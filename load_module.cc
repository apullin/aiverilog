...
bool load_module(const char* type, int& parser_errors) {
    ...
    for (struct module_library* lcur = library_list; lcur != 0; lcur = lcur->next) {
        ...
        snprintf(path, sizeof(path), "%s%c%s", lcur->dir, dir_character, (*cur).second);
...
int build_library_index(const char* path, bool key_case_sensitive) {
    ...
    while (const struct dirent* de = readdir(dir)) {
        ...
        for (list<const char*>::iterator suf = library_suff.begin(); suf != library_suff.end();
             ++suf) {
