...
static bool get_real_arguments(const NetExpr* le, const NetExpr* re, double& lval, double& rval) {
    verireal val;

    if (!get_real_arg_(le, val))
        return false;
    lval = val.as_double();

    if (!get_real_arg_(re, val))
        return false;
    rval = val.as_double();
    ...
    if (lc != 0 && rc != 0) {
        ...
        if (debug_eval_tree) {
            cerr << get_fileline() << ": debug: "
                 << "Partially evaluate " << *this << " using (a+2)-1 --> (a+1) transform." << endl;
        }
...
NetEConst* NetEBComp::eval_leeq_real_(const NetExpr* le, const NetExpr* re, bool eq_flag) const {
    ...
    if (lval < rval)
        tmp = true;
    if (tmp == false && eq_flag && lval == rval)
        tmp = true;
...
NetEConst* NetEBComp::eval_arguments_(const NetExpr* l, const NetExpr* r) const {
    ...
    eval_debug(this, res, l->expr_type() == IVL_VT_REAL || r->expr_type() == IVL_VT_REAL);
    return res;
}
...
NetEConst* NetEConcat::eval_arguments_(const vector<NetExpr*>& vals, unsigned gap) const {
    ...
    switch (const_logical(cond_)) {
        case C_0:
            ...
            if (debug_eval_tree) {
                ...
                cerr << get_fileline() << ":      : Selecting false case: " << *false_val_ << endl;
            }
        ...
        case C_1:
            ...
            if (debug_eval_tree) {
                cerr << get_fileline() << ": debug: Evaluate ternary with "
                     << "constant condition value: ";
                print_ternary_cond(cond_);
                cerr << get_fileline() << ":      : Selecting true case: " << *true_val_ << endl;
            }
...
NetExpr* NetETernary::blended_arguments_(const NetExpr* te, const NetExpr* fe) const {
    ...
    if (t == 0 || f == 0) {
        verireal tv, fv;
        if (!get_real_arg_(te, tv))
            return 0;
        if (!get_real_arg_(fe, fv))
            return 0;
        ...
        if (debug_eval_tree) {
            ...
            cerr << get_fileline() << ":      : Blending real cases "
                 << "true=" << tv.as_double() << ", false=" << fv.as_double() << ", to get " << val
                 << endl;
        }
...
NetECReal*
NetESFunc::evaluate_math_two_arg_(ID id, const NetExpr* arg0_, const NetExpr* arg1_) const {
...
NetExpr* NetESFunc::evaluate_min_max_(ID id, const NetExpr* arg0_, const NetExpr* arg1_) const {
    const NetEConst* tmpi0 = dynamic_cast<const NetEConst*>(arg0_);
    const NetECReal* tmpr0 = dynamic_cast<const NetECReal*>(arg0_);
    const NetEConst* tmpi1 = dynamic_cast<const NetEConst*>(arg1_);
    const NetECReal* tmpr1 = dynamic_cast<const NetECReal*>(arg1_);
...
static void no_string_arg(const NetESFunc* info, unsigned arg_num) {
    cerr << info->get_fileline() << ": error: constant function " << info->name()
         << "() does not support a string argument (" << arg_num + 1 << ")." << endl;
}
...
static bool check_dimension(const NetExpr* dim_expr, long& dim) {
    ...
    if (dimi)
        dim = dimi->value().as_long();
    if (dimr)
        dim = dimr->value().as_long();
    return false;
}
...
static bool get_array_info(const NetExpr* arg, long dim, long& left, long& right, bool& defer) {
...
NetEConst* NetESFunc::evaluate_array_funcs_(ID id, const NetExpr* arg0, const NetExpr* arg1) const {
    long dim = 0;
    /* Check to see if the dimension argument is constant. */
    if (check_dimension(arg1, dim))
        return 0;
...
NetExpr* NetESFunc::evaluate_two_arg_(ID id, const NetExpr* arg0, const NetExpr* arg1) const {
    switch (id) {
        case CTBITS:
...
NetESFunc::ID NetESFunc::built_in_id_() const {
     ...
     * flag was given. */
    if (funcs_need_init && ((generation_flag >= GN_VER2005) || gn_icarus_misc_flag)) {
    ...
    switch (parms_.size()) {
        ...
        default:
            ...
            if (!takes_nargs_(id, parms_.size())) {
                cerr << get_fileline() << ": error: constant function " << name_
                     << "() does not support " << parms_.size() << " arguments." << endl;
                return 0;
            }
            if (id == CTBITS) {
                ...
                cerr << get_fileline() << ": sorry: constant functions with " << parms_.size()
                     << " arguments are not supported: " << name_ << "()." << endl;
            }
            return 0;
    }
}
