...
static bool process_binary_args(Design* des,
                                NetScope* scope,
                                NetExpr* root,
                                NetExpr* left,
                                NetExpr* right,
...
NetNet* NetExpr::synthesize(Design* des, NetScope*, NetExpr*) {
    cerr << get_fileline() << ": internal error: cannot synthesize expression: " << *this << endl;
    des->errors += 1;
    return 0;
}
...
NetNet* NetEBAdd::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (process_binary_args(des, scope, root, left_, right_, lsig, rsig, real_args)) {
        return 0;
    }
...
NetNet* NetEBBits::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    NetNet* osig = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, osig_vec);
    osig->set_line(*this);
    osig->local_flag(true);
...
NetNet* NetEBComp::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (op_ == 'E' || op_ == 'N') {
        NetCaseCmp* gate = new NetCaseCmp(
            scope, scope->local_symbol(), width, op_ == 'E' ? NetCaseCmp::EEQ : NetCaseCmp::NEQ);
        gate->set_line(*this);
    ...
    if (op_ == 'w' || op_ == 'W') {
        NetCaseCmp* gate = new NetCaseCmp(
            scope, scope->local_symbol(), width, op_ == 'w' ? NetCaseCmp::WEQ : NetCaseCmp::WNE);
        gate->set_line(*this);
        connect(gate->pin(0), osig->pin(0));
        connect(gate->pin(1), lsig->pin(0));
        connect(gate->pin(2), rsig->pin(0));
    ...
    if ((width == 1) && (op_ == 'e') && !real_args) {
        NetLogic* gate = new NetLogic(scope, scope->local_symbol(), 3, NetLogic::XNOR, 1, true);
        gate->set_line(*this);
        connect(gate->pin(0), osig->pin(0));
        connect(gate->pin(1), lsig->pin(0));
        connect(gate->pin(2), rsig->pin(0));
    ...
    if ((width == 1) && (op_ == 'n') && !real_args) {
        NetLogic* gate = new NetLogic(scope, scope->local_symbol(), 3, NetLogic::XOR, 1, true);
        gate->set_line(*this);
        connect(gate->pin(0), osig->pin(0));
        connect(gate->pin(1), lsig->pin(0));
        connect(gate->pin(2), rsig->pin(0));
...
NetNet* NetEBPow::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    NetPow* powr =
        new NetPow(scope, scope->local_symbol(), width, lsig->vector_width(), rsig->vector_width());
    powr->set_line(*this);
    des->add_node(powr);
...
NetNet* NetEBMult::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    NetMult* mult = new NetMult(
        scope, scope->local_symbol(), width, lsig->vector_width(), rsig->vector_width());
    mult->set_line(*this);
    des->add_node(mult);
...
NetNet* NetEBLogic::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    NetNet* osig = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, osig_tmp);
    osig->set_line(*this);
    osig->local_flag(true);
...
NetNet* NetEBShift::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (lsig->data_type() == IVL_VT_REAL) {
        cerr << get_fileline() << ": error: shift operator (" << human_readable_op(op_)
    ...
    if (const NetEConst* rcon = dynamic_cast<NetEConst*>(right_)) {
        ...
        NetPartSelect* psel = new NetPartSelect(
            lsig, shift < 0 ? ushift : 0, part_width, NetPartSelect::VP, signed_flag && right_flag);
        psel->set_line(*this);
        des->add_node(psel);

        const netvector_t* psig_vec = new netvector_t(expr_type(), part_width - 1, 0);
        NetNet* psig = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, psig_vec);
        psig->set_line(*this);
        ...
        const netvector_t* zsig_vec = new netvector_t(osig->data_type(), znum.len() - 1, 0);
        NetNet* zsig = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, zsig_vec);
        zsig->set_line(*this);
        zsig->local_flag(true);
        connect(zcon->pin(0), zsig->pin(0));

        NetConcat* ccat = new NetConcat(scope, scope->local_symbol(), osig->vector_width(), 2);
        ccat->set_line(*this);
        des->add_node(ccat);
    ...
    NetCLShift* dev = new NetCLShift(scope,
                                     scope->local_symbol(),
                                     osig->vector_width(),
                                     rsig->vector_width(),
                                     right_flag,
...
NetNet* NetEConcat::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (num_parms != 1) {
        ...
        osig = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, osig_vec);
        osig->set_line(*this);
        osig->local_flag(true);

        auto cncat = new NetConcat(scope, scope->local_symbol(), osig->vector_width(), num_parms);
        cncat->set_line(*this);
        des->add_node(cncat);
    ...
    if (repeat() != 1) {
        auto rep = new NetReplicate(scope, scope->local_symbol(), expr_width(), repeat());
        rep->set_line(*this);
        des->add_node(rep);
...
NetNet* NetEArrayPattern::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    NetNet* osig = new NetNet(scope, path, NetNet::IMPLICIT, dims, array_type->element_type());
    osig->set_line(*this);
    osig->local_flag(true);
...
NetNet* NetEUBits::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (isig->data_type() == IVL_VT_REAL) {
        cerr << get_fileline() << ": error: bit-wise negation (" << human_readable_op(op_)
...
NetNet* NetEUnary::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (op_ == 'm') {
        ...
        const netvector_t* sig_vec = new netvector_t(sub->data_type(), sub->vector_width() - 1, 0);
...
NetNet* NetEUReduce::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (isig->data_type() == IVL_VT_REAL) {
        ...
        cerr << get_fileline() << ": error: reduction operator (" << human_readable_op(op_)
             << ") may not have a REAL operand." << endl;
        des->errors += 1;
        return 0;
    }
    ...
    NetUReduce* gate = new NetUReduce(scope, scope->local_symbol(), rtype, isig->vector_width());
    gate->set_line(*this);
    des->add_node(gate);
...
NetNet* NetESelect::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (const NetEConst* base_const = dynamic_cast<NetEConst*>(base_)) {
        ...
        NetPartSelect* sel = new NetPartSelect(sub, base_val, select_width, NetPartSelect::VP);
        des->add_node(sel);

        ivl_assert(*this, select_width > 0);
        const netvector_t* tmp_vec = new netvector_t(sub->data_type(), select_width - 1, 0);
        ...
        if (concat_count > 1) {
            NetConcat* cat =
                new NetConcat(scope, scope->local_symbol(), expr_width(), concat_count);
            cat->set_line(*this);
            des->add_node(cat);
            if (below) {
    ...
    if (base_ != 0) {
        ...
        NetPartSelect* sel = new NetPartSelect(sub, off, expr_width(), base_->has_sign());
        sel->set_line(*this);
        des->add_node(sel);

        const netvector_t* tmp_vec = new netvector_t(sub->data_type(), expr_width() - 1, 0);
        NetNet* tmp = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, tmp_vec);
        tmp->local_flag(true);
        tmp->set_line(*this);
        sub = tmp;
    ...
    NetNet* net = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, net_vec);
    net->set_line(*this);
    net->local_flag(true);
    ...
    if (expr_width() < sub->vector_width()) {
        NetPartSelect* sel = new NetPartSelect(sub, 0, expr_width(), NetPartSelect::VP, has_sign());
        sel->set_line(*this);
        des->add_node(sel);
        ...
        NetSignExtend* pad = new NetSignExtend(scope, scope->local_symbol(), expr_width());
        pad->set_line(*this);
        des->add_node(pad);
        ...
        NetConcat* cat = new NetConcat(scope, scope->local_symbol(), expr_width(), 2);
        cat->set_line(*this);
        des->add_node(cat);
...
NetNet* NetETernary::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    NetMux* mux = new NetMux(csig->scope(), oname, width, 2, csig->vector_width());
    mux->set_line(*this);
...
NetNet* NetESignal::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    const netvector_t* tmp_vec = new netvector_t(net_->data_type(), net_->vector_width() - 1, 0);
    NetNet* tmp = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, tmp_vec);
    tmp->set_line(*this);
    tmp->local_flag(true);
    ...
    if (const NetEConst* index_co = dynamic_cast<NetEConst*>(word_)) {
        ...
        NetArrayDq* mux = new NetArrayDq(scope, scope->local_symbol(), net_, selwid);
        mux->set_line(*this);
        des->add_node(mux);
...
NetNet* NetESFunc::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": debug: Net system function " << name_ << " returns "
             << def->type << endl;
    }
    ...
    NetSysFunc* net = new NetSysFunc(scope, scope->local_symbol(), def, 1 + parms_.size(), trigger);
    net->set_line(*this);
    des->add_node(net);

    netvector_t* osig_vec = new netvector_t(def->type, def->wid - 1, 0);
    osig_vec->set_signed(def->type == IVL_VT_REAL ? true : false);
    NetNet* osig = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, osig_vec);
    osig->set_line(*this);
    osig->local_flag(true);
...
NetNet* NetEUFunc::synthesize(Design* des, NetScope* scope, NetExpr* root) {
    ...
    NetUserFunc* net = new NetUserFunc(scope_, scope_->local_symbol(), func_, trigger);
    net->set_line(*this);
    des->add_node(net);

    /* Create an output signal and connect it to the function. */
    const netvector_t* osig_vec =
        new netvector_t(result_sig_->expr_type(), result_sig_->vector_width() - 1, 0);
    NetNet* osig = new NetNet(scope_, scope_->local_symbol(), NetNet::WIRE, osig_vec);
    osig->set_line(*this);
    osig->local_flag(true);
    connect(net->pin(0), osig->pin(0));
