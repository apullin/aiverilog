...
ufunc_core::ufunc_core(unsigned owid,
                       vvp_net_t* ptr,
                       unsigned nports,
                       vvp_net_t** ports,
                       vvp_code_t sa,
                       __vpiScope* call_scope__,
                       char* scope_label)
...
ufunc_vec4::ufunc_vec4(unsigned ow,
                       vvp_net_t* ptr,
                       unsigned nports,
                       vvp_net_t** ports,
                       vvp_code_t start_address,
...
ufunc_real::ufunc_real(unsigned ow,
                       vvp_net_t* ptr,
                       unsigned nports,
                       vvp_net_t** ports,
                       vvp_code_t start_address,
 ...
 */
void compile_ufunc_real(char* label,
                        char* code,
                        unsigned wid,
                        unsigned argc,
                        struct symb_s* argv,
    ...
    ufunc_core* fcore = new ufunc_real(wid, ptr, portc, ports, exec_code, call_scope, scope_label);
    ptr->fun = fcore;
...
void compile_ufunc_vec4(char* label,
                        char* code,
                        unsigned wid,
                        unsigned argc,
                        struct symb_s* argv,
    ...
    /* Create the output functor and attach it to the label. Tell
       ...
       that will contain the execution. */
    vvp_net_t* ptr = new vvp_net_t;
    ufunc_core* fcore = new ufunc_vec4(wid, ptr, portc, ports, exec_code, call_scope, scope_label);
    ptr->fun = fcore;
    define_functor_symbol(label, ptr);
    free(label);
