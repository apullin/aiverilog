...
bool vvp_fun_delay::clean_pulse_events_(vvp_time64_t use_delay, const vvp_vector4_t& bit) {
    if (list_ == 0)
        return false;
...
bool vvp_fun_delay::clean_pulse_events_(vvp_time64_t use_delay, const vvp_vector8_t& bit) {
    if (list_ == 0)
        return false;
 ...
 */
void vvp_fun_delay::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    if (port.port() > 0) {
    ...
    if (initial_) {
        ...
        const vvp_vector4_t& use_vec4 = (list_ && list_->next->sim_time == schedule_simtime())
                                            ? list_->next->ptr_vec4
                                            : cur_vec4_;
...
void vvp_fun_delay::recv_vec4_pv(
...
void vvp_fun_delay::recv_vec8(vvp_net_ptr_t port, const vvp_vector8_t& bit) {
    ...
    if (initial_) {
        ...
        // special case that the event list contains the current
        // value as a zero-delay-remaining event.
        const vvp_vector8_t& use_vec8 = (list_ && list_->next->sim_time == schedule_simtime())
                                            ? list_->next->ptr_vec8
                                            : cur_vec8_;
        ...
        /* Scan the vectors looking for delays. Select the maximum
           delay encountered. */
        use_delay = delay_.get_delay(use_vec8.value(0).value(), bit.value(0).value());

        for (unsigned idx = 1; idx < use_wid; idx += 1) {
            vvp_time64_t tmp;
            tmp = delay_.get_delay(use_vec8.value(idx).value(), bit.value(idx).value());
            if (tmp > use_delay)
                use_delay = tmp;
        }
    }
...
void vvp_fun_delay::recv_vec8_pv(vvp_net_ptr_t ptr,
...
void vvp_fun_delay::recv_real(vvp_net_ptr_t port, double bit, vvp_context_t) {
    if (port.port() > 0) {
        ...
        if (bit > -0.5) {
            ...
            vvp_vector4_t vec4(8 * sizeof(vvp_time64_t), floor(-bit * round_ + 0.5) * -1 * scale_);
...
static vvp_time64_t delay_from_edge(vvp_bit4_t a, vvp_bit4_t b, vvp_time64_t const array[12]) {
...
void vvp_fun_modpath::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    ...
    for (vvp_fun_modpath_src* cur = ifnone_list_; cur; cur = cur->next_) {
        ...
        } else if (cur->wake_time_ == ifnone_wake_time && ifnone_wake_time > candidate_wake_time) {
            candidate_list.push_back(cur);
    ...
    vvp_time64_t use_delay = delay_from_edge(cur_vec4_.value(0), bit.value(0), out_at);
    ...
    for (unsigned idx = 1; idx < bit.size(); idx += 1) {
        vvp_time64_t tmp = delay_from_edge(cur_vec4_.value(idx), bit.value(idx), out_at);
        /* If the current and new bit values match then no delay
         * is needed for this bit. */
        if (cur_vec4_.value(idx) == bit.value(idx))
            continue;
        if (tmp != use_delay) {
            fprintf(stderr,
                    "sorry: multi-bit module path delays are "
...
void vvp_fun_modpath_src::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    if (port.port() == 0) {
        // The modpath input...
        if (test_vec4(bit))
...
vvp_fun_modpath_edge::vvp_fun_modpath_edge(vvp_time64_t del[12], bool pos, bool neg)
...
static void modpath_src_put_delays(vpiHandle ref, p_vpi_delay delays) {
    ...
    if (delays->time_type == vpiSimTime) {
        ...
        for (idx = 0; idx < delays->no_of_delays; idx += 1) {
            if (delays->da[idx].real < 0.0)
                delays->da[idx].real = 0.0;
        }
        for (idx = 0; idx < 12; idx += 1) {
            tmp[idx] =
                vpip_scaled_real_to_time64(delays->da[use_map[0][idx]].real, src->dest->scope);
        }
    }
    ...
    if (delays->no_of_delays <= 6) {
        ...
        tmp[DELAY_EDGE_0x] =
            tmp[DELAY_EDGE_0z] < tmp[DELAY_EDGE_01] ? tmp[DELAY_EDGE_0z] : tmp[DELAY_EDGE_01];
        /* x->1 is the maximum of z->1 and 0->1. */
        tmp[DELAY_EDGE_x1] =
            tmp[DELAY_EDGE_z1] > tmp[DELAY_EDGE_01] ? tmp[DELAY_EDGE_z1] : tmp[DELAY_EDGE_01];
        /* 1->x is the minimum of 1->z and 1->0. */
        tmp[DELAY_EDGE_1x] =
            tmp[DELAY_EDGE_1z] < tmp[DELAY_EDGE_10] ? tmp[DELAY_EDGE_1z] : tmp[DELAY_EDGE_10];
        /* x->0 is the maximum of z->0 and 1->0. */
        tmp[DELAY_EDGE_x0] =
            tmp[DELAY_EDGE_z0] > tmp[DELAY_EDGE_10] ? tmp[DELAY_EDGE_z0] : tmp[DELAY_EDGE_10];
        /* x->z is the maximum of 1->z and 0->z. */
        tmp[DELAY_EDGE_xz] =
            tmp[DELAY_EDGE_1z] > tmp[DELAY_EDGE_0z] ? tmp[DELAY_EDGE_1z] : tmp[DELAY_EDGE_0z];
        /* z->x is the minimum of z->1 and z->0. */
        tmp[DELAY_EDGE_zx] =
            tmp[DELAY_EDGE_z1] < tmp[DELAY_EDGE_z0] ? tmp[DELAY_EDGE_z1] : tmp[DELAY_EDGE_z0];
    }
...
struct __vpiModPath* vpip_make_modpath(vvp_net_t* net) {
    ...
    mp_list = static_cast<struct __vpiModPath**>(
        realloc(mp_list, mp_count * sizeof(struct __vpiModPath**)));
...
struct __vpiModPathSrc* vpip_make_modpath_src(struct __vpiModPath* path, vvp_net_t* net) {
...
vvp_fun_intermodpath::vvp_fun_intermodpath(vvp_net_t* net, unsigned width) : net_(net) {
    for (unsigned idx = 0; idx < 12; idx += 1)
        delay_[idx] = 0;
 ...
 * multiple vectors approaching the result should be scheduled.
 */
void vvp_fun_intermodpath::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    /* Only the first port is used. */
    if (port.port() > 0)
        return;
    ...
    /* Given the scheduled output time, create an output event. */
    vvp_time64_t use_delay = delay_from_edge(cur_vec4_.value(0), bit.value(0), delay_);

    /* FIXME: This bases the edge delay on only the least
       bit. This is WRONG! I need to find all the possible delays,
       and schedule an event for each partial change. Hard! */
    for (unsigned idx = 1; idx < bit.size(); idx += 1) {
        vvp_time64_t tmp = delay_from_edge(cur_vec4_.value(idx), bit.value(idx), delay_);
        /* If the current and new bit values match then no delay
         * is needed for this bit. */
        if (cur_vec4_.value(idx) == bit.value(idx))
            continue;
...
static void intermodpath_put_delays(vpiHandle ref, p_vpi_delay delays) {
    ...
    if (delays->time_type == vpiSimTime) {
        ...
        for (idx = 0; idx < 12; idx += 1) {
            tmp[idx] = vpip_scaled_real_to_time64(delays->da[use_map[0][idx]].real, src->scope);
        }
    }
...
struct __vpiInterModPath*
vpip_make_intermodpath(vvp_net_t* net, vpiPortInfo* port1, vpiPortInfo* port2) {
    ...
    imp_list = static_cast<struct __vpiInterModPath**>(
        realloc(imp_list, imp_count * sizeof(struct __vpiInterModPath**)));
