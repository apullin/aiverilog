...
resolv_core::resolv_core(unsigned nports, vvp_net_t* net) : nports_(nports), net_(net) {
    count_functors_resolv += 1;
}

resolv_core::~resolv_core() {}

void resolv_core::recv_vec4_pv_(unsigned port,
...
void resolv_core::recv_vec8_pv_(unsigned port,
...
resolv_wired_logic::resolv_wired_logic(unsigned nports, vvp_net_t* net) : resolv_core(nports, net) {
    // count the input (leaf) nodes
    unsigned nnodes = nports;
...
resolv_triand::resolv_triand(unsigned nports, vvp_net_t* net) : resolv_wired_logic(nports, net) {}
...
resolv_trior::resolv_trior(unsigned nports, vvp_net_t* net) : resolv_wired_logic(nports, net) {}
