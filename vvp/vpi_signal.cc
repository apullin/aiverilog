...
char* generic_get_str(int code, vpiHandle ref, const char* name, const char* index) {
    ...
    if (code == vpiFullName) {
        ...
        len += bn_len;
        if (!is_bn_a_pkg)
            len += 1;  // include space for "." separator
    }
    if (index != NULL)
...
static vpiHandle fill_in_var4(struct __vpiSignal* obj,
    ...
    return fill_in_net4(obj, vpip_peek_current_scope(), name, msb, lsb, signed_flag, node);
}
...
static void
format_vpiBinStrVal(const vvp_signal_value* sig, int base, unsigned wid, s_vpi_value* vp) {
...
static void
format_vpiOctStrVal(const vvp_signal_value* sig, int base, unsigned wid, s_vpi_value* vp) {
...
static void
format_vpiHexStrVal(const vvp_signal_value* sig, int base, unsigned wid, s_vpi_value* vp) {
...
static void format_vpiDecStrVal(
    const vvp_signal_value* sig, int base, unsigned wid, int signed_flag, s_vpi_value* vp) {
...
static void
format_vpiStringVal(const vvp_signal_value* sig, int base, unsigned wid, s_vpi_value* vp) {
    ...
    for (long idx = base + (signed)wid - 1; idx >= base; idx -= 1) {
        ...
        if (idx >= 0 && idx < (signed)sig->value_size() && sig->value(idx) == BIT4_1) {
            tmp |= 1;
        }
...
static void format_vpiScalarVal(const vvp_signal_value* sig, int base, s_vpi_value* vp) {
...
static void
format_vpiStrengthVal(const vvp_signal_value* sig, int base, unsigned wid, s_vpi_value* vp) {
    long end = base + (signed)wid;
    s_vpi_strengthval* op;

    op =
        static_cast<s_vpi_strengthval*>(need_result_buf(wid * sizeof(s_vpi_strengthval), RBUF_VAL));
...
static void
format_vpiVectorVal(const vvp_signal_value* sig, int base, unsigned wid, s_vpi_value* vp) {
    long end = base + (signed)wid;
    unsigned int obit = 0;
    unsigned hwid = (wid + 31) / 32;

    s_vpi_vecval* op =
        static_cast<p_vpi_vecval>(need_result_buf(hwid * sizeof(s_vpi_vecval), RBUF_VAL));
    vp->value.vector = op;
...
vpiHandle __vpiSignal::get_index(int idx) {
    ...
    if (msb.get_value() >= lsb.get_value()) {
        if ((idx > msb.get_value()) || (idx < lsb.get_value()))
            return 0;
    } else {
        if ((idx < msb.get_value()) || (idx > lsb.get_value()))
            return 0;
...
vpiHandle __vpiSignal::put_bit_value(struct __vpiBit* bit, p_vpi_value vp, int flags) {
    ...
    if ((flags == vpiForceFlag) || (flags == vpiReleaseFlag)) {
        fprintf(stderr,
                "Sorry: vpi_put_value() for %s does not "
                "currently support force/release.\n",
                bit->as_bit.vpi_get_str(vpiFullName));
        return NULL;
    }

    if ((get_type_code() == vpiNet) && !dynamic_cast<vvp_island_port*>(node->fun)) {
        node->send_vec4_pv(val, index, width(), vthread_get_wt_context());
    } else {
        vvp_send_vec4_pv(dest, val, index, width(), vthread_get_wt_context());
    }
...
unsigned __vpiSignal::width(void) const {
    unsigned wid = (msb.get_value() >= lsb.get_value()) ? (msb.get_value() - lsb.get_value() + 1)
 ...
 */
vpiHandle vpip_make_int2(const char* name, int msb, int lsb, bool signed_flag, vvp_net_t* vec) {
    __vpiSignal* obj;
 ...
 */
vpiHandle vpip_make_var4(const char* name, int msb, int lsb, bool signed_flag, vvp_net_t* vec) {
...
void* __vpiSignal::operator new(size_t siz) {
    ...
    if ((alloc_array == 0) || (alloc_index == alloc_count)) {
        alloc_array =
            static_cast<struct vpiSignal_plug*>(calloc(alloc_count, sizeof(struct vpiSignal_plug)));
        alloc_index = 0;
#ifdef CHECK_WITH_VALGRIND
        VALGRIND_MAKE_MEM_NOACCESS(alloc_array, alloc_count * sizeof(struct vpiSignal_plug));
        VALGRIND_CREATE_MEMPOOL(alloc_array, 0, 1);
        signal_pool_count += 1;
        signal_pool = static_cast<vpiSignal_plug**>(
            realloc(signal_pool, signal_pool_count * sizeof(vpiSignal_plug**)));
    ...
    switch (vp->format) {
        ...
        default:
            fprintf(stderr,
                    ...
                    (int)vp->format,
                    vpi_get_str(vpiFullName, rfp->parent));
            assert(0);
    }
}
...
static vpiHandle PV_put_value(vpiHandle ref, p_vpi_value vp, int flags) {
    ...
    if (flags == vpiForceFlag) {
        ...
        if (full_sig) {
            ...
            rfp->net->send_vec4_pv(val, base, sig_size, vthread_get_wt_context());
        }
    } else {
        if (full_sig) {
            vvp_send_vec4(dest, val, vthread_get_wt_context());
        } else {
            vvp_send_vec4_pv(dest, val, base, sig_size, vthread_get_wt_context());
        }
    }
    return 0;
}
