...
class vvp_island_tran : public vvp_island {
  public:
    ...
    void count_drivers(vvp_island_port* port, unsigned bit_idx, unsigned counts[3]) override;
};
...
struct vvp_island_branch_tran : public vvp_island_branch {
    vvp_island_branch_tran(vvp_net_t* en__,
                           bool active_high__,
                           unsigned width__,
                           unsigned part__,
                           unsigned offset__,
...
static void count_drivers_(vvp_branch_ptr_t cur,
                           bool other_side_visited,
                           unsigned bit_idx,
                           unsigned counts[3]) {
...
void vvp_island_tran::count_drivers(vvp_island_port* port, unsigned bit_idx, unsigned counts[3]) {
...
inline vvp_vector8_t resolve_ambiguous(const vvp_vector8_t& a,
    ...
    for (unsigned idx = 0; idx < out.size(); idx += 1) {
        ...
        b_bit = vvp_scalar_t(b_bit.value(), str_map[b_bit.strength0()], str_map[b_bit.strength1()]);
...
static void push_value_through_branch(const vvp_vector8_t& val, vvp_branch_ptr_t cur) {
    ...
    if (branch->width == 0) {
        ...
        dst_port->value = resolve_ambiguous(
            dst_port->value, val, branch->state, vvp_switch_strength_map[branch->resistive]);
...
void compile_island_tranif(int sense, char* island, char* pa, char* pb, char* pe, bool resistive) {
...
void compile_island_tranvp(
    char* island, char* pa, char* pb, unsigned wid, unsigned par, unsigned off) {
    vvp_island* use_island = compile_find_island(island);
    assert(use_island);
    free(island);

    vvp_island_branch_tran* br = new vvp_island_branch_tran(NULL, false, wid, par, off, false);
