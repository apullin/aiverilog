...
vvp_net_fil_t::filter_mask_(const T& val, const T& force, T& filter, unsigned base) {
...
template <class T>
vvp_net_fil_t::prop_t
vvp_net_fil_t::filter_input_mask_(const T& val, const T& force, T& rep) const {
    if (test_force_mask_is_zero())
        return PROP;
...
vvp_fun_signal4_sa::vvp_fun_signal4_sa(unsigned wid, vvp_bit4_t init) : bits4_(wid, init) {}
 ...
 */
void vvp_fun_signal4_sa::recv_vec4(vvp_net_ptr_t ptr, const vvp_vector4_t& bit, vvp_context_t) {
    switch (ptr.port()) {
...
void vvp_fun_signal4_sa::recv_vec4_pv(
    vvp_net_ptr_t ptr, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t) {
...
void vvp_fun_signal4_aa::reset_instance(vvp_context_t context) {
    vvp_vector4_t* bits = static_cast<vvp_vector4_t*>(vvp_get_context_item(context, context_idx_));
 ...
 */
void vvp_fun_signal4_aa::recv_vec4(vvp_net_ptr_t ptr,
                                   const vvp_vector4_t& bit,
                                   vvp_context_t context) {
    assert(ptr.port() == 0);
    assert(context);

    vvp_vector4_t* bits4 = static_cast<vvp_vector4_t*>(vvp_get_context_item(context, context_idx_));
...
void vvp_fun_signal_real_sa::recv_real(vvp_net_ptr_t ptr, double bit, vvp_context_t) {
    switch (ptr.port()) {
        case 0:
...
void vvp_fun_signal_real_aa::reset_instance(vvp_context_t context) {
    double* bits = static_cast<double*>(vvp_get_context_item(context, context_idx_));
...
double vvp_fun_signal_real_aa::real_unfiltered_value() const {
    const double* bits = static_cast<double*>(vthread_get_rd_context_item(context_idx_));
...
void vvp_fun_signal_real_aa::recv_real(vvp_net_ptr_t ptr, double bit, vvp_context_t context) {
    assert(ptr.port() == 0);
    assert(context);
...
void vvp_fun_signal_string_aa::reset_instance(vvp_context_t context) {
    string* bits = static_cast<std::string*>(vvp_get_context_item(context, context_idx_));
    *bits = "";
}
...
const std::string& vvp_fun_signal_string_aa::get_string() const {
    const string* bits = static_cast<std::string*>(vthread_get_rd_context_item(context_idx_));
...
vvp_fun_signal_object_sa::vvp_fun_signal_object_sa(unsigned size) : vvp_fun_signal_object(size) {}

#ifdef CHECK_WITH_VALGRIND
void vvp_fun_signal_object_aa::free_instance(vvp_context_t context) {
    vvp_object_t* bits = static_cast<vvp_object_t*>(vvp_get_context_item(context, context_idx_));
    delete bits;
}
#endif

void vvp_fun_signal_object_sa::recv_object(vvp_net_ptr_t ptr, vvp_object_t bit, vvp_context_t) {
    assert(ptr.port() == 0);
...
vvp_fun_signal_object_aa::vvp_fun_signal_object_aa(unsigned size) : vvp_fun_signal_object(size) {
    context_idx_ = vpip_add_item_to_context(this, vpip_peek_context_scope());
}
...
vvp_object_t vvp_fun_signal_object_aa::get_object() const {
    const vvp_object_t* bits =
        static_cast<vvp_object_t*>(vthread_get_rd_context_item(context_idx_));
    return *bits;
}

void vvp_fun_signal_object_aa::recv_object(vvp_net_ptr_t ptr,
                                           vvp_object_t bit,
                                           vvp_context_t context) {
    assert(ptr.port() == 0);
    assert(context);
...
void vvp_fun_force::recv_vec4(vvp_net_ptr_t ptr, const vvp_vector4_t& bit, vvp_context_t) {
    assert(ptr.port() == 0);
...
vvp_wire_vec4::vvp_wire_vec4(unsigned wid, vvp_bit4_t init) : bits4_(wid, init) {
    needs_init_ = true;
}
...
vvp_net_fil_t::prop_t vvp_wire_vec4::filter_vec4(const vvp_vector4_t& bit,
    ...
    if (vwid != bits4_.size()) {
        cerr << "Internal error: Input vector expected width=" << bits4_.size() << ", got "
    ...
    if (base == 0 && bit.size() == vwid) {
        if (bits4_.eeq(bit) && !needs_init_)
            return STOP;
        bits4_ = bit;
    } else {
        bool rc = bits4_.set_vec(base, bit);
        if (rc == false && !needs_init_)
            return STOP;
    }
...
vvp_net_fil_t::prop_t vvp_wire_vec4::filter_vec8(const vvp_vector8_t& bit,
    ...
    if (base == 0 && bit4.size() == vwid) {
        if (bits4_.eeq(bit4) && !needs_init_)
            return STOP;
        bits4_ = bit4;
    } else {
...
void vvp_wire_vec4::release_pv(vvp_net_ptr_t ptr, unsigned base, unsigned wid, bool net_flag) {
    ...
    if (net_flag) {
        ...
        ptr.ptr()->send_vec4_pv(bits4_.subvalue(base, wid), base, bits4_.size(), 0);
        run_vpi_callbacks();
    } else {
        // Variables keep the current value.
...
vvp_wire_vec8::vvp_wire_vec8(unsigned wid) : bits8_(wid) {
    needs_init_ = true;
}
...
vvp_net_fil_t::prop_t vvp_wire_vec8::filter_input_vec8(const vvp_vector8_t& bit,
                                                       vvp_vector8_t& rep) const {
...
void vvp_wire_vec8::release_pv(vvp_net_ptr_t ptr, unsigned base, unsigned wid, bool net_flag) {
    ...
    if (net_flag) {
        ...
        ptr.ptr()->send_vec8_pv(bits8_.subvalue(base, wid), base, bits8_.size());
        run_vpi_callbacks();
    } else {
        // Variable do not know about strength so this should not be able
        // to happen. If for some reason it can then it should not be too
