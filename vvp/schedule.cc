...
void assign_vector4_event_s::single_step_display(void) {
    cerr << "assign_vector4_event: Propagate val=" << val << ", vwid=" << vwid << ", base=" << base
         << endl;
}
...
unsigned long count_assign4_pool(void) {
    return assign4_heap.pool;
}
...
unsigned long count_assign8_pool() {
    return assign8_heap.pool;
}
...
unsigned long count_assign_real_pool(void) {
    return assignr_heap.pool;
}
...
unsigned long count_assign_aword_pool(void) {
    return array_w_heap.pool;
}
...
void force_vector4_event_s::single_step_display(void) {
    cerr << "force_vector4_event: Force val=" << val << ", vwid=" << vwid << ", base=" << base
         << endl;
}
...
unsigned long count_force4_pool(void) {
    return force4_heap.pool;
}
...
unsigned long count_assign_arword_pool(void) {
    return array_r_w_heap.pool;
}
...
unsigned long count_gen_pool(void) {
    return generic_event_heap.pool;
}
...
unsigned long count_time_pool(void) {
    return event_time_heap.pool;
}
...
static void schedule_event_(struct event_s* cur, vvp_time64_t delay, event_queue_t select_queue) {
    cur->next = cur;
...
void schedule_assign_vector(
    vvp_net_ptr_t ptr, unsigned base, unsigned vwid, const vvp_vector4_t& bit, vvp_time64_t delay) {
    struct assign_vector4_event_s* cur = new struct assign_vector4_event_s(bit);
    cur->ptr = ptr;
    cur->base = base;
    cur->vwid = vwid;
...
void schedule_force_vector(
    vvp_net_t* net, unsigned base, unsigned vwid, const vvp_vector4_t& bit, vvp_time64_t delay) {
...
void schedule_propagate_vector(vvp_net_t* net, vvp_time64_t delay, const vvp_vector4_t& src) {
    struct propagate_vector4_event_s* cur = new struct propagate_vector4_event_s(src);
    cur->net = net;
    schedule_event_(cur, delay, SEQ_NBASSIGN);
}
...
void schedule_propagate_event(vvp_net_t* net, vvp_time64_t delay) {
    vvp_vector4_t tmp(1, BIT4_X);
    struct propagate_vector4_event_s* cur = new struct propagate_vector4_event_s(tmp);
    cur->net = net;
    schedule_event_(cur, delay, SEQ_NBASSIGN);
}
...
void schedule_generic(
    vvp_gen_event_t obj, vvp_time64_t delay, bool sync_flag, bool ro_flag, bool delete_when_done) {
    struct generic_event_s* cur = new generic_event_s;

    cur->obj = obj;
    cur->delete_obj_when_done = delete_when_done;
    schedule_event_(cur, delay, sync_flag ? (ro_flag ? SEQ_ROSYNC : SEQ_RWSYNC) : SEQ_ACTIVE);
