...
static void print_copy_is_too_big(size_t src_size, unsigned max_size, const string& qtype) {
    cerr << get_fileline() << "Warning: queue<" << qtype << "> is bounded to have at most "
...
static void copy_elements(QTYPE* queue, SRC_TYPE* src, unsigned max_size) {
    ...
    unsigned copy_size = ((src_size < max_size) || (max_size == 0)) ? src_size : max_size;
...
void vvp_queue_real::set_word_max(unsigned adr, double value, unsigned max_size) {
    if (adr == queue.size())
        if (!max_size || (queue.size() < max_size))
            ...
            cerr << get_fileline() << "Warning: assigning to queue<real>[" << adr
                 << "] is"
                    " outside bound ("
                 << max_size << "). " << value << " was not added." << endl;
    else
        set_word(adr, value);
}
...
void vvp_queue_real::set_word(unsigned adr, double value) {
    if (adr < queue.size())
        ...
        cerr << get_fileline() << "Warning: assigning to queue<real>[" << adr
             << "] is outside "
                "of size ("
             << queue.size() << "). " << value << " was not added." << endl;
}
...
void vvp_queue_real::insert(unsigned idx, double value, unsigned max_size) {
    ...
    if (idx > queue.size())
        cerr << get_fileline() << "Warning: inserting to queue<real>[" << idx
             << "] is "
                "outside of size ("
             << queue.size() << "). " << value << " was not added." << endl;
    // Inserting at the end
        ...
        if (max_size && (queue.size() == max_size)) {
            cerr << get_fileline() << "Warning: insert(" << idx << ", " << value << ") removed "
                 << queue.back() << " from already full bounded queue<real> [" << max_size << "]."
                 << endl;
            queue.pop_back();
        }
...
void vvp_queue_real::push_back(double value, unsigned max_size) {
    if (!max_size || (queue.size() < max_size))
        ...
        cerr << get_fileline() << "Warning: push_back(" << value
             << ") skipped for already full bounded queue<real> [" << max_size << "]." << endl;
}

void vvp_queue_real::push_front(double value, unsigned max_size) {
    if (max_size && (queue.size() == max_size)) {
        cerr << get_fileline() << "Warning: push_front(" << value << ") removed " << queue.back()
             << " from already full bounded queue<real> [" << max_size << "]." << endl;
        queue.pop_back();
    }
...
void vvp_queue_string::set_word_max(unsigned adr, const string& value, unsigned max_size) {
    if (adr == queue.size())
        if (!max_size || (queue.size() < max_size))
            ...
            cerr << get_fileline() << "Warning: assigning to queue<string>[" << adr
                 << "] is"
                    " outside bound ("
                 << max_size << "). \"" << value << "\" was not added." << endl;
    else
...
void vvp_queue_string::insert(unsigned idx, const string& value, unsigned max_size) {
    // Inserting past the end of the queue
    if (idx > queue.size())
        cerr << get_fileline() << "Warning: inserting to queue<string>[" << idx
             << "] is "
                "outside of size ("
             << queue.size() << "). \"" << value << "\" was not added." << endl;
    // Inserting at the end
        ...
        if (max_size && (queue.size() == max_size)) {
            cerr << get_fileline() << "Warning: insert(" << idx << ", \"" << value
                 << "\") removed \"" << queue.back()
                 << "\" from already full bounded queue<string> [" << max_size << "]." << endl;
            queue.pop_back();
        }
        queue.insert(queue.begin() + idx, value);
    }
...
void vvp_queue_string::push_back(const string& value, unsigned max_size) {
    if (!max_size || (queue.size() < max_size))
        ...
        cerr << get_fileline() << "Warning: push_back(\"" << value
             << "\") skipped for already full bounded queue<string> [" << max_size << "]." << endl;
}

void vvp_queue_string::push_front(const string& value, unsigned max_size) {
    if (max_size && (queue.size() == max_size)) {
        cerr << get_fileline() << "Warning: push_front(\"" << value << "\") removed \""
             << queue.back() << "\" from already full bounded queue<string> [" << max_size << "]."
             << endl;
        queue.pop_back();
    }
    queue.push_front(value);
...
void vvp_queue_vec4::set_word_max(unsigned adr, const vvp_vector4_t& value, unsigned max_size) {
    if (adr == queue.size())
        if (!max_size || (queue.size() < max_size))
            ...
            cerr << get_fileline() << "Warning: assigning to queue<vector>[" << adr
                 << "] is"
                    " outside bound ("
                 << max_size << "). " << value << " was not added." << endl;
    else
...
void vvp_queue_vec4::insert(unsigned idx, const vvp_vector4_t& value, unsigned max_size) {
    // Inserting past the end of the queue
    if (idx > queue.size())
        cerr << get_fileline() << "Warning: inserting to queue<vector[" << value.size() << "]>["
             << idx << "] is outside of size (" << queue.size() << "). " << value
             << " was not added." << endl;
    // Inserting at the end
    else if (idx == queue.size())
        if (!max_size || (queue.size() < max_size))
            ...
            cerr << get_fileline() << "Warning: inserting to queue<vector[" << value.size() << "]>["
                 << idx << "] is outside bound (" << max_size << "). " << value << " was not added."
                 << endl;
    else {
        if (max_size && (queue.size() == max_size)) {
            cerr << get_fileline() << "Warning: insert(" << idx << ", " << value << ") removed "
                 << queue.back() << " from already full bounded queue<vector[" << value.size()
                 << "]> [" << max_size << "]." << endl;
            queue.pop_back();
        }
        queue.insert(queue.begin() + idx, value);
...
void vvp_queue_vec4::push_back(const vvp_vector4_t& value, unsigned max_size) {
    if (!max_size || (queue.size() < max_size))
        ...
        cerr << get_fileline() << "Warning: push_back(" << value
             << ") skipped for already full bounded queue<vector[" << value.size() << "]> ["
             << max_size << "]." << endl;
}
...
void vvp_queue_vec4::push_front(const vvp_vector4_t& value, unsigned max_size) {
    if (max_size && (queue.size() == max_size)) {
        cerr << get_fileline() << "Warning: push_front(" << value << ") removed " << queue.back()
             << " from already full bounded queue<vector[" << value.size() << "]> [" << max_size
             << "]." << endl;
        queue.pop_back();
    }
    queue.push_front(value);
