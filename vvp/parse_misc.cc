...
void symbv_add(struct symbv_s* obj, struct symb_s item) {
    obj->vect =
        static_cast<struct symb_s*>(realloc(obj->vect, (obj->cnt + 1) * sizeof(struct symb_s)));
...
void argv_add(struct argv_s* obj, vpiHandle item) {
    obj->argv = static_cast<vpiHandle*>(realloc(obj->argv, (obj->argc + 1) * sizeof(vpiHandle)));
...
void argv_sym_add(struct argv_s* obj, char* item) {
    argv_add(obj, 0x0);
    obj->syms = static_cast<char**>(realloc(obj->syms, (obj->argc) * sizeof(char*)));
