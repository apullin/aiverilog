const char COPYRIGHT[] = "Copyright (c) 2001-2025 Stephen Williams (steve@icarus.com)";
/*
...
static void print_rusage(struct rusage* a, struct rusage* b) {
    double delta = a->ru_utime.tv_sec + a->ru_utime.tv_usec / 1E6 + a->ru_stime.tv_sec +
                   a->ru_stime.tv_usec / 1E6 - b->ru_utime.tv_sec - b->ru_utime.tv_usec / 1E6 -
...
static void final_cleanup() {
    ...
    for (vector<const char*>::iterator cur = file_names.begin(); cur != file_names.end(); ++cur) {
        delete[] *cur;
    }
...
static void report_used(void) {
    fprintf(stderr, "This VVP simulation has already run and can not be reused\n");
}
...
int vvp_run(const char* design_path) {
    ...
    if (verbose_flag) {
        vpi_mcd_printf(1,
                       " ... %8lu functors (net_fun pool=%zu bytes)\n",
                       count_functors,
        ...
        vpi_mcd_printf(1,
                       " ... %8lu filters (net_fil pool=%zu bytes)\n",
                       count_filters,
