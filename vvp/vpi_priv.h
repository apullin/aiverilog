...
extern vpiHandle vpip_build_file_line(const char* description, long file_idx, long lineno);
...
extern vpiHandle vpip_make_iterator(unsigned nargs, vpiHandle* args, bool free_args_flag);
...
class __vpiScope : public __vpiHandle {
  ...
  public:
    ...
    inline const char* scope_name() const {
        return name_;
    }

    inline const char* scope_def_name() const {
        return tname_;
    }
...
class vpiScopeFunction : public __vpiScope {
  public:
    inline vpiScopeFunction(const char* nam,
                            const char* tnam,
                            bool auto_flag,
                            int func_type,
                            unsigned func_wid,
                            vvp_bit4_t func_init_val)
        : __vpiScope(nam, tnam, auto_flag),
          func_type_(func_type),
          func_wid_(func_wid),
          func_init_val_(func_init_val) {}
  ...
  public:
    inline unsigned get_func_width(void) const {
        return func_wid_;
    }
    inline vvp_bit4_t get_func_init_val(void) const {
        return func_init_val_;
    }
...
extern unsigned vpip_add_item_to_context(automatic_hooks_s* item, __vpiScope* scope);
extern vpiHandle vpip_make_root_iterator(int type_code);
extern void vpip_make_root_iterator(class __vpiHandle**& table, unsigned& ntable);
...
extern vpiHandle
vpip_make_int2(const char* name, int msb, int lsb, bool signed_flag, vvp_net_t* vec);
extern vpiHandle vpip_make_int4(const char* name, int msb, int lsb, vvp_net_t* vec);
extern vpiHandle
vpip_make_var4(const char* name, int msb, int lsb, bool signed_flag, vvp_net_t* net);
extern vpiHandle vpip_make_net4(
    __vpiScope* scope, const char* name, int msb, int lsb, bool signed_flag, vvp_net_t* node);
...
struct __vpiBit {
    ...
    inline unsigned get_norm_index() const {
        return this - bit0;
    }
    inline struct __vpiSignal* get_parent() const {
        return (bit0 - 1)->parent;
    }
...
class vpiPortInfo : public __vpiHandle {
  public:
    ...
    void add_port_bit(vpiPortBitInfo* port_bit) {
        port_bits_.push_back(port_bit);
    }

    int vpi_get(int code) override;
    char* vpi_get_str(int code) override;
    vpiHandle vpi_handle(int code) override;
    vvp_net_t* get_port(void) const {
        return ref_;
    }
    vpiHandle vpi_iterate(int code) override;
...
extern struct __vpiModPathSrc* vpip_make_modpath_src(struct __vpiModPath* path, vvp_net_t* net);
...
extern struct __vpiInterModPath*
...
class __vpiNamedEvent : public __vpiHandle {
  public:
    ...
    int get_type_code(void) const override;
    __vpiScope* get_scope(void) const {
        return scope_;
    }
    int vpi_get(int code) override;
    char* vpi_get_str(int code) override;
...
extern vpiHandle vpip_make_real_net(__vpiScope* scope, const char* name, vvp_net_t* net);
...
class __vpiBaseVar : public __vpiHandle {
  public:
    ...
    inline vvp_net_t* get_net() const {
        return net_;
    }
...
struct __vpiArrayBase {
    ...
    virtual void put_word_value(struct __vpiArrayWord* word, p_vpi_value vp, int flags) = 0;
...
struct __vpiArray : public __vpiArrayBase, public __vpiHandle {
    ...
    char* vpi_get_str(int code) override;
    vpiHandle vpi_handle(int code) override;
    inline vpiHandle vpi_iterate(int code) override {
  ...
  private:
    ...
    friend vpiHandle vpip_make_array(
        const char* label, const char* name, int first_addr, int last_addr, bool signed_flag);
...
vpiHandle
vpip_make_string_param(const char* name, char* value, bool local_flag, long file_idx, long lineno);
...
vpiHandle vpip_make_binary_param(const char* name,
                                 const vvp_vector4_t& bits,
                                 bool signed_flag,
                                 bool local_flag,
                                 long file_idx,
                                 long lineno);
...
vpiHandle
vpip_make_real_param(const char* name, double value, bool local_flag, long file_idx, long lineno);
 ...
 */
extern unsigned
vpip_vec4_to_dec_str(const vvp_vector4_t& vec4, char* buf, unsigned int nbuf, int signed_flag);


extern void vpip_vec4_to_hex_str(const vvp_vector4_t& bits, char* buf, unsigned nbuf);

extern void vpip_vec4_to_oct_str(const vvp_vector4_t& bits, char* buf, unsigned nbuf);
...
extern void vpip_vec4_get_value(const vvp_vector4_t& word_val,
                                unsigned width,
                                bool signed_flag,
                                s_vpi_value* vp);
extern void vpip_vec2_get_value(const vvp_vector2_t& word_val,
                                unsigned width,
                                bool signed_flag,
                                s_vpi_value* vp);
