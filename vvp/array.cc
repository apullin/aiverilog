...
void __vpiArray::get_word_value(struct __vpiArrayWord* word, p_vpi_value vp) {
    ...
    if (vals4) {
        vpip_vec4_get_value(vals4->get_word(index), vals4->width(), signed_flag, vp);
    } else if (vals) {
        switch (vp->format) {
            case vpiBinStrVal:
            case vpiOctStrVal:
        ...
        if (part_off != 0 || val.size() != vals_width) {
            ...
            if ((part_off + val.size()) > tmp.size()) {
                cerr << "part_off=" << part_off << " val.size()=" << val.size()
                     << " vals[address].size()=" << tmp.size() << " vals_width=" << vals_width
                     << endl;
                assert(0);
            }
...
void compile_var_array(
    char* label, char* name, int last, int first, int msb, int lsb, char signed_flag) {
...
void vvp_fun_arrayport_sa::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
...
class vvp_fun_arrayport_aa : public vvp_fun_arrayport, public automatic_hooks_s {
  public:
    ...
    explicit vvp_fun_arrayport_aa(__vpiScope* context_scope,
                                  vvp_array_t mem,
                                  vvp_net_t* net,
                                  long addr);
...
vvp_fun_arrayport_aa::vvp_fun_arrayport_aa(__vpiScope* context_scope,
                                           vvp_array_t mem,
                                           vvp_net_t* net)
...
void vvp_fun_arrayport_aa::reset_instance(vvp_context_t context) {
    unsigned long* addr = static_cast<unsigned long*>(vvp_get_context_item(context, context_idx_));
...
void vvp_fun_arrayport_aa::check_word_change_(unsigned long addr, vvp_context_t context) {
    const unsigned long* port_addr =
        static_cast<unsigned long*>(vvp_get_context_item(context, context_idx_));
...
static void array_attach_port(vvp_array_t array, vvp_fun_arrayport* fun) {
    ...
    array->ports_ = fun;
    if (!array->get_scope()->is_automatic() && (array->vals4 || array->vals)) {
            ...
            if (cur->test_value_callback_ready()) {
                if (cur->cb_data.value) {
                    if (vpi_array_is_real(this)) {
                        ...
                        vpip_vec4_get_value(
                            vals4->get_word(addr), vals_width, signed_flag, cur->cb_data.value);
                        ...
                        if (addr < vals->get_size())
                            vals->get_word(addr, val);
                        vpip_vec4_get_value(val, vals_width, signed_flag, cur->cb_data.value);
                    } else {
                        assert(0);
                    }
                }
...
array_port_resolv_list_t::array_port_resolv_list_t(char* lab, bool use_addr__, long addr__)
...
void compile_array_port(char* label, char* array, char* addr) {
    array_port_resolv_list_t* resolv_mem = new array_port_resolv_list_t(array, false, 0);
...
void compile_array_port(char* label, char* array, long addr) {
    array_port_resolv_list_t* resolv_mem = new array_port_resolv_list_t(array, true, addr);
...
void memory_delete(vpiHandle item) {
    ...
    if (arr->nets) {
        for (unsigned idx = 0; idx < arr->get_size(); idx += 1) {
            if (struct __vpiSignal* sig = dynamic_cast<__vpiSignal*>(arr->nets[idx])) {
                // Delete the individual words?
                constant_delete(sig->id.index);
                /* These should only be the real words. */
            } else {
                assert(arr->nets[idx]->get_type_code() == vpiRealVar);
                struct __vpiRealVar* sigr = static_cast<struct __vpiRealVar*>(arr->nets[idx]);
