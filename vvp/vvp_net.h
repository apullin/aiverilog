...
inline vvp_context_item_t vvp_get_context_item(vvp_context_t context, unsigned item_idx) {
    return (vvp_context_item_t)context[item_idx];
}

inline void
vvp_set_context_item(vvp_context_t context, unsigned item_idx, vvp_context_item_t item) {
...
inline char vvp_bit4_to_ascii(vvp_bit4_t a) {
    return "01zx"[a];
}
...
inline bool bit4_is_xz(vvp_bit4_t a) {
    return a >= 2;
}
 ...
 */
static inline unsigned long add_with_carry(unsigned long a, unsigned long b, unsigned long& carry) {
...
extern unsigned long multiply_with_carry(unsigned long a, unsigned long b, unsigned long& carry);
...
class vvp_vector4_t {
  ...
  private:
    ...
    enum {
        WORD_0_ABITS = 0x00000000UL,
        WORD_0_BBITS = 0x00000000UL
    };
    enum {
        WORD_1_ABITS = 0xFFFFFFFFUL,
        WORD_1_BBITS = 0x00000000UL
    };
    enum {
        WORD_X_ABITS = 0xFFFFFFFFUL,
        WORD_X_BBITS = 0xFFFFFFFFUL
    };
    enum {
        WORD_Z_ABITS = 0x00000000UL,
        WORD_Z_BBITS = 0xFFFFFFFFUL
    };
#else
...
inline vvp_vector4_t::vvp_vector4_t(unsigned size__, vvp_bit4_t val) : size_(size__) {
...
extern vvp_bit4_t
compare_gtge(const vvp_vector4_t& a, const vvp_vector4_t& b, vvp_bit4_t val_if_equal);
extern vvp_bit4_t
compare_gtge_signed(const vvp_vector4_t& a, const vvp_vector4_t& b, vvp_bit4_t val_if_equal);
...
template <class T>
extern bool
vector4_to_value(const vvp_vector4_t& a, T& val, bool is_signed, bool is_arithmetic = true);

template <class T>
extern bool vector4_to_value(const vvp_vector4_t& a, bool& overflow_flag, T& val);
...
class vvp_scalar_t {
  ...
  public:
    ...
    bool eeq(vvp_scalar_t that) const {
        return value_ == that.value_;
    }
    bool is_hiz() const {
        return (value_ & 0x77) == 0;
    }
...
class vvp_vector8_t {
  ...
  public:
    ...
    // strength.
    explicit vvp_vector8_t(const vvp_vector4_t& that, unsigned str0, unsigned str1);
    explicit vvp_vector8_t(const vvp_vector2_t& that, unsigned str0, unsigned str1);
...
template <class T> class vvp_sub_pointer_t {
  public:
    ...
    bool operator==(vvp_sub_pointer_t that) const {
...
class vvp_net_t {
  ...
  public:  // Methods to propagate output from this node.
    ...
    void
    send_vec4_pv(const vvp_vector4_t& val, unsigned base, unsigned vwid, vvp_context_t context);
    void send_vec8_pv(const vvp_vector8_t& val, unsigned base, unsigned vwid);
...
class vvp_net_fun_t {
  public:
    ...
    virtual void recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t context);
    ...
    virtual void recv_string(vvp_net_ptr_t port, const std::string& bit, vvp_context_t context);
    virtual void recv_object(vvp_net_ptr_t port, vvp_object_t bit, vvp_context_t context);

    // Part select variants of above
    virtual void recv_vec4_pv(vvp_net_ptr_t p,
                              const vvp_vector4_t& bit,
                              unsigned base,
                              unsigned vwid,
                              vvp_context_t context);
    virtual void
    recv_vec8_pv(vvp_net_ptr_t p, const vvp_vector8_t& bit, unsigned base, unsigned vwid);
  ...
  protected:
    void recv_vec4_pv_(vvp_net_ptr_t p,
                       const vvp_vector4_t& bit,
                       unsigned base,
                       unsigned vwid,
                       vvp_context_t context);
    void recv_vec8_pv_(vvp_net_ptr_t p, const vvp_vector8_t& bit, unsigned base, unsigned vwid);
  ...
  public:  // These objects are only permallocated.
    ...
    static std::size_t heap_total() {
        return heap_.heap_total();
    }
...
class vvp_net_fil_t : public vvp_vpi_callback {
  ...
  public:
    ...
    virtual prop_t
    filter_vec4(const vvp_vector4_t& bit, vvp_vector4_t& rep, unsigned base, unsigned vwid);
    virtual prop_t
    filter_vec8(const vvp_vector8_t& val, vvp_vector8_t& rep, unsigned base, unsigned vwid);
...
class vvp_wide_fun_t : public vvp_net_fun_t {
  public:
    ...
    void recv_vec4_pv(vvp_net_ptr_t p,
                      const vvp_vector4_t& bit,
                      unsigned base,
                      unsigned vwid,
 ...
 */
inline void vvp_send_vec4_pv(vvp_net_ptr_t ptr,
...
inline void
vvp_send_vec8_pv(vvp_net_ptr_t ptr, const vvp_vector8_t& val, unsigned base, unsigned vwid) {
    while (class vvp_net_t* cur = ptr.ptr()) {
        vvp_net_ptr_t next_val = cur->port[ptr.port()];
...
inline void vvp_net_t::send_vec8_pv(const vvp_vector8_t& val, unsigned base, unsigned vwid) {
    if (fil == 0) {
