...
void vvp_fun_pmos_::recv_vec4(vvp_net_ptr_t ptr, const vvp_vector4_t& bit, vvp_context_t) {
...
void vvp_fun_pmos_::recv_vec4_pv(
...
void vvp_fun_pmos_::recv_vec8_pv(vvp_net_ptr_t ptr,
...
void vvp_fun_pmos_::recv_real(vvp_net_ptr_t ptr, double real, vvp_context_t ctx) {
...
void vvp_fun_pmos_::generate_output_(vvp_net_ptr_t ptr) {
    ...
    for (unsigned idx = 0; idx < out.size(); idx += 1) {
        ...
        b_bit = vvp_scalar_t(
            b_bit.value(), strength_map[b_bit.strength0()], strength_map[b_bit.strength1()]);
...
vvp_fun_pmos::vvp_fun_pmos(bool enable_invert) : vvp_fun_pmos_(enable_invert, false) {}
...
vvp_fun_rpmos::vvp_fun_rpmos(bool enable_invert) : vvp_fun_pmos_(enable_invert, true) {}
...
void vvp_fun_cmos_::recv_vec4(vvp_net_ptr_t ptr, const vvp_vector4_t& bit, vvp_context_t) {
...
void vvp_fun_cmos_::recv_vec4_pv(
    vvp_net_ptr_t ptr, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t ctx) {
    recv_vec4_pv_(ptr, bit, base, vwid, ctx);
}

void vvp_fun_cmos_::recv_vec8_pv(vvp_net_ptr_t ptr,
                                 const vvp_vector8_t& bit,
                                 unsigned base,
                                 unsigned vwid) {
    recv_vec8_pv_(ptr, bit, base, vwid);
...
void vvp_fun_cmos_::recv_real(vvp_net_ptr_t ptr, double real, vvp_context_t ctx) {
    recv_vec4(ptr, double_to_vector4_LSB(real), ctx);
}
