 ...
 */
static inline int write_digits(unsigned long v, char** buf, unsigned int* nbuf, int zero_suppress) {
...
#define ALLOC_MARGIN 4
    if (!valv || vlen > vlen_alloc) {
        if (valv)
            free(valv);
        valv = static_cast<unsigned long*>(calloc(vlen + ALLOC_MARGIN, sizeof(*valv)));
