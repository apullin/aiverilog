...
static bool check_callback_time(p_cb_data data, bool allow_suppress) {
    ...
    if (!data->time) {
        if (!allow_suppress) {
            fprintf(stderr,
                    "VPI error: null value passed in cb_data.time "
    ...
    switch (data->time->type) {
        ...
        case vpiSuppressTime:
            if (!allow_suppress) {
                fprintf(stderr,
                        "VPI error: vpiSuppressTime is not valid "
                        "when registering %s callback\n.",
                        cb_reason_name(data->reason));
                return false;
            }
        ...
        default:
            fprintf(stderr,
                    "VPI error: invalid type passed in cb_data time "
...
inline value_part_callback::value_part_callback(p_cb_data data) : value_callback(data) {
...
static vvp_time64_t get_sync_cb_time(sync_callback* obj) {
    ...
    switch (obj->cb_time.type) {
        ...
        default:
            fprintf(
                stderr, "get_sync_cb_time: Unsupported time type %d.\n", (int)obj->cb_time.type);
            assert(0);
            break;
    }
    return tv;
