...
static void delete_sub_scopes(__vpiScope* scope) {
    ...
    for (citer = scope->classes.begin(); citer != scope->classes.end(); ++citer) {
        class_list_count += 1;
        class_list =
            static_cast<class_type**>(realloc(class_list, class_list_count * sizeof(class_type**)));
...
static void construct_scope_fullname(__vpiScope* ref, char* buf) {
    ...
    if (ref->get_type_code() == vpiPackage)
        strcat(buf, "::");
}
...
static int compare_types(int code, int type) {
    ...
    if (code == vpiInternalScope &&
        (type == vpiModule || type == vpiGenScope || type == vpiFunction || type == vpiTask ||
         type == vpiNamedBegin || type == vpiNamedFork))
        return 1;

    if (code == vpiInstance &&
        (type == vpiModule || type == vpiProgram || type == vpiInterface || type == vpiPackage))
        return 1;

    if (code == vpiVariables && (type == vpiIntegerVar || type == vpiBitVar || type == vpiByteVar ||
                                 type == vpiShortIntVar || type == vpiIntVar ||
                                 type == vpiLongIntVar || type == vpiTimeVar || type == vpiRealVar))
        return 1;
...
class vpiScopeModule : public __vpiScope {
  public:
    inline vpiScopeModule(const char* nam, const char* tnam) : __vpiScope(nam, tnam, false) {}
...
struct vpiScopePackage : public __vpiScope {
    inline vpiScopePackage(const char* nam, const char* tnam) : __vpiScope(nam, tnam) {}
    int get_type_code(void) const override {
        return vpiPackage;
    }
};
...
struct vpiScopeTask : public __vpiScope {
    inline vpiScopeTask(const char* nam, const char* tnam) : __vpiScope(nam, tnam) {}
    int get_type_code(void) const override {
        return vpiTask;
    }
};
...
struct vpiScopeTaskAuto : public __vpiScope {
    inline vpiScopeTaskAuto(const char* nam, const char* tnam) : __vpiScope(nam, tnam, true) {}
    int get_type_code(void) const override {
        return vpiTask;
    }
};
...
struct vpiScopeBegin : public __vpiScope {
    inline vpiScopeBegin(const char* nam, const char* tnam) : __vpiScope(nam, tnam, false) {}
    int get_type_code(void) const override {
        return vpiNamedBegin;
    }
};
...
class vpiScopeBeginAuto : public __vpiScope {
  public:
    inline vpiScopeBeginAuto(const char* nam, const char* tnam) : __vpiScope(nam, tnam, true) {}
    int get_type_code(void) const override {
        return vpiNamedBegin;
    }
};
...
struct vpiScopeGenerate : public __vpiScope {
    inline vpiScopeGenerate(const char* nam, const char* tnam) : __vpiScope(nam, tnam) {}
    int get_type_code(void) const override {
        return vpiGenScope;
    }
};
...
struct vpiScopeFork : public __vpiScope {
    inline vpiScopeFork(const char* nam, const char* tnam) : __vpiScope(nam, tnam, false) {}
    int get_type_code(void) const override {
        return vpiNamedFork;
    }
};
...
class vpiScopeForkAuto : public __vpiScope {
  public:
    inline vpiScopeForkAuto(const char* nam, const char* tnam) : __vpiScope(nam, tnam, true) {}
    int get_type_code(void) const override {
        return vpiNamedFork;
    }
};
...
struct vpiScopeClass : public __vpiScope {
    inline vpiScopeClass(const char* nam, const char* tnam) : __vpiScope(nam, tnam) {}
    int get_type_code(void) const override {
 ...
 */
void compile_scope_decl(char* label,
                        char* type,
                        char* name,
                        char* tname,
                        char* parent,
...
unsigned vpip_add_item_to_context(automatic_hooks_s* item, __vpiScope* scope) {
    ...
    if (scope->item == 0)
        scope->item = static_cast<automatic_hooks_s**>(malloc(sizeof(automatic_hooks_s*)));
    else
...
vpiPortInfo::vpiPortInfo(__vpiScope* parent,
                         ...
                         const char* name,
                         char* buffer)
    : parent_(parent), index_(index), direction_(vpi_direction), width_(width), name_(name) {
    if (buffer != nullptr)
        functor_ref_lookup(&ref_, buffer);
    else
        ref_ = nullptr;
}
...
vpiPortBitInfo::vpiPortBitInfo(vpiPortInfo* parent, unsigned bit) : parent_(parent), bit_(bit) {}
 ...
 */
void compile_port_info(
    unsigned index, int vpi_direction, unsigned width, const char* name, char* buffer) {
    vpiPortInfo* obj =
        new vpiPortInfo(vpip_peek_current_scope(), index, vpi_direction, width, name, buffer);
