...
template <size_t SLAB_SIZE, size_t CHUNK_COUNT> slab_t<SLAB_SIZE, CHUNK_COUNT>::slab_t() {
    pool = CHUNK_COUNT;
...
inline void* slab_t<SLAB_SIZE, CHUNK_COUNT>::alloc_slab() {
    if (heap_ == 0) {
        ...
        slab_pool_count += 1;
        slab_pool =
            static_cast<item_cell_u**>(realloc(slab_pool, slab_pool_count * sizeof(item_cell_u**)));
