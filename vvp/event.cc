...
evctl_real::evctl_real(__vpiHandle* handle, double value, unsigned long ecount) : evctl(ecount) {
    handle_ = handle;
    value_ = value;
}
...
void schedule_evctl(__vpiHandle* handle, double value, vvp_net_t* event, unsigned long ecount) {
...
evctl_vector::evctl_vector(
    vvp_net_ptr_t ptr, const vvp_vector4_t& value, unsigned off, unsigned wid, unsigned long ecount)
...
void schedule_evctl(vvp_net_ptr_t ptr,
...
evctl_array::evctl_array(vvp_array_t memory,
                         unsigned index,
                         const vvp_vector4_t& value,
                         unsigned off,
                         unsigned long ecount)
...
void schedule_evctl(vvp_array_t memory,
                    unsigned index,
                    const vvp_vector4_t& value,
                    unsigned offset,
                    vvp_net_t* event,
...
evctl_array_r::evctl_array_r(vvp_array_t memory, unsigned index, double value, unsigned long ecount)
    : evctl(ecount) {
    mem_ = memory;
    idx_ = index;
    value_ = value;
...
const vvp_fun_edge::edge_t vvp_edge_posedge = VVP_EDGE(BIT4_0, BIT4_1) | VVP_EDGE(BIT4_0, BIT4_X) |
                                              VVP_EDGE(BIT4_0, BIT4_Z) | VVP_EDGE(BIT4_X, BIT4_1) |
                                              VVP_EDGE(BIT4_Z, BIT4_1);

const vvp_fun_edge::edge_t vvp_edge_negedge = VVP_EDGE(BIT4_1, BIT4_0) | VVP_EDGE(BIT4_1, BIT4_X) |
                                              VVP_EDGE(BIT4_1, BIT4_Z) | VVP_EDGE(BIT4_X, BIT4_0) |
                                              VVP_EDGE(BIT4_Z, BIT4_0);

const vvp_fun_edge::edge_t vvp_edge_edge = VVP_EDGE(BIT4_0, BIT4_1) | VVP_EDGE(BIT4_1, BIT4_0) |
                                           VVP_EDGE(BIT4_0, BIT4_X) | VVP_EDGE(BIT4_X, BIT4_0) |
                                           VVP_EDGE(BIT4_0, BIT4_Z) | VVP_EDGE(BIT4_Z, BIT4_0) |
                                           VVP_EDGE(BIT4_X, BIT4_1) | VVP_EDGE(BIT4_1, BIT4_X) |
                                           VVP_EDGE(BIT4_Z, BIT4_1) | VVP_EDGE(BIT4_1, BIT4_Z);
...
bool vvp_fun_edge::recv_vec4_(const vvp_vector4_t& bit, vvp_bit4_t& old_bit, vthread_t& threads) {
...
void vvp_fun_edge_sa::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
...
void vvp_fun_edge_sa::recv_vec4_pv(
    vvp_net_ptr_t port, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t) {
    assert(base == 0);
    if (recv_vec4_(bit, bits_[port.port()], threads_)) {
        vvp_net_t* net = port.ptr();
...
void vvp_fun_edge_aa::reset_instance(vvp_context_t context) {
    vvp_fun_edge_state_s* state =
        static_cast<vvp_fun_edge_state_s*>(vvp_get_context_item(context, context_idx_));
...
vthread_t vvp_fun_edge_aa::add_waiting_thread(vthread_t thread) {
    vvp_fun_edge_state_s* state =
        static_cast<vvp_fun_edge_state_s*>(vthread_get_wt_context_item(context_idx_));
...
void vvp_fun_edge_aa::recv_vec4(vvp_net_ptr_t port,
...
class anyedge_vec4_value : public anyedge_value {
  public:
    ...
    void set(const vvp_vector4_t& bit) {
        old_bits = bit;
    };
    ...
    bool recv_vec4_pv(const vvp_vector4_t& bit, unsigned base, unsigned vwid);
...
bool anyedge_vec4_value::recv_vec4_pv(const vvp_vector4_t& bit, unsigned base, unsigned vwid) {
...
void vvp_fun_anyedge_sa::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
...
void vvp_fun_anyedge_sa::recv_vec4_pv(
    vvp_net_ptr_t port, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t) {
    anyedge_vec4_value* value = get_vec4_value(last_value_[port.port()]);
    assert(value);
...
void vvp_fun_anyedge_sa::recv_real(vvp_net_ptr_t port, double bit, vvp_context_t) {
...
void vvp_fun_anyedge_sa::recv_string(vvp_net_ptr_t port, const std::string& bit, vvp_context_t) {
 ...
 */
void vvp_fun_anyedge_sa::recv_object(vvp_net_ptr_t port, vvp_object_t, vvp_context_t) {
    run_waiting_threads_(threads_);
    vvp_net_t* net = port.ptr();
    net->send_vec4(vvp_vector4_t(), 0);
}
...
void vvp_fun_anyedge_aa::reset_instance(vvp_context_t context) {
    vvp_fun_anyedge_state_s* state =
        static_cast<vvp_fun_anyedge_state_s*>(vvp_get_context_item(context, context_idx_));
...
vthread_t vvp_fun_anyedge_aa::add_waiting_thread(vthread_t thread) {
    vvp_fun_anyedge_state_s* state =
        static_cast<vvp_fun_anyedge_state_s*>(vthread_get_wt_context_item(context_idx_));
...
void vvp_fun_anyedge_aa::recv_vec4(vvp_net_ptr_t port,
                                   const vvp_vector4_t& bit,
                                   vvp_context_t context) {
    if (context) {
        vvp_fun_anyedge_state_s* state =
...
void vvp_fun_anyedge_aa::recv_real(vvp_net_ptr_t port, double bit, vvp_context_t context) {
    if (context) {
        vvp_fun_anyedge_state_s* state =
            static_cast<vvp_fun_anyedge_state_s*>(vvp_get_context_item(context, context_idx_));
...
void vvp_fun_anyedge_aa::recv_string(vvp_net_ptr_t port,
...
vvp_fun_event_or::vvp_fun_event_or(vvp_net_t* base_net) : base_net_(base_net) {}
...
void vvp_fun_event_or_sa::recv_vec4(vvp_net_ptr_t, const vvp_vector4_t& bit, vvp_context_t) {
    run_waiting_threads_(threads_);
    base_net_->send_vec4(bit, 0);
}

vvp_fun_event_or_aa::vvp_fun_event_or_aa(vvp_net_t* base_net) : vvp_fun_event_or(base_net) {
    context_scope_ = vpip_peek_context_scope();
    context_idx_ = vpip_add_item_to_context(this, context_scope_);
}
...
void vvp_fun_event_or_aa::reset_instance(vvp_context_t context) {
    waitable_state_s* state =
        static_cast<waitable_state_s*>(vvp_get_context_item(context, context_idx_));
...
vvp_named_event_sa::vvp_named_event_sa(__vpiHandle* h) : vvp_named_event(h), threads_(0) {}
...
void vvp_named_event_sa::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    run_waiting_threads_(threads_);
    vvp_net_t* net = port.ptr();
    net->send_vec4(bit, 0);
...
void vvp_named_event_aa::recv_vec4(vvp_net_ptr_t port,
                                   const vvp_vector4_t& bit,
                                   vvp_context_t context) {
    assert(context);
