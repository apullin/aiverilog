    ...
    switch (vp->format) {
        ...
        case vpiVectorVal:
            vp->value.vector = static_cast<p_vpi_vecval>(
                need_result_buf((size + 3) / 4 * sizeof(s_vpi_vecval), RBUF_VAL));
            uint_value = 0;
...
class __vpiStringParam : public __vpiStringConst {
  public:
    __vpiStringParam(__vpiScope* scope,
                     char* txt,
                     const char* name,
                     bool local_flag,
                     long file_idx,
...
inline __vpiStringParam::__vpiStringParam(
    __vpiScope* scope, char* txt, const char* nam, bool local_flag, long file_idx, long lineno)
...
vpiHandle
vpip_make_string_param(const char* name, char* text, bool local_flag, long file_idx, long lineno) {
    __vpiStringParam* obj =
        new __vpiStringParam(vpip_peek_current_scope(), text, name, local_flag, file_idx, lineno);

    return obj;
}


inline __vpiBinaryConst::__vpiBinaryConst() : signed_flag(0), sized_flag(0) {}
...
struct __vpiBinaryParam : public __vpiBinaryConst {
    __vpiBinaryParam(__vpiScope* scope,
...
inline __vpiBinaryParam::__vpiBinaryParam(__vpiScope* scope,
                                          const vvp_vector4_t& b,
                                          bool is_signed,
                                          const char* nam,
                                          bool local_flag,
...
vpiHandle vpip_make_binary_param(const char* name,
    ...
    struct __vpiBinaryParam* obj = new __vpiBinaryParam(
        vpip_peek_current_scope(), bits, signed_flag, name, local_flag, file_idx, lineno);
...
__vpiDecConst::__vpiDecConst(const __vpiDecConst& that) : __vpiHandle(), value(that.value) {}
...
struct __vpiRealParam : public __vpiRealConst {
    __vpiRealParam(__vpiScope* scope,
                   double val,
                   const char* name,
                   bool local_flag,
                   long file_idx,
...
inline __vpiRealParam::__vpiRealParam(
    __vpiScope* scope, double val, const char* name, bool local_flag, long file_idx, long lineno)
    : __vpiRealConst(val) {
    scope_ = scope;
    basename_ = name;
    local_flag_ = local_flag;
...
vpip_make_real_param(const char* name, double value, bool local_flag, long file_idx, long lineno) {
    struct __vpiRealParam* obj =
        new __vpiRealParam(vpip_peek_current_scope(), value, name, local_flag, file_idx, lineno);
    return obj;
}
...
void __vpiNullConst::vpi_get_value(p_vpi_value val) {
    ...
    switch (val->format) {
        ...
        case vpiVectorVal:
            val->value.vector =
                static_cast<p_vpi_vecval>(need_result_buf(2 * sizeof(s_vpi_vecval), RBUF_VAL));
