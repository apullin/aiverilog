...
__vpiScope* vpip_scope(__vpiSignal* sig) {
    if (sig->is_netarray)
        return static_cast<__vpiScope*>(vpi_handle(vpiScope, sig->within.parent));
    else
    ...
    if (property == vpiType) {
        if (vpi_trace) {
            fprintf(vpi_trace,
                    "vpi_get(vpiType, %p) --> %s\n",
                    ref,
                    vpi_type_as_string(ref->get_type_code()));
        }
...
__vpiScope* vpip_timescale_scope_from_handle(vpiHandle obj) {
    ...
    switch (obj->get_type_code()) {
        ...
        default:
            fprintf(stderr,
                    "ERROR: vpip_scope_from_handle called with "
...
double vpip_scaled_time_from_handle(vvp_time64_t time, vpiHandle obj) {
    int scale = vpip_get_time_precision() - vpip_time_units_from_handle(obj);
    if (scale >= 0)
...
static void vec4_get_value_string(const vvp_vector4_t& word_val, unsigned width, s_vpi_value* vp) {
 ...
 */
void vpip_vec4_get_value(const vvp_vector4_t& word_val,
                         unsigned width,
                         bool signed_flag,
                         s_vpi_value* vp) {
    char* rbuf = 0;
...
void vpip_vec2_get_value(const vvp_vector2_t& word_val,
                         unsigned width,
                         bool signed_flag,
                         s_vpi_value* vp) {
    switch (vp->format) {
...
void vpi_get_value(vpiHandle expr, s_vpi_value* vp) {
    ...
    if (vpi_trace)
        switch (vp->format) {
            ...
            case vpiIntVal:
                fprintf(vpi_trace,
                        "vpi_get_value(<%d>...) -> int=%d\n",
                        expr->get_type_code(),
                        (int)vp->value.integer);
                break;
...
vpiHandle vpi_put_value(vpiHandle obj, s_vpi_value* vp, s_vpi_time* when, PLI_INT32 flags) {
    assert(obj);
    ...
    if (flags != vpiNoDelay && flags != vpiForceFlag && flags != vpiReleaseFlag) {
        ...
        switch (put->value.format) {
            ...
            case vpiVectorVal:
                put->value.value.vector = vectordup(put->value.value.vector, vpi_get(vpiSize, obj));
                break;
...
static vpiHandle find_name(const char* name, vpiHandle handle) {
    ...
    for (unsigned i = 0; i < ref->intern.size(); i += 1) {
        ...
        if (vpi_get(vpiType, ref->intern[i]) == vpiPort)
            continue;
...
static vpiHandle find_scope(const char* name, vpiHandle handle, int depth) {
    vpiHandle iter =
        handle == 0 ? vpi_iterate(vpiModule, NULL) : vpi_iterate(vpiInternalScope, handle);
...
bool check_connected_to_concat8(vvp_net_t* current_net, vvp_net_t* net2) {
    if (!dynamic_cast<vvp_fun_concat8*>(current_net->fun))
        return false;
...
vpiHandle vpi_handle_multi(PLI_INT32 type, vpiHandle ref1, vpiHandle ref2) {
    ...
    if (type != vpiInterModPath) {
        fprintf(stderr,
                "sorry: vpi_handle_multi currently supports"
    ...
    if (!port1) {
        fprintf(stderr,
                "sorry: second argument of vpi_handle_multi"
                "must be a vpiPort\n");
        return nullptr;
    }
    ...
    if (!port2) {
        fprintf(stderr,
                "sorry: third argument of vpi_handle_multi"
                "must be a vpiPort\n");
        return nullptr;
    }
    ...
    if (net1 == net2) {
        fprintf(stderr,
                "Error: Net for both ports is the same. "
    ...
    while (cur.ptr()) {
        ...
        if ((!port2_has_index && cur.ptr() == net2) ||
            ...
            if (prev.ptr()) {
                prev.ptr()->port[prev.port()] =
                    vvp_net_ptr_t(new_net, 0);  // Point to port 0 of vvp_fun_intermodpath
                new_net->port[0] = cur.ptr()->port[cur.port()];  // Connect the next net in list
                cur.ptr()->port[cur.port()] =
                    vvp_net_ptr_t(nullptr, 0);  // Only port2 is connected to intermodpath
    ...
    fprintf(stderr,
            "\tport1 = %s, port1_has_index = %d, port1_bit_index = %d\n",
            port1_name.c_str(),
            port1_has_index,
            port1_bit_index);
    fprintf(stderr,
            "\tport2 = %s, port2_has_index = %d, port2_bit_index = %d\n",
            port2_name.c_str(),
            port2_has_index,
            port2_bit_index);
    return nullptr;
 ...
 */
extern "C" void vpip_count_drivers(vpiHandle ref, unsigned idx, unsigned counts[4]) {
