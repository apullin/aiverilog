...
void* vvp_net_t::operator new(size_t size) {
    ...
    if (vvp_net_alloc_remaining == 0) {
        ...
        vvp_net_pool = static_cast<vvp_net_t**>(
            realloc(vvp_net_pool, vvp_net_pool_count * sizeof(vvp_net_t**)));
...
#ifdef CHECK_WITH_VALGRIND
    VALGRIND_MEMPOOL_ALLOC(vvp_net_pool[vvp_net_pool_count - 1], return_this, size);
...
void pool_local_net(vvp_net_t* net) {
    ...
    local_net_pool = static_cast<vvp_net_t**>(
        realloc(local_net_pool, local_net_pool_count * sizeof(vvp_net_t**)));
...
vvp_net_fil_t::prop_t
vvp_net_fil_t::filter_vec4(const vvp_vector4_t&, vvp_vector4_t&, unsigned, unsigned) {
    return PROP;
}

vvp_net_fil_t::prop_t
vvp_net_fil_t::filter_vec8(const vvp_vector8_t&, vvp_vector8_t&, unsigned, unsigned) {
    return PROP;
}
...
const edge_t vvp_edge_posedge = VVP_EDGE(BIT4_0, BIT4_1) | VVP_EDGE(BIT4_0, BIT4_X) |
                                VVP_EDGE(BIT4_0, BIT4_Z) | VVP_EDGE(BIT4_X, BIT4_1) |
                                VVP_EDGE(BIT4_Z, BIT4_1);

const edge_t vvp_edge_negedge = VVP_EDGE(BIT4_1, BIT4_0) | VVP_EDGE(BIT4_1, BIT4_X) |
...
unsigned long multiply_with_carry(unsigned long a, unsigned long b, unsigned long& carry) {
...
void vvp_vector4_t::copy_bits(const vvp_vector4_t& that) {
    ...
    if (size_ <= BITS_PER_WORD && that.size_ <= BITS_PER_WORD) {
        ...
        unsigned long mask = ((bits_to_copy < BITS_PER_WORD) ? (1UL << bits_to_copy) : (0UL)) - 1UL;
        abits_val_ &= ~mask;
        bbits_val_ &= ~mask;
...
vvp_vector4_t::vvp_vector4_t(const vvp_vector4_t& that, unsigned adr, unsigned wid) {
...
void vvp_vector4_t::setarray(unsigned adr, unsigned wid, const unsigned long* val) {
    ...
    if (size_ <= BITS_PER_WORD) {
        ...
        unsigned long hmask = ((adr + wid) < BITS_PER_WORD) ? -1UL << (adr + wid) : 0;
        ...
        while (wid > 0) {
            ...
            unsigned long lmask = (1UL << off) - 1UL;
            unsigned long hmask = ((off + trans) < BITS_PER_WORD) ? -1UL << (off + trans) : 0;
            unsigned long mask = ~(hmask | lmask);
...
void vvp_vector4_t::fill_bits(vvp_bit4_t bit) {
    ...
    static const unsigned long init_atable[4] = {
        WORD_0_ABITS, WORD_1_ABITS, WORD_Z_ABITS, WORD_X_ABITS};
    static const unsigned long init_btable[4] = {
        WORD_0_BBITS, WORD_1_BBITS, WORD_Z_BBITS, WORD_X_BBITS};
...
vvp_vector4_t& vvp_vector4_t::operator&=(const vvp_vector4_t& that) {
    ...
    if (size_ <= BITS_PER_WORD) {
        ...
        for (unsigned idx = 0; idx < words; idx += 1) {
            ...
            bbits_ptr_[idx] = (tmp1 & that.bbits_ptr_[idx]) | (tmp2 & bbits_ptr_[idx]);
        }
    }
...
vvp_vector4_t& vvp_vector4_t::operator|=(const vvp_vector4_t& that) {
    ...
    //  10 11 01 11 11
    if (size_ <= BITS_PER_WORD) {
        unsigned long tmp = abits_val_ | bbits_val_ | that.abits_val_ | that.bbits_val_;
        ...
        for (unsigned idx = 0; idx < words; idx += 1) {
            unsigned long tmp =
                abits_ptr_[idx] | bbits_ptr_[idx] | that.abits_ptr_[idx] | that.bbits_ptr_[idx];
            bbits_ptr_[idx] = ((~abits_ptr_[idx] | bbits_ptr_[idx]) & that.bbits_ptr_[idx]) |
...
template <class INT>
bool vector4_to_value(const vvp_vector4_t& vec, INT& val, bool is_signed, bool is_arithmetic) {
    INT res = 0;
    INT msk = 1;
    bool rc_flag = true;
...
template bool
vector4_to_value(const vvp_vector4_t& vec, int8_t& val, bool is_signed, bool is_arithmetic);
template bool
vector4_to_value(const vvp_vector4_t& vec, int16_t& val, bool is_signed, bool is_arithmetic);
template bool
vector4_to_value(const vvp_vector4_t& vec, int32_t& val, bool is_signed, bool is_arithmetic);
template bool
vector4_to_value(const vvp_vector4_t& vec, int64_t& val, bool is_signed, bool is_arithmetic);
template bool
vector4_to_value(const vvp_vector4_t& vec, uint8_t& val, bool is_signed, bool is_arithmetic);
template bool
vector4_to_value(const vvp_vector4_t& vec, uint16_t& val, bool is_signed, bool is_arithmetic);
template bool
vector4_to_value(const vvp_vector4_t& vec, uint32_t& val, bool is_signed, bool is_arithmetic);
template bool
vector4_to_value(const vvp_vector4_t& vec, uint64_t& val, bool is_signed, bool is_arithmetic);

template <class T> bool vector4_to_value(const vvp_vector4_t& vec, bool& overflow_flag, T& val) {
    T res = 0;
    T msk = 1;
...
template bool vector4_to_value(const vvp_vector4_t& vec, bool& overflow_flag, unsigned long& val);
#ifndef UL_AND_TIME64_SAME
template bool vector4_to_value(const vvp_vector4_t& vec, bool& overflow_flag, vvp_time64_t& val);
#endif
...
void vvp_vector4array_aa::reset_instance(vvp_context_t context) {
    v4cell* cell = static_cast<v4cell*>(vvp_get_context_item(context, context_idx_));
...
void vvp_vector4array_aa::set_word(unsigned index, const vvp_vector4_t& that) {
    ...
    v4cell* cell = static_cast<v4cell*>(vthread_get_wt_context_item(context_idx_)) + index;
...
vvp_vector4_t vvp_vector4array_aa::get_word(unsigned index) const {
    ...
    v4cell* cell = static_cast<v4cell*>(vthread_get_rd_context_item(context_idx_)) + index;
...
static unsigned long add_carry(unsigned long a, unsigned long b, unsigned long& carry) {
...
static void
multiply_long(unsigned long a, unsigned long b, unsigned long& low, unsigned long& high) {
    ...
    for (unsigned bdx = 0; bdx < words; bdx += 1) {
        ...
        for (unsigned adx = 0; adx < words; adx += 1) {
            ...
            for (unsigned sdx = 0; (adx + bdx + sdx) < words; sdx += 1) {
                r.vec_[adx + bdx + sdx] = add_carry(r.vec_[adx + bdx + sdx], low, carry);
                low = hig;
                hig = 0;
            }
        }
...
static void div_mod(vvp_vector2_t dividend,
                    vvp_vector2_t divisor,
                    vvp_vector2_t& quotient,
                    vvp_vector2_t& remainder) {
...
vvp_vector2_t operator/(const vvp_vector2_t& dividend, const vvp_vector2_t& divisor) {
...
bool operator>(const vvp_vector2_t& a, const vvp_vector2_t& b) {
    const unsigned awords =
        (a.wid_ + vvp_vector2_t::BITS_PER_WORD - 1) / vvp_vector2_t::BITS_PER_WORD;
    const unsigned bwords =
        (b.wid_ + vvp_vector2_t::BITS_PER_WORD - 1) / vvp_vector2_t::BITS_PER_WORD;
...
vvp_vector8_t::vvp_vector8_t(const vvp_vector4_t& that, unsigned str0, unsigned str1)
    : size_(that.size()) {
    if (size_ == 0)
        return;
...
vvp_vector8_t::vvp_vector8_t(const vvp_vector2_t& that, unsigned str0, unsigned str1)
    : size_(that.size()) {
    if (size_ == 0)
        return;
...
void vvp_net_fun_t::recv_vec4(vvp_net_ptr_t, const vvp_vector4_t&, vvp_context_t) {
    fprintf(stderr, "internal error: %s: recv_vec4 not implemented\n", typeid(*this).name());
    assert(0);
}

void vvp_net_fun_t::recv_vec4_pv_(
    vvp_net_ptr_t p, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t) {
...
void vvp_net_fun_t::recv_vec4_pv(
    vvp_net_ptr_t, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t) {
    cerr << "internal error: " << typeid(*this).name() << ": "
         << "recv_vec4_pv(" << bit << ", " << base << ", " << vwid << ") not implemented" << endl;
    assert(0);
}
...
void vvp_net_fun_t::recv_vec8_pv_(vvp_net_ptr_t p,
...
void vvp_net_fun_t::recv_vec8_pv(vvp_net_ptr_t port,
                                 const vvp_vector8_t& bit,
                                 unsigned base,
                                 unsigned vwid) {
...
void vvp_fun_drive::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    assert(port.port() == 0);
    port.ptr()->send_vec8(vvp_vector8_t(bit, drive0_, drive1_));
}

void vvp_fun_drive::recv_vec4_pv(
    vvp_net_ptr_t ptr, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t ctx) {
...
void vvp_wide_fun_core::propagate_vec4(const vvp_vector4_t& bit, vvp_time64_t delay) {
    if (delay)
...
void vvp_wide_fun_core::dispatch_vec4_from_input_(unsigned port, const vvp_vector4_t& bit) {
    assert(port < nports_);
    if (port_values_ == 0)
        port_values_ = new vvp_vector4_t[nports_];
...
vvp_wide_fun_t::vvp_wide_fun_t(vvp_wide_fun_core* c, unsigned base) : core_(c), port_base_(base) {}

vvp_wide_fun_t::~vvp_wide_fun_t() {}

void vvp_wide_fun_t::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    unsigned pidx = port_base_ + port.port();
    core_->dispatch_vec4_from_input_(pidx, bit);
}

void vvp_wide_fun_t::recv_vec4_pv(
    vvp_net_ptr_t ptr, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t ctx) {
    recv_vec4_pv_(ptr, bit, base, vwid, ctx);
}
...
vvp_bit4_t
compare_gtge(const vvp_vector4_t& lef, const vvp_vector4_t& rig, vvp_bit4_t out_if_equal) {
...
vvp_bit4_t
compare_gtge_signed(const vvp_vector4_t& a, const vvp_vector4_t& b, vvp_bit4_t out_if_equal) {
    assert(a.size() == b.size());
