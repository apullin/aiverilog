...
static const unsigned opcode_count = sizeof(opcode_table) / sizeof(*opcode_table) - 1;
...
void functor_ref_lookup(vvp_net_t** ref, char* lab) {
    struct functor_gen_resolv_list_s* res = new struct functor_gen_resolv_list_s(lab);
...
bool vpi_handle_resolv_list_s::resolve(bool mes) {
    ...
    if (!val.ptr) {
        ...
        size_t n = 0;
        char ss[32];
        if (2 == sscanf(label(), "W<%u,%[r]>%zn", &base, ss, &n) && n == strlen(label())) {
            val.ptr = vpip_make_vthr_word(base, ss);
            sym_set_value(sym_vpi, label(), val);

        } else if (1 == sscanf(label(), "S<%u,str>%zn", &base, &n) && n == strlen(label())) {
...
void compile_vpi_lookup(vpiHandle* handle, char* label) {
    ...
    struct vpi_handle_resolv_list_s* res = new struct vpi_handle_resolv_list_s(label);
...
void code_label_lookup(struct vvp_code_s* code, char* label, bool cptr2) {
    struct code_label_resolv_list_s* res = new struct code_label_resolv_list_s(label, cptr2);
...
static void compile_array_lookup(struct vvp_code_s* code, char* label) {
    struct code_array_resolv_list_s* res = new struct code_array_resolv_list_s(label);
...
void wide_inputs_connect(vvp_wide_fun_core* core, unsigned argc, struct symb_s* argv) {
...
template <class T_> void make_arith(T_* arith, char* label, unsigned argc, struct symb_s* argv) {
    vvp_net_t* ptr = new vvp_net_t;
    ptr->fun = arith;
...
void compile_arith_cast_int(char* label, long width, unsigned argc, struct symb_s* argv) {
...
void compile_arith_cast_vec2(char* label, long width, unsigned argc, struct symb_s* argv) {
...
void compile_arith_cast_real(char* label, bool signed_flag, unsigned argc, struct symb_s* argv) {
...
void compile_arith_div(
    char* label, long wid, bool signed_flag, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_arith_mult(char* label, long wid, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_cmp_eeq(char* label, long wid, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_cmp_nee(char* label, long wid, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_cmp_ge(char* label, long wid, bool signed_flag, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_cmp_gt(char* label, long wid, bool signed_flag, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_cmp_weq(char* label, long wid, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_cmp_wne(char* label, long wid, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_delay(char* label, unsigned width, vvp_delay_t* delay, struct symb_s arg) {
...
void compile_delay(
    char* label, unsigned width, unsigned argc, struct symb_s* argv, bool ignore_decay) {
    vvp_delay_t stub(0, 0, 0);
    if (ignore_decay)
        stub.set_ignore_decay();
    vvp_net_t* net = new vvp_net_t;
...
void compile_modpath_src(struct __vpiModPath* dst,
                         char edge,
                         const struct symb_s& src,
                         struct numbv_s& vals,
...
void compile_shiftr(char* label, long wid, bool signed_flag, unsigned argc, struct symb_s* argv) {
    assert(wid > 0);
...
void compile_udp_def(int sequ, char* label, char* name, unsigned nin, unsigned init, char** table) {
    if (sequ) {
        vvp_bit4_t init4;
        if (init == 0)
            init4 = BIT4_0;
...
void compile_code(char* label, char* mnem, comp_operands_t opa) {
    ...
    struct opcode_table_s* op = static_cast<struct opcode_table_s*>(
        bsearch(mnem, opcode_table, opcode_count, sizeof(struct opcode_table_s), &opcode_compare));
    if (op == 0) {
    ...
    for (unsigned idx = 0; idx < op->argc; idx += 1) {
        switch (op->argt[idx]) {
            ...
            case OA_CODE_PTR2:
                ...
                code_label_lookup(code, opa->argv[idx].symb.text, op->argt[idx] == OA_CODE_PTR2);
                break;
...
void compile_file_line(char* label, long file_idx, long lineno, char* description) {
    if (label)
        compile_codelabel(label);
...
void compile_vpi_call(char* label,
                      char* name,
...
void compile_vpi_func_call(char* label,
                           char* name,
                           int val_type,
                           unsigned val_wid,
                           long file_idx,
...
void compile_param_logic(char* label,
                         const char* name,
                         char* value,
                         bool signed_flag,
                         bool local_flag,
