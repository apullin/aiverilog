...
void vpip_load_module(const char* name) {
    ...
    if (strchr(name, sep)) {
        ...
        rc = -1;
        for (unsigned idx = 0; (rc != 0) && (idx < vpip_module_path_cnt); idx += 1) {
            export_flag = false;
            snprintf(buf, sizeof(buf), "%s%c%s.vpi", vpip_module_path[idx], sep, name);
            rc = stat(buf, &sb);

            if (rc != 0) {
                export_flag = true;
                snprintf(buf, sizeof(buf), "%s%c%s.vpl", vpip_module_path[idx], sep, name);
                rc = stat(buf, &sb);
            }
        }
    ...
    if (function) {
        ...
        if (!set_callback(&vpi_routines, vpip_routines_version)) {
            fprintf(stderr,
                    "Failed to link VPI module %s. Try rebuilding it with iverilog-vpi.\n",
                    name);
            ivl_dlclose(dll);
            return;
        }
    ...
    dll_list_cnt += 1;
    dll_list = static_cast<ivl_dll_t*>(realloc(dll_list, dll_list_cnt * sizeof(ivl_dll_t)));
