...
struct systask_def : public __vpiSysTaskCall {
    virtual int get_type_code(void) const override {
        return vpiSysTaskCall;
    }
    virtual int vpi_get(int code) override {
        return systask_get(code, this);
    }
    virtual char* vpi_get_str(int code) override {
        return systask_get_str(code, this);
    }
    virtual vpiHandle vpi_handle(int code) override {
        return systask_handle(code, this);
    }
    virtual vpiHandle vpi_iterate(int code) override {
...
class sysfunc_vec4 : public sysfunc_def {
  public:
    ...
    int vpi_get(int code) override;
    vpiHandle vpi_put_value(p_vpi_value val, int flags) override;
    inline const vvp_vector4_t& return_value() const {
        return return_value_;
    }
...
static struct __vpiUserSystf* allocate_def(void) {
    if (def_table == 0) {
        def_table = static_cast<struct __vpiUserSystf**>(malloc(sizeof(struct __vpiUserSystf*)));
...
static void
add_vpi_call_error(vpi_call_error_type type, const char* name, long file_idx, long lineno) {
    vpi_call_error_lst = static_cast<vpi_call_error_p>(
        realloc(vpi_call_error_lst, (vpi_call_error_num + 1) * sizeof(vpi_call_error_s)));
        ...
        case vpiSysFunc:
            if (val_code == 0 && fnet == 0) {
                if (func_as_task_err) {
                    ...
                    add_vpi_call_error(VPI_CALL_FUNC_AS_TASK_WARN, name, file_idx, lineno);
                }
            }
            break;
