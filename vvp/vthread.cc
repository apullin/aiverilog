...
template vvp_vector4_t coerce_to_width(const vvp_vector4_t& that, unsigned width);


static void multiply_array_imm(unsigned long* res,
...
void vthreads_delete(class __vpiScope* scope) {
    for (std::set<vthread_t>::iterator cur = scope->threads.begin(); cur != scope->threads.end();
         ++cur) {
        delete *cur;
    }
    scope->threads.clear();
...
static void vthread_reap(vthread_t thr) {
    if (!thr->children.empty()) {
        for (set<vthread_t>::iterator cur = thr->children.begin(); cur != thr->children.end();
             ++cur) {
            vthread_t child = *cur;
            assert(child);
 ...
 */
static bool resize_rval_vec(vvp_vector4_t& val, int64_t& off, unsigned int sig_wid) {
...
bool of_CAST_VEC4_STR(vthread_t thr, vvp_code_t cp) {
    ...
    if (wid != 8 * str.length()) {
        cerr << thr->get_fileline() << "VVP error: size mismatch when casting string to vector."
             << endl;
        thr->push_vec4(vec);
        schedule_stop(0);
        return false;
...
static void of_CMPU_the_hard_way(vthread_t thr,
                                 unsigned wid,
...
static void do_CMPU(vthread_t thr, const vvp_vector4_t& lval, const vvp_vector4_t& rval) {
    ...
    if (rval.size() != lval.size()) {
        cerr << thr->get_fileline() << "VVP ERROR: %cmp/u operand width mismatch: lval=" << lval
             << ", rval=" << rval << endl;
    }
    assert(rval.size() == lval.size());
    unsigned wid = lval.size();
...
bool of_DEASSIGN(vthread_t, vvp_code_t cp) {
    ...
    if (base >= fil->value_size())
        return true;
    if (base + width > fil->value_size())
        width = fil->value_size() - base;
...
bool of_DELETE_ELEM(vthread_t thr, vvp_code_t cp) {
    ...
    if (thr->flags[4] == BIT4_1) {
        cerr << thr->get_fileline() << "Warning: skipping queue delete() with undefined index."
             << endl;
        return true;
    }
    if (idx_val < 0) {
        cerr << thr->get_fileline() << "Warning: skipping queue delete() with negative index."
             << endl;
        return true;
    }
    size_t idx = idx_val;
    ...
    vvp_queue* queue = obj->get_object().peek<vvp_queue>();
    if (queue == 0) {
        cerr << thr->get_fileline() << "Warning: skipping delete(" << idx << ") on empty queue."
             << endl;
    } else {
        size_t size = queue->get_size();
        if (idx >= size) {
            cerr << thr->get_fileline() << "Warning: skipping out of range delete(" << idx
 ...
 */
static unsigned long divide2words(unsigned long a, unsigned long b, unsigned long high) {
...
static unsigned long* divide_bits(unsigned long* ap, const unsigned long* bp, unsigned wid) {
    ...
    for (unsigned cur = words - btop; cur > 0; cur -= 1) {
        ...
        if (ap[cur_ptr + btop] >= bp[btop]) {
            ...
            cur_res = divide2words(ap[cur_ptr + btop], bp[btop], ap[cur_ptr + btop + 1]);
        }
...
bool of_IX_GETV(vthread_t thr, vvp_code_t cp) {
    ...
    if (sig == 0) {
        assert(net->fil);
        cerr << thr->get_fileline() << "%%ix/getv error: Net arg not a vector signal? "
...
bool of_IX_GETV_S(vthread_t thr, vvp_code_t cp) {
    ...
    if (sig == 0) {
        assert(net->fil);
        cerr << thr->get_fileline() << "%%ix/getv/s error: Net arg not a vector signal? "
...
static bool do_join_opcode(vthread_t thr) {
    ...
    // join with that one.
    for (set<vthread_t>::iterator cur = thr->children.begin(); cur != thr->children.end(); ++cur) {
        vthread_t curp = *cur;
...
template <typename ELEM> static bool load_dar(vthread_t thr, vvp_code_t cp) {
    ...
    ELEM word;
    if (darray && (adr >= 0) && (thr->flags[4] == BIT4_0))  // A defined address >= 0
...
bool of_LOAD_VEC4(vthread_t thr, vvp_code_t cp) {
    ...
    vvp_signal_value* sig = dynamic_cast<vvp_signal_value*>(net->fil);
    if (sig == 0) {
        cerr << thr->get_fileline() << "%load/v error: Net arg not a signal? "
...
static void do_verylong_mod(vvp_vector4_t& vala,
                            const vvp_vector4_t& valb,
                            bool left_is_neg,
                            bool right_is_neg) {
...
bool of_NEW_DARRAY(vthread_t thr, vvp_code_t cp) {
    ...
    if (strcmp(text, "b8") == 0) {
    ...
    } else if ((1 == sscanf(text, "b%u%zn", &word_wid, &n)) && (n == strlen(text))) {
        obj = new vvp_darray_vec2(size, word_wid);
    } else if ((1 == sscanf(text, "sb%u%zn", &word_wid, &n)) && (n == strlen(text))) {
        obj = new vvp_darray_vec2(size, word_wid);
    } else if ((1 == sscanf(text, "v%u%zn", &word_wid, &n)) && (n == strlen(text))) {
        obj = new vvp_darray_vec4(size, word_wid);
    } else if ((1 == sscanf(text, "sv%u%zn", &word_wid, &n)) && (n == strlen(text))) {
        obj = new vvp_darray_vec4(size, word_wid);
        ...
        cerr << get_fileline() << "Internal error: Unsupported dynamic array type: " << text << "."
             << endl;
        assert(0);
    }
...
template <typename ELEM> static bool prop(vthread_t thr, vvp_code_t cp) {
    unsigned pid = cp->number;
...
bool of_PUSHV_STR(vthread_t thr, vvp_code_t) {
    ...
    string val;
    for (vector<char>::reverse_iterator cur = buf.rbegin(); cur != buf.rend(); ++cur) {
        val.push_back(*cur);
    }
...
static bool qinsert(vthread_t thr, vvp_code_t cp, unsigned wid = 0) {
    ...
    assert(queue);
    if (idx < 0) {
        cerr << thr->get_fileline() << "Warning: cannot insert at a negative "
             << get_queue_type(value) << " index (" << idx << "). ";
        print_queue_value(value);
        cerr << " was not added." << endl;
    } else if (thr->flags[4] != BIT4_0) {
        cerr << thr->get_fileline() << "Warning: cannot insert at an undefined "
...
template <typename ELEM, class QTYPE>
static bool q_pop(vthread_t thr,
                  vvp_code_t cp,
    ...
    if (size) {
        ...
        cerr << thr->get_fileline() << "Warning: pop_" << loc << "() on empty "
...
template <typename ELEM> static void get_back_value(vvp_queue* queue, ELEM& value) {
...
template <typename ELEM> static void get_front_value(vvp_queue* queue, ELEM& value) {
...
template <typename ELEM> static bool ret(vthread_t thr, vvp_code_t cp) {
...
template <typename ELEM> static bool retload(vthread_t thr, vvp_code_t cp) {
    size_t index = cp->number;
    ELEM type;
...
template <typename ELEM> static bool set_dar_obj(vthread_t thr, vvp_code_t cp) {
...
template <typename ELEM> static bool store_dar(vthread_t thr, vvp_code_t cp) {
    int64_t adr = thr->words[3].w_int;
    ELEM value;
    ...
    if (adr < 0)
        cerr << thr->get_fileline() << "Warning: cannot write to a negative "
             << get_darray_type(value) << " index (" << adr << ")." << endl;
    else if (thr->flags[4] != BIT4_0)
        cerr << thr->get_fileline() << "Warning: cannot write to an undefined "
             << get_darray_type(value) << " index." << endl;
    else if (darray)
        darray->set_word(adr, value);
    else
        cerr << thr->get_fileline() << "Warning: cannot write to an undefined "
             << get_darray_type(value) << "." << endl;
...
template <typename ELEM> static bool store_prop(vthread_t thr, vvp_code_t cp, unsigned wid = 0) {
    size_t pid = cp->number;
    ELEM val;
...
static bool store_qdar(vthread_t thr, vvp_code_t cp, unsigned wid = 0) {
    ...
    assert(queue);
    if (idx < 0) {
        cerr << thr->get_fileline() << "Warning: cannot assign to a negative "
             << get_queue_type(value) << " index (" << idx << "). ";
        print_queue_value(value);
        cerr << " was not added." << endl;
    } else if (thr->flags[4] != BIT4_0) {
        cerr << thr->get_fileline() << "Warning: cannot assign to an undefined "
             << get_queue_type(value) << " index. ";
        print_queue_value(value);
        cerr << " was not added." << endl;
    } else {
...
template <typename ELEM> static bool store(vthread_t thr, vvp_code_t cp) {
    ELEM val;
    pop_value(thr, val, 0);
    /* set the value into port 0 of the destination. */
    vvp_net_ptr_t ptr(cp->net, 0);
...
template <typename ELEM> static bool storea(vthread_t thr, vvp_code_t cp) {
    unsigned idx = cp->bit_idx[0];
    ELEM val;
    pop_value(thr, val, 0);
...
bool of_STORE_VEC4(vthread_t thr, vvp_code_t cp) {
    ...
    if (val_size < wid) {
        cerr << thr->get_fileline() << "XXXX Internal error: val.size()=" << val_size
             << ", expecting >= " << wid << endl;
    }
    assert(val_size >= wid);
    if (val_size > wid) {
...
bool of_FILE_LINE(vthread_t thr, vvp_code_t cp) {
    ...
    thr->set_fileline(vpi_get_str(vpiFile, handle), vpi_get(vpiLineNo, handle));

    if (show_file_line)
        cerr << thr->get_fileline() << vpi_get_str(_vpiDescription, handle) << endl;
