...
static void cmd_call(unsigned argc, const char* argv[]) {
    ...
    for (unsigned idx = 0; idx < vpi_argc; idx += 1) {
        ...
        if (handle == 0 && strspn(argv[idx + 1], "0123456789") == strlen(argv[idx + 1])) {
    ...
    if (errors == 0) {
        vpiHandle call_handle =
            vpip_build_vpi_call(argv[0], 0, 0, 0, true, false, vpi_argc, vpi_argv, 0, 0, 0, 1, 0);
        if (call_handle == 0)
            goto out;
...
static struct {
    ...
    {"exit",   &cmd_finish,  "Synonym for finish."                                              },
    ...
    {"trace",  &cmd_trace,   "Control statement tracing (on/off) when the code is instrumented."},
