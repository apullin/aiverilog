...
vpiHandle __vpiEnumTypespec::vpi_iterate(int code) {
    if (code == vpiEnumConst) {
        vpiHandle* args = static_cast<vpiHandle*>(calloc(names.size(), sizeof(vpiHandle*)));
...
void compile_enum2_type(char* label,
                        long width,
                        bool signed_flag,
    ...
    size_t idx = 0;
    for (list<struct enum_name_s>::iterator cur = names->begin(); cur != names->end();
         ++cur, ++idx) {
        assert(cur->val4 == 0);
...
void compile_enum4_type(char* label,
                        long width,
                        bool signed_flag,
                        std::list<struct enum_name_s>* names) {
    struct __vpiEnumTypespec* spec = new struct __vpiEnumTypespec;
...
void enum_delete(vpiHandle item) {
    ...
    for (vector<enumconst_s>::iterator iter = obj->names.begin(); iter != obj->names.end();
         ++iter) {
        delete[] iter->name;
    }
