...
ostream& operator<<(ostream& o, const struct udp_levels_table& table) {
    o << "[" << hex << table.mask0 << "/" << hex << table.mask1 << "/" << hex << table.maskx << "]";
    return o;
}

vvp_udp_s::vvp_udp_s(const char* label, char* name__, unsigned ports, vvp_bit4_t init, bool type)
...
#ifdef CHECK_WITH_VALGRIND
    udp_defns_count += 1;
    udp_defns = static_cast<vvp_udp_s**>(realloc(udp_defns, udp_defns_count * sizeof(vvp_udp_s**)));
...
vvp_bit4_t
vvp_udp_comb_s::calculate_output(const udp_levels_table& cur, const udp_levels_table&, vvp_bit4_t) {
    return test_levels(cur);
}

static void or_based_on_char(udp_levels_table& cur, char flag, unsigned long mask_bit) {
    switch (flag) {
        case '0':
            cur.mask0 |= mask_bit;
            break;
...
vvp_udp_seq_s::vvp_udp_seq_s(const char* label, char* name__, unsigned ports, vvp_bit4_t init)
...
vvp_bit4_t vvp_udp_seq_s::test_edges_(const udp_levels_table& cur, const udp_levels_table& prev) {
 ...
 */
void compile_udp_functor(char* label, char* type, unsigned argc, struct symb_s* argv) {
