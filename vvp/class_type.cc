...
template <class T> class property_atom : public class_property_t {
  public:
    ...
    size_t instance_size() const override {
        return sizeof(T);
    }
...
class property_string : public class_property_t {
  ...
  public:
    void construct(char* buf) const override { /* string*tmp = */
...
class property_object : public class_property_t {
  public:
    ...
    size_t instance_size() const override {
        return array_size_ * sizeof(vvp_object_t);
    }
...
class_type::class_type(const string& nam, size_t nprop) : class_name_(nam), properties_(nprop) {
    instance_size_ = 0;
}
...
void class_type::set_property(size_t idx,
                              const string& name,
                              const string& type,
                              uint64_t array_size) {
...
void class_type::set_vec4(class_type::inst_t obj, size_t pid, const vvp_vector4_t& val) const {
    char* buf = reinterpret_cast<char*>(obj);
    assert(pid < properties_.size());
    properties_[pid].type->set_vec4(buf, val);
}

void class_type::get_vec4(class_type::inst_t obj, size_t pid, vvp_vector4_t& val) const {
    char* buf = reinterpret_cast<char*>(obj);
    assert(pid < properties_.size());
    properties_[pid].type->get_vec4(buf, val);
}

void class_type::set_real(class_type::inst_t obj, size_t pid, double val) const {
    char* buf = reinterpret_cast<char*>(obj);
    assert(pid < properties_.size());
...
void class_type::set_string(class_type::inst_t obj, size_t pid, const string& val) const {
    char* buf = reinterpret_cast<char*>(obj);
    assert(pid < properties_.size());
...
void class_type::set_object(class_type::inst_t obj,
                            size_t pid,
...
void class_type::get_object(class_type::inst_t obj,
                            size_t pid,
                            vvp_object_t& val,
                            size_t idx) const {
    char* buf = reinterpret_cast<char*>(obj);
