...
class class_type : public __vpiHandle {
  ...
  public:
    ...
    inline const std::string& class_name(void) const {
        return class_name_;
    }
    // Number of properties in the class definition.
    inline size_t property_count(void) const {
    ...
    void
    set_property(size_t idx, const std::string& name, const std::string& type, uint64_t array_size);
