...
vvp_fun_concat::vvp_fun_concat(unsigned w0, unsigned w1, unsigned w2, unsigned w3)
...
void vvp_fun_concat::recv_vec4(vvp_net_ptr_t port,
                               const vvp_vector4_t& bit,
                               vvp_context_t context) {
    recv_vec4_pv(port, bit, 0, bit.size(), context);
}

void vvp_fun_concat::recv_vec4_pv(
...
void compile_concat(char* label,
                    unsigned w0,
                    unsigned w1,
                    unsigned w2,
                    unsigned w3,
...
vvp_fun_concat8::vvp_fun_concat8(unsigned w0, unsigned w1, unsigned w2, unsigned w3)
    : val_(w0 + w1 + w2 + w3) {
    wid_[0] = w0;
    wid_[1] = w1;
    wid_[2] = w2;
...
void vvp_fun_concat8::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    vvp_vector8_t bit8(bit, 6, 6);
    recv_vec8_pv(port, bit8, 0, bit8.size());
}

void vvp_fun_concat8::recv_vec4_pv(
    vvp_net_ptr_t port, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t) {
    vvp_vector8_t bit8(bit, 6, 6);
...
void vvp_fun_concat8::recv_vec8_pv(vvp_net_ptr_t port,
...
void compile_concat8(char* label,
                     unsigned w0,
                     unsigned w1,
                     unsigned w2,
                     unsigned w3,
...
vvp_fun_repeat::vvp_fun_repeat(unsigned width, unsigned repeat) : wid_(width), rep_(repeat) {}

vvp_fun_repeat::~vvp_fun_repeat() {}

void vvp_fun_repeat::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
...
void vvp_fun_repeat::recv_vec4_pv(vvp_net_ptr_t port,
                                  const vvp_vector4_t& bit,
                                  unsigned base,
                                  unsigned vwid,
                                  vvp_context_t context) {
