...
vvp_fun_part::vvp_fun_part(unsigned base, unsigned wid) : base_(base), wid_(wid) {}

vvp_fun_part::~vvp_fun_part() {}

vvp_fun_part_sa::vvp_fun_part_sa(unsigned base, unsigned wid) : vvp_fun_part(base, wid) {
    net_ = 0;
}

vvp_fun_part_sa::~vvp_fun_part_sa() {}

void vvp_fun_part_sa::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    assert(port.port() == 0);
 ...
 * defined behavior.
 */
void vvp_fun_part_sa::recv_vec4_pv(
...
vvp_fun_part_aa::vvp_fun_part_aa(unsigned base, unsigned wid) : vvp_fun_part(base, wid) {
...
void vvp_fun_part_aa::reset_instance(vvp_context_t context) {
    vvp_vector4_t* val = static_cast<vvp_vector4_t*>(vvp_get_context_item(context, context_idx_));
...
void vvp_fun_part_aa::recv_vec4(vvp_net_ptr_t port,
 ...
 * defined behavior.
 */
void vvp_fun_part_aa::recv_vec4_pv(vvp_net_ptr_t port,
                                   const vvp_vector4_t& bit,
                                   unsigned base,
                                   unsigned vwid,
                                   vvp_context_t context) {
...
void vvp_fun_part_pv::recv_vec4(vvp_net_ptr_t port,
                                const vvp_vector4_t& bit,
                                vvp_context_t context) {
    assert(port.port() == 0);
    ...
    if (bit.size() != wid_) {
        cerr << "internal error: part_pv data mismatch. "
             << "base_=" << base_ << ", wid_=" << wid_ << ", vwid_=" << vwid_ << ", bit=" << bit
             << endl;
    }
...
void vvp_fun_part_pv::recv_vec4_pv(
    vvp_net_ptr_t port, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t ctx) {
    assert(port.port() == 0);
...
vvp_fun_part_var::vvp_fun_part_var(unsigned w, bool is_signed) : wid_(w), is_signed_(is_signed) {}

vvp_fun_part_var::~vvp_fun_part_var() {}

bool vvp_fun_part_var::recv_vec4_(vvp_net_ptr_t port,
                                  const vvp_vector4_t& bit,
                                  int& base,
...
void vvp_fun_part_var_sa::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t& bit, vvp_context_t) {
    if (recv_vec4_(port, bit, base_, source_, ref_)) {
        port.ptr()->send_vec4(ref_, 0);
    }
}

void vvp_fun_part_var_sa::recv_vec4_pv(
    vvp_net_ptr_t port, const vvp_vector4_t& bit, unsigned base, unsigned vwid, vvp_context_t) {
...
void vvp_fun_part_var_aa::reset_instance(vvp_context_t context) {
    vvp_fun_part_var_state_s* state =
        static_cast<vvp_fun_part_var_state_s*>(vvp_get_context_item(context, context_idx_));
...
void vvp_fun_part_var_aa::recv_vec4_pv(vvp_net_ptr_t port,
                                       ...
                                       vvp_context_t context) {
    if (context) {
        const vvp_fun_part_var_state_s* state =
            static_cast<vvp_fun_part_var_state_s*>(vvp_get_context_item(context, context_idx_));
...
void compile_part_select(char* label, char* source, unsigned base, unsigned wid) {
    vvp_fun_part* fun = 0;
...
void compile_part_select_pv(
    char* label, char* source, unsigned base, unsigned wid, unsigned vector_wid) {
    vvp_fun_part_pv* fun = new vvp_fun_part_pv(base, wid, vector_wid);
    link_node_1(label, source, fun);
}

void compile_part_select_var(char* label, char* source, char* var, unsigned wid, bool is_signed) {
