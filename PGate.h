...
class PGate : public PNamedItem {
  public:
    explicit PGate(perm_string name, std::list<PExpr*>* pins, const std::list<PExpr*>* del);

    explicit PGate(perm_string name, std::list<PExpr*>* pins, PExpr* del);
  ...
  protected:
    const std::vector<PExpr*>& get_pins() const {
        return pins_;
    }

    unsigned calculate_array_size_(Design*, NetScope*, long& high, long& low) const;
...
class PGBuiltin : public PGate {
  ...
  public:
    explicit PGBuiltin(Type t,
                       perm_string name,
                       std::list<PExpr*>* pins,
                       const std::list<PExpr*>* del);
    explicit PGBuiltin(Type t, perm_string name, std::list<PExpr*>* pins, PExpr* del);
    ~PGBuiltin() override;
  ...
  private:
    void calculate_gate_and_lval_count_(unsigned& gate_count, unsigned& lval_count) const;

    NetNode* create_gate_for_output_(Design*,
                                     NetScope*,
...
class PGModule : public PGate {
  public:
    ...
    explicit PGModule(perm_string type, perm_string name, std::list<PExpr*>* pins);

    // If the binding of ports is by name, this constructor takes
    // the bindings and stores them for later elaboration.
    explicit PGModule(perm_string type, perm_string name, named_pexpr_t* pins, unsigned npins);
  ...
  private:
    ...
    NetNet* resize_net_to_port_(Design* des,
                                NetScope* scope,
                                NetNet* sig,
                                unsigned port_wid,
                                NetNet::PortType dir,
