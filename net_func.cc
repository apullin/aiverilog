 ...
 * that is the result.
 */
NetUserFunc::NetUserFunc(NetScope* s, perm_string n, NetScope* d, NetEvWait* trigger__)
    : NetNode(s, n, d->func_def()->port_count() + 1), def_(d), trigger_(trigger__) {
...
bool PECallFunction::check_call_matches_definition_(Design* des, NetScope* dscope) const {
    ...
    if (dscope->type() != NetScope::FUNC) {
        cerr << get_fileline() << ": error: Attempt to call scope " << scope_path(dscope)
             << " as a function." << endl;
        des->errors += 1;
        return false;
    }
...
NetSysFunc::NetSysFunc(NetScope* s,
                       perm_string n,
