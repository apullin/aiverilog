...
static void qualify_enable(Design* des,
                           NetScope* scope,
                           NetNet* qualifier,
                           bool active_state,
                           NetLogic::TYPE gate_type,
                           Link& enable_i,
                           Link& enable_o) {
    ...
    if (active_state == false) {
        NetLogic* gate = new NetLogic(scope, scope->local_symbol(), 2, NetLogic::NOT, 1);
        des->add_node(gate);
        connect(gate->pin(1), qualifier->pin(0));

        NetNet* sig =
            new NetNet(scope, scope->local_symbol(), NetNet::WIRE, &netvector_t::scalar_logic);
        sig->local_flag(true);
...
static void multiplex_enables(
    Design* des, NetScope* scope, NetNet* select, Link& enable_1, Link& enable_0, Link& enable_o) {
    if (!enable_1.is_linked() && !enable_0.is_linked())
        return;

    if (enable_1.is_linked(scope->tie_hi()) && enable_0.is_linked(scope->tie_hi())) {
        connect(enable_o, scope->tie_hi());
        return;
    }

    if (enable_1.is_linked(scope->tie_lo()) || !enable_1.is_linked()) {
        qualify_enable(des, scope, select, false, NetLogic::AND, enable_0, enable_o);
        return;
    }
...
static void
merge_sequential_enables(Design* des, NetScope* scope, Link& top_enable, Link& sub_enable) {
    ...
    if (top_enable.is_linked()) {
        NetLogic* gate = new NetLogic(scope, scope->local_symbol(), 3, NetLogic::OR, 1);
        des->add_node(gate);
    ...
    for (unsigned idx = 0; idx < nex_in.pin_count(); idx += 1) {
        if (nex_in.pin(idx).nexus()->has_floating_input()) {
            if (all_bits_driven(bitmasks[idx])) {
                ...
                NetLogic* gate =
                    new NetLogic(scope(), scope()->local_symbol(), 1, NetLogic::PULLDOWN, width);
                des->add_node(gate);
                ...
                NetNet* sig = new NetNet(scope(), scope()->local_symbol(), NetNet::WIRE, tmp_vec);
                sig->local_flag(true);
                connect(sig->pin(0), gate->pin(0));
            } else if (is_ff_input) {
                ...
                cerr << get_fileline() << ": warning: A latch "
                     ...
                     << nex_map[idx].lnk.nexus()->pick_any_net()->name() << "'." << endl;
 ...
 * r-value.
 */
bool NetAssignBase::synth_async(Design* des,
                                NetScope* scope,
                                NexusSet& nex_map,
                                NetBus& nex_out,
                                NetBus& enables,
    ...
    if (is_part_select && !scope->loop_index_tmp.empty()) {
        ...
        if (isig) {
            ...
            isig = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_type);
...
bool NetProc::synth_async_block_substatement_(Design* des,
                                              NetScope* scope,
                                              NexusSet& nex_map,
                                              NetBus& nex_out,
                                              NetBus& enables,
    ...
    if (debug_synth2) {
        for (unsigned idx = 0; idx < nex_map.size(); idx += 1) {
            cerr << get_fileline() << ": NetProc::synth_async_block_substatement_: nex_map[" << idx
                 << "] dump link, base=" << nex_map[idx].base << ", wid=" << nex_map[idx].wid
                 << endl;
            nex_map[idx].lnk.dump_link(cerr, 8);
        }
        for (unsigned idx = 0; idx < tmp_map.size(); idx += 1) {
            cerr << get_fileline() << ": NetProc::synth_async_block_substatement_: tmp_map[" << idx
                 << "] dump link, base=" << tmp_map[idx].base << ", wid=" << tmp_map[idx].wid
                 << endl;
            tmp_map[idx].lnk.dump_link(cerr, 8);
        }
        for (unsigned idx = 0; idx < tmp_out.pin_count(); idx += 1) {
            cerr << get_fileline() << ": NetProc::synth_async_block_substatement_: tmp_out[" << idx
                 << "] dump link" << endl;
    ...
    if (debug_synth2) {
        cerr << get_fileline()
             << ": NetProc::synth_async_block_substatement_: "
                "substmt->synch_async(...) --> "
             << (flag ? "true" : "false") << " for statement at " << substmt->get_fileline() << "."
             << endl;
    }
 ...
 * the substatements.
 */
bool NetBlock::synth_async(Design* des,
                           NetScope* scope,
                           NexusSet& nex_map,
                           NetBus& nex_out,
                           NetBus& enables,
    ...
    do {
        ...
        bool sub_flag =
            synth_async_block_substatement_(des, scope, nex_map, nex_out, enables, bitmasks, cur);
        flag = flag && sub_flag;
 ...
 */
static NetNet* mux_selector_reduce_width(
    Design* des, NetScope* scope, const LineInfo& loc, NetNet* esig, unsigned sel_need) {
    ...
    if (debug_synth2) {
        cerr << loc.get_fileline() << ": mux_selector_reduce_width: "
             << "Reduce selector width=" << sel_got << " to " << sel_need << " bits." << endl;
    }
    ...
    NetConcat* osig_cat =
        new NetConcat(scope, scope->local_symbol(), sel_need, 2, !disable_concatz_generation);
    osig_cat->set_line(loc);
    des->add_node(osig_cat);
    connect(osig_cat->pin(0), osig->pin(0));

    // Create the part select esig[N-2:0]...
    NetPartSelect* ps0 = new NetPartSelect(esig, 0, sel_need - 1, NetPartSelect::VP);
    ps0->set_line(loc);
    des->add_node(ps0);
    connect(ps0->pin(1), esig->pin(0));

    const netvector_t* ps0_vec = new netvector_t(osig_data_type, sel_need - 2, 0);
    NetNet* ps0_sig = new NetNet(scope, scope->local_symbol(), NetNet::TRI, ps0_vec);
    ...
    NetPartSelect* ps1 =
        new NetPartSelect(esig, sel_need - 1, sel_got - sel_need + 1, NetPartSelect::VP);
    ps1->set_line(loc);
    des->add_node(ps1);
    connect(ps1->pin(1), esig->pin(0));

    const netvector_t* ps1_vec = new netvector_t(osig_data_type, sel_got - sel_need, 0);
    NetNet* ps1_sig = new NetNet(scope, scope->local_symbol(), NetNet::TRI, ps1_vec);
    ...
    NetUReduce* ered =
        new NetUReduce(scope, scope->local_symbol(), NetUReduce::OR, sel_got - sel_need + 1);
    ered->set_line(loc);
    des->add_node(ered);
    connect(ered->pin(1), ps1_sig->pin(0));

    NetNet* ered_sig =
        new NetNet(scope, scope->local_symbol(), NetNet::TRI, &netvector_t::scalar_logic);
...
bool NetCase::synth_async(Design* des,
                          NetScope* scope,
                          NexusSet& nex_map,
                          NetBus& nex_out,
                          NetBus& enables,
    ...
    if (debug_synth2) {
        cerr << get_fileline() << ": NetCase::synth_async: "
             << "Adjusted mux_size is " << mux_size << " (max_guard_value=" << max_guard_value
             << ", sel_need=" << sel_need << ", sel_width=" << sel_width << ")." << endl;
    }
    ...
    if (default_statement) {
        bool flag = synth_async_block_substatement_(
            des, scope, nex_map, default_out, default_ena, default_masks, default_statement);
        if (!flag)
            return false;
    ...
    for (size_t mdx = 0; mdx < nex_out.pin_count(); mdx += 1) {
        out_mux[mdx] = new NetMux(scope, scope->local_symbol(), mux_width[mdx], mux_size, sel_need);
        ...
        ena_mux[mdx] = new NetMux(scope, scope->local_symbol(), 1, mux_size, sel_need);
    ...
    for (unsigned idx = 0; idx < mux_size; idx += 1) {
        ...
        bool flag =
            synth_async_block_substatement_(des, scope, nex_map, tmp_out, tmp_ena, tmp_masks, stmt);
        if (!flag)
            return false;
 ...
 */
bool NetCase::synth_async_casez_(Design* des,
                                 NetScope* scope,
                                 NexusSet& nex_map,
                                 NetBus& nex_out,
                                 NetBus& enables,
    ...
    for (size_t idx = 0; idx < items_.size(); idx += 1) {
        ...
        NetCaseCmp* condit_dev = new NetCaseCmp(scope, scope->local_symbol(), sel_width, case_kind);
        ...
        NetNet* condit = new NetNet(scope, scope->local_symbol(), NetNet::TRI, condit_type);
        ...
        for (unsigned mdx = 0; mdx < nex_out.pin_count(); mdx += 1) {
            NetMux* mux = new NetMux(scope, scope->local_symbol(), mux_width[mdx], 2, 1);
            des->add_node(mux);
            mux->set_line(*this);
            ...
            multiplex_enables(
                des, scope, condit, tmp_ena.pin(mdx), prev_ena.pin(mdx), enables.pin(mdx));
 ...
 */
bool NetCondit::synth_async(Design* des,
                            NetScope* scope,
                            NexusSet& nex_map,
                            NetBus& nex_out,
                            NetBus& enables,
    ...
    if (if_) {
        ...
        bool flag =
            synth_async_block_substatement_(des, scope, nex_map, a_out, a_ena, a_masks, if_);
        if (!flag)
            return false;
    ...
    if (else_) {
        ...
        bool flag =
            synth_async_block_substatement_(des, scope, nex_map, b_out, b_ena, b_masks, else_);
        if (!flag)
            return false;
    ...
    for (unsigned idx = 0; idx < nex_out.pin_count(); idx += 1) {
        ...
        if (debug_synth2 && apv) {
            cerr << get_fileline() << ": NetCondit::synth_async: "
                 << "Assign-to-part apv base=" << apv->base() << ", width=" << apv->width() << endl;
        }

        NetPartSelect* bpv = detect_partselect_lval(b_out.pin(idx));
        if (debug_synth2 && bpv) {
            cerr << get_fileline() << ": NetCondit::synth_async: "
                 << "Assign-to-part bpv base=" << bpv->base() << ", width=" << bpv->width() << endl;
        }
        ...
        NetNet* otmp = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_type);
        ...
        if (mux_width < mux_lwidth) {
            ...
            if (itmp == 0) {
                itmp = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_type);
...
bool NetEvWait::synth_async(Design* des,
                            NetScope* scope,
                            NexusSet& nex_map,
                            NetBus& nex_out,
                            NetBus& enables,
...
bool NetForLoop::synth_async(Design* des,
                             NetScope* scope,
                             NexusSet& nex_map,
                             NetBus& nex_out,
                             NetBus& enables,
    ...
    if (!index_) {
        cerr << get_fileline()
             << ": sorry: Unable to synthesize for-loop without explicit index variable." << endl;
        return false;
    }

    if (!step_statement_) {
        cerr << get_fileline()
             << ": sorry: Unable to synthesize for-loop without for_step statement." << endl;
        return false;
    }
    ...
    for (;;) {
        ...
        if (debug_synth2) {
            cerr << get_fileline() << ": NetForLoop::synth_async: "
                 << "Synthesis iteration with " << index_->name() << "=" << *index_var.value
                 << endl;
        }
        ...
        rc = synth_async_block_substatement_(
            des, scope, nex_map, nex_out, tmp_ena, tmp_masks, statement_);
...
bool NetProcTop::synth_async(Design* des) {
    ...
    for (unsigned idx = 0; idx < nex_set.size(); idx += 1) {
        if (enables.pin(idx).is_linked(scope()->tie_hi())) {
            ...
            NetLatch* latch = new NetLatch(scope(), scope()->local_symbol(), nex_set[idx].wid);
            des->add_node(latch);
            latch->set_line(*this);
...
bool NetProc::synth_sync(Design* des,
                         NetScope* scope,
                         bool& /* ff_negedge */,
                         NetNet* /* ff_clk */,
                         NetBus& ff_ce,
 ...
 */
bool NetBlock::synth_sync(Design* des,
                          NetScope* scope,
                          bool& ff_negedge,
                          NetNet* ff_clk,
                          NetBus& ff_ce,
    ...
    do {
        ...
        bool ok_flag = cur->synth_sync(des,
                                       scope,
                                       ff_negedge,
                                       ff_clk,
                                       tmp_ce,
 ...
 */
bool NetCondit::synth_sync(Design* des,
                           NetScope* scope,
                           bool& ff_negedge,
                           NetNet* ff_clk,
                           NetBus& ff_ce,
    ...
    for (unsigned idx = 0; idx < events_in.size(); idx += 1) {
        ...
        for (unsigned pin = 0; pin < tmp_out.pin_count(); pin += 1) {
            ...
            if (!rst_nex->drivers_constant() || !tmp_ena.pin(pin).is_linked(scope->tie_hi())) {
...
bool NetEvWait::synth_sync(Design* des,
                           NetScope* scope,
                           bool& ff_negedge,
                           NetNet* ff_clk,
                           NetBus& ff_ce,
    ...
    /* Synthesize the input to the DFF. */
    return statement_->synth_sync(des,
                                  scope,
                                  ff_negedge,
                                  ff_clk,
                                  ff_ce,
...
bool NetProcTop::synth_sync(Design* des) {
    ...
    /* Synthesize the input to the DFF. */
    bool negedge = false;
    bool flag = statement_->synth_sync(des,
                                       scope(),
                                       negedge,
                                       clock,
                                       ce,
    ...
    for (unsigned idx = 0; idx < nex_set.size(); idx += 1) {
        ...
        NetFF* ff2 = new NetFF(scope(), scope()->local_symbol(), negedge, nex_set[idx].wid);
        des->add_node(ff2);
        ff2->set_line(*this);
