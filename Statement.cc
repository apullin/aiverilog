...
PAssign_::PAssign_(PExpr* lval__, PExpr* ex, bool is_constant, bool is_init)
    : event_(0), count_(0), lval_(lval__), rval_(ex), is_constant_(is_constant), is_init_(is_init) {
    delay_ = 0;
}
...
PAssign::PAssign(PExpr* lval__, PExpr* ex) : PAssign_(lval__, ex, false), op_(0) {}

PAssign::PAssign(PExpr* lval__, char op, PExpr* ex) : PAssign_(lval__, ex, false), op_(op) {}

PAssign::PAssign(PExpr* lval__, PExpr* d, PExpr* ex) : PAssign_(lval__, d, ex), op_(0) {}
...
PAssignNB::PAssignNB(PExpr* lval__, PExpr* d, PExpr* ex) : PAssign_(lval__, d, ex) {}
...
PBlock::PBlock(perm_string n, LexicalScope* parent, BL_TYPE t) : PScope(n, parent), bl_type_(t) {}
...
PChainConstructor::PChainConstructor(const vector<named_pexpr_t>& parms) : parms_(parms) {}

PChainConstructor::~PChainConstructor() {}

PCondit::PCondit(PExpr* ex, Statement* i, Statement* e) : expr_(ex), if_(i), else_(e) {}
...
PDelayStatement::PDelayStatement(PExpr* d, Statement* st) : delay_(d), statement_(st) {}
...
PEventStatement::PEventStatement(PEEvent* ee) : expr_(1), statement_(0), always_sens_(false) {
    expr_[0] = ee;
}

PEventStatement::PEventStatement(bool always_sens) : statement_(0), always_sens_(always_sens) {}
...
PForStatement::PForStatement(PExpr* n1, PExpr* e1, PExpr* cond, Statement* step, Statement* st)
