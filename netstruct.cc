...
netstruct_t::netstruct_t() : union_(false), packed_(false), signed_(false) {}
    ...
    if (union_ && packed_ && members_.size() > 1) {
        ...
        if (expect_wid != got_wid) {
            cerr << get_fileline() << ": error: "
                 ...
                 << " is " << got_wid << " bits, expecting " << expect_wid << " bits." << endl;
            des->errors += 1;
        }
    }
}

const netstruct_t::member_t* netstruct_t::packed_member(perm_string name,
