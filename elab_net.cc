 ...
 */
NetNet* PEConcat::elaborate_lnet_common_(Design* des,
                                         NetScope* scope,
    ...
    if (repeat_) {
        cerr << repeat_->get_fileline() << ": error: "
             << "repeat concatenations are not allowed in net l-values." << endl;
        des->errors += 1;
        return 0;
    }
    ...
    for (unsigned idx = 0; idx < nets.size(); idx += 1) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": debug: Elaborate subexpression " << idx << " of "
                 << nets.size() << " l-values: " << *parms_[idx] << endl;
        }
    ...
    NetNet* osig = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, tmp2_vec);
    ...
    if (bidirectional_flag) {
        ...
        for (unsigned idx = 0; idx < nets.size(); idx += 1) {
            ...
            NetTran* ps = new NetTran(scope, scope->local_symbol(), osig->vector_width(), wid, off);
            des->add_node(ps);
            ps->set_line(*this);
...
NetNet* PEConcat::elaborate_lnet(Design* des, NetScope* scope, bool var_allowed_in_sv) const {
    return elaborate_lnet_common_(des, scope, false, var_allowed_in_sv);
}

NetNet* PEConcat::elaborate_bi_net(Design* des, NetScope* scope, bool var_allowed_in_sv) const {
    return elaborate_lnet_common_(des, scope, true, var_allowed_in_sv);
}

bool PEConcat::is_collapsible_net(Design* des, NetScope* scope, NetNet::PortType port_type) const {
    ivl_assert(*this, scope);
 ...
 */
bool PEIdent::eval_part_select_(
    ...
    switch (index_tail.sel) {
        ...
        case index_component_t::SEL_PART: {
            ...
            if (lidx > midx) {
                cerr << get_fileline() << ": error: Part select " << sig->name() << "[" << msb
                     << ":" << lsb << "] indices reversed." << endl;
                cerr << get_fileline() << ":      : Did you mean " << sig->name() << "[" << lsb
                     << ":" << msb << "]?" << endl;
                des->errors += 1;
            ...
            if (name_tail.index.size() > sig->unpacked_dimensions()) {
                ...
                if (prefix_indices.size() + 2 <= sig->packed_dims().size()) {
                    ...
                    if (rcl) {
                        ...
                        cerr << get_fileline() << ": error: Index " << sig->name() << "[" << msb
 ...
 */
NetNet* PEIdent::elaborate_lnet_common_(Design* des,
                                        NetScope* scope,
                                        bool bidirectional_flag,
                                        bool var_allowed_in_sv) const {
    ivl_assert(*this, scope);
    ...
    if (sig == 0) {
        cerr << get_fileline() << ": error: Net " << path_ << " is not defined in this context."
             << endl;
        if (sr.is_found()) {
            cerr << sr.scope->get_fileline() << ":      : Found a " << sr.result_type()
                 << " with this name here." << endl;
        }
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": " << __func__ << ": "
             << "Found l-value path_=" << path_ << " as sig=" << sig->name()
             << " base_path=" << base_path << " member_path=" << member_path
             << " unpacked_dimensions()=" << sig->unpacked_dimensions() << endl;
    }
    ...
    // wire.
    if (gn_var_can_be_uwire() && var_allowed_in_sv && (sig->type() == NetNet::REG) &&
        (sig->peek_lref() == 0)) {
    ...
    if (sig->type() == NetNet::REG) {
        ...
        if (var_allowed_in_sv)
            cerr << "continuous assignment/module";
        else
            cerr << "primitive";
        if (gn_var_can_be_uwire()) {
            ...
            if (var_allowed_in_sv) {
                cerr << get_fileline() << ":      : "
                     << "This is allowed when SystemVerilog is enabled." << endl;
            }
        }
        des->errors += 1;
        return nullptr;
    ...
    if ((struct_type = sig->struct_type()) && !member_path.empty()) {
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": " << __func__ << ": "
                 << "Final, calculated member " << member_path << " offset=" << member_off
                 << " width=" << member_width << endl;
        }
        ...
        if (sig->packed_dimensions() > 1) {
            ...
            if (debug_elaborate) {
                cerr << get_fileline() << ": PEIdent::elaborate_lnet_common_: "
                     << "packed_base=" << *packed_base << ", member_off=" << member_off << endl;
            }
        }
        ...
        if (path_tail.index.size() > sig->unpacked_dimensions()) {
            if (sig->get_scalar()) {
                ...
                if (sig->data_type() == IVL_VT_REAL)
                    ...
                    cerr << "scalar";
                cerr << " array word: " << sig->name() << as_indices(unpacked_indices_const)
                     << endl;
                des->errors += 1;
                return 0;
            }
    ...
    if (sig->pin_count() > 1 && widx_flag) {
        if (widx < 0 || widx >= (long)sig->pin_count())
            return 0;
        NetNet* tmp = new NetNet(scope, scope->local_symbol(), sig->type(), sig->net_type());
        tmp->set_line(*this);
        tmp->local_flag(true);
    ...
    if (subnet_wid != sig->vector_width()) {
        ...
        const netvector_t* tmp2_vec = new netvector_t(sig->data_type(), subnet_wid - 1, 0);
        NetNet* subsig =
            new NetNet(sig->scope(), sig->scope()->local_symbol(), NetNet::WIRE, tmp2_vec);
        ...
        if (bidirectional_flag) {
            // Make a tran(VP)
            NetTran* sub =
                new NetTran(scope, scope->local_symbol(), sig->vector_width(), subnet_wid, lidx);
            sub->set_line(*this);
            des->add_node(sub);
            ...
            NetPartSelect* sub = new NetPartSelect(sig, lidx, subnet_wid, NetPartSelect::PV);
            des->add_node(sub);
            sub->set_line(*this);
 ...
 */
NetNet* PEIdent::elaborate_lnet(Design* des, NetScope* scope, bool var_allowed_in_sv) const {
    return elaborate_lnet_common_(des, scope, false, var_allowed_in_sv);
}

NetNet* PEIdent::elaborate_bi_net(Design* des, NetScope* scope, bool var_allowed_in_sv) const {
    return elaborate_lnet_common_(des, scope, true, var_allowed_in_sv);
}
...
NetNet* PEIdent::elaborate_subport(Design* des, NetScope* scope) const {
    ...
    switch (sig->port_type()) {
        ...
        case NetNet::PIMPLICIT:
            cerr << get_fileline() << ": internal error: signal " << path_ << " in module "
                 << scope_path(scope) << " is left as "
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_subport: "
             << "path_ = \"" << path_ << "\", unpacked_dimensions=" << sig->unpacked_dimensions()
    ...
    if (sig->unpacked_dimensions()) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PEIdent::elaborate_subport: "
                 << "path_=\"" << path_ << "\" is an unpacked array with " << sig->pin_count()
                 << " elements." << endl;
        }
    ...
    NetNet* tmp = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp2_vec);
    ...
    switch (sig->port_type()) {
        ...
        case NetNet::PINOUT:
            ps = new NetTran(scope, scope->local_symbol(), sig->vector_width(), swid, lidx);
...
bool PEIdent::is_collapsible_net(Design* des, NetScope* scope, NetNet::PortType port_type) const {
    ivl_assert(*this, scope);
    ...
    /* If this is SystemVerilog and the variable is not yet
       assigned by anything, then convert it to an unresolved
       wire. */
    if (gn_var_can_be_uwire() && (sig->type() == NetNet::REG) && (sig->peek_eref() == 0) &&
