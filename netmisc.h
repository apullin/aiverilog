...
static inline bool test_function_return_value(const symbol_search_results& search_results) {
    ...
    if (search_results.scope->type() != NetScope::FUNC)
        return false;
    if (search_results.net->name() != search_results.scope->basename())
        return false;
    return true;
}

extern bool symbol_search(const LineInfo* li,
                          Design* des,
                          NetScope* scope,
                          pform_name_t path,
                          unsigned lexical_pos,
                          struct symbol_search_results* res,
                          NetScope* start_scope = nullptr,
 ...
 * signed_flag.
 */
extern NetExpr* pad_to_width(
    NetExpr* expr, unsigned wid, bool signed_flag, const LineInfo& info, ivl_type_t use_type = 0);
/*
 * This version determines the extension method from the base expression type.
 */
inline NetExpr*
pad_to_width(NetExpr* expr, unsigned wid, const LineInfo& info, ivl_type_t use_type = 0) {
 ...
 */
extern NetExpr* cast_to_width(NetExpr* expr, unsigned wid, bool signed_flag, const LineInfo& info);

extern NetNet* pad_to_width(Design* des, NetNet* n, unsigned w, const LineInfo& info);

extern NetNet* pad_to_width_signed(Design* des, NetNet* n, unsigned w, const LineInfo& info);
...
extern bool calculate_part(const LineInfo* li,
                           Design* des,
                           NetScope* scope,
 ...
 */
extern NetExpr* normalize_variable_base(
    NetExpr* base, long msb, long lsb, unsigned long wid, bool is_up, long slice_off = 0);
 ...
 */
extern NetExpr*
normalize_variable_bit_base(const std::list<long>& indices, NetExpr* base, const NetNet* reg);
 ...
 * expression is (x+8).
 */
extern NetExpr* normalize_variable_part_base(const std::list<long>& indices,
                                             NetExpr* base,
                                             const NetNet* reg,
                                             unsigned long wid,
                                             bool is_up);
 ...
 * lwid set to (8).
 */
extern NetExpr* normalize_variable_slice_base(const std::list<long>& indices,
                                              NetExpr* base,
                                              const NetNet* reg,
                                              unsigned long& lwid);
...
extern void indices_to_expressions(Design* des,
                                   NetScope* scope,
                                   ...
                                   indices_flags& flags,
                                   std::list<NetExpr*>& indices,
                                   std::list<long>& indices_const);
...
extern NetExpr* normalize_variable_unpacked(const LineInfo& loc,
                                            ...
                                            const std::list<NetExpr*>& indices);

extern NetExpr* make_canonical_index(Design* des,
                                     NetScope* scope,
...
extern NetExpr* elab_and_eval(Design* des,
                              NetScope* scope,
                              PExpr* pe,
                              int context_width,
 ...
 */
extern NetExpr*
elab_and_eval(Design* des, NetScope* scope, PExpr* expr, ivl_type_t lv_net_type, bool need_const);

/*
 * This function is a variant of elab_and_eval that elaborates and
 * evaluates the arguments of a system task.
 */
extern NetExpr* elab_sys_task_arg(Design* des,
                                  NetScope* scope,
                                  perm_string name,
                                  unsigned arg_idx,
                                  PExpr* pe,
 ...
 */
extern NetExpr* elaborate_rval_expr(Design* des,
                                    NetScope* scope,
...
extern bool evaluate_range(Design* des,
                           NetScope* scope,
                           const LineInfo* li,
...
extern bool evaluate_ranges(Design* des,
                            NetScope* scope,
                            const LineInfo* li,
                            netranges_t& llist,
 ...
 */
extern std::list<hname_t> eval_scope_path(Design* des, NetScope* scope, const pform_name_t& path);
extern hname_t
 ...
 * processing.
 */
extern uint64_t get_scaled_time_from_real(const Design* des, NetScope* scope, const NetECReal* val);

extern void collapse_partselect_pv_to_concat(Design* des, NetNet* sig);

extern bool evaluate_index_prefix(Design* des,
                                  NetScope* scope,
                                  std::list<long>& prefix_indices,
                                  const std::list<index_component_t>& indices);

extern NetExpr* collapse_array_indices(Design* des,
                                       NetScope* scope,
                                       const NetNet* net,
                                       const std::list<index_component_t>& indices);

extern NetExpr* collapse_array_exprs(Design* des,
                                     NetScope* scope,
                                     const LineInfo* loc,
                                     const NetNet* net,
                                     const std::list<index_component_t>& indices);

extern void assign_unpacked_with_bufz(
