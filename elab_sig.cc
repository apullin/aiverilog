...
static void
sig_check_data_type(Design* des, const NetScope* scope, const PWire* wire, NetNet* sig) {
    ...
    if ((sig->type() == NetNet::WIRE) && (sig->data_type() != IVL_VT_LOGIC)) {
        if (gn_cadence_types_flag) {
            ...
            cerr << wire->get_fileline() << ": error: Net `" << wire->basename()
                 << "` can not be of type `" << sig->data_type() << "`." << endl;
            des->errors++;
        }
    }
    ...
    if (wire->symbol_type() == PNamedItem::NET) {
        cerr << wire->get_fileline() << ": error: Net `" << wire->basename()
             << "` can not be of type `" << sig->data_type() << "`." << endl;
        des->errors++;
    } else if (scope->type() == NetScope::MODULE && sig->port_type() != NetNet::NOT_A_PORT) {
        // Module ports only support wire types a the moment
        cerr << wire->get_fileline() << ": sorry: Port `" << wire->basename() << "` of module `"
             << scope->module_name() << "` with type `" << sig->data_type() << "` is not supported."
             << endl;
        des->errors++;
    }
}
...
static void
sig_check_port_type(Design* des, const NetScope* scope, const PWire* wire, const NetNet* sig) {
    ...
    if (sig->port_type() == NetNet::PINPUT && sig->type() == NetNet::REG &&
        !gn_var_can_be_uwire()) {
        cerr << wire->get_fileline() << ": error: Port `" << wire->basename() << "` of module `"
             << scope->module_name() << "` is declared as input and as a reg type." << endl;
        des->errors += 1;
    }

    if (sig->port_type() == NetNet::PINOUT && sig->type() == NetNet::REG) {
        cerr << wire->get_fileline() << ": error: Port `" << wire->basename() << "` of module `"
             << scope->module_name() << "` is declared as inout and as a reg type." << endl;
        des->errors += 1;
    }

    if (sig->port_type() == NetNet::PINOUT && sig->data_type() == IVL_VT_REAL) {
        cerr << wire->get_fileline() << ": error: Port `" << wire->basename() << "` of module `"
             << scope->module_name() << "` is declared as a real inout port." << endl;
        des->errors += 1;
    }
}
...
bool PScope::elaborate_sig_wires_(Design* des, NetScope* scope) const {
    ...
    for (map<perm_string, PWire*>::const_iterator wt = wires.begin(); wt != wires.end(); ++wt) {
        PWire* cur = (*wt).second;
...
static void
elaborate_sig_funcs(Design* des, NetScope* scope, const map<perm_string, PFunction*>& funcs) {
    ...
    for (mfunc_it_t cur = funcs.begin(); cur != funcs.end(); ++cur) {
        ...
        if (debug_elaborate) {
            cerr << cur->second->get_fileline() << ": elaborate_sig_funcs: "
                 << "Elaborate function " << use_name << " in " << scope_path(fscope) << endl;
        }
...
static void
elaborate_sig_tasks(Design* des, NetScope* scope, const map<perm_string, PTask*>& tasks) {
...
static void
elaborate_sig_classes(Design* des, NetScope* scope, const map<perm_string, PClass*>& classes) {
        ...
        for (unsigned cc = 0; cc < pp->expr.size(); cc += 1) {
            ...
            if (wt == wires.end()) {
                cerr << get_fileline() << ": error: "
                     << "Port " << port_path << " (" << (idx + 1) << ") of module " << mod_name()
            ...
            if ((*wt).second->get_port_type() == NetNet::NOT_A_PORT) {
                cerr << get_fileline() << ": error: "
                     << "Port " << pp->expr[cc]->path() << " (" << (idx + 1) << ") of module "
                     << mod_name() << " has no direction declaration." << endl;
                des->errors += 1;
            }
        }
    }
    ...
    for (map<perm_string, struct class_type_t::prop_info_t>::iterator cur =
        ...
        if (debug_elaborate) {
            cerr << pclass->get_fileline() << ": netclass_t::elaborate_sig: "
                 << "Elaborate static property " << cur->first << " as signal in scope "
                 << scope_path(class_scope_) << "." << endl;
        }

        /* NetNet*sig = */ new NetNet(class_scope_, cur->first, NetNet::REG, use_type);
    }
...
bool PGModule::elaborate_sig_mod_(Design* des, NetScope* scope, const Module* rmod) const {
    bool flag = true;
    ...
    for (unsigned idx = 0; idx < instance.size(); idx += 1) {
        ...
        if (my_scope->parent() != scope) {
            cerr << get_fileline() << ": internal error: "
                 ...
                 << scope_path(my_scope->parent()) << " instead of " << scope_path(scope) << endl;
        }
    ...
    for (scope_list_it_t cur = scope_list_.begin(); cur != scope_list_.end(); ++cur) {
        ...
        if (debug_elaborate)
            cerr << get_fileline() << ": debug: Elaborate nets in "
                 << "scope " << scope_path(*cur) << " in generate " << id_number << endl;
...
bool PGenerate::elaborate_sig_(Design* des, NetScope* scope) const {
    ...
    for (wires_it_t wt = wires.begin(); wt != wires.end(); ++wt) {
        ...
        if (debug_elaborate)
            cerr << get_fileline() << ": debug: Elaborate PWire " << cur->basename() << " in scope "
                 << scope_path(scope) << endl;
...
void PFunction::elaborate_sig(Design* des, NetScope* scope) const {
    ...
    if (gn_system_verilog() && (fname == "new" || fname == "new@")) {
        ...
        if (debug_elaborate)
            cerr << get_fileline() << ": PFunction::elaborate_sig: "
                 << "Scope " << scope_path(scope) << " is a CONSTRUCTOR, so use \"this\" argument"
    ...
    if (debug_elaborate)
        cerr << get_fileline() << ": PFunction::elaborate_sig: "
             ...
             << " with ret_sig width=" << (ret_sig ? ret_sig->vector_width() : 0) << "." << endl;
...
void PTaskFunc::elaborate_sig_ports_(Design* des,
                                     NetScope* scope,
                                     vector<NetNet*>& ports,
                                     vector<NetExpr*>& pdefs,
    ...
    for (size_t idx = 0; idx < ports_->size(); idx += 1) {
        ...
        if (tmp == 0) {
            cerr << get_fileline() << ": internal error: "
                 << "task/function " << scope_path(scope) << " is missing port " << port_name << "."
                 << endl;
            scope->dump(cerr);
            ...
            if (tmp->port_type() == NetNet::PINPUT) {
                ...
                if (tmp->data_type() == IVL_VT_CLASS) {
                    ...
                    tmp_def = elab_and_eval(
                        des, scope, ports_->at(idx).defe, -1, scope->need_const_func());
                }
                if (tmp_def == 0) {
                    cerr << get_fileline() << ": error: Unable to evaluate "
                         << *ports_->at(idx).defe << " as a port default expression." << endl;
                    des->errors += 1;
                }
            } else {
                cerr << get_fileline()
...
void PBlock::elaborate_sig(Design* des, NetScope* scope) const {
    ...
    if (pscope_name() != 0) {
        ...
        if (my_scope == 0) {
            cerr << get_fileline() << ": internal error: "
                 << "Unable to find child scope " << pscope_name() << " in this context?" << endl;
            des->errors += 1;
            my_scope = scope;
        } else {
            if (debug_elaborate)
                cerr << get_fileline() << ": debug: "
                     << "elaborate_sig descending into " << scope_path(my_scope) << "." << endl;
...
ivl_type_t
PWire::elaborate_type(Design* des, NetScope* scope, const netranges_t& packed_dimensions) const {
    ...
    ivl_assert(*this, use_data_type == IVL_VT_LOGIC || use_data_type == IVL_VT_BOOL);
...
NetNet* PWire::elaborate_sig(Design* des, NetScope* scope) {
    ...
    if (debug_elaborate) {
        ...
        cerr << ", unpacked_.size()=" << unpacked_.size() << endl;
    }
    ...
    if (port_set_ || net_set_) {
        if (warn_implicit_dimensions && port_set_ && net_set_ && net_.empty() && !port_.empty()) {
            cerr << get_fileline() << ": warning: "
                 << "var/net declaration of " << basename()
                 << " inherits dimensions from port declaration." << endl;
        }

        if (warn_implicit_dimensions && port_set_ && net_set_ && port_.empty() && !net_.empty()) {
            cerr << get_fileline() << ": warning: "
                 << "Port declaration of " << basename() << " inherits dimensions from var/net."
                 << endl;
        }
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": PWire::elaborate_sig: "
                 << "Calculated ranges for " << basename() << ". Now check for consistency."
                 << endl;
        }
        ...
        if (port_set_ && net_set_ && !test_ranges_eeq(plist, nlist) && dimensions_ok) {
            ...
            if (net_.empty()) {
                cerr << port_.front().first->get_fileline() << ": error: Vectored port ``" << name_
                     << "'' " << plist << " has a scalar net declaration at " << get_fileline()
                     << "." << endl;
                des->errors += 1;
            }
            ...
            if (!port_.empty() && !net_.empty()) {
                cerr << port_.front().first->get_fileline() << ": error: Vectored port ``" << name_
                     << "'' " << plist << " has a net declaration " << nlist << " at "
                     << net_.front().first->get_fileline() << " that does not match." << endl;
                des->errors += 1;
            }
        }
    ...
    unsigned nattrib = 0;
    const attrib_list_t* attrib_list = evaluate_attributes(attributes, nattrib, des, scope);
    ...
    if (debug_elaborate) {
        ...
        if (set_data_type_)
            cout << " " << *set_data_type_;
        cout << " " << name_ << unpacked_dimensions << " in scope " << scope_path(scope) << endl;
    }
