...
void pform_end_nature(const struct vlltype& loc) {
    ...
    if (ivl_nature_t dup_access_nat = access_function_nature[nature_access]) {
        cerr << tmp->get_fileline() << ": error: "
             << "Access function name " << nature_access << " is already used by nature "
             << dup_access_nat->name() << " declared at " << dup_access_nat->get_fileline() << "."
             << endl;
        error_count += 1;
    }
...
void pform_discipline_domain(const struct vlltype& loc, ivl_dis_domain_t use_domain) {
    ...
    if (discipline_domain != IVL_DIS_NONE) {
        cerr << loc.text << ":" << loc.first_line << ": error: "
             << "Too many domain attributes for discipline " << discipline_name << "." << endl;
        error_count += 1;
        return;
    }
...
void pform_discipline_potential(const struct vlltype& loc, const char* name) {
    if (discipline_potential) {
        cerr << loc.text << ":" << loc.first_line << ": error: "
             << "Too many potential natures for discipline " << discipline_name << "." << endl;
        error_count += 1;
        return;
    }
...
void pform_discipline_flow(const struct vlltype& loc, const char* name) {
    if (discipline_flow) {
        cerr << loc.text << ":" << loc.first_line << ": error: "
             << "Too many flow natures for discipline " << discipline_name << "." << endl;
        error_count += 1;
        return;
    }
...
void pform_end_discipline(const struct vlltype& loc) {
    ...
    ivl_discipline_t tmp = new ivl_discipline_s(
        discipline_name, discipline_domain, discipline_potential, discipline_flow);
...
void pform_attach_discipline(const struct vlltype& loc,
    ...
    for (list<pform_ident_t>::iterator cur = names->begin(); cur != names->end(); ++cur) {
        ...
        if (ivl_discipline_t tmp = cur_net->get_discipline()) {
            cerr << loc.text << ":" << loc.first_line << ": error: "
                 << "discipline " << discipline->name() << " cannot override existing discipline "
                 << tmp->name() << " on net " << cur_net->basename() << endl;
            error_count += 1;
