...
static void collect_parm_item(Design* des,
                              NetScope* scope,
                              perm_string name,
...
static void collect_scope_parameters(
    Design* des, NetScope* scope, const map<perm_string, LexicalScope::param_expr_t*>& parameters) {
    if (debug_scopes) {
...
static void collect_scope_specparams(
    Design* des, NetScope* scope, const map<perm_string, LexicalScope::param_expr_t*>& specparams) {
    if (debug_scopes) {
        cerr << scope->get_fileline() << ": " << __func__ << ": "
...
static void collect_scope_signals(NetScope* scope, const map<perm_string, PWire*>& wires) {
    for (map<perm_string, PWire*>::const_iterator cur = wires.begin(); cur != wires.end(); ++cur) {
        PWire* wire = (*cur).second;
        if (debug_scopes) {
            cerr << wire->get_fileline() << ": " << __func__ << ": "
                 << "adding placeholder for signal '" << wire->basename() << "' in scope '"
 ...
 */
static void elaborate_scope_enumeration(Design* des, NetScope* scope, enum_type_t* enum_type) {
    bool rc_flag;
    ...
    for (list<named_pexpr_t>::const_iterator cur = enum_type->names->begin();
        ...
        if (cur->parm) {
            ...
            if (val_const == 0) {
                cerr << use_enum->get_fileline() << ": error: Enumeration expression for "
                     << cur->name << " is not an integer constant." << endl;
                des->errors += 1;
                continue;
            }
            ...
            if (use_enum->base_type() == IVL_VT_BOOL && !cur_value.is_defined()) {
                cerr << use_enum->get_fileline() << ": error: Enumeration name " << cur->name
            ...
            if (((cur_value.len() != enum_width) || !cur_value.has_len()) && !is_signed &&
                cur_value.is_negative()) {
                cerr << use_enum->get_fileline() << ": error: Enumeration name " << cur->name
            ...
            if (cur_value.len() > enum_width) {
                ...
                for (unsigned idx = enum_width; idx < cur_value.len(); idx += 1) {
                    if (cur_value[idx] != cur_value[check_width]) {
                        ...
                        if (!is_signed && (idx == enum_width) && (cur_value[idx] == verinum::V0)) {
                            check_width += 1;
                            continue;
                        }
                ...
                if (!cur_value.has_sign() && (cur_value[enum_width] == verinum::V1)) {
                    cerr << use_enum->get_fileline() << ": error: Enumeration name " << cur->name
                         << " has a value that is too large: " << cur_value << "." << endl;
                    des->errors += 1;
                    break;
                }
        ...
        if (dup_name) {
            cerr << use_enum->get_fileline() << ": error: Enumeration name " << cur->name << " and "
                 << dup_name << " have the same value: " << cur_value << endl;
            des->errors += 1;
        }
        ...
        if (!rc_flag) {
            cerr << use_enum->get_fileline() << ": error: Duplicate enumeration name " << cur->name
                 << endl;
            des->errors += 1;
        }
...
static void
elaborate_scope_enumerations(Design* des, NetScope* scope, const vector<enum_type_t*>& enum_types) {
    if (debug_scopes) {
        cerr << scope->get_fileline() << ": " << __func__ << ": "
    ...
    for (vector<enum_type_t*>::const_iterator cur = enum_types.begin(); cur != enum_types.end();
         ++cur) {
...
static void elaborate_scope_class(Design* des, NetScope* scope, PClass* pclass) {
    ...
    if (debug_scopes) {
        cerr << pclass->get_fileline() << ": elaborate_scope_class: "
             << "Elaborate scope class " << pclass->pscope_name() << " within scope "
             << scope_path(scope) << endl;
    }
    ...
    NetScope* class_scope =
        new NetScope(scope, hname_t(pclass->pscope_name()), NetScope::CLASS, scope->unit());
    ...
    if (debug_scopes) {
        cerr << pclass->get_fileline() << ": elaborate_scope_class: "
             ...
             << " in class " << scope_path(class_scope) << ", scope=" << scope_path(scope) << "."
             << endl;
    }
    ...
    for (map<perm_string, PTask*>::iterator cur = pclass->tasks.begin(); cur != pclass->tasks.end();
        ...
        if (!cur->second->is_auto()) {
            cerr << "error: Lifetime of method `" << scope_path(method_scope)
        ...
        if (debug_scopes) {
            cerr << cur->second->get_fileline() << ": elaborate_scope_class: "
                 << "Elaborate method (task) scope " << scope_path(method_scope) << endl;
        }
    ...
    for (map<perm_string, PFunction*>::iterator cur = pclass->funcs.begin();
        ...
        if (debug_scopes) {
            cerr << cur->second->get_fileline() << ": elaborate_scope_class: "
                 << "Elaborate method (function) scope " << scope_path(method_scope) << endl;
        }
...
static void elaborate_scope_classes(Design* des, NetScope* scope, const vector<PClass*>& classes) {
    if (debug_scopes) {
        cerr << scope->get_fileline() << ": " << __func__ << ": "
...
static void replace_scope_parameters(Design* des,
                                     NetScope* scope,
                                     const LineInfo& loc,
    ...
    for (Module::replace_t::const_iterator cur = replacements.begin(); cur != replacements.end();
         ++cur) {
        PExpr* val = (*cur).second;
        if (val == 0) {
            cerr << loc.get_fileline() << ": internal error: "
                 << "Missing expression in parameter replacement for " << (*cur).first << endl;
            ;
        }
        ivl_assert(loc, val);
        if (debug_scopes) {
            cerr << loc.get_fileline() << ": debug: "
                 << "Replace " << (*cur).first << " with expression " << *val << " from "
                 << val->get_fileline() << "." << endl;
...
static void
elaborate_scope_events_(Design* des, NetScope* scope, const map<perm_string, PEvent*>& events) {
    for (map<perm_string, PEvent*>::const_iterator et = events.begin(); et != events.end(); ++et) {
...
static void
elaborate_scope_tasks(Design* des, NetScope* scope, const map<perm_string, PTask*>& tasks) {
...
static void elaborate_scope_func(Design* des, NetScope* scope, PFunction* task) {
    ...
    if (debug_scopes) {
        cerr << task->get_fileline() << ": elaborate_scope_func: "
             << "Elaborate function scope " << scope_path(task_scope) << endl;
    }
...
static void
elaborate_scope_funcs(Design* des, NetScope* scope, const map<perm_string, PFunction*>& funcs) {
...
bool Module::elaborate_scope(Design* des, NetScope* scope, const replace_t& replacements) {
    if (debug_scopes) {
...
void PGenerate::check_for_valid_genvar_value_(long value) {
    if (generation_flag < GN_VER2005 && value < 0) {
        cerr << get_fileline() << ": warning: A negative value (" << value
             << ") has been assigned to genvar '" << loop_index << "'." << endl;
        cerr << get_fileline()
             << ":        : This is illegal in "
...
bool PGenerate::generate_scope_loop_(Design* des, NetScope* container) {
    ...
    if (test == 0) {
        cerr << get_fileline()
             << ": error: Cannot evaluate generate \"loop\" "
    ...
    while (test->value().as_long()) {
        ...
        // genvar as a constant.
        {
            ...
            NetEConstParam* gp = new NetEConstParam(scope, loop_index, genvar_verinum);
        ...
        if (step == 0) {
            cerr << get_fileline()
                 << ": error: Cannot evaluate generate "
...
bool PGenerate::generate_scope_condit_(Design* des, NetScope* container, bool else_flag) {
    ...
    if (debug_scopes)
        cerr << get_fileline() << ": debug: Generate condition " << (else_flag ? "(else)" : "(if)")
             << " value=" << test->value() << ": Generate scope=" << use_name << endl;

    if (directly_nested) {
        if (debug_scopes)
            cerr << get_fileline() << ": debug: Generate condition "
                 << (else_flag ? "(else)" : "(if)") << " detected direct nesting." << endl;
...
bool PGenerate::generate_scope_case_(Design* des, NetScope* container) {
    ...
    while (cur != generate_schemes.end()) {
        ...
        for (unsigned idx = 0; idx < item->item_test.size() && !match_flag; idx += 1) {
            NetExpr* item_value_ex = elab_and_eval(des, container, item->item_test[idx], -1, true);
            NetEConst* item_value_co = dynamic_cast<NetEConst*>(item_value_ex);
            if (item_value_co == 0) {
                cerr << get_fileline() << ": error: Cannot evaluate "
                     << " genvar case item expression: " << *item->item_test[idx] << endl;
                des->errors += 1;
                return false;
            }
    ...
    for (generate_it_t cur = generate_schemes.begin(); cur != generate_schemes.end(); ++cur) {
        ...
        if (debug_scopes) {
            cerr << get_fileline() << ": elaborate_subscope_direct_: "
                 << "Elaborate direct subscope " << curp->scope_name << " within scope "
                 << scope_name << endl;
        }
...
void delayed_elaborate_scope_mod_instances::elaborate_runrun() {
    if (debug_scopes)
        cerr << obj_->get_fileline() << ": debug: "
             << "Resume scope elaboration of instances of " << mod_->mod_name() << "." << endl;
...
void PGModule::elaborate_scope_mod_(Design* des, Module* mod, NetScope* sc) const {
    if (get_name() == "") {
        cerr << get_fileline() << ": error: Instantiation of module " << mod->mod_name()
             << " requires an instance name." << endl;
        des->errors += 1;
        return;
    }
    ...
    for (NetScope* scn = sc; scn; scn = scn->parent()) {
        ...
        if (in_genblk) {
            ...
            if (rl_count > recursive_mod_limit) {
                cerr << get_fileline() << ": error: instance " << scope_path(sc) << "."
                     << get_name() << " of module " << mod->mod_name() << " is nested too deep."
                     << endl;
                cerr << get_fileline()
                     << ":      : check for "
                        "proper recursion termination or increase the "
                        "limit ("
                     << recursive_mod_limit << ") with the -pRECURSIVE_MOD_LIMIT flag." << endl;
                des->errors += 1;
                return;
            }
            continue;
        ...
        cerr << get_fileline() << ":      : The offending instance is " << get_name() << " within "
             << scope_path(scn) << "." << endl;
        des->errors += 1;
        return;
    }
    ...
    if (is_array()) {
        ...
        if (debug_scopes)
            cerr << get_fileline() << ": debug: delay elaborate_scope"
                 << " of array of " << get_name() << " in scope " << scope_path(sc) << "." << endl;

        elaborator_work_item_t* tmp = new delayed_elaborate_scope_mod_instances(des, this, mod, sc);
...
void PGModule::elaborate_scope_mod_instances_(Design* des, Module* mod, NetScope* sc) const {
    ...
    for (int idx = 0; idx < instance_count; idx += 1) {
        ...
        NetScope* my_scope = new NetScope(sc,
                                          use_name,
                                          NetScope::MODULE,
                                          0,
                                          ...
                                          mod->program_block,
                                          mod->is_interface);
        my_scope->set_line(get_file(), mod->get_file(), get_lineno(), mod->get_lineno());
            ...
            for (;;) {
                ...
                if (cur == mod->param_names.end()) {
                    cerr << get_fileline()
                         << ": warning: "
                            "ignoring "
                         << overrides_->size() - mod->param_names.size()
                         << " extra parameter override(s) for "
                            "instance '"
                         << use_name << "' of module '" << mod->mod_name() << "' which expects "
                         << mod->param_names.size() << " parameter(s)." << endl;
                    break;
                }
...
void PBlock::elaborate_scope(Design* des, NetScope* scope) const {
    ...
    if (pscope_name() != 0) {
        ...
        if (debug_scopes)
            cerr << get_fileline() << ": debug: "
                 << "Elaborate block scope " << use_name << " within " << scope_path(scope) << endl;

        // The scope type is begin-end or fork-join. The
        // sub-types of fork-join are not interesting to the scope.
        my_scope = new NetScope(
            scope, use_name, bl_type_ != BL_SEQ ? NetScope::FORK_JOIN : NetScope::BEGIN_END);
