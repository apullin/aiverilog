...
NetNet* sub_net_from(Design* des, NetScope* scope, long val, NetNet* sig) {
    const netvector_t* zero_vec = new netvector_t(sig->data_type(), sig->vector_width() - 1, 0);
    NetNet* zero_net = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, zero_vec);
    ...
    const netvector_t* tmp_vec = new netvector_t(sig->data_type(), sig->vector_width() - 1, 0);
...
NetNet* cast_to_int2(Design* des, NetScope* scope, NetNet* src, unsigned wid) {
    ...
    const netvector_t* tmp_vec = new netvector_t(IVL_VT_BOOL, wid - 1, 0, src->get_signed());
    NetNet* tmp = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_vec);
    tmp->set_line(*src);
    tmp->local_flag(true);
...
static NetExpr* make_add_expr(NetExpr* expr, long val) {
    ...
    NetEBAdd* res = new NetEBAdd(add_op, expr, val_c, expr->expr_width(), expr->has_sign());
    res->set_line(*expr);
...
static NetExpr* make_sub_expr(long val, NetExpr* expr) {
    ...
    NetEBAdd* res = new NetEBAdd('-', val_c, expr, expr->expr_width(), expr->has_sign());
    res->set_line(*expr);
...
static NetExpr* make_sub_expr(NetExpr* expr, long val) {
    ...
    NetEBAdd* res = new NetEBAdd('-', expr, val_c, expr->expr_width(), expr->has_sign());
    res->set_line(*expr);
 ...
 */
NetExpr* normalize_variable_base(
    NetExpr* base, long msb, long lsb, unsigned long wid, bool is_up, long soff) {
    bool msb_lo = msb < lsb;
...
NetExpr* normalize_variable_bit_base(const list<long>& indices, NetExpr* base, const NetNet* reg) {
...
NetExpr* normalize_variable_part_base(
    const list<long>& indices, NetExpr* base, const NetNet* reg, unsigned long wid, bool is_up) {
    const netranges_t& packed_dims = reg->packed_dims();
    ivl_assert(*base, indices.size() + 1 == packed_dims.size());
...
NetExpr* normalize_variable_slice_base(const list<long>& indices,
                                       NetExpr* base,
                                       const NetNet* reg,
                                       unsigned long& lwid) {
    const netranges_t& packed_dims = reg->packed_dims();
    ...
    if (pcur->get_msb() >= pcur->get_lsb()) {
        ...
        if (min_wid < num_bits(loff))
            min_wid = num_bits(loff);
        if (loff != 0)
            min_wid += 1;
        base = pad_to_width(base, min_wid, *base);
...
ostream& operator<<(ostream& o, __IndicesManip<long> val) {
    for (list<long>::const_iterator cur = val.val.begin(); cur != val.val.end(); ++cur) {
        o << "[" << *cur << "]";
    }
    return o;
}
...
ostream& operator<<(ostream& o, __IndicesManip<NetExpr*> val) {
    for (list<NetExpr*>::const_iterator cur = val.val.begin(); cur != val.val.end(); ++cur) {
        o << "[" << *(*cur) << "]";
    }
    return o;
}
 ...
 */
void indices_to_expressions(Design* des,
                            NetScope* scope,
    ...
    for (list<index_component_t>::const_iterator cur = src.begin(); count > 0; ++cur, --count) {
...
static NetExpr* normalize_variable_unpacked(const netranges_t& dims, const list<long>& indices) {
    ...
    int idx = 0;
    for (list<long>::const_iterator cur = indices.begin(); cur != indices.end(); ++cur, ++idx) {
        long tmp = *cur;
...
NetExpr* normalize_variable_unpacked(const LineInfo& loc,
    ...
    int idx = 0;
    for (list<NetExpr*>::const_iterator cur = indices.begin(); cur != indices.end(); ++cur, ++idx) {
        NetExpr* tmp = *cur;
        ...
        if (canonical_expr == 0) {
            ...
            canonical_expr = new NetEBAdd(
                '+', canonical_expr, tmp_scaled, canonical_expr->expr_width() + 1, expr_has_sign);
        }
    }
...
NetExpr* make_canonical_index(Design* des,
                              NetScope* scope,
                              const LineInfo* loc,
    ...
    indices_flags flags;
    indices_to_expressions(
        des, scope, loc, src, src.size(), need_const, flags, indices_expr, indices_const);
...
NetExpr* elab_and_eval(Design* des,
                       NetScope* scope,
                       PExpr* pe,
                       int context_width,
                       bool need_const,
    ...
    if (debug_elaborate) {
        cerr << pe->get_fileline() << ": elab_and_eval: test_width of " << *pe << endl;
        cerr << pe->get_fileline() << ":              : "
             << "returns type=" << pe->expr_type() << ", context_width=" << context_width
             << ", signed=" << pe->has_sign() << ", expr_width=" << expr_width
    ...
    if ((min_width != UINT_MAX) && (pe->expr_type() != IVL_VT_REAL) && (pos_context_width > 0) &&
        (expr_width > pos_context_width)) {
    ...
    if ((mode >= PExpr::LOSSLESS) && (expr_width > width_cap) && (expr_width > pos_context_width)) {
...
NetExpr*
elab_and_eval(Design* des, NetScope* scope, PExpr* pe, ivl_type_t lv_net_type, bool need_const) {
    if (debug_elaborate) {
...
NetExpr* elab_sys_task_arg(
    Design* des, NetScope* scope, perm_string name, unsigned arg_idx, PExpr* pe, bool need_const) {
    if (!pe)
        return nullptr;
    ...
    if (debug_elaborate) {
        cerr << pe->get_fileline() << ": " << __func__ << ": "
             << "test_width of " << name << " argument " << (arg_idx + 1) << " " << *pe << endl;
        cerr << pe->get_fileline() << ":        "
...
bool evaluate_range(Design* des,
                    NetScope* scope,
                    const LineInfo* li,
...
bool evaluate_ranges(Design* des,
                     NetScope* scope,
                     const LineInfo* li,
                     netranges_t& llist,
    ...
    for (list<pform_range_t>::const_iterator cur = rlist.begin(); cur != rlist.end(); ++cur) {
 ...
 */
hname_t
eval_path_component(Design* des, NetScope* scope, const name_component_t& comp, bool& error_flag) {
    ...
    for (list<index_component_t>::const_iterator cur = comp.index.begin(); cur != comp.index.end();
         ++cur) {
...
std::list<hname_t> eval_scope_path(Design* des, NetScope* scope, const pform_name_t& path) {
...
const char* human_readable_op(const char op, bool unary) {
    ...
    switch (op) {
        ...
        case 'q':
            type = "->";
            break;  // Logical implication
        case 'Q':
            type = "<->";
            break;  // Logical equivalence
...
void collapse_partselect_pv_to_concat(Design* des, NetNet* sig) {
    ...
    if (debug_elaborate) {
        cerr << sig->get_fileline() << ": debug: "
             << "Collapse " << device_count << " NetPartSelect::PV devices into a concatenation."
             << endl;
    }

    NetConcat* cat = new NetConcat(scope, scope->local_symbol(), ps_map.size(), device_count, true);
    des->add_node(cat);
    cat->set_line(*sig);
 ...
 */
bool evaluate_index_prefix(Design* des,
                           NetScope* scope,
 ...
 * replace the exprs.
 */
NetExpr* collapse_array_exprs(Design* des,
                              NetScope* scope,
                              const LineInfo* loc,
                              const NetNet* net,
                              const list<index_component_t>& indices) {
    ...
    indices_flags flags;
    indices_to_expressions(
        des, scope, loc, indices, net->packed_dimensions(), false, flags, exprs, exprs_const);
    ...
    for (size_t idx = 0; idx < net->packed_dimensions(); idx += 1, ++pcur, ++ecur) {
        ...
        // the msb/lsb values.
        NetExpr* tmp = normalize_variable_base(*ecur, msb, lsb, cur_slice_width, msb > lsb);
 ...
 */
NetExpr* collapse_array_indices(Design* des,
                                NetScope* scope,
                                const NetNet* net,
                                const list<index_component_t>& indices) {
    list<long> prefix_indices;
...
static void assign_unpacked_with_bufz_dim(Design* des,
                                          NetScope* scope,
                                          const LineInfo* loc,
                                          NetNet* lval,
                                          NetNet* rval,
    ...
    for (unsigned int idx = 0; idx < l_dims[dim].width(); idx++) {
        if (dim == l_dims.size() - 1) {
            ...
            assign_unpacked_with_bufz_dim(
                des, scope, loc, lval, rval, stride, dim + 1, idx_l, idx_r);
        }
...
void assign_unpacked_with_bufz(
    Design* des, NetScope* scope, const LineInfo* loc, NetNet* lval, NetNet* rval) {
...
void check_for_inconsistent_delays(const NetScope* scope) {
    ...
    if (display_ts_dly_warning && used_explicit_timescale && used_implicit_timescale) {
 ...
 */
bool calculate_param_range(
    const LineInfo& line, ivl_type_t par_type, long& par_msv, long& par_lsv, long length) {
