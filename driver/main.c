...
static int t_version_only(void)
{
      ...
      fflush(0);
      snprintf(tmp, sizeof tmp, CMD_EXE_QUOTE "%s%civlpp" CMD_EXE_QUOTE " -V",
	       ivlpp_dir, sep);
      rc = system(tmp);
      if (rc != 0) {
      ...
      fflush(0);
      snprintf(tmp, sizeof tmp, CMD_EXE_QUOTE "%s%civl" CMD_EXE_QUOTE
	       " -V -C\"%s\" -C\"%s\"", ivl_dir, sep,
...
{
      snprintf(tmp, sizeof tmp, CMD_EXE_QUOTE "%s%civlpp" CMD_EXE_QUOTE
	       "%s%s%s -F\"%s\" -f\"%s\" -p\"%s\"%s",
	       ivlpp_dir, sep,
               verbose_flag ? " -v" : "",
	       e_flag ? "" : " -L",
...
int main(int argc, char **argv)
{
	...
	/* Write the preprocessor command needed to preprocess a
	   ...
	   files. */
      fprintf(iconfig_file, "ivlpp:" CMD_EXE_QUOTE "%s%civlpp" CMD_EXE_QUOTE
	      " %s -L -F\"%s\" -P\"%s\"\n",
	      ivlpp_dir, sep,
              strchr(warning_flags, 'r') ? "-Wredef-all" :
              strchr(warning_flags, 'R') ? "-Wredef-chg" : "",
