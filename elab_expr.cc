...
NetExpr* elaborate_rval_expr(Design* des,
                             NetScope* scope,
                             ivl_type_t lv_net_type,
                             PExpr* expr,
                             bool need_const,
                             bool force_unsigned) {
    return elaborate_rval_expr(des,
                               scope,
                               lv_net_type,
...
NetExpr* elaborate_rval_expr(Design* des,
    ...
    if (lv_net_type && typed_elab) {
        ...
        rval = elab_and_eval(
            des, scope, expr, context_wid, need_const, false, lv_type, force_unsigned);
    }
...
unsigned PExpr::fix_width_(width_mode_t mode) {
    ...
    if ((mode == UPSIZE) && type_is_vectorable(expr_type_) && (width < integer_width))
...
NetExpr* PEAssignPattern::elaborate_expr(Design* des,
                                         NetScope* scope,
                                         ivl_type_t ntype,
    ...
    if (auto uarray_type = dynamic_cast<const netuarray_t*>(ntype)) {
        return elaborate_expr_uarray_(
            des, scope, uarray_type, uarray_type->static_dimensions(), 0, need_const);
    }

    if (auto parray_type = dynamic_cast<const netparray_t*>(ntype)) {
        return elaborate_expr_packed_(des,
                                      scope,
                                      parray_type->base_type(),
                                      parray_type->packed_width(),
                                      parray_type->slice_dimensions(),
                                      0,
                                      need_const);
    }
    ...
    if (auto struct_type = dynamic_cast<const netstruct_t*>(ntype)) {
        return elaborate_expr_struct_(des, scope, struct_type, need_const);
    }
...
NetExpr* PEAssignPattern::elaborate_expr_array_(
    Design* des, NetScope* scope, const netarray_t* array_type, bool need_const, bool up) const {
    ...
    for (size_t idx = 0; idx < parms_.size(); idx += 1) {
        elem_exprs[elem_idx] = elaborate_rval_expr(des, scope, elem_type, parms_[idx], need_const);
        if (up)
            elem_idx++;
        else
            elem_idx--;
...
NetExpr* PEAssignPattern::elaborate_expr_uarray_(Design* des,
                                                 NetScope* scope,
    ...
    if (dims[cur_dim].width() != parms_.size()) {
        cerr << get_fileline() << ": error: Unpacked array assignment pattern expects "
             ...
             << get_fileline() << ":      : Found " << parms_.size() << " element(s)." << endl;
        des->errors++;
    }
    ...
    for (size_t idx = 0; idx < parms_.size(); idx++) {
        ...
        if (const auto ap = dynamic_cast<PEAssignPattern*>(parms_[idx])) {
            expr = ap->elaborate_expr_uarray_(des, scope, uarray_type, dims, cur_dim, need_const);
        } else if (dynamic_cast<PEConcat*>(parms_[idx])) {
            cerr << get_fileline() << ": sorry: "
                 << "Array concatenation is not yet supported." << endl;
            des->errors++;
            ...
            cerr << get_fileline() << ": error: Expression " << *parms_[idx]
                 << " is not compatible with this context."
                 << " Expected array or array-like expression." << endl;
            des->errors++;
        }
...
NetExpr* PEAssignPattern::elaborate_expr_packed_(Design* des,
                                                 NetScope* scope,
    ...
    for (size_t idx = 0; idx < parms_.size(); idx++) {
        ...
        if (ap)
            expr =
                ap->elaborate_expr_packed_(des, scope, base_type, width, dims, cur_dim, need_const);
        else
            expr =
                elaborate_rval_expr(des, scope, nullptr, base_type, width, parms_[idx], need_const);
        if (expr)
...
NetExpr* PEAssignPattern::elaborate_expr_struct_(Design* des,
                                                 NetScope* scope,
    ...
    if (members.size() != parms_.size()) {
        cerr << get_fileline() << ": error: Struct assignment pattern expects " << members.size()
             << " element(s) in this context.\n"
             << get_fileline() << ":      : Found " << parms_.size() << " element(s)." << endl;
        des->errors++;
    }
    ...
    for (size_t idx = 0; idx < std::min(parms_.size(), members.size()); idx++) {
        auto expr = elaborate_rval_expr(des, scope, members[idx].net_type, parms_[idx], need_const);
        if (expr)
            neconcat->set(idx, expr);
    }
...
unsigned PEBinary::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
    ...
    if (l_type == IVL_VT_CLASS || r_type == IVL_VT_CLASS) {
        cerr << get_fileline() << ": error: "
             << "Class/null is not allowed with the '" << human_readable_op(op_) << "' operator."
             << endl;
        des->errors += 1;
    }
 ...
 */
NetExpr*
PEBinary::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    ...
    if (left_->expr_type() == IVL_VT_REAL && type_is_vectorable(right_->expr_type())) {
        r_width = right_->expr_width();
    } else {
        right_->cast_signed(signed_flag_);
    }
    if (right_->expr_type() == IVL_VT_REAL && type_is_vectorable(left_->expr_type())) {
 ...
 */
NetExpr*
PEBinary::elaborate_expr_base_(Design* des, NetExpr* lp, NetExpr* rp, unsigned expr_wid) const {
    if (debug_elaborate) {
...
NetExpr*
PEBinary::elaborate_expr_base_div_(Design* des, NetExpr* lp, NetExpr* rp, unsigned expr_wid) const {
...
NetExpr*
PEBinary::elaborate_expr_base_mult_(Design*, NetExpr* lp, NetExpr* rp, unsigned expr_wid) const {
...
unsigned PEBComp::test_width(Design* des, NetScope* scope, width_mode_t&) {
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEBComp::test_width: "
             << "Comparison expression operands are " << l_type << " " << l_width << " bits and "
             << r_type << " " << r_width << " bits. Resorting to " << l_width_ << " bits and "
             << r_width_ << " bits." << endl;
    }
    ...
    switch (op_) {
        ...
        case 'E': /* === */
        case 'N': /* !== */
            if ((l_type == IVL_VT_CLASS || r_type == IVL_VT_CLASS) && l_type != r_type) {
                cerr << get_fileline() << ": error: "
                     << "Both arguments (" << l_type << ", " << r_type
                     << ") must be class/null for '" << human_readable_op(op_) << "' operator."
                     << endl;
                des->errors += 1;
            }
            break;
        default:
            if (l_type == IVL_VT_CLASS || r_type == IVL_VT_CLASS) {
                cerr << get_fileline() << ": error: "
                     << "Class/null is not allowed with the '" << human_readable_op(op_)
                     << "' operator." << endl;
                des->errors += 1;
            }
    }
...
NetExpr*
PEBComp::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag
    ...
    if (debug_elaborate) {
        ...
        cerr << get_fileline() << ": PEBComp::elaborate_expr: "
             << "op_: " << human_readable_op(op_) << ", expr_wid=" << expr_wid << ", flags=0x"
             << hex << flags << dec << endl;
    }
...
NetExpr*
PEBLogic::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    ivl_assert(*this, left_);
    ivl_assert(*this, right_);
...
unsigned PEBLeftWidth::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
    ...
    if ((mode >= EXPAND) && type_is_vectorable(expr_type_)) {
        ...
        if (!right_->has_sign())
            c_width -= 1;
        if (rc && (r_width <= c_width))
            r_val = rc->value().as_long();

        if (debug_elaborate && rc) {
            cerr << get_fileline() << ": PEBLeftWidth::test_width: "
                 << "Evaluated rc=" << *rc << ", r_val=" << r_val << ", width_cap=" << width_cap
                 << endl;
        }
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEBLeftWidth::test_width: "
             << "Done calculating expr_width_=" << expr_width_ << ", min_width_=" << min_width_
             << ", mode=" << width_mode_name(mode) << endl;
    }
...
NetExpr* PEBLeftWidth::elaborate_expr(Design* des,
                                      NetScope* scope,
                                      unsigned expr_wid,
                                      unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag
...
NetExpr* PEBPower::elaborate_expr_leaf(Design*, NetExpr* lp, NetExpr* rp, unsigned expr_wid) const {
    if (debug_elaborate) {
        cerr << get_fileline() << ": debug: elaborate expression " << *this
...
static unsigned int
sign_cast_width(Design* des, NetScope* scope, PExpr& expr, PExpr::width_mode_t& mode) {
    unsigned int width;
...
NetExpr*
PEBShift::elaborate_expr_leaf(Design* des, NetExpr* lp, NetExpr* rp, unsigned expr_wid) const {
    switch (op_) {
        case 'l':  // <<
        case 'r':  // >>
        case 'R':  // >>>
    ...
    if (const NetEConst* lpc = dynamic_cast<NetEConst*>(lp)) {
        ...
        if ((op_ == 'R' && signed_flag_) && (shift >= expr_wid)) {
            if (debug_elaborate)
                cerr << get_fileline() << ": debug: "
                     << "Value signed-right-shifted " << shift << " beyond width of " << expr_wid
...
unsigned PECallFunction::test_width_sfunc_(Design* des, NetScope* scope, width_mode_t& mode) {
    ...
    if (name == "$ivlh_to_unsigned") {
        ...
        if (nexpr == 0) {
            cerr << get_fileline() << ": error: "
                 << "Unable to evaluate " << name << " width argument: " << *pexpr << endl;
            return 0;
        }
    ...
    if (name == "$signed" || name == "$unsigned") {
        ...
        if (debug_elaborate)
            cerr << get_fileline() << ": debug: " << name << " argument width = " << expr_width_
                 << "." << endl;
    ...
    if (debug_elaborate)
        cerr << get_fileline() << ": " << __func__ << ": "
             << "test_width of system function " << name << " returns wid=" << expr_width_
 ...
 */
static NetFuncDef* find_function_definition(Design* des, NetScope*, NetScope* func) {
...
unsigned PECallFunction::test_width_method_(Design*,
                                            NetScope*,
    ...
    if (search_results.path_tail.size() > 1) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PECallFunction::test_width_method_: "
                 << "Chained path tail (" << search_results.path_tail << ") not supported." << endl;
        }
        return 0;
    }
    ...
    if (search_results.net && search_results.net->enumeration()) {
        ...
        if (method_name == "first" || method_name == "last" || method_name == "prev" ||
            method_name == "next") {
            expr_type_ = IVL_VT_BOOL;
...
unsigned PECallFunction::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
    if (debug_elaborate) {
    ...
    if (test_function_return_value(search_results)) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PECallFunction::test_width: "
                 << "Net " << search_results.net->name() << " is actually a function call to "
                 << scope_path(search_results.scope) << "." << endl;
        }
...
NetExpr* PECallFunction::cast_to_width_(NetExpr* expr, unsigned wid) const {
    if (debug_elaborate) {
        cerr << get_fileline() << ": PECallFunction::cast_to_width_: "
             << "cast to " << wid << " bits " << (signed_flag_ ? "signed" : "unsigned")
 ...
 */
NetExpr* PECallFunction::elaborate_sfunc_(Design* des,
                                          NetScope* scope,
                                          unsigned expr_wid,
                                          unsigned flags) const {
    perm_string name = peek_tail_name(path_);
    ...
    for (const auto& parm : parms_) {
        if (!parm.name.nil()) {
            ...
            cerr << parm.get_fileline() << ": error: "
                 << "The system function `" << name << "` has no argument called `" << parm.name
                 << "`." << endl;
        }
    }
    ...
    if (name == "$signed" || name == "$unsigned") {
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": PECallFunction::elaborate_sfunc_: " << name
                 << " expression is the argument cast to expr_wid=" << expr_wid << endl;
        }
        const PExpr* expr = parms_[0].parm;
    ...
    if (name == "$sizeof" || name == "$bits") {
        ...
        if (const PETypename* type_expr = dynamic_cast<PETypename*>(expr)) {
            ...
            use_width = 1;
            while (const netuarray_t* utype = dynamic_cast<const netuarray_t*>(data_type)) {
                use_width = netrange_width(utype->static_dimensions(), use_width);
                data_type = utype->element_type();
            }
            if (!data_type->packed()) {
                use_width = 0;
                cerr << get_fileline() << ": error: "
                     << "Invalid data type for $bits()." << endl;
                des->errors++;
            } else {
...
NetExpr*
PECallFunction::elaborate_access_func_(Design* des, NetScope* scope, ivl_nature_t nature) const {
    ...
    if (parms_.size() == 1) {
        ...
        if ((branch = find_existing_implicit_branch(sig, gnd))) {
            if (debug_elaborate)
                cerr << get_fileline() << ": debug: "
                     << "Re-use implicit branch from " << branch->get_fileline() << endl;
        } else {
...
static NetExpr* check_for_enum_methods(const LineInfo* li,
    ...
    if (debug_elaborate) {
        cerr << li->get_fileline() << ": " << __func__ << ": "
             << "Check for method " << method_name << " of enumeration at "
             << netenum->get_fileline() << endl;
        cerr << li->get_fileline() << ": " << __func__ << ": "
    ...
    if (method_name == "name") {
        ...
        sys_expr = new NetESFunc("$ivl_enum_method$name", &netstring_t::type_string, 2);
        ...
        if (args.size() != 0 && args[0]) {
            count = elaborate_rval_expr(des, scope, &netvector_t::atom2u32, args[0]);
            if (!count) {
                cerr << li->get_fileline()
                     << ": error: unable to elaborate "
                        "enumeration method argument "
                     << use_path << "." << method_name << "(" << args[0] << ")." << endl;
                des->errors++;
            } else if (const NetEEvent* evt = dynamic_cast<NetEEvent*>(count)) {
                cerr << evt->get_fileline() << ": error: An event '" << evt->event()->name()
        ...
        sys_expr = new NetESFunc(func_name, netenum, 2 + (count != nullptr));
        NetENetenum* def = new NetENetenum(netenum);
        def->set_line(*li);
        sys_expr->parm(0, def);
        sys_expr->parm(1, expr);
        if (count)
            sys_expr->parm(2, count);
    ...
    if (debug_elaborate) {
        cerr << li->get_fileline() << ": " << __func__ << ": Generate " << sys_expr->name() << "("
             << use_path << ")" << endl;
    }
...
bool calculate_part(const LineInfo* li,
                    Design* des,
                    NetScope* scope,
    ...
    do {
        ...
        if (member == 0) {
            cerr << li->get_fileline() << ": error: Member " << member_name
                 << " is not a member of struct type of " << net->name() << "." << completed_path
                 << endl;
            des->errors += 1;
            return 0;
        }
        ...
        if (debug_elaborate) {
            cerr << li->get_fileline() << ": check_for_struct_members: "
                 << "Member type: " << *member_type << " (" << typeid(*member_type).name() << ")"
                 << endl;
        }
        ...
        if (const netstruct_t* tmp_struct = dynamic_cast<const netstruct_t*>(member_type)) {
            ...
            if (!member_comp.index.empty()) {
                ...
                bool rc =
                    calculate_part(li, des, scope, member_comp.index.back(), tail_off, tail_wid);
                if (!rc)
                    return 0;
            ...
            if (debug_elaborate) {
                cerr << li->get_fileline() << ": check_for_struct_members: "
                     << "member_comp=" << member_comp << " has " << member_comp.index.size()
                     << " indices." << endl;
            }
            ...
            if (!member_comp.index.empty()) {
                ...
                if (debug_elaborate) {
                    cerr << li->get_fileline() << ": check_for_struct_members: "
                         ...
                         << ", mem_packed_dims.size()==" << mem_packed_dims.size() << endl;
                }
                ...
                if (debug_elaborate) {
                    cerr << li->get_fileline() << ": check_for_struct_members: "
                         ...
                         << " tail_off=" << tail_off << " tail_wid=" << tail_wid << " off=" << off
                         << " use_width=" << use_width << endl;
                }
            ...
            if (member_comp.index.size() > mem_packed_dims.size()) {
                cerr << li->get_fileline() << ": error: "
                     << "Too many index expressions for member " << member_name << "." << endl;
                des->errors += 1;
                return 0;
            }
            ...
            cerr << li->get_fileline() << ": internal error: "
                 << "Unexpected member type? " << *member_type << endl;
            des->errors += 1;
            struct_type = 0;
        }
...
static NetExpr* elaborate_struct_member_access(const PExpr* expr,
    ...
    return check_for_struct_members(expr, des, scope, net, packed_index, path_tail, word_index);
}
...
NetExpr* PEIdent::elaborate_expr_class_field_(Design* des,
                                              NetScope* scope,
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_expr: "
             << "Ident " << sr.path_head << " look for property " << comp << endl;
    }

    if (sr.path_tail.size() > 1) {
        cerr << get_fileline() << ": sorry: "
             << "Nested member path not yet supported for class properties." << endl;
        return nullptr;
    }
    ...
    if (par_val)
        return elaborate_expr_param_(
            des, scope, par_val, class_type->class_scope(), par_type, expr_wid, flags);

    int pidx = class_type->property_idx_from_name(comp.name);
    if (pidx < 0) {
        cerr << get_fileline() << ": error: "
             << "Class " << class_type->get_name() << " has no property " << comp.name << "."
             << endl;
        des->errors += 1;
        return 0;
    }
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": check_for_class_property: "
             << "Property " << comp.name << " of net " << sr.net->name()
    ...
    if (qual.test_static()) {
        ...
        return class_static_property_expression(this, class_type, prop_name);
    }
    ...
    if (const netuarray_t* tmp_ua = dynamic_cast<const netuarray_t*>(tmp_type)) {
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": PEIdent::elaborate_expr_class_member_: "
                 << "Property " << class_type->get_prop_name(pidx) << " has " << dims.size()
                 << " dimensions, "
        ...
        if (dims.size() != comp.index.size()) {
            ...
            canon_index = make_canonical_index(des, scope, this, comp.index, tmp_ua, false);
        }
    }
...
NetExpr* PECallFunction::elaborate_expr(Design* des,
                                        NetScope* scope,
                                        unsigned expr_wid,
                                        unsigned flags) const {
    if (debug_elaborate) {
...
NetExpr* PECallFunction::elaborate_expr_(Design* des, NetScope* scope, unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag
        ...
        if (!search_results.path_tail.empty()) {
            ...
            if (tmp) {
                ...
                cerr << get_fileline() << ": error: "
                     ...
                     << search_results.path_head.back() << " has no method \""
                     << search_results.path_tail << "(...)\"." << endl;
                des->errors += 1;
                return 0;
            }
        }

        cerr << get_fileline() << ": error: Object " << search_results.path_head.back() << " in "
             << scope_path(search_results.scope) << " is not a function." << endl;
        des->errors += 1;
        return 0;
    }
    ...
    if (gn_system_verilog() && (path_.size() == 1)) {
        ...
        if (c_scope && (c_scope == dscope->get_class_scope())) {
            ...
            use_search_results.path_head.push_back(
                name_component_t(perm_string::literal(THIS_TOKEN)));
    ...
    if (dscope->is_const_func()) {
        ...
        is_const_func_call =
            (callee_scope == caller_scope) || (callee_scope->type() == NetScope::PACKAGE);
    }
...
NetExpr* PECallFunction::elaborate_base_(Design* des,
                                         NetScope* scope,
                                         NetScope* dscope,
                                         unsigned flags) const {
    ...
    if (need_const && !def->proc()) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PECallFunction::elaborate_base_: "
                 << "Try to elaborate " << scope_path(dscope) << " as constant function." << endl;
        }
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PECallFunction::elaborate_base_: "
             << "Expecting " << parms_count << " argument for function " << scope_path(dscope)
             << "." << endl;
    }
    ...
    unsigned parm_errors = elaborate_arguments_(des, scope, def, need_const, parms, 0);
    ...
    if (def->is_void()) {
        cerr << get_fileline() << ": error: void function `" << dscope->basename()
             << "` can not be called in an expression." << endl;
        des->errors++;
        return nullptr;
    }
 ...
 */
unsigned PECallFunction::elaborate_arguments_(Design* des,
                                              NetScope* scope,
                                              const NetFuncDef* def,
                                              bool need_const,
    ...
    if (actual_count > parm_count) {
        cerr << get_fileline() << ": error: "
             << "Too many arguments (" << actual_count << ", expecting " << parm_count << ")"
    ...
    for (unsigned idx = 0; idx < parm_count; idx += 1) {
        ...
        if (tmp) {
            parms[pidx] =
                elaborate_rval_expr(des, scope, def->port(pidx)->net_type(), tmp, need_const);
            if (parms[pidx] == 0) {
                parm_errors += 1;
                continue;
            }
            ...
            if (debug_elaborate)
                cerr << get_fileline() << ": debug:"
                     ...
                     << " argwid=" << parms[pidx]->expr_width() << ": " << *parms[idx] << endl;
 ...
 */
NetExpr* PECallFunction::elaborate_expr_method_(Design* des,
                                                NetScope* scope,
    ...
    if (search_results.net && search_results.net->data_type() == IVL_VT_QUEUE &&
        ...
        NetESelect* tmp =
            new NetESelect(sub_expr, mux, darray->element_width(), darray->element_type());
        tmp->set_line(*this);
        sub_expr = tmp;
    }
    ...
    if (search_results.net && search_results.net->data_type() == IVL_VT_QUEUE &&
        ...
        if (method_name == "pop_back") {
            ...
            NetESFunc* sys_expr = new NetESFunc("$ivl_queue_method$pop_back", element_type, 1);
            sys_expr->set_line(*this);
            sys_expr->parm(0, sub_expr);
            return sys_expr;
        }
        ...
        if (method_name == "pop_front") {
            ...
            NetESFunc* sys_expr = new NetESFunc("$ivl_queue_method$pop_front", element_type, 1);
            sys_expr->set_line(*this);
            sys_expr->parm(0, sub_expr);
            return sys_expr;
        }

        cerr << get_fileline() << ": error: Method " << method_name << " is not a queue method."
             << endl;
        des->errors += 1;
        return 0;
    }
    ...
    if (search_results.net && search_results.net->enumeration()) {
        ...
        // Get the method name that we are looking for.
        perm_string method_name = search_results.path_tail.back().name;
        return check_for_enum_methods(
            this, des, scope, netenum, path_, method_name, sub_expr, parms_);
    }
    ...
    if (sub_expr->expr_type() == IVL_VT_CLASS) {
        ...
        if (method == 0) {
            cerr << get_fileline() << ": Error: " << method_name << " is not a method of class "
    ...
    if (sub_expr->expr_type() == IVL_VT_STRING) {
        ...
        if (method_name == "len") {
            NetESFunc* sys_expr =
                new NetESFunc("$ivl_string_method$len", &netvector_t::atom2u32, 1);
            sys_expr->parm(0, sub_expr);
            return sys_expr;
        }
        ...
        if (method_name == "atoi") {
            NetESFunc* sys_expr =
                new NetESFunc("$ivl_string_method$atoi", netvector_t::integer_type(), 1);
            sys_expr->parm(0, sub_expr);
            return sys_expr;
        }
        ...
        if (method_name == "atoreal") {
            NetESFunc* sys_expr =
                new NetESFunc("$ivl_string_method$atoreal", &netreal_t::type_real, 1);
            sys_expr->parm(0, sub_expr);
            return sys_expr;
        }
        ...
        if (method_name == "atohex") {
            NetESFunc* sys_expr =
                new NetESFunc("$ivl_string_method$atohex", netvector_t::integer_type(), 1);
            sys_expr->parm(0, sub_expr);
            return sys_expr;
        }
        ...
        if (method_name == "substr") {
            if (parms_.size() != 2)
                cerr << get_fileline() << ": error: Method `substr()`"
                     << " requires 2 arguments, got " << parms_.size() << "." << endl;

            static const std::vector<perm_string> parm_names = {perm_string::literal("i"),
                                                                perm_string::literal("j")};
            auto args = map_named_args(des, parm_names, parms_);

            NetESFunc* sys_expr =
                new NetESFunc("$ivl_string_method$substr", &netstring_t::type_string, 3);
            sys_expr->set_line(*this);
            ...
            for (int i = 0; i < 2; i++) {
                ...
                auto expr = elaborate_rval_expr(des, scope, &netvector_t::atom2u32, args[i], false);
        ...
        cerr << get_fileline() << ": error: Method " << method_name << " is not a string method."
             << endl;
        return 0;
    }
 ...
 */
NetExpr* PECallFunction::elaborate_expr_method_par_(
    Design* des, const NetScope* scope, const symbol_search_results& search_results) const {
    ...
    if (dynamic_cast<const netstring_t*>(par_type)) {
        ...
        cerr << get_fileline() << ": error: "
             << "Unknown or unsupport string method: " << method_name << endl;
        return 0;
    }
    ...
    cerr << get_fileline()
         << ": sorry: Don't know how to handle methods of parameters of type:" << endl;
...
NetExpr*
PECastSize::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag
...
NetExpr*
PECastType::elaborate_expr(Design* des, NetScope* scope, ivl_type_t type, unsigned flags) const {
...
NetExpr*
PECastType::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag
    ...
    if (dynamic_cast<const netreal_t*>(target_type_)) {
        ...
        cerr << get_fileline() << " error: Expression of type `" << sub->expr_type()
             << "` can not be cast to target type `real`." << endl;
        des->errors++;
        return nullptr;
...
NetExpr*
PECastSign::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    ivl_assert(*this, base_);
...
NetExpr*
PEConcat::elaborate_expr(Design* des, NetScope* scope, ivl_type_t ntype, unsigned flags) const {
...
NetExpr*
PEConcat::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag

    concat_depth += 1;

    if (debug_elaborate) {
        cerr << get_fileline() << ": debug: Elaborate expr=" << *this << ", expr_wid=" << expr_wid
             << endl;
    }
    ...
    for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
        ...
        if (width_modes_[idx] != SIZED) {
            cerr << ex->get_fileline() << ": error: "
                 << "Concatenation operand \"" << *parms_[idx] << "\" has indefinite width."
                 << endl;
            des->errors += 1;
            parm_errors += 1;
            continue;
...
bool PEIdent::calculate_packed_indices_(Design* des,
                                        NetScope* scope,
                                        const NetNet* net,
    ...
    if (path_.back().index.size() > dimensions) {
        cerr << get_fileline() << ": error: the number of indices (" << path_.back().index.size()
             << ") is greater than the number of dimensions (" << dimensions << ")." << endl;
        des->errors += 1;
        return false;
    }
...
bool PEIdent::calculate_bits_(Design* des, NetScope* scope, long& msb, bool& defined) const {
    defined = true;
 ...
 */
void PEIdent::calculate_parts_(
    Design* des, NetScope* scope, long& msb, long& lsb, bool& defined) const {
    defined = true;
    const name_component_t& name_tail = path_.back();
    ivl_assert(*this, !name_tail.index.empty());
...
bool PEIdent::calculate_up_do_width_(Design* des, NetScope* scope, unsigned long& wid) const {
    const name_component_t& name_tail = path_.back();
    ivl_assert(*this, !name_tail.index.empty());
 ...
 */
NetExpr* PEIdent::calculate_up_do_base_(Design* des, NetScope* scope, bool need_const) const {
    const name_component_t& name_tail = path_.back();
    ivl_assert(*this, !name_tail.index.empty());
...
ivl_type_t PEIdent::resolve_type_(Design* des,
                                  const symbol_search_results& sr,
    ...
    while (type) {
        ...
        if (auto class_type = dynamic_cast<const netclass_t*>(type)) {
            ...
            if (name == "num")
                type = &netvector_t::atom2s32;
            else if ((name == "first") || (name == "last") || (name == "next") || (name == "prev"))
                type = netenum;
            else
                return nullptr;
        } else {
...
unsigned PEIdent::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
    ...
    if (type) {
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": PEIdent::test_width: " << path_ << ", type=" << expr_type_
                 << ", width=" << expr_width_ << ", signed_=" << (signed_flag_ ? "true" : "false")
                 << ", use_depth=" << use_depth << endl;
        }
        return expr_width_;
    }
...
NetExpr*
PEIdent::elaborate_expr(Design* des, NetScope* scope, ivl_type_t ntype, unsigned flags) const {
    bool need_const = NEED_CONST & flags;
    ...
    if (!sr.net) {
        cerr << get_fileline() << ": error: Unable to bind variable `" << path_ << "' in `"
    ...
    if (!sr.path_tail.empty()) {
        if (net->struct_type()) {
            return elaborate_struct_member_access(
                this, des, scope, net, sr.path_head, sr.path_tail);
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_expr: "
             << "Typed ident " << net->name() << " with " << use_comp.index.size() << " indices"
    ...
    if (net->unpacked_dimensions() != use_comp.index.size()) {
        cerr << get_fileline() << ": sorry: "
             << "Net " << net->name() << " expects " << net->unpacked_dimensions() << ", but got "
             << use_comp.index.size() << "." << endl;
        des->errors += 1;
 ...
 */
NetExpr*
PEIdent::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    NetExpr* result;
...
NetExpr*
PEIdent::elaborate_expr_(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    ...
    if (sr.par_val != 0) {
        if (!sr.path_tail.empty()) {
            cerr << get_fileline() << ": error: Parameter name " << sr.path_head
                 << " can't have member names (" << sr.path_tail << ")." << endl;
            des->errors += 1;
        }

        return elaborate_expr_param_or_specparam_(
            des, scope, sr.par_val, sr.scope, sr.type, expr_wid, flags);
    }
    ...
    if (sr.net != 0) {
            ...
            if (debug_elaborate) {
                cerr << get_fileline()
                     << ": PEIdent::elaborate_expr: "
                        "Ident "
                     << sr.path_head << " look for struct member " << sr.path_tail << endl;
            }

            return elaborate_struct_member_access(
                this, des, scope, sr.net, sr.path_head, sr.path_tail);
        }
        ...
        if (sr.net->darray_type() && !sr.path_tail.empty()) {
            if (debug_elaborate) {
                cerr << get_fileline()
                     << ": PEIdent::elaborate_expr: "
                        "Ident "
                     << sr.path_head << " looking for array property " << sr.path_tail << endl;
            }
        ...
        if (sr.net->queue_type() && !sr.path_tail.empty()) {
            if (debug_elaborate) {
                cerr << get_fileline() << ": PEIdent::elaborate_expr: "
                     << "Ident " << sr.path_head << " looking for queue property " << sr.path_tail
                     << endl;
            }
            ...
            if (member_comp.name == "pop_back") {
                NetESFunc* fun = new NetESFunc("$ivl_queue_method$pop_back", element_type, 1);
                fun->set_line(*this);
            ...
            if (member_comp.name == "pop_front") {
                NetESFunc* fun = new NetESFunc("$ivl_queue_method$pop_front", element_type, 1);
                fun->set_line(*this);
        ...
        if ((sr.net->data_type() == IVL_VT_STRING) && !sr.path_tail.empty()) {
            if (debug_elaborate) {
                cerr << get_fileline()
                     << ": PEIdent::elaborate_expr: "
                        "Ident "
                     << sr.path_head << " looking for string property " << sr.path_tail << endl;
            }

            ivl_assert(*this, sr.path_tail.size() == 1);
            const name_component_t member_comp = sr.path_tail.front();
            cerr << get_fileline() << ": sorry: String method '" << member_comp.name
                 << "' currently requires ()." << endl;
            des->errors += 1;
            return 0;
        }

        if (dynamic_cast<const netclass_t*>(sr.type) && !sr.path_tail.empty()) {
            return elaborate_expr_class_field_(des, scope, sr, expr_wid, flags);
        }
        ...
        if (sr.net->enumeration() && !sr.path_tail.empty()) {
            ...
            if (debug_elaborate) {
                cerr << get_fileline()
                     << ": PEIdent::elaborate_expr: "
                        "Ident "
                     << sr.path_head << " look for enumeration method " << sr.path_tail << endl;
            }
            ...
            return check_for_enum_methods(this,
                                          des,
                                          scope,
                                          netenum,
        ...
        if (!sr.path_tail.empty()) {
            cerr << get_fileline() << ": error: Variable " << sr.path_head
                 << " does not have a field named: " << sr.path_tail << "." << endl;
            des->errors += 1;
            return 0;
        }

        NetExpr* tmp = elaborate_expr_net(des, scope, sr.net, sr.scope, expr_wid, flags);
        ...
        if (debug_elaborate) {
            cerr << get_fileline()
                 << ": PEIdent::elaborate_expr: "
                    "Expression as net. expr_wid="
                 << expr_wid << ", tmp->expr_width()=" << tmp->expr_width() << ", tmp=" << *tmp
                 << endl;
        }
    ...
    if (sr.eve != 0) {
        ...
        if (!sr.path_tail.empty()) {
            cerr << get_fileline() << ": error: Event name " << sr.path_head
                 << " can't have member names (" << sr.path_tail << ")" << endl;
            des->errors += 1;
        }
    ...
    if (!(SYS_TASK_ARG & flags)) {
        ...
        cerr << get_fileline() << ": error: Unable to bind "
             << ((NEED_CONST & flags) ? "parameter" : "wire/reg/memory") << " `" << path_
    ...
    if ((path_.size() == 1)) {
        ...
        if (NetScope* nsc = scope->child(use_name)) {
            ...
            if (debug_elaborate)
                cerr << get_fileline() << ": debug: Found scope " << use_name << " in scope "
                     << scope->basename() << endl;
    ...
    if (NetScope* nsc = des->find_scope(scope, spath)) {
        ...
        if (debug_elaborate)
            cerr << get_fileline() << ": debug: Found scope " << nsc->basename() << " in "
                 << scope_path(scope) << endl;
...
static verinum param_part_select_bits(const verinum& par_val, long wid, long lsv) {
...
NetExpr* PEIdent::elaborate_expr_param_bit_(Design* des,
                                            NetScope* scope,
                                            const NetExpr* par,
    ...
    if (!calculate_param_range(*this, par_type, par_msv, par_lsv, par_ex->value().len()))
        return 0;
    ...
    if (sel->expr_type() == IVL_VT_REAL) {
        cerr << get_fileline() << ": error: Index expression for " << name << "[" << *sel
    ...
    if (debug_elaborate)
        cerr << get_fileline() << ": debug: Calculate bit select " << name << "[" << *sel
             << "] from range "
...
NetExpr* PEIdent::elaborate_expr_param_part_(Design* des,
                                             NetScope* scope,
                                             const NetExpr* par,
                                             const NetScope*,
                                             ivl_type_t par_type,
    ...
    if ((msv > lsv && par_msv < par_lsv) || (msv < lsv && par_msv >= par_lsv)) {
        perm_string name = peek_tail_name(path_);
        cerr << get_fileline() << ": error: Part select " << name << "[" << msv << ":" << lsv
    ...
    if (warn_ob_select) {
        ...
        if (par_ex->value().has_len() && (base + wid > (long)par->expr_width())) {
            perm_string name = peek_tail_name(path_);
            cerr << get_fileline() << ": warning: Part select " << name << "[" << msv << ":" << lsv
                 << "] is selecting "
                    "after the parameter "
...
static void warn_param_ob(long par_msv,
                          long par_lsv,
                          bool defined,
                          long par_base,
                          unsigned long wid,
...
NetExpr* PEIdent::elaborate_expr_param_idx_up_(Design* des,
                                               NetScope* scope,
                                               const NetExpr* par,
                                               const NetScope* found_in,
                                               ivl_type_t par_type,
    ...
    if (debug_elaborate)
        cerr << get_fileline() << ": debug: Calculate part select " << name << "[" << *base
             << "+:" << wid << "] from range "
             << "[" << par_msv << ":" << par_lsv << "]." << endl;

    if (base->expr_type() == IVL_VT_REAL) {
        cerr << get_fileline()
             << ": error: Indexed part select base "
                "expression for "
             << name << "[" << *base << "+:" << wid << "] cannot be a real value." << endl;
        des->errors += 1;
        return 0;
    }
    ...
    if (const NetEConst* base_c = dynamic_cast<NetEConst*>(base)) {
        ...
        if (warn_ob_select) {
            ...
            warn_param_ob(par_msv, par_lsv, defined, lsv - par_base, wid, pwid, this, name, true);
        }
        verinum result = param_part_select_bits(par_ex->value(), wid, lsv - par_base);
        NetEConst* result_ex = new NetEConst(result);
        result_ex->set_line(*this);
        return result_ex;
    }
...
NetExpr* PEIdent::elaborate_expr_param_idx_do_(Design* des,
                                               NetScope* scope,
                                               const NetExpr* par,
                                               const NetScope* found_in,
                                               ivl_type_t par_type,
    ...
    if (const NetEConst* base_c = dynamic_cast<NetEConst*>(base)) {
        ...
        if (warn_ob_select) {
            ...
            if (par_ex->has_width())
                pwid = par_ex->expr_width() - 1;
            warn_param_ob(par_msv, par_lsv, defined, lsv - par_base, wid, pwid, this, name, false);
        }
...
NetExpr* PEIdent::elaborate_expr_param_or_specparam_(Design* des,
    ...
    return elaborate_expr_param_(des, scope, par, found_in, par_type, expr_wid, flags);
}
...
NetExpr* PEIdent::elaborate_expr_param_(Design* des,
    ...
    if (par->expr_type() == IVL_VT_REAL && use_sel != index_component_t::SEL_NONE) {
        perm_string name = peek_tail_name(path_);
        cerr << get_fileline() << ": error: "
    ...
    if (use_sel == index_component_t::SEL_BIT)
        return elaborate_expr_param_bit_(des, scope, par, found_in, par_type, need_const);

    if (use_sel == index_component_t::SEL_PART)
        return elaborate_expr_param_part_(des, scope, par, found_in, par_type, expr_wid);

    if (use_sel == index_component_t::SEL_IDX_UP)
        return elaborate_expr_param_idx_up_(des, scope, par, found_in, par_type, need_const);

    if (use_sel == index_component_t::SEL_IDX_DO)
        return elaborate_expr_param_idx_do_(des, scope, par, found_in, par_type, need_const);
    ...
    if (etmp) {
        if (debug_elaborate)
            cerr << get_fileline() << ": debug: "
                 << "Elaborate parameter <" << path_ << "> as enumeration constant." << *etmp
                 << endl;
        tmp = etmp->dup_expr();
    } else {
        perm_string name = peek_tail_name(path_);
        ...
        if (ctmp) {
            ...
            if (debug_elaborate)
                cerr << get_fileline() << ": debug: "
                     << "Elaborate parameter <" << name << "> as constant " << *tmp << endl;
        }
 ...
 */
NetExpr* PEIdent::elaborate_expr_net_word_(Design* des,
                                           NetScope* scope,
                                           NetNet* net,
                                           NetScope* found_in,
                                           unsigned expr_wid,
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_net_word_: "
             << "expr_wid=" << expr_wid
             << ", net->get_scalar()==" << (net->get_scalar() ? "true" : "false") << endl;
    }
    ...
    if (name_tail.index.empty()) {
        cerr << get_fileline() << ": error: Array " << path() << " needs an array index here."
             << endl;
        des->errors += 1;
        return 0;
    }
    ...
    if (net->get_scalar() && word_sel != index_component_t::SEL_NONE) {
        ...
        if (res->expr_type() == IVL_VT_REAL)
            ...
            cerr << "scalar";
        cerr << " array word: " << net->name() << as_indices(unpacked_indices) << endl;
        des->errors += 1;
        delete res;
        return 0;
    }
 ...
 */
NetExpr* PEIdent::elaborate_expr_net_part_(
    Design* des, NetScope* scope, NetESignal* net, NetScope*, unsigned expr_wid) const {
    ...
    if (prefix_indices.size() + 1 < net->sig()->packed_dims().size()) {
        ...
        if (!mrc || !lrc) {
            ...
            cerr << net->sig()->name();
            if (net->sig()->unpacked_dimensions() > 0)
                cerr << "[]";
            cerr << "." << endl;
            des->errors += 1;
            return 0;
    ...
    if (warn_ob_select) {
        if ((sb_lsb >= (signed)net->vector_width()) || (sb_msb >= (signed)net->vector_width())) {
            cerr << get_fileline()
                 << ": warning: "
                    "Part select "
 ...
 */
NetExpr* PEIdent::elaborate_expr_net_idx_up_(
    Design* des, NetScope* scope, NetESignal* net, NetScope*, bool need_const) const {
    if (net->sig()->data_type() == IVL_VT_STRING) {
        cerr << get_fileline()
             << ": error: Cannot take the index part "
                "select of a string ('"
             << net->name() << "')." << endl;
        des->errors += 1;
        return 0;
    ...
    if (base->expr_type() == IVL_VT_REAL) {
        cerr << get_fileline()
             << ": error: Indexed part select base "
                "expression for "
             << net->sig()->name() << "[" << *base << "+:" << wid << "] cannot be a real value."
             << endl;
        des->errors += 1;
        return 0;
    }
        ...
        if (base_c->value().is_defined()) {
            ...
            if (warn_ob_select) {
                if (rel_base < 0) {
                    ...
                    if (net->word_index())
                        cerr << "[]";
                    cerr << "[" << lsv << "+:" << wid << "] is selecting before vector." << endl;
                }
                if (rel_base + wid > net->vector_width()) {
                    cerr << get_fileline() << ": warning: " << net->name();
                    if (net->word_index())
                        cerr << "[]";
                    cerr << "[" << lsv << "+:" << wid << "] is selecting after vector." << endl;
                }
            }
        } else {
 ...
 */
NetExpr* PEIdent::elaborate_expr_net_idx_do_(
    Design* des, NetScope* scope, NetESignal* net, NetScope*, bool need_const) const {
    if (net->sig()->data_type() == IVL_VT_STRING) {
        cerr << get_fileline()
             << ": error: Cannot take the index part "
...
NetExpr* PEIdent::elaborate_expr_net_bit_(
    ...
    if (mux->expr_type() == IVL_VT_REAL) {
        cerr << get_fileline() << ": error: Index expression for " << net->sig()->name() << "["
             << *mux << "] cannot be a real value." << endl;
        des->errors += 1;
        return 0;
    }
    ...
    if (const NetEConst* msc = dynamic_cast<NetEConst*>(mux)) {
        ...
        if (prefix_indices.size() + 2 <= sig_packed.size()) {
            ...
            if (!rc) {
                cerr << get_fileline() << ": error: Index " << net->sig()->name() << "[" << msv
...
NetExpr* PEIdent::elaborate_expr_net_bit_last_(
    Design*, NetScope*, NetESignal* net, NetScope* /* found_in */, bool need_const) const {
    if (need_const) {
        cerr << get_fileline() << ": error: "
...
NetExpr* PEIdent::elaborate_expr_net(Design* des,
                                     NetScope* scope,
                                     NetNet* net,
                                     NetScope* found_in,
                                     unsigned expr_wid,
...
NetExpr*
PENewArray::elaborate_expr(Design* des, NetScope* scope, ivl_type_t ntype, unsigned flags) const {
 ...
 */
NetExpr* PENewClass::elaborate_expr_constructor_(
    Design* des, NetScope* scope, const netclass_t* ctype, NetExpr* obj, unsigned /*flags*/) const {
    ...
    if (new_scope == 0) {
        ...
        if (parms_.size() > 0) {
            cerr << get_fileline() << ": error: "
                 << "Class " << ctype->get_name() << " has no constructor, but you passed "
                 << parms_.size() << " arguments to the new operator." << endl;
            des->errors += 1;
        }
        return obj;
    }
    ...
    if (def == 0) {
        cerr << get_fileline() << ": internal error: "
             << "Scope " << scope_path(new_scope) << " is missing constructor definition." << endl;
        des->errors += 1;
    }
    ivl_assert(*this, def);
    ...
    if ((parms_.size() + 1) > def->port_count()) {
        cerr << get_fileline() << ": error: Argument count mismatch."
             ...
             << " to constructor expecting " << (def->port_count() - 1) << " arguments." << endl;
        des->errors += 1;
    }
    ...
    for (size_t idx = 1; idx < parms.size(); idx += 1) {
        ...
        if (args[idx - 1]) {
            parms[idx] =
                elaborate_rval_expr(des, scope, def->port(idx)->net_type(), args[idx - 1], false);
...
NetExpr*
PENewClass::elaborate_expr(Design* des, NetScope* scope, ivl_type_t ntype, unsigned flags) const {
    ...
    if (class_type_) {
        ivl_type_t elab_class_type = class_type_->elaborate_type(des, scope);
        ctype = dynamic_cast<const netclass_t*>(elab_class_type);
        if (!ctype) {
            cerr << get_fileline() << ": error: Incompatible type in"
                 << " typed constructor call.\n"
                 << get_fileline() << ":      : Constructor type `" << *elab_class_type
                 << "` is not a class type." << endl;
            des->errors++;
            return nullptr;
        }
        ...
        if (!ntype->type_compatible(ctype)) {
            cerr << get_fileline() << ": error: Incompatible type in"
                 << " typed constructor call.\n"
                 << get_fileline() << ":      : Constructor type `" << *ctype
                 << "` is not compatible with the target type `" << *ntype << "`." << endl;
            des->errors++;
            return nullptr;
        }
    }
    ...
    if (ctype->is_virtual()) {
        cerr << get_fileline() << ": error: "
             << "Can not create object of virtual class `" << ctype->get_name() << "`." << endl;
        des->errors++;
        return 0;
    }
...
NetExpr*
PENewCopy::elaborate_expr(Design* des, NetScope* scope, ivl_type_t obj_type, unsigned) const {
...
NetEConst* PENumber::elaborate_expr(Design*, NetScope*, unsigned expr_wid, unsigned) const {
 ...
 */
NetEConst* PEString::elaborate_expr(Design*, NetScope*, unsigned expr_wid, unsigned) const {
    verinum val(value());
...
unsigned PETernary::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
    ...
    if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Ternary expression type=" << expr_type_ << ", width=" << expr_width_
             << " (tru_type=" << tru_type << ", fal_type=" << fal_type << ")" << endl;

    return fix_width_(mode);
}

bool NetETernary::test_operand_compat(ivl_variable_type_t l, ivl_variable_type_t r) {
 ...
 */
NetExpr*
PETernary::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag
    ...
    if (const NetEConst* tmp = dynamic_cast<NetEConst*>(con)) {
        ...
        if (cval.get(0) == verinum::V1) {
            ...
            NetExpr* dmy = elab_and_eval_alternative_(des, scope, fal_, expr_wid, flags, true);
            delete dmy;

            delete con;
            return elab_and_eval_alternative_(des, scope, tru_, expr_wid, flags, true);
        }
        ...
        if (cval.get(0) == verinum::V0) {
            ...
            // Evaluate the alternate expression to find any errors.
            NetExpr* dmy = elab_and_eval_alternative_(des, scope, tru_, expr_wid, flags, true);
            delete dmy;

            delete con;
            return elab_and_eval_alternative_(des, scope, fal_, expr_wid, flags, true);
        }
    ...
    NetExpr* tru = elab_and_eval_alternative_(des, scope, tru_, expr_wid, flags, false);
    if (tru == 0) {
        delete con;
        return 0;
    }

    NetExpr* fal = elab_and_eval_alternative_(des, scope, fal_, expr_wid, flags, false);
    if (fal == 0) {
        delete con;
        delete tru;
        return 0;
    ...
    if (!NetETernary::test_operand_compat(tru->expr_type(), fal->expr_type())) {
        cerr << get_fileline() << ": error: Data types " << tru->expr_type() << " and "
 ...
 * self-determined.
 */
NetExpr* PETernary::elab_and_eval_alternative_(Design* des,
                                               NetScope* scope,
                                               PExpr* expr,
                                               unsigned expr_wid,
                                               unsigned flags,
    ...
    if (short_cct && (expr_type_ == IVL_VT_REAL) && (expr->expr_type() != IVL_VT_REAL))
...
NetExpr* PETypename::elaborate_expr(Design* des, NetScope*, ivl_type_t, unsigned) const {
...
unsigned PEUnary::test_width(Design* des, NetScope* scope, width_mode_t& mode) {
    ...
    switch (op_) {
        ...
        case '!': {
            ...
            if (debug_elaborate)
                cerr << get_fileline() << ": debug: "
                     << "Test width of sub-expression of " << op_ << " returns " << sub_width << "."
                     << endl;
        }
            return expr_width_;
    }
...
NetExpr*
PEUnary::elaborate_expr(Design* des, NetScope* scope, unsigned expr_wid, unsigned flags) const {
    flags &= ~SYS_TASK_ARG;  // don't propagate the SYS_TASK_ARG flag
        ...
        case 'd':
            ...
            if (expr_wid != expr_->expr_width()) {
                ...
                if (dynamic_cast<NetEConst*>(ip) || dynamic_cast<NetECReal*>(ip)) {
                     ...
                     */
                    cerr << get_fileline() << ": error: "
                         << "inappropriate use of " << human_readable_op(op_, true) << " operator."
                         << endl;
                    des->errors += 1;
                    return 0;
                }
...
NetNet* Design::find_discipline_reference(ivl_discipline_t dis, NetScope* scope) {
    ...
    if (debug_elaborate)
        cerr << gnd->get_fileline() << ": debug: "
             ...
             << " for discipline=" << dis->name() << " in scope=" << scope_path(scope) << endl;
