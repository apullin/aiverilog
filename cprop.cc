...
void cprop_functor::lpm_ff(Design*, NetFF* obj) {
    ...
    if (connected(obj->pin_Data(), obj->pin_Q()) && (!obj->pin_Sclr().is_linked()) &&
        (!obj->pin_Sset().is_linked()) && (!obj->pin_Aclr().is_linked()) &&
...
void cprop_functor::lpm_part_select(Design* des, NetPartSelect* obj) {
        ...
        if (top > obj_set[idx]->base()) {
            if (debug_optimizer)
                cerr << obj->get_fileline() << ": cprop::lpm_part_select: "
                     ...
                     << ") overlaps PV starting at " << obj_set[idx]->base() << ". Give up."
                     << endl;
            return;
        }
    }
    ...
    NetConcat* cncat = new NetConcat(scope, scope->local_symbol(), sig_width, part_count);
    cncat->set_line(*obj);
    des->add_node(cncat);
    ...
    for (size_t idx = 0; idx < obj_set.size(); idx += 1) {
        ...
        if (cobj->base() > off) {
            NetNet* val = output_2_state ? make_const_0(des, scope, cobj->base() - off)
    ...
    if (off < sig_width) {
        NetNet* val = output_2_state ? make_const_0(des, scope, sig_width - off)
...
void cprop_dc_functor::lpm_const(Design*, NetConst* obj) {
    ...
    for (unsigned idx = 0; idx < obj->pin_count(); idx += 1) {
        for (Link* clnk = nexus_info[idx].nex->first_nlink(); clnk; clnk = clnk->next_nlink()) {
            NetPins* cur;
            unsigned pin;
            ...
            if ((tmp->port_type() != NetNet::NOT_A_PORT) && (tmp->scope()->parent() == 0))
                return;
        }
    }
...
void cprop(Design* des) {
    ...
    do {
        ...
        des->functor(&prop);
        if (verbose_flag) {
            cout << " ... Iteration detected " << prop.count << " optimizations." << endl << flush;
        }
    } while (prop.count > 0);
