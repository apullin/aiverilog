...
void nodangle_f::event(Design*, NetEvent* ev) {
    ...
    if (iteration == 0) {
        ...
        for (list<NetEvent*>::iterator idx = match.begin(); idx != match.end(); ++idx) {
            NetEvent* tmp = *idx;
            assert(tmp != ev);
...
static void check_is_floating(NetNet* sig) {
    ...
    if (sig->type() == NetNet::TRI1)
        return;
    if (sig->type() == NetNet::IMPLICIT_REG)
        return;
    ...
    for (unsigned idx = 0; idx < sig->pin_count(); idx += 1) {
        ...
        if (sig->port_type() == PortType::NOT_A_PORT && sig->pin_count() == 1) {
            ...
            cerr << sig->get_fileline() << ": warning: "
                 << "Signal " << scope_path(sig->scope()) << "." << sig->name() << "[" << idx << "]"
                 << " has no drivers." << endl;
        } else {
            cerr << sig->get_fileline() << ": warning: "
...
void nodangle_f::signal(Design*, NetNet* sig) {
    ...
    if ((sig->port_type() != NetNet::NOT_A_PORT) &&
        ((sig->scope()->type() == NetScope::TASK) || (sig->scope()->type() == NetScope::FUNC) ||
