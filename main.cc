const char COPYRIGHT[] = "Copyright (c) 1998-2026 Stephen Williams (steve@icarus.com)";
/*
...
const char NOTICE[] = "  This program is free software; you can redistribute it and/or modify\n"
...
void add_vpi_module(const char* name) {
    if (vpi_module_list == 0) {
        ...
        char* tmp = static_cast<char*>(
            realloc(vpi_module_list, strlen(vpi_module_list) + strlen(name) + 2));
        strcat(tmp, ",");
        strcat(tmp, name);
        vpi_module_list = tmp;
    }
...
static void read_sources_file(const char* path) {
    ...
    while (fgets(line_buf, sizeof line_buf, fd) != 0) {
        ...
        if ((strlen(line_buf) == ((sizeof line_buf) - 1)) &&
            ...
            cerr << "WARNING: Line buffer overflow reading sources file: " << path << "." << endl;
            assert(0);
        }
...
inline static double cycles_diff(struct tms*, struct tms*) {
    return 0;
}
...
static void EOC_cleanup(void) {
    ...
    for (list<const char*>::iterator suf = library_suff.begin(); suf != library_suff.end(); ++suf) {
        free((void*)*suf);
    }
    library_suff.clear();
    ...
    for (map<string, const char*>::iterator flg = flags.begin(); flg != flags.end(); ++flg) {
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, "C:F:f:hN:P:p:Vv")) != EOF)
        switch (opt) {
            case 'C':
    ...
    if (version_flag) {
        cout << "\nIcarus Verilog Parser/Elaborator version " << VERSION << " (" << VERSION_TAG
             << ")" << endl
             << endl;
    ...
    if (help_flag) {
        cout << "Icarus Verilog Parser/Elaborator version " << VERSION << " (" << VERSION_TAG << ")"
             << endl
    ...
    if (flag_tmp)
        disable_concatz_generation = strcmp(flag_tmp, "true") == 0;
    ...
    if (pf_path) {
        ...
        for (map<perm_string, ivl_nature_t>::iterator cur = natures.begin(); cur != natures.end();
             ++cur) {
        ...
        for (vector<PPackage*>::iterator pac = pform_units.begin(); pac != pform_units.end();
             ++pac) {
            pform_dump(out, *pac);
        }
        out << "PFORM DUMP PACKAGES:" << endl;
        for (vector<PPackage*>::iterator pac = pform_packages.begin(); pac != pform_packages.end();
             ++pac) {
            pform_dump(out, *pac);
        }
    ...
    if (pre_process_fail_count) {
        cerr << "Preprocessor failed with " << pre_process_fail_count << " errors." << endl;
    ...
    if (verbose_flag) {
        if (times_flag) {
            times(cycles + 1);
            cerr << " ... done, " << cycles_diff(cycles + 1, cycles + 0) << " seconds." << endl;
        }
    ...
    if (verbose_flag) {
        if (times_flag) {
            times(cycles + 2);
            cerr << " ... done, " << cycles_diff(cycles + 2, cycles + 1) << " seconds." << endl;
        }
    ...
    if (verbose_flag) {
        if (times_flag) {
            times(cycles + 3);
            cerr << " ... done, " << cycles_diff(cycles + 3, cycles + 2) << " seconds." << endl;
        }
    }
    ...
    if (verbose_flag) {
        if (times_flag) {
            times(cycles + 4);
            cerr << " ... done, " << cycles_diff(cycles + 4, cycles + 3) << " seconds." << endl;
        } else {
            cout << "DONE." << endl;
        }
    }
...
errors_summary:
    if (!missing_modules.empty()) {
        ...
        for (idx = missing_modules.begin(); idx != missing_modules.end(); ++idx)
            cerr << "        " << (*idx).first << " referenced " << (*idx).second << " times."
                 << endl;
...
static void find_module_mention(map<perm_string, bool>& check_map, Module* mod) {
    ...
    for (cur = mod->generate_schemes.begin(); cur != mod->generate_schemes.end(); ++cur) {
...
static void find_module_mention(map<perm_string, bool>& check_map, PGenerate* schm) {
    ...
    list<PGenerate*>::const_iterator cur;
    for (cur = schm->generate_schemes.begin(); cur != schm->generate_schemes.end(); ++cur) {
        find_module_mention(check_map, *cur);
    }
}
