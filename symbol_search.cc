...
bool symbol_search(const LineInfo* li,
                   Design* des,
                   NetScope* scope,
                   pform_name_t path,
    ...
    if (!path.empty()) {
        bool flag =
            symbol_search(li, des, scope, path, lexical_pos, res, start_scope, prefix_scope);
        if (!flag)
            return false;
    ...
    while (scope) {
        if (debug_scopes || debug_elaborate) {
            cerr << li->get_fileline() << ": symbol_search: "
                 << "Looking for " << path_tail << " in scope " << scope_path(scope)
...
bool symbol_search(const LineInfo* li,
    ...
    return symbol_search(
        li, des, search_scope, path.name, lexical_pos, res, search_scope, prefix_scope);
}
