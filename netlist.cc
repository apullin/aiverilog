...
void NetPins::devirtualize_pins(void) {
    ...
    if (npins_ > array_size_limit) {
        cerr << get_fileline() << ": error: pin count " << npins_ << " exceeds " << array_size_limit
...
Link& NetPins::pin(unsigned idx) {
    ...
    ivl_assert(*this,
               idx == 0 ? (pins_[0].pin_zero_ && pins_[0].node_ == this) : pins_[idx].pin_ == idx);
...
NetDelaySrc::NetDelaySrc(
    NetScope* s, perm_string n, unsigned npins, bool condit_src, bool conditional, bool parallel)
...
void NetDelaySrc::set_delays(
    uint64_t t01, uint64_t t10, uint64_t t0z, uint64_t tz1, uint64_t t1z, uint64_t tz0) {
...
void NetDelaySrc::set_delays(uint64_t t01,
                             uint64_t t10,
                             uint64_t t0z,
                             uint64_t tz1,
                             uint64_t t1z,
...
void NetNet::initialize_dir_() {
    ...
    if (pins_are_virtual()) {
        if (0)
            cerr << "NetNet setting Link default dir" << endl;
        set_default_dir(dir);
    } else {
...
NetNet::NetNet(
    NetScope* s, perm_string n, Type t, const netranges_t& unpacked, ivl_type_t use_net_type)
    : NetObj(s, n, calculate_count(unpacked)),
      type_(t),
      port_type_(NOT_A_PORT),
      coerced_to_uwire_(false),
      local_flag_(false),
      lexical_pos_(0),
      net_type_(use_net_type),
      discipline_(0),
      unpacked_dims_(unpacked),
      eref_count_(0),
      lref_count_(0) {
...
bool NetNet::sb_to_slice(const list<long>& indices,
                         long sb,
                         long& loff,
...
NetPartSelect::NetPartSelect(
    NetNet* sig, unsigned off, unsigned wid, NetPartSelect::dir_t dir__, bool signed_flag__)
...
NetPartSelect::NetPartSelect(NetNet* sig, NetNet* sel, unsigned wid, bool signed_flag__)
...
NetReplicate::NetReplicate(NetScope* scope__, perm_string n, unsigned wid, unsigned rpt)
...
NetAbs::NetAbs(NetScope* s, perm_string n, unsigned w) : NetNode(s, n, 2), width_(w) {
    pin(0).set_dir(Link::OUTPUT);
    pin(1).set_dir(Link::INPUT);
}
 ...
 */
NetAddSub::NetAddSub(NetScope* s, perm_string n, unsigned w) : NetNode(s, n, 4), width_(w) {
 ...
 *    2  -- Distance
 */
NetCLShift::NetCLShift(NetScope* s,
                       perm_string n,
                       unsigned width__,
                       unsigned width_dist__,
                       bool right_flag__,
                       bool signed_flag__)
    : NetNode(s, n, 3),
      width_(width__),
...
NetCompare::NetCompare(NetScope* s, perm_string n, unsigned wi) : NetNode(s, n, 8), width_(wi) {
    signed_flag_ = false;
...
NetDivide::NetDivide(NetScope* sc, perm_string n, unsigned wr, unsigned wa, unsigned wb)
    : NetNode(sc, n, 3), width_r_(wr), width_a_(wa), width_b_(wb), signed_flag_(false) {
    pin(0).set_dir(Link::OUTPUT);  // Result
    pin(1).set_dir(Link::INPUT);   // DataA
    pin(2).set_dir(Link::INPUT);   // DataB
}
...
NetMult::NetMult(NetScope* sc, perm_string n, unsigned wr, unsigned wa, unsigned wb)
    : NetNode(sc, n, 3), signed_(false), width_r_(wr), width_a_(wa), width_b_(wb) {
    pin(0).set_dir(Link::OUTPUT);  // Result
    pin(1).set_dir(Link::INPUT);   // DataA
    pin(2).set_dir(Link::INPUT);   // DataB
}
...
NetPow::NetPow(NetScope* sc, perm_string n, unsigned wr, unsigned wa, unsigned wb)
    : NetNode(sc, n, 3), signed_(false), width_r_(wr), width_a_(wa), width_b_(wb) {
    pin(0).set_dir(Link::OUTPUT);  // Result
    pin(1).set_dir(Link::INPUT);   // DataA
    pin(2).set_dir(Link::INPUT);   // DataB
...
NetMux::NetMux(NetScope* s, perm_string n, unsigned wi, unsigned si, unsigned sw)
...
NetSignExtend::NetSignExtend(NetScope* s, perm_string n, unsigned w) : NetNode(s, n, 2), width_(w) {
    pin(0).set_dir(Link::OUTPUT);
    pin(1).set_dir(Link::INPUT);
}
...
NetCondit::NetCondit(NetExpr* ex, NetProc* i, NetProc* e) : expr_(ex), if_(i), else_(e) {}
...
NetConst::NetConst(NetScope* s, perm_string n, verinum::V v) : NetNode(s, n, 1), value_(v, 1) {
    pin(0).set_dir(Link::OUTPUT);
}

NetConst::NetConst(NetScope* s, perm_string n, const verinum& val) : NetNode(s, n, 1), value_(val) {
    pin(0).set_dir(Link::OUTPUT);
}
...
NetFuncDef::NetFuncDef(NetScope* s,
                       NetNet* result,
                       const vector<NetNet*>& po,
...
NetSTask::NetSTask(const char* na, ivl_sfunc_as_task_t sfat, const vector<NetExpr*>& pa)
...
NetEUFunc::NetEUFunc(NetScope* scope, NetScope* def, NetESignal* res, vector<NetExpr*>& p, bool nc)
    : NetExpr(res->net_type()),
      scope_(scope),
      func_(def),
      result_sig_(res),
      parms_(p),
...
ivl_variable_type_t NetEBBits::expr_type() const {
    if (left_->expr_type() == IVL_VT_LOGIC || right_->expr_type() == IVL_VT_LOGIC)
        return IVL_VT_LOGIC;
...
NetEBLogic::NetEBLogic(char op__, NetExpr* l, NetExpr* r) : NetEBinary(op__, l, r, 1, false) {}
...
NetEConst::NetEConst(ivl_type_t type, const verinum& val) : NetExpr(type), value_(val) {
    ivl_assert(*this, type->packed());
    ivl_assert(*this,
               type->packed_width() >= 0 && (unsigned long)type->packed_width() == val.len());
...
NetEEvent::NetEEvent(NetEvent* e) : event_(e) {
    e->exprref_ += 1;
}
...
NetESignal::NetESignal(NetNet* n) : NetExpr(n->net_type()), net_(n), word_(0) {
    net_->incr_eref();
    set_line(*n);
}

NetESignal::NetESignal(NetNet* n, NetExpr* w) : NetExpr(n->vector_width()), net_(n), word_(w) {
    net_->incr_eref();
    set_line(*n);
 ...
 */
NetETernary::NetETernary(NetExpr* c, NetExpr* t, NetExpr* f, unsigned wid, bool signed_flag)
...
NetEUReduce::NetEUReduce(char op__, NetExpr* ex) : NetEUnary(op__, ex, 1, false) {}
...
NetLogic::NetLogic(
    NetScope* s, perm_string n, unsigned pins, TYPE t, unsigned wid, bool is_cassign__)
...
NetUReduce::NetUReduce(NetScope* scope__, perm_string n, NetUReduce::TYPE t, unsigned wid)
...
static DelayType combine_delays(const DelayType a, const DelayType b) {
    ...
    if ((a == NO_DELAY || a == ZERO_DELAY) && (b == NO_DELAY || b == ZERO_DELAY)) {
        result = ZERO_DELAY;
    }
...
DelayType NetBlock::delay_type(bool print_delay) const {
    ...
    if (type() == PARA_JOIN_ANY) {
        ...
        for (const NetProc* cur = proc_first(); cur; cur = proc_next(cur)) {
            ...
            if (dt > result)
                result = dt;
            if ((dt == DEFINITE_DELAY) && !print_delay)
                break;
        }
    }
...
DelayType NetPDelay::delay_type(bool print_delay) const {
    ...
    if (expr_) {
        if (statement_) {
            return combine_delays(delay_type_from_expr(expr_), statement_->delay_type(print_delay));
        } else {
...
static bool do_expr_event_match(const NetExpr* expr, const NetEvWait* evwt) {
    ...
    for (unsigned idx = 0; idx < prb->pin_count(); idx += 1) {
        ...
        ns_evwt->add(
            const_cast<Nexus*>(prb->pin(idx).nexus()), 0, prb->pin(idx).nexus()->vector_width());
    }
...
static bool while_is_wait(const NetExpr* expr, const NetProc* stmt) {
    if (const NetEvWait* evwt = dynamic_cast<const NetEvWait*>(stmt)) {
        ...
        if (val.len() != 1)
            return false;
        if (val.get(0) != verinum::V1)
            return false;
        if (!do_expr_event_match(cond->left(), evwt))
            return false;
        if (evwt->get_lineno() != cond->get_lineno())
            return false;
        if (evwt->get_file() != cond->get_file())
            return false;
        return true;
    }
    return false;
...
static void
print_synth_warning(const NetProc* net_proc, const char* name, ivl_process_type_t pr_type) {
    cerr << net_proc->get_fileline() << ": warning: " << name << " statement cannot be synthesized "
         << get_process_type_as_string(pr_type) << endl;
}

static void check_if_logic_l_value(const NetAssignBase* base, ivl_process_type_t pr_type) {
    if (base->l_val_count() != 1)
        return;
    ...
    if ((sig->data_type() != IVL_VT_BOOL) && (sig->data_type() != IVL_VT_LOGIC)) {
        cerr << base->get_fileline()
             << ": warning: Assigning to a "
                "non-integral variable ("
             << sig->name() << ") cannot be synthesized " << get_process_type_as_string(pr_type)
             << endl;
    }
}

/* By default elements can be synthesized or ignored. */
bool NetProc::check_synth(ivl_process_type_t /* pr_type */, const NetScope* /* scope */) const {
    return false;
}
...
bool NetAssign::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
...
bool NetAssignNB::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    bool result = false;
...
bool NetBlock::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
    ...
    if (type() != SEQU) {
        ...
        cerr << " statement cannot be synthesized " << get_process_type_as_string(pr_type) << endl;
        result = true;
    }
...
bool NetCase::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
    for (unsigned idx = 0; idx < nitems(); idx += 1) {
        if (stat(idx))
            result |= stat(idx)->check_synth(pr_type, scope);
    }
    // FIXME: Check for ff/latch/comb structures.
    return result;
}

bool NetCAssign::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    print_synth_warning(this, "A procedural assign", pr_type);
    return false;
}

bool NetCondit::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
    if (if_)
        result |= if_->check_synth(pr_type, scope);
    if (else_)
        result |= else_->check_synth(pr_type, scope);
    // FIXME: Check for ff/latch/comb structures.
    return result;
}

bool NetDeassign::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    print_synth_warning(this, "A procedural deassign", pr_type);
    return false;
}

bool NetDisable::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    while (scope) {
        if (scope != target_)
            scope = scope->parent();
        else
...
bool NetDoWhile::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
    print_synth_warning(this, "A do/while", pr_type);
    if (proc_)
        result |= proc_->check_synth(pr_type, scope);
    return result;
}

bool NetEvTrig::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    print_synth_warning(this, "An event trigger", pr_type);
    return false;
}

bool NetEvNBTrig::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    print_synth_warning(this, "A non-blocking event trigger", pr_type);
    return false;
}

// The delay check above has already marked this as an error.
bool NetEvWait::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
    if (statement_)
        result |= statement_->check_synth(pr_type, scope);
    return result;
}

bool NetForce::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    print_synth_warning(this, "A force", pr_type);
    return false;
}

bool NetForever::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
 ...
 */
static void print_for_idx_warning(const NetProc* proc,
                                  const char* check,
    ...
    cerr << proc->get_fileline()
         << ": warning: A for statement must use "
            "the index ("
         << idx->name() << ") in the " << check << " expression to be synthesized "
         << get_process_type_as_string(pr_type) << endl;
}

static void check_for_const_synth(const NetExpr* expr,
                                  const NetProc* proc,
                                  const char* str,
                                  ivl_process_type_t pr_type) {
    if (!dynamic_cast<const NetEConst*>(expr)) {
        cerr << proc->get_fileline()
             << ": warning: A for "
                "statement must "
             << str << " value to be synthesized " << get_process_type_as_string(pr_type) << endl;
    }
}

static void check_for_bin_synth(const NetExpr* left,
                                const NetExpr* right,
                                const char* str,
                                const char* check,
                                const NetProc* proc,
...
static void print_for_step_warning(const NetProc* proc, ivl_process_type_t pr_type) {
    cerr << proc->get_fileline()
...
static void
print_for_step_warning(const NetProc* proc, ivl_process_type_t pr_type, const NetNet* idx) {
    cerr << proc->get_fileline()
         << ": warning: A for statement step must "
            "be an assignment to the index variable ("
         << idx->name() << ") to be synthesized " << get_process_type_as_string(pr_type) << endl;
}

static void check_for_bstep_synth(const NetExpr* expr,
                                  const NetProc* proc,
                                  ivl_process_type_t pr_type,
                                  const NetNet* index) {
    ...
    if (const NetEBAdd* tmp = dynamic_cast<const NetEBAdd*>(expr)) {
        check_for_bin_synth(
            tmp->left(), tmp->right(), "change by a constant", "step", proc, pr_type, index);
    } else {
        cerr << proc->get_fileline()
...
static void check_for_step_synth(const NetAssign* assign,
                                 const NetProc* proc,
                                 ivl_process_type_t pr_type,
                                 const NetNet* index) {
    if (assign->l_val_count() != 1) {
        ...
        switch (assign->assign_operator()) {
            case '+':
            case '-':
                check_for_const_synth(assign->rval(), proc, "have a constant step", pr_type);
                break;
            case 0:
            ...
            default:
                cerr << proc->get_fileline()
                     ...
                     << assign->assign_operator() << "' it must be +/- to be synthesized "
                     << get_process_type_as_string(pr_type) << endl;
                break;
        }
    }
...
bool NetForLoop::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
...
bool NetRelease::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    print_synth_warning(this, "A release", pr_type);
    return false;
}

bool NetRepeat::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
...
bool NetScope::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    bool result = false;
    ...
    for (const NetEvent* cur = events_; cur; cur = cur->snext_) {
        ...
        cerr << cur->get_fileline() << ": warning: An event (" << cur->name()
             << ") cannot be synthesized " << get_process_type_as_string(pr_type) << endl;
    }
    for (signals_map_iter_t cur = signals_map_.begin(); cur != signals_map_.end(); ++cur) {
...
bool NetSTask::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    if (strcmp(name(), "$ivl_darray_method$delete") == 0) {
        ...
        cerr << get_fileline() << ": warning: System task (" << name() << ") cannot be synthesized "
             << get_process_type_as_string(pr_type) << endl;
    }
    return false;
}
 ...
 */
bool NetBaseDef::check_synth(ivl_process_type_t pr_type, const NetScope* /* scope */) const {
    bool result = false;
    ...
    if (!tscope->is_auto()) {
        cerr << tscope->get_def_file() << ":" << tscope->get_def_lineno()
             ...
             << ") must be automatic to be synthesized " << get_process_type_as_string(pr_type)
             << endl;
    }
    if (proc_)
        result |= proc_->check_synth(pr_type, tscope);
    return result;
}

bool NetUTask::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
...
bool NetWhile::check_synth(ivl_process_type_t pr_type, const NetScope* scope) const {
    bool result = false;
