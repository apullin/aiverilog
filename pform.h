...
extern void pform_set_default_nettype(NetNet::Type net, const char* file, unsigned lineno);
 ...
 */
extern void pform_startmodule(const struct vlltype& loc,
                              const char* name,
                              bool program_block,
                              bool is_interface,
...
extern Module::port_t* pform_module_port_reference(const struct vlltype& loc, perm_string name);
extern void pform_endmodule(const char*, bool inside_celldefine, Module::UCDriveType uc_drive_def);
...
extern void pform_make_udp(const struct vlltype& loc,
                           perm_string name,
                           std::list<pform_ident_t>* parms,
                           std::vector<PWire*>* decl,
...
extern void pform_package_import(const struct vlltype& loc, PPackage* pkg, const char* ident);
extern void pform_package_export(const struct vlltype& loc, PPackage* pkg, const char* ident);
PPackage* pform_package_importable(PPackage* pkg, perm_string name);
PPackage* pform_find_potential_import(const struct vlltype& loc,
                                      LexicalScope* scope,
                                      perm_string name,
                                      bool tf_call,
                                      bool make_explicit);
...
extern PExpr*
pform_package_ident(const struct vlltype& loc, PPackage* pkg, const pform_name_t* ident);
...
extern PTrigger* pform_new_trigger(const struct vlltype& loc,
                                   PPackage* pkg,
...
extern PTask* pform_push_task_scope(const struct vlltype& loc,
                                    const char* name,
                                    LexicalScope::lifetime_t lifetime);
...
extern PBlock*
pform_push_block_scope(const struct vlltype& loc, const char* name, PBlock::BL_TYPE tt);

extern void pform_put_behavior_in_scope(AProcess* proc);

extern verinum*
pform_verinum_with_size(verinum* s, verinum* val, const char* file, unsigned lineno);
...
extern void pform_set_typedef(const struct vlltype& loc,
                              perm_string name,
                              data_type_t* data_type,
                              std::list<pform_range_t>* unp_ranges = nullptr);
extern void pform_forward_typedef(const struct vlltype& loc,
                                  perm_string name,
...
extern void pform_make_var_init(const struct vlltype& li, const pform_ident_t& name, PExpr* expr);
...
extern void pform_set_type_attrib(perm_string name, const std::string& key, char* value);

extern LexicalScope::range_t* pform_parameter_value_range(
...
extern void pform_set_parameter(const struct vlltype& loc,
                                ...
                                bool is_type,
                                data_type_t* data_type,
                                const std::list<pform_range_t>* udims,
                                PExpr* expr,
...
extern PSpecPath* pform_make_specify_path(const struct vlltype& li,
                                          std::list<perm_string>* src,
                                          char pol,
                                          bool full_flag,
                                          std::list<perm_string>* dst);
 ...
 */
extern PProcess*
pform_make_behavior(ivl_process_type_t, Statement*, std::list<named_pexpr_t>* attr);
extern void pform_mc_translate_on(bool flag);

extern std::vector<PWire*>* pform_make_udp_input_ports(std::list<pform_ident_t>*);

extern void pform_make_events(const struct vlltype& loc, const std::list<pform_ident_t>* names);
/*
 ...
 */
extern PAssign* pform_compressed_assign_from_inc_dec(const struct vlltype& loc, PExpr* exp);
 ...
 */
extern PExpr* pform_genvar_inc_dec(const struct vlltype& loc, const char* name, bool inc_flag);

extern PExpr*
pform_genvar_compressed(const struct vlltype& loc, const char* name, char op, PExpr* rval);
 ...
 */
extern void
pform_make_analog_behavior(const struct vlltype& loc, ivl_process_type_t type, Statement* st);

extern AContrib* pform_contribution_statement(const struct vlltype& loc, PExpr* lval, PExpr* rval);
...
extern PExpr*
pform_make_branch_probe_expression(const struct vlltype& loc, const char* name, const char* branch);
