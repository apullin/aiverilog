...
void PEIdent::report_mixed_assignment_conflict_(const char* category) const {
    cerr << get_fileline()
         << ": error: Cannot perform procedural "
            "assignment to "
         << category << " '" << path_ << "' because it is also continuously assigned." << endl;
}
 ...
 * NetAssign_ object.
 */
NetAssign_* PEConcat::elaborate_lval(
    Design* des, NetScope* scope, bool is_cassign, bool is_force, bool is_init) const {
    if (repeat_) {
        cerr << get_fileline()
    ...
    for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
        ...
        NetAssign_* tmp = parms_[idx]->elaborate_lval(des, scope, is_cassign, is_force, is_init);
...
NetAssign_* PEIdent::elaborate_lval(
    ...
    if (reg == 0) {
        if (scope->type() == NetScope::FUNC && scope->func_def()->is_void() &&
            scope->basename() == peek_tail_name(path_)) {
            cerr << get_fileline() << ": error: "
                 << "Cannot assign to " << path_ << " because function " << scope_path(scope)
                 << " is void." << endl;
        } else {
            cerr << get_fileline() << ": error: Could not find variable ``" << path_ << "'' in ``"
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": " << __func__ << ": "
             << "Found l-value path_=" << path_ << " as reg=" << reg->name() << endl;
        cerr << get_fileline() << ": " << __func__ << ": "
             << "reg->type()=" << reg->type()
             << ", reg->unpacked_dimensions()=" << reg->unpacked_dimensions() << endl;
        if (reg->net_type())
            cerr << get_fileline() << ": " << __func__ << ": "
        ...
        cerr << get_fileline() << ": " << __func__ << ": "
             << " base_path=" << base_path << ", member_path=" << member_path << endl;
    }

    if (reg->get_const() && !is_init) {
        cerr << get_fileline() << ": error: Assignment to const signal `" << reg->name()
             << "` is not allowed." << endl;
        des->errors++;
        return nullptr;
    }
    ...
    if ((reg->type() != NetNet::REG) &&
        ((reg->type() != NetNet::UNRESOLVED_WIRE) || !reg->coerced_to_uwire()) && !is_force) {
        cerr << get_fileline() << ": error: '" << path_
             << "' is not a valid l-value for a procedural assignment." << endl;
        cerr << reg->get_fileline() << ":      : '" << path_ << "' is declared here as a "
             << reg->type() << "." << endl;
        des->errors += 1;
        return 0;
    }

    return elaborate_lval_var_(des, scope, is_force, is_cassign, reg, sr.type, member_path);
}

NetAssign_* PEIdent::elaborate_lval_var_(Design* des,
                                         NetScope* scope,
                                         bool is_force,
                                         bool is_cassign,
                                         NetNet* reg,
...
NetAssign_*
PEIdent::elaborate_lval_array_(Design* des, NetScope*, bool is_force, NetNet* reg) const {
    if (!gn_system_verilog()) {
        cerr << get_fileline()
             << ": error: Assignment to an entire"
...
NetAssign_* PEIdent::elaborate_lval_net_word_(
    Design* des, NetScope* scope, NetNet* reg, bool need_const_idx, bool is_force) const {
    const name_component_t& name_tail = path_.back();
    ...
    if ((reg->type() == NetNet::UNRESOLVED_WIRE) && !is_force) {
        ...
        if (!canon_const ||
            reg->test_part_driven(reg->vector_width() - 1, 0, canon_const->value().as_long())) {
    ...
    if (reg->get_scalar() && use_sel != index_component_t::SEL_NONE) {
        ...
        if (reg->data_type() == IVL_VT_REAL)
            ...
            cerr << "scalar";
        cerr << " array word: " << reg->name() << as_indices(unpacked_indices) << endl;
        des->errors += 1;
        return 0;
    }
...
bool PEIdent::elaborate_lval_net_bit_(
    Design* des, NetScope* scope, NetAssign_* lv, bool need_const_idx, bool is_force) const {
    NetNet* reg = lv->sig();
    ivl_assert(*this, reg);
    ...
    if (has_variable_prefix) {
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": PEIdent::elaborate_lval_net_bit_: "
                 << "Variable packed prefix, base_expr=" << *base_expr << endl;
        }
    ...
    if (mux && mux->expr_type() == IVL_VT_REAL) {
        cerr << get_fileline() << ": error: Index expression for " << reg->name() << "[" << *mux
    ...
    if (const NetEConst* index_con = dynamic_cast<NetEConst*>(mux)) {
        ...
        if (index_con->value().is_defined()) {
            ...
            if (warn_ob_select) {
                cerr << get_fileline() << ": warning: L-value bit select of " << reg->name();
                if (reg->unpacked_dimensions() > 0)
                    cerr << "[]";
                cerr << " has an undefined index." << endl;
            }
    ...
    if (debug_elaborate && (reg->type() == NetNet::UNRESOLVED_WIRE)) {
        cerr << get_fileline() << ": PEIdent::elaborate_lval_net_bit_: "
             << "Try to assign bits of variable which is also continuously assigned." << endl;
    }
    ...
    if (prefix_indices.size() + 2 <= reg->packed_dims().size()) {
        ...
        if (warn_ob_select && (loff < 0 || loff >= (long)reg->vector_width())) {
            cerr << get_fileline() << ": warning: bit select " << reg->name() << "[" << lsb << "]"
...
bool PEIdent::elaborate_lval_net_part_(Design* des,
    ...
    if (!parts_defined_flag) {
        if (warn_ob_select) {
            cerr << get_fileline() << ": warning: L-value part select of " << reg->name();
            if (reg->unpacked_dimensions() > 0)
                cerr << "[]";
            cerr << " has an undefined index." << endl;
        }
    ...
    if (moff < loff) {
        cerr << get_fileline() << ": error: part select " << reg->name() << "[" << msb << ":" << lsb
             << "]"
             << " is reversed." << endl;
        des->errors += 1;
        return false;
    ...
    if (warn_ob_select && (loff < 0 || moff >= (long)reg->vector_width())) {
        cerr << get_fileline() << ": warning: Part select " << reg->name() << "[" << msb << ":"
             << lsb << "]"
             << " is out of range." << endl;
    }
    ...
    if (const NetEConst* base_c = dynamic_cast<NetEConst*>(base)) {
        ...
        if (base_c->value().is_defined()) {
            ...
            if (!base_c->has_sign() && (int32_t)lsv < 0) {
                ...
                if (warn_ob_select) {
                    ...
                    cerr << "[" << (unsigned long)lsv
                         << (index_tail.sel == index_component_t::SEL_IDX_UP ? "+:" : "-:") << wid
            ...
            if (prefix_indices.size() + 1 < reg->packed_dims().size()) {
                ...
                if (((rng.get_msb() < rng.get_lsb()) && use_sel == index_component_t::SEL_IDX_UP) ||
                    ((rng.get_msb() > rng.get_lsb()) && use_sel == index_component_t::SEL_IDX_DO)) {
                    offset = -wid + 1;
                }
        ...
        if (use_sel == index_component_t::SEL_IDX_UP) {
            ...
            base = normalize_variable_part_base(prefix_indices, base, reg, wid, false);
            sel_type = IVL_SEL_IDX_DOWN;
        }
    }

    if (debug_elaborate)
        cerr << get_fileline() << ": debug: Set part select width=" << wid << ", base=" << *base
             << endl;
 ...
 */
NetAssign_* PEIdent::elaborate_lval_net_class_member_(Design* des,
                                                      NetScope* scope,
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_lval_net_class_member_: "
             << "l-value is property " << member_path << " of " << sig->name() << "." << endl;
    }
    ...
    do {
            ...
            if (class_type->get_prop_initialized(pidx)) {
                ...
                if (debug_elaborate) {
                    cerr << get_fileline() << ": PEIdent::elaborate_lval_method_class_member_: "
                         << "Found initializers for property " << class_type->get_prop_name(pidx)
                         << endl;
                }
            }
        }
        ...
        if (!member_cur.index.empty()) {
            if (const netsarray_t* stype = dynamic_cast<const netsarray_t*>(ptype)) {
                canon_index =
                    make_canonical_index(des, scope, this, member_cur.index, stype, false);
...
bool PEIdent::elaborate_lval_net_packed_member_(
    Design* des, NetScope* scope, NetAssign_* lv, pform_name_t member_path, bool is_force) const {
    if (debug_elaborate) {
    ...
    do {
        ...
        if (member == 0) {
            cerr << get_fileline() << ": error: Member " << member_name
                 << " is not a member of struct type of " << reg->name() << "." << completed_path
                 << endl;
            des->errors += 1;
            return false;
        }
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PEIdent::elaborate_lval_net_packed_member_: "
             ...
             << ", completed_path=" << completed_path << ", member_path=" << member_path << endl;
    }
