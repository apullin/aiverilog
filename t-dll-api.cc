...
extern "C" int ivl_design_process(ivl_design_t des, ivl_process_f func, void* cd) {
    assert(des);
...
extern "C" void ivl_design_roots(ivl_design_t des, ivl_scope_t** scopes, unsigned int* nscopes) {
    assert(des);
...
extern "C" ivl_attribute_t ivl_logic_attr_val(ivl_net_logic_t net, unsigned idx) {
    assert(net);
...
extern "C" ivl_attribute_t ivl_process_attr_val(ivl_process_t net, unsigned idx) {
    assert(net);
    assert(idx < net->nattr);
    return net->attr + idx;
}
...
extern "C" ivl_statement_t ivl_stmt_block_stmt(ivl_statement_t net, unsigned i) {
    assert(net);
    switch (net->type_) {
        case IVL_ST_BLOCK:
        case IVL_ST_FORK:
