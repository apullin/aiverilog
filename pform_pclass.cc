...
void pform_start_class_declaration(const struct vlltype& loc,
    ...
    if (base_args) {
        type->base_args.insert(type->base_args.begin(), base_args->begin(), base_args->end());
        delete base_args;
    }
}
...
void pform_class_property(const struct vlltype& loc,
    ...
    // type.
    for (list<decl_assignment_t*>::iterator cur = decls->begin(); cur != decls->end(); ++cur) {
...
void pform_set_this_class(const struct vlltype& loc, PTaskFunc* net) {
    ...
    vector<pform_tf_port_t>* this_port =
        pform_make_task_ports(loc, NetNet::PINPUT, pform_cur_class->type, this_name);
...
void pform_end_class_declaration(const struct vlltype& loc) {
    ...
    if (!pform_cur_class->type->initialize.empty() || !pform_cur_class->type->base_args.empty()) {
        PFunction* func = pform_push_function_scope(loc, "new@", LexicalScope::AUTOMATIC);
        func->set_ports(0);
