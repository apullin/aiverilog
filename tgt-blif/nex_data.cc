...
inline blif_nex_data_t::blif_nex_data_t(ivl_nexus_t nex) : nex_(nex), name_(0) {}
...
void blif_nex_data_t::make_name_from_sig_(ivl_signal_t sig) {
    ...
    for (ivl_scope_t sscope = ivl_signal_scope(sig); ivl_scope_parent(sscope);
         sscope = ivl_scope_parent(sscope)) {
