...
static int print_lpm_mux_s1(FILE* fd, ivl_lpm_t net) {
    ...
    fprintf(fd,
            "# IVL_LPM_MUX ivl_lpm_width(net)=%u, Q=%s, D0=%s, D1=%s\n",
            ivl_lpm_width(net),
            ned_out->get_name(),
            ned_d0->get_name(),
            ned_d1->get_name());
    ...
    for (unsigned idx = 0; idx < ivl_lpm_width(net); idx += 1) {
        fprintf(fd,
                ...
                "1-1 1\n",
                ned_sel->get_name(),
                ned_sel->get_name_index(0),
                ned_d0->get_name(),
                ned_d0->get_name_index(idx),
                ned_d1->get_name(),
                ned_d1->get_name_index(idx),
                ned_out->get_name(),
                ned_out->get_name_index(idx));
    }
...
static int print_lpm_mux_sN(FILE* fd, ivl_lpm_t net) {
    ...
    for (unsigned wid = 0; wid < ivl_lpm_width(net); wid += 1) {
        ...
        for (size_t idx = 0; idx < ned_d.size(); idx += 1) {
            fprintf(fd, " %s%s", ned_d[idx]->get_name(), ned_d[idx]->get_name_index(wid));
        }
