...
int print_lpm_shift(FILE* fd, ivl_lpm_t net, bool left) {
    ...
    for (size_t lvl = 0; lvl < shiftw; lvl += 1) {
        for (unsigned idx = 0; idx < dataw; idx += 1) {
            ...
            // First arg (shift)
            fprintf(fd, ".names %s%s", s_ned->get_name(), s_ned->get_name_index(lvl));
            ...
            if (!borrow) {
                if (lvl == 0) {
                    fprintf(fd,
                            " %s%s %s%s",
                            d_ned->get_name(),
                            d_ned->get_name_index(idx),
                            d_ned->get_name(),
                            d_ned->get_name_index(idx_2));
                } else {
                    fprintf(fd,
                            " %s/%zu/%u %s/%zu/%u",
                            q_ned->get_name(),
                ...
                if (lvl == 0) {
                    fprintf(fd, " %s%s", d_ned->get_name(), d_ned->get_name_index(idx));
                } else {
                    fprintf(fd, " %s/%zu/%u", q_ned->get_name(), lvl - 1, idx);
                }
            }
