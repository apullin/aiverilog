...
static int print_lpm_cmp_gt_s(FILE* fd, ivl_lpm_t net) {
    ...
    if (ivl_lpm_width(net) == 1) {
        fprintf(fd,
                ...
                "01 1\n",
                a_ned->get_name(),
                a_ned->get_name_index(0),
                b_ned->get_name(),
                b_ned->get_name_index(0),
                q_ned->get_name(),
                q_ned->get_name_index(0));
        if (ge_flag)
            fprintf(fd, "00 1\n11 1\n");
        return 0;
    }
    ...
    fprintf(stderr,
            "%s:%u: sorry: blif: Signed magnitude compare not implemented yet\n",
            ivl_lpm_file(net),
            ivl_lpm_lineno(net));
...
static int print_lpm_cmp_gt_u(FILE* fd, ivl_lpm_t net) {
    ...
    for (unsigned idx = 0; idx < ivl_lpm_width(net); idx += 1) {
        fprintf(fd,
                ".names %s%s %s%s %s%s/G%u\n"
                "10 1\n",
                a_ned->get_name(),
                a_ned->get_name_index(idx),
                b_ned->get_name(),
                b_ned->get_name_index(idx),
                q_ned->get_name(),
                q_ned->get_name_index(0),
                idx);
    }
    ...
    fprintf(fd, ".names");
    for (unsigned idx = 0; idx < ivl_lpm_width(net); idx += 1) {
        fprintf(fd, " %s%s/G%u", q_ned->get_name(), q_ned->get_name_index(0), idx);
    }
    for (unsigned idx = ge_flag ? 0 : 1; idx < ivl_lpm_width(net); idx += 1) {
        fprintf(fd, " %s%s/E%u", q_ned->get_name(), q_ned->get_name_index(0), idx);
    }
