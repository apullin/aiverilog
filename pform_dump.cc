...
static std::ostream& operator<<(std::ostream& out, const std::vector<named_pexpr_t>& exprs) {
    for (size_t idx = 0; idx < exprs.size(); idx++) {
        if (idx != 0)
            out << ", ";
...
void struct_type_t::pform_dump(ostream& out, unsigned indent) const {
    out << setw(indent) << "" << "Struct " << (packed_flag ? "packed" : "unpacked") << " with "
        << (members.get() == 0 ? 0 : members->size()) << " members" << endl;
    if (members.get() == 0)
        return;

    for (list<struct_member_t*>::iterator cur = members->begin(); cur != members->end(); ++cur) {
...
void vector_type_t::pform_dump(ostream& fd, unsigned indent) const {
    ...
    if (pdims.get()) {
        for (list<pform_range_t>::iterator cur = pdims->begin(); cur != pdims->end(); ++cur) {
            fd << "[";
            if (cur->first)
                fd << *(cur->first);
            if (cur->second)
...
void class_type_t::pform_dump_init(ostream& out, unsigned indent) const {
    for (vector<Statement*>::const_iterator cur = initialize.begin(); cur != initialize.end();
         ++cur) {
        Statement* curp = *cur;
...
void struct_member_t::pform_dump(ostream& out, unsigned indent) const {
    ...
    for (list<decl_assignment_t*>::iterator cur = names->begin(); cur != names->end(); ++cur) {
...
static void dump_attributes_map(ostream& out, const map<perm_string, PExpr*>& attributes, int ind) {
    for (map<perm_string, PExpr*>::const_iterator idx = attributes.begin(); idx != attributes.end();
         ++idx) {
...
void PWire::dump(ostream& out, unsigned ind) const {
    ...
    if (port_set_) {
        if (port_.empty()) {
            ...
            out << " port";
            for (list<pform_range_t>::const_iterator cur = port_.begin(); cur != port_.end();
                 ++cur) {
                out << "[";
                if (cur->first)
                    out << *cur->first;
    ...
    if (net_set_) {
        if (net_.empty()) {
            ...
            out << " net";
            for (list<pform_range_t>::const_iterator cur = net_.begin(); cur != net_.end(); ++cur) {
                out << "[";
                if (cur->first)
                    out << *cur->first;
                if (cur->second)
    ...
    for (list<pform_range_t>::const_iterator cur = unpacked_.begin(); cur != unpacked_.end();
         ++cur) {
        out << "[";
        if (cur->first)
            out << *cur->first;
...
void PGate::dump_ranges(ostream& out) const {
    if (ranges_ == 0)
        return;
    for (list<pform_range_t>::iterator cur = ranges_->begin(); cur != ranges_->end(); ++cur) {
        out << "[";
        if (cur->first)
            out << *(cur->first);
        if (cur->second)
...
void PCAssign::dump(ostream& out, unsigned ind) const {
    out << setw(ind) << "" << "assign " << *lval_ << " = " << *expr_ << "; /* " << get_fileline()
        << " */" << endl;
}

void PDeassign::dump(ostream& out, unsigned ind) const {
    out << setw(ind) << "" << "deassign " << *lval_ << "; /* " << get_fileline() << " */" << endl;
}

void PDelayStatement::dump(ostream& out, unsigned ind) const {
    out << setw(ind) << "" << "#" << *delay_ << " /* " << get_fileline() << " */";
    if (statement_) {
        out << endl;
...
void PForce::dump(ostream& out, unsigned ind) const {
    out << setw(ind) << "" << "force " << *lval_ << " = " << *expr_ << "; /* " << get_fileline()
        << " */" << endl;
}
...
void PRelease::dump(ostream& out, unsigned ind) const {
    out << setw(ind) << "" << "release " << *lval_ << "; /* " << get_fileline() << " */" << endl;
}
...
void PGenerate::dump(ostream& out, unsigned indent) const {
    ...
    for (list<PGate*>::const_iterator idx = gates.begin(); idx != gates.end(); ++idx) {
        (*idx)->dump(out, indent + 2);
    }

    dump_var_inits_(out, indent + 2);

    for (list<PProcess*>::const_iterator idx = behaviors.begin(); idx != behaviors.end(); ++idx) {
        (*idx)->dump(out, indent + 2);
    }

    for (list<AProcess*>::const_iterator idx = analog_behaviors.begin();
         idx != analog_behaviors.end();
         ++idx) {
        (*idx)->dump(out, indent + 2);
    }

    for (list<PCallTask*>::const_iterator idx = elab_tasks.begin(); idx != elab_tasks.end();
         ++idx) {
        (*idx)->dump(out, indent + 2);
    }
...
void LexicalScope::dump_parameters_(ostream& out, unsigned indent) const {
    ...
    for (parm_iter_t cur = parameters.begin(); cur != parameters.end(); ++cur) {
        ...
        if (cur->second->data_type)
            ...
            out << "(nil type)";
        if ((*cur).second->expr)
            out << " " << (*cur).first << " = " << *(*cur).second->expr;
        for (LexicalScope::range_t* tmp = (*cur).second->range; tmp; tmp = tmp->next) {
            if (tmp->exclude_flag)
                out << " exclude ";
            else
                out << " from ";
...
void LexicalScope::dump_enumerations_(ostream& out, unsigned indent) const {
    for (vector<enum_type_t*>::const_iterator cur = enum_sets.begin(); cur != enum_sets.end();
         ++cur) {
...
void LexicalScope::dump_events_(ostream& out, unsigned indent) const {
    for (map<perm_string, PEvent*>::const_iterator cur = events.begin(); cur != events.end();
         ++cur) {
        PEvent* ev = (*cur).second;
        out << setw(indent) << "" << "event " << ev->name() << "; // " << ev->get_fileline()
            << endl;
    }
}
...
void LexicalScope::dump_wires_(ostream& out, unsigned indent) const {
    ...
    for (map<perm_string, PWire*>::const_iterator wire = wires.begin(); wire != wires.end();
         ++wire) {
...
void Module::dump(ostream& out) const {
    ...
    for (genscheme_iter_t cur = generate_schemes.begin(); cur != generate_schemes.end(); ++cur) {
        (*cur)->dump(out, 4);
    }
    ...
    for (list<PGate*>::const_iterator gate = gates_.begin(); gate != gates_.end(); ++gate) {
        (*gate)->dump(out);
    }

    dump_var_inits_(out, 4);

    for (list<PProcess*>::const_iterator behav = behaviors.begin(); behav != behaviors.end();
         ++behav) {
        (*behav)->dump(out, 4);
    }
    ...
    for (list<PSpecPath*>::const_iterator spec = specify_paths.begin(); spec != specify_paths.end();
         ++spec) {
        (*spec)->dump(out, 4);
    }
...
void PUdp::dump(ostream& out) const {
    ...
    // statements.
    for (map<string, PExpr*>::const_iterator idx = attributes.begin(); idx != attributes.end();
         ++idx) {
