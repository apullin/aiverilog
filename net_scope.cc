...
void NetScope::set_line(perm_string file,
 ...
 * member.
 */
void NetScope::set_parameter(perm_string key,
                             bool is_annotatable,
 ...
 */
void NetScope::set_parameter(perm_string key, NetExpr* val, const LineInfo& file_line) {
    param_expr_t& ref = parameters[key];
 ...
 */
void NetScope::replace_parameter(
    Design* des, perm_string key, PExpr* val, NetScope* scope, bool defparam) {
    if (parameters.find(key) == parameters.end()) {
        cerr << val->get_fileline() << ": error: parameter `" << key << "` not found in `"
    ...
    if (ref.local_flag) {
        cerr << val->get_fileline() << ": error: "
             << "Cannot override localparam `" << key << "` in `" << scope_path(this) << "`."
             << endl;
        des->errors++;
        return;
    }
    if (!ref.overridable) {
        cerr << val->get_fileline() << ": error: "
             << "Cannot override parameter `" << key << "` in `" << scope_path(this)
             << "`. Parameter cannot be overridden "
             << "in the scope it has been declared in." << endl;
        des->errors++;
        return;
    }
    ...
    if (ref.type_flag && defparam) {
        cerr << val->get_fileline() << ": error: "
             << "Cannot override type parameter `" << key << "` in `" << scope_path(this)
             << "`. It is not allowed to override type"
             << " parameters using a defparam statement." << endl;
        des->errors++;
        return;
    }
 ...
 */
const NetExpr* NetScope::get_parameter(Design* des, const char* key, ivl_type_t& ivl_type) {
    return get_parameter(des, perm_string::literal(key), ivl_type);
}

const NetExpr* NetScope::get_parameter(Design* des, perm_string key, ivl_type_t& ivl_type) {
...
void NetScope::add_module_port_info(unsigned idx,
                                    perm_string name,
                                    PortType::Enum ptype,
                                    unsigned long width) {
    ivl_assert(*this, type_ == MODULE);
...
void NetScope::add_tie_hi(Design* des) {
    if (tie_hi_ == 0) {
        NetNet* sig =
            new NetNet(this, lex_strings.make("_LOGIC1"), NetNet::WIRE, &netvector_t::scalar_logic);
        sig->local_flag(true);
...
void NetScope::add_tie_lo(Design* des) {
    if (tie_lo_ == 0) {
        NetNet* sig =
            new NetNet(this, lex_strings.make("_LOGIC0"), NetNet::WIRE, &netvector_t::scalar_logic);
        sig->local_flag(true);
