...
void NetEvent::find_similar_event(list<NetEvent*>& event_list) {
    ...
    for (list<NetEvProbe*>::iterator idx = first_probes.begin(); idx != first_probes.end(); ++idx) {
    ...
    for (NetEvProbe* cur = probes_->enext_; cur; cur = cur->enext_) {
        ...
        for (list<NetEvProbe*>::iterator idx = similar_probes.begin(); idx != similar_probes.end();
             ++idx) {
    ...
    for (set<NetEvent*>::iterator idx = candidate_events.begin(); idx != candidate_events.end();
         ++idx) {
        NetEvent* tmp = *idx;
...
NetEvNBTrig::NetEvNBTrig(NetEvent* ev, NetExpr* dly) : event_(ev), dly_(dly) {
...
NetEvProbe::NetEvProbe(NetScope* s, perm_string n, NetEvent* tgt, edge_t t, unsigned p)
