...
static ivl_variable_type_t arith_expr_type(const NetExpr* l, const NetExpr* r) {
    ...
    if (l_expr_type == IVL_VT_REAL || r_expr_type == IVL_VT_REAL)
        return IVL_VT_REAL;

    if (l_expr_type == IVL_VT_LOGIC || r_expr_type == IVL_VT_LOGIC)
        return IVL_VT_LOGIC;
 ...
 */
NetEBComp::NetEBComp(char op__, NetExpr* l, NetExpr* r) : NetEBinary(op__, l, r, 1, false) {}
...
NetEConstEnum::NetEConstEnum(perm_string n, const netenum_t* enum_set, const verinum& val)
...
NetECString::NetECString(const std::string& val) : NetEConst(verinum(val)) {}
...
NetENew::NetENew(ivl_type_t t) : NetExpr(t), size_(0), init_val_(0) {}
...
NetESelect::NetESelect(NetExpr* exp, NetExpr* base, unsigned wid, ivl_select_type_t sel_type)
    : expr_(exp), base_(base), sel_type_(sel_type) {
    expr_width(wid);
}

NetESelect::NetESelect(NetExpr* exp, NetExpr* base, unsigned wid, ivl_type_t use_type)
...
NetESFunc::NetESFunc(
    const char* n, ivl_variable_type_t t, unsigned width, unsigned np, bool is_overridden)
...
NetESFunc::NetESFunc(const char* n, ivl_type_t rtype, unsigned np)
    : NetExpr(rtype), name_(0), type_(rtype->base_type()), parms_(np), is_overridden_(false) {
    name_ = lex_strings.add(n);
}
...
NetEShallowCopy::NetEShallowCopy(NetExpr* arg1, NetExpr* arg2) : arg1_(arg1), arg2_(arg2) {}
...
NetEAccess::NetEAccess(NetBranch* br, ivl_nature_t nat) : branch_(br), nature_(nat) {
    cast_signed_base_(true);
}
