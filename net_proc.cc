...
void NetCase::prune() {
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": debug: pruning case expressions to " << prune_width << " bits."
             << endl;
    }

    // Prune the case expression
    expr_ = pad_to_width(unpadded_expr->dup_expr(), prune_width, padded_expr->has_sign(), *expr_);
    delete padded_expr;
...
NetPDelay::NetPDelay(uint64_t d, NetProc* st) : delay_(d), expr_(0), statement_(st) {}

NetPDelay::NetPDelay(NetExpr* d, NetProc* st) : delay_(0), expr_(d), statement_(st) {}
