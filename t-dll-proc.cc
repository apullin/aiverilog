...
bool dll_target::process(const NetProcTop* net) {
    ...
    ivl_process_t obj = static_cast<struct ivl_process_s*>(calloc(1, sizeof(struct ivl_process_s)));
...
bool dll_target::make_single_lval_(const LineInfo* li,
                                   struct ivl_lval_s* cur,
    ...
    if (net->nevents() > 0) {
        ...
        if (net->nevents() > 1) {
            stmt_cur_->u_.assign_.events =
                static_cast<ivl_event_t*>(calloc(net->nevents(), sizeof(ivl_event_t*)));
        }
            ...
            if (ev->nprobe() >= 1) {
                ...
                for (unsigned idx = 0; idx < ev->nprobe(); idx += 1) {
                    ...
                    for (unsigned bit = 0; bit < pr->pin_count(); bit += 1) {
                        ivl_nexus_t nex =
                            static_cast<ivl_nexus_t>(pr->pin(bit).nexus()->t_cookie());
                        assert(nex);
...
bool dll_target::proc_block(const NetBlock* net) {
    ...
    if ((count == 1) && (net->subscope() == 0) && (net->type() != NetBlock::PARA_JOIN_NONE)) {
    ...
    stmt_cur_->u_.block_.stmt_ =
        static_cast<struct ivl_statement_s*>(calloc(count, sizeof(struct ivl_statement_s)));
    ...
    for (const NetProc* cur = net->proc_first(); cur; cur = net->proc_next(cur), idx += 1) {
        assert(idx < count);
...
bool dll_target::proc_condit(const NetCondit* net) {
    ...
    stmt_cur_->u_.condit_.stmt_ =
        static_cast<struct ivl_statement_s*>(calloc(2, sizeof(struct ivl_statement_s)));
...
bool dll_target::proc_delay(const NetPDelay* net) {
    ...
    ivl_statement_t tmp =
        static_cast<struct ivl_statement_s*>(calloc(1, sizeof(struct ivl_statement_s)));
...
void dll_target::proc_stask(const NetSTask* net) {
    ...
    stmt_cur_->u_.stask_.parms_ = static_cast<ivl_expr_t*>(calloc(nparms, sizeof(ivl_expr_t)));
