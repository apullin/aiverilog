...
class PAssign_ : public Statement {
  public:
    explicit PAssign_(PExpr* lval, PExpr* ex, bool is_constant, bool is_init = false);
  ...
  protected:
    ...
    NetExpr* elaborate_rval_(Design*,
                             NetScope*,
    ...
    NetExpr* elaborate_rval_obj_(Design*, NetScope*, ivl_variable_type_t type) const;
...
class PAssignNB : public PAssign_ {
  ...
  private:
    NetProc* assign_to_memory_(class NetMemory*, PExpr*, Design* des, NetScope* scope) const;
};
...
class PCallTask : public Statement {
  ...
  private:
    ...
    NetProc* elaborate_method_(Design* des, NetScope* scope, bool add_this_flag = false) const;
    NetProc* elaborate_function_(Design* des, NetScope* scope) const;
    NetProc* elaborate_void_function_(Design* des, NetScope* scope, NetFuncDef* def) const;
    NetProc* elaborate_non_void_function_(Design* des, NetScope* scope) const;

    NetProc*
    elaborate_build_call_(Design* des, NetScope* scope, NetScope* task, NetExpr* use_this) const;
    NetProc* elaborate_sys_task_method_(Design* des,
                                        NetScope* scope,
                                        NetNet* net,
                                        perm_string method_name,
                                        const char* sys_task_name,
                                        const std::vector<perm_string>& parm_names = {}) const;
    NetProc* elaborate_queue_method_(Design* des,
                                     NetScope* scope,
                                     NetNet* net,
                                     perm_string method_name,
                                     const char* sys_task_name,
...
class PForeach : public Statement {
  ...
  private:
    NetProc* elaborate_static_array_(Design* des, NetScope* scope, const netranges_t& dims) const;
...
class PForStatement : public Statement {
  public:
    PForStatement(PExpr* n1, PExpr* e1, PExpr* cond, Statement* step, Statement* body);
