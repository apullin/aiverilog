...
vpiHandle vpi_handle_by_name(const char*, vpiHandle) {
    return 0;
}
...
PLI_INT32 vpi_get(int, vpiHandle) {
    return 0;
}
...
void* vpi_get_userdata(vpiHandle) {
    return 0;
}
PLI_INT32 vpi_put_userdata(vpiHandle, void*) {
    return 0;
}

// I/O routines

PLI_UINT32 vpi_mcd_open(char*) {
    return 0;
}
PLI_UINT32 vpi_mcd_close(PLI_UINT32) {
    return 0;
}
PLI_INT32 vpi_mcd_flush(PLI_UINT32) {
    return 0;
}
char* vpi_mcd_name(PLI_UINT32) {
    return 0;
}
PLI_INT32 vpi_mcd_printf(PLI_UINT32, const char*, ...) {
    return 0;
}
PLI_INT32 vpi_mcd_vprintf(PLI_UINT32, const char*, va_list) {
    return 0;
}
...
PLI_INT32 vpi_vprintf(const char*, va_list) {
    return 0;
}
...
PLI_INT32 vpi_release_handle(vpiHandle) {
    return 0;
}
...
PLI_INT32 vpi_sim_control(PLI_INT32, ...) {
    return 0;
}

// proposed standard extensions

PLI_INT32 vpi_fopen(const char*, const char*) {
    return 0;
}
FILE* vpi_get_file(PLI_INT32) {
    return 0;
}
...
PLI_INT32 vpi_vcontrol(PLI_INT32, va_list) {
    return 0;
}
...
bool load_vpi_module(const char* path) {
    ...
    if (!set_callback(&vpi_routines, vpip_routines_version)) {
        cerr << "error: Failed to link '" << path
             << "'. "
                "Try rebuilding it with iverilog-vpi."
             << endl;
        ivl_dlclose(dll);
        return true;
    }
