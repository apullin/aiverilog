...
NexusSet* NetExpr::nex_input(bool, bool, bool) const {
    cerr << get_fileline() << ": internal error: nex_input not implemented: " << *this << endl;
    return new NexusSet;
}

NexusSet* NetProc::nex_input(bool, bool, bool) const {
    cerr << get_fileline() << ": internal error: NetProc::nex_input not implemented" << endl;
    return new NexusSet;
}
...
NexusSet* NetESignal::nex_input_base(
    ...
    if (word_) {
        ...
        if (!always_sens && warn_sens_entire_arr) {
            cerr << get_fileline() << ": warning: @* is sensitive to all " << net_->unpacked_count()
                 << " words in array '" << name() << "'." << endl;
        }
        if (always_sens)
            if (const NetEConst* val = dynamic_cast<NetEConst*>(word_)) {
                const_select = true;
...
static void func_always_sens(NetFuncDef* func, NexusSet* result, bool rem_out, bool nested_func) {
