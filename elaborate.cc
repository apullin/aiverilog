...
unsigned PGate::calculate_array_size_(Design* des, NetScope* scope, long& high, long& low) const {
    ...
    if (ranges_) {
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": debug: PGate: Make array "
                 << "[" << high << ":" << low << "]" << " of " << size << " instances for "
                 << get_name() << endl;
        }
    }
...
void PGAssign::elaborate(Design* des, NetScope* scope) const {
    ...
    bool var_allowed_in_sv = (drive0 == IVL_DR_STRONG && drive1 == IVL_DR_STRONG) ? true : false;
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PGAssign::elaborate: elaborated l-value"
             << " width=" << lval->vector_width() << ", pin_count=" << lval->pin_count() << endl;
    }

    NetExpr* rval_expr = elaborate_rval_expr(des, scope, lval->net_type(), pin(1));

    if (rval_expr == 0) {
        cerr << get_fileline() << ": error: Unable to elaborate r-value: " << *pin(1) << endl;
        des->errors += 1;
        return;
    }
    ...
    if (lval->vector_width() < rval->vector_width()) {
        NetPartSelect* tmp = new NetPartSelect(rval, 0, lval->vector_width(), NetPartSelect::VP);
        des->add_node(tmp);
        tmp->set_line(*this);
        const netvector_t* osig_vec =
            new netvector_t(rval->data_type(), lval->vector_width() - 1, 0);
        NetNet* osig = new NetNet(scope, scope->local_symbol(), NetNet::TRI, osig_vec);
        osig->set_line(*this);
    ...
    if ((drive0 != IVL_DR_STRONG || drive1 != IVL_DR_STRONG) &&
        ((dynamic_cast<NetESelect*>(rval_expr)) || (dynamic_cast<NetEConcat*>(rval_expr)))) {
        need_driver_flag = true;
    }

    if (need_driver_flag) {
        NetBUFZ* driver = new NetBUFZ(scope, scope->local_symbol(), rval->vector_width(), false);
...
NetNet* elaborate_unpacked_array(
    Design* des, NetScope* scope, const LineInfo& loc, const NetNet* lval, PExpr* expr) {
    NetNet* expr_net;
    ...
    if (!ident) {
        if (dynamic_cast<PEConcat*>(expr)) {
            cout << loc.get_fileline() << ": sorry: Continuous assignment"
                 << " of array concatenation is not yet supported." << endl;
            des->errors++;
            return nullptr;
            ...
            cout << loc.get_fileline() << ": error: Can not assign"
                 << " non-array expression `" << *expr << "` to array." << endl;
            des->errors++;
            return nullptr;
        }
    } else {
    ...
    if (expr_dims.empty()) {
        cerr << loc.get_fileline() << ": error: Can not assign"
             << " non-array identifier `" << *expr << "` to array." << endl;
        des->errors++;
        return nullptr;
    }
...
void PGBuiltin::calculate_gate_and_lval_count_(unsigned& gate_count, unsigned& lval_count) const {
    switch (type()) {
        case BUF:
        case NOT:
            if (pin_count() > 2)
...
NetNode* PGBuiltin::create_gate_for_output_(Design* des,
                                            NetScope* scope,
    ...
    switch (type()) {
        case AND:
            if (pin_count() < 2) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::AND, instance_width);
            }
            break;
        ...
        case BUFIF0:
            if (pin_count() != 3) {
                ...
                gate =
                    new NetLogic(scope, inst_name, pin_count(), NetLogic::BUFIF0, instance_width);
            }
            break;
        ...
        case BUFIF1:
            if (pin_count() != 3) {
                ...
                gate =
                    new NetLogic(scope, inst_name, pin_count(), NetLogic::BUFIF1, instance_width);
            }
            break;
        ...
        case CMOS:
            if (pin_count() != 4) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::CMOS, instance_width);
            }
            break;
        ...
        case NAND:
            if (pin_count() < 2) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NAND, instance_width);
            }
            break;
        ...
        case NMOS:
            if (pin_count() != 3) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NMOS, instance_width);
            }
            break;
        ...
        case NOR:
            if (pin_count() < 2) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::NOR, instance_width);
            }
            break;
        ...
        case NOTIF0:
            if (pin_count() != 3) {
                ...
                gate =
                    new NetLogic(scope, inst_name, pin_count(), NetLogic::NOTIF0, instance_width);
            }
            break;
        ...
        case NOTIF1:
            if (pin_count() != 3) {
                ...
                gate =
                    new NetLogic(scope, inst_name, pin_count(), NetLogic::NOTIF1, instance_width);
            }
            break;
        ...
        case OR:
            if (pin_count() < 2) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::OR, instance_width);
            }
            break;
        ...
        case RCMOS:
            if (pin_count() != 4) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::RCMOS, instance_width);
            }
            break;
        ...
        case RNMOS:
            if (pin_count() != 3) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::RNMOS, instance_width);
            }
            break;
        ...
        case RPMOS:
            if (pin_count() != 3) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::RPMOS, instance_width);
            }
            break;
        ...
        case PMOS:
            if (pin_count() != 3) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::PMOS, instance_width);
            }
            break;

        case PULLDOWN:
            gate = new NetLogic(scope, inst_name, 1, NetLogic::PULLDOWN, instance_width);
            break;

        case PULLUP:
            gate = new NetLogic(scope, inst_name, 1, NetLogic::PULLUP, instance_width);
            break;
        ...
        case XNOR:
            if (pin_count() < 2) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::XNOR, instance_width);
            }
            break;
        ...
        case XOR:
            if (pin_count() < 2) {
                ...
                gate = new NetLogic(scope, inst_name, pin_count(), NetLogic::XOR, instance_width);
            }
            break;
    ...
    for (unsigned idx = 0; idx < lval_count; idx += 1) {
        ...
        if (lval_sigs[idx] == 0) {
            cerr << get_fileline() << ": error: "
                 << "Failed to elaborate primitive output expression " << scope_path(scope) << "."
                 << *pin(idx) << "." << endl;
            des->errors += 1;
            return;
        }
        ...
        ivl_assert(*this,
                   idx == 0 || lval_sigs[idx]->vector_width() == lval_sigs[0]->vector_width());
    }
    ...
    for (unsigned idx = 0; idx < pin_count(); idx += 1) {
        ...
        if (array_count == 1) {
            ...
            if (1 == sig->vector_width() && instance_width != 1) {
                ...
                NetReplicate* rep =
                    new NetReplicate(scope, scope->local_symbol(), instance_width, instance_width);
                rep->set_line(*this);
                des->add_node(rep);
                connect(rep->pin(1), sig->pin(0));

                const netvector_t* osig_vec = new netvector_t(IVL_VT_LOGIC, instance_width - 1, 0);
            ...
            if (instance_width != sig->vector_width()) {
                cerr << get_fileline() << ": error: "
                     << "Expression width " << sig->vector_width() << " does not match width "
                     << instance_width << " of logic gate array port " << idx + 1 << "." << endl;
                des->errors += 1;
            }
            ...
            if (idx < gate_count) {
                NetConcat* cc =
                    new NetConcat(scope, scope->local_symbol(), sig->vector_width(), array_count);
                cc->set_line(*this);
                des->add_node(cc);
                ...
                for (unsigned gdx = 0; gdx < array_count; gdx += 1) {
                    ...
                    NetNet* tmp2 = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp2_vec);
                    tmp2->set_line(*this);
                ...
                for (unsigned gdx = 0; gdx < array_count; gdx += 1) {
                    ...
                    NetPartSelect* tmp1 = new NetPartSelect(sig, gdx, 1, NetPartSelect::VP);
                    tmp1->set_line(*this);
                    des->add_node(tmp1);
            ...
            cerr << get_fileline() << ": error: Gate count of " << array_count
                 << " does not match net width of " << sig->vector_width() << " at pin " << idx
                 << "." << endl;
            des->errors += 1;
        }
    }
...
NetNet* PGModule::resize_net_to_port_(Design* des,
                                      NetScope* scope,
                                      NetNet* sig,
                                      unsigned port_wid,
                                      NetNet::PortType dir,
    ...
    NetNet* tmp = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, tmp_type);
    tmp->local_flag(true);
    tmp->set_line(*this);
    ...
    if (dir == NetNet::PINOUT) {
        ...
        NetTran* node = new NetTran(
            scope, scope->local_symbol(), part_b ? wida : widb, part_b ? widb : wida, 0);
        if (part_b) {
    ...
    switch (dir) {
        case NetNet::POUTPUT:
            if (pwidth > swidth) {
                NetPartSelect* node = new NetPartSelect(tmp, 0, swidth, NetPartSelect::VP);
                connect(node->pin(0), sig->pin(0));
                des->add_node(node);
            } else {
                NetNet* osig;
        ...
        case NetNet::PINPUT:
            if (pwidth > swidth) {
                ...
                NetPartSelect* node = new NetPartSelect(sig, 0, pwidth, NetPartSelect::VP);
                connect(node->pin(0), tmp->pin(0));
                des->add_node(node);
            }
            break;
 ...
 */
static void convert_net(Design* des, const LineInfo* line, NetNet* net, NetNet::Type type) {
...
static void isolate_and_connect(Design* des,
                                NetScope* scope,
                                const PGModule* mod,
                                NetNet* port,
                                NetNet* sig,
    ...
    switch (ptype) {
        case NetNet::POUTPUT: {
            NetBUFZ* tmp =
                new NetBUFZ(scope, scope->local_symbol(), sig->vector_width(), true, idx);
            tmp->set_line(*mod);
            des->add_node(tmp);
        ...
        case NetNet::PINOUT: {
            NetTran* tmp = new NetTran(
                scope, scope->local_symbol(), sig->vector_width(), sig->vector_width(), 0);
            tmp->set_line(*mod);
            des->add_node(tmp);
            connect(tmp->pin(1), port->pin(0));
            connect(tmp->pin(0), sig->pin(0));
...
void elaborate_unpacked_port(Design* des,
                             NetScope* scope,
                             NetNet* port_net,
                             PExpr* expr,
    ...
    NetNet* expr_net = elaborate_unpacked_array(des, scope, *expr, port_net, expr);
    if (!expr_net) {
        perm_string port_name = mod->get_port_name(port_idx);
        cerr << expr->get_fileline() << ":      : Port " << port_idx + 1 << " (" << port_name
             << ") of " << mod->mod_name() << " is connected to " << *expr << endl;
        ...
        if (gn_var_can_be_uwire() && (expr_net->type() == NetNet::REG)) {
            if (expr_net->peek_lref() > 0) {
                ...
                cerr << expr->get_fileline()
                     << ": error: "
                        "Cannot connect port '"
                     << port_name << "' to variable '" << expr_net->name()
...
void PGModule::elaborate_mod_(Design* des, Module* rmod, NetScope* scope) const {
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": debug: Instantiate module " << rmod->mod_name()
             << " with instance name " << get_name() << " in scope " << scope_path(scope) << endl;
    }
    ...
    if (debug_elaborate)
        cerr << get_fileline()
             ...
             << instance.size() << " instance(s) of " << get_name() << "..." << endl;
    ...
    for (unsigned idx = 0; idx < pins.size(); idx += 1) {
            ...
            if (tmp->port_type() == NetNet::PINPUT) {
                ...
                if (warn_portbinding) {
                    cerr << get_fileline() << ": warning: "
                         ...
                         << " with dangling input port " << (idx + 1) << " (" << port_name;
                    switch (rmod->uc_drive) {
                        case Module::UCD_PULL0:
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": debug: " << get_name() << ": Port " << (idx + 1) << " ("
                 << port_name << ") has " << prts.size() << " sub-ports." << endl;
        }
        ...
        for (unsigned inst = 0; inst < instance.size(); inst += 1) {
            ...
            if (using_default)
                elab_scope_inst[inst] = inst_scope;
        ...
        if (!prts.empty() && (prts[0]->port_type() == NetNet::PINPUT) &&
            ...
            pins[idx]->is_collapsible_net(des, scope, prts[0]->port_type())) {
            prts[0]->port_type(NetNet::PINOUT);

            cerr << pins[idx]->get_fileline() << ": warning: input port " << prts[0]->name()
                 << " is coerced to inout." << endl;
        }

        if (!prts.empty() && (prts[0]->port_type() == NetNet::POUTPUT) &&
            (prts[0]->type() != NetNet::REG) && prts[0]->pin(0).nexus()->has_floating_input() &&
            pins[idx]->is_collapsible_net(des, scope, prts[0]->port_type())) {
            prts[0]->port_type(NetNet::PINOUT);

            cerr << pins[idx]->get_fileline() << ": warning: output port " << prts[0]->name()
                 << " is coerced to inout." << endl;
        }
        ...
        if (prts.empty() || (ptype == NetNet::PINPUT)) {
            ...
            if (!prts.empty() && prts[0]->unpacked_dimensions() > 0) {
                ...
                elaborate_unpacked_port(des, scope, prts[0], pins[idx], ptype, rmod, idx);
                continue;
            }
            ...
            if (using_default) {
                ...
                //	         << " has scope: " << elab_scope_inst[inst]->fullname() <<
                // endl;
                //       }
                // }
                if (instance.size() > 1) {
            ...
            if (tmp_expr == 0) {
                cerr << pins[idx]->get_fileline() << ": error: Failed to elaborate input port '"
                     << port_name << "' " << (using_default ? "default value" : "expression")
                     << " (" << *pins[idx] << ") in instance " << scope->fullname() << "."
                     << get_name() << " of module: " << rmod->mod_name() << "." << endl;
                des->errors += 1;
                continue;
            }
            ...
            if (sig == 0) {
                cerr << pins[idx]->get_fileline() << ": internal error: Port expression "
            ...
            if (need_bufz_for_input_port(prts) || gn_interconnect_flag == true) {
                ...
                NetBUFZ* tmp = new NetBUFZ(inner_scope,
                                           inner_scope->local_symbol(),
                                           sig->vector_width(),
                                           true,
                ...
                const netvector_t* tmp2_vec =
                    new netvector_t(sig->data_type(), sig->vector_width() - 1, 0);
                NetNet* tmp2 =
                    new NetNet(inner_scope, inner_scope->local_symbol(), NetNet::WIRE, tmp2_vec);
                tmp2->local_flag(true);
                tmp2->set_line(*this);
            ...
            if ((sig->data_type() == IVL_VT_REAL) && !prts.empty() &&
                ...
                sig = cast_to_int4(des, scope, sig, prts_vector_width / instance.size());
            }
            ...
            if (sig == 0) {
                ...
                cerr << pins[idx]->get_fileline() << ":      : Port " << (idx + 1) << " ("
                     << port_name << ") of " << rmod->mod_name() << " is connected to "
                     << *pins[idx] << endl;
                des->errors += 1;
                continue;
            }
            ...
            if ((sig->data_type() == IVL_VT_REAL) && !prts.empty() &&
                ...
                cerr << pins[idx]->get_fileline() << ": error: "
                     << "Cannot automatically connect bit based "
                        "inout port "
                     << (idx + 1) << " (" << port_name << ") of module " << rmod->mod_name()
            ...
            if ((sig->data_type() != IVL_VT_REAL) && !prts.empty() &&
                ...
                for (unsigned pidx = 0; pidx < prts.size(); pidx += 1) {
                    prts[pidx] =
                        cast_to_int4(des, scope, prts[pidx], prts_vector_width / instance.size());
                }
            }
            ...
            if ((sig->data_type() == IVL_VT_BOOL) && !prts.empty() &&
                ...
                for (unsigned pidx = 0; pidx < prts.size(); pidx += 1) {
                    prts[pidx] = cast_to_int2(des, scope, prts[pidx], prts[pidx]->vector_width());
                }
            }
            ...
            if ((sig->data_type() == IVL_VT_REAL) && !prts.empty() &&
                ...
                cerr << pins[idx]->get_fileline() << ": error: "
                     ...
                     << " cannot have a real port (port " << (idx + 1) << " : " << port_name
                     << ") connected to a "
                        "real signal ("
                     << sig->name() << ")." << endl;
                des->errors += 1;
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": debug: " << get_name() << ": Port " << (idx + 1) << " ("
                 << port_name << ") has vector width of " << prts_vector_width << "." << endl;
        }
        ...
        if ((instance.size() == 1) && (prts_vector_width != sig->vector_width())) {
            ...
            cerr << get_fileline() << ": warning: Port " << (idx + 1) << " (" << port_name
                 << ") of module " << type_ << " expects " << prts_vector_width << " bit(s), given "
                 << sig->vector_width() << "." << endl;
            ...
            if (ptype == NetNet::PINOUT) {
                ...
                if (as_signed)
                    cerr << "(signed) ";
                cerr << (prts_vector_width - sig->vector_width()) << " high bits of the port."
                     << endl;
            } else {
                ...
                if (as_signed)
                    cerr << "(signed) ";
                cerr << (sig->vector_width() - prts_vector_width) << " high bits of the expression."
                     << endl;
            }

            sig = resize_net_to_port_(des, scope, sig, prts_vector_width, ptype, as_signed);
        }
        ...
        if (prts.size() == 1) {
            ...
            if (prts[0]->delay_paths() > 0 ||
                ...
                isolate_and_connect(
                    des, inner_scope, this, prts[0], sig, ptype, gn_interconnect_flag ? idx : -1);
            } else {
            ...
            switch (ptype) {
                case NetNet::POUTPUT:
                    NetConcat* ctmp;
                    ctmp =
                        new NetConcat(scope, scope->local_symbol(), prts_vector_width, prts.size());
                    ctmp->set_line(*this);
                    des->add_node(ctmp);
                ...
                case NetNet::PINPUT:
                    if (debug_elaborate) {
                        cerr << get_fileline() << ": debug: " << get_name() << ": Dividing "
                             << prts_vector_width << " bits across all "
                             << prts_vector_width / instance.size() << " input sub-ports of port "
                             << (idx + 1) << "." << endl;
                    }
                    ...
                    for (unsigned ldx = 0, spin = 0; ldx < prts.size(); ldx += 1) {
                        ...
                        NetPartSelect* ptmp =
                            new NetPartSelect(sig, spin, sp->vector_width(), NetPartSelect::VP);
                        ptmp->set_line(*this);
                        des->add_node(ptmp);
...
void PGModule::elaborate_udp_(Design* des, PUdp* udp, NetScope* scope) const {
    ...
    if (inst_count != 1) {
        cerr << get_fileline() << ": sorry: UDPs with a range (" << my_name << " [" << high << ":"
             << low << "]) are "
             << "not supported." << endl;
        des->errors += 1;
        return;
    ...
    if (pins_) {
        ...
        if (pin_count() != udp->ports.size()) {
            cerr << get_fileline()
                 << ": error: Wrong number "
                    "of ports. Expecting "
                 << udp->ports.size() << ", got " << pin_count() << "." << endl;
            des->errors += 1;
            return;
        }
    ...
    if (pins[0] == 0) {
        ...
        if (sig == 0) {
            ...
            cerr << get_fileline() << ":      : Output "
                 << "port of " << udp->name_ << " is " << udp->ports[0] << "." << endl;
            des->errors += 1;
            return;
        } else {
        ...
        if (sig->vector_width() != 1) {
            cerr << get_fileline() << ": error: "
                 << "Output port expression " << *pins[0] << " is too wide (" << sig->vector_width()
                 << ") expected 1." << endl;
            des->errors += 1;
        }
    }
...
void PGModule::elaborate(Design* des, NetScope* scope) const {
    ...
    if (!ignore_missing_modules) {
        cerr << get_fileline() << ": internal error: Unknown module type: " << type_ << endl;
    }
}
...
NetAssign_* PAssign_::elaborate_lval(Design* des, NetScope* scope) const {
    ...
    if (!lval_) {
        ...
        NetNet* tmp;
        const netvector_t* tmp_vec =
            new netvector_t(rval_->expr_type(), rval_->expr_width() - 1, 0, rval_->has_sign());
...
NetExpr* PAssign_::elaborate_rval_(Design* des, NetScope* scope, ivl_type_t net_type) const {
    ivl_assert(*this, rval_);

    NetExpr* rv = elaborate_rval_expr(des, scope, net_type, rval_, is_constant_);
...
NetExpr* PAssign_::elaborate_rval_(Design* des,
                                   NetScope* scope,
    ...
    NetExpr* rv = elaborate_rval_expr(
        des, scope, lv_net_type, lv_type, lv_width, rval(), is_constant_, force_unsigned);
...
NetProc* PAssign::elaborate_compressed_(Design* des, NetScope* scope) const {
    ...
    NetExpr* rv =
        elaborate_rval_(des, scope, 0, lv->expr_type(), count_lval_width(lv), force_unsigned);
    if (rv == 0)
        return 0;
...
NetProc* PAssign::elaborate(Design* des, NetScope* scope) const {
    ...
    if (delay || event_) {
        ...
        NetNet* tmp = new NetNet(scope, scope->local_symbol(), NetNet::REG, tmp2_vec);
        tmp->local_flag(true);
        tmp->set_line(*this);
...
NetProc* PAssignNB::elaborate(Design* des, NetScope* scope) const {
    ...
    NetExpr* rv =
        elaborate_rval_(des, scope, lv->net_type(), lv->expr_type(), count_lval_width(lv));
    if (rv == 0)
        return 0;
        ...
        if (const PChainConstructor* supernew = dynamic_cast<PChainConstructor*>(list_[idx])) {
            if (debug_elaborate) {
                cerr << get_fileline() << ": PBlock::elaborate: "
                     ...
                     << " at " << supernew->get_fileline() << "." << endl;
            }
            if (idx > 0) {
                des->errors += 1;
                cerr << supernew->get_fileline() << ": error: "
                     << "super.new(...) must be the first statement in a block." << endl;
            }
        }
...
static int test_case_width(Design* des, NetScope* scope, PExpr* pe, PExpr::width_mode_t& mode) {
    ...
    if (debug_elaborate) {
        ...
        cerr << pe->get_fileline() << ":        "
             ...
             << ", signed=" << pe->has_sign() << ", mode=" << PExpr::width_mode_name(mode) << endl;
    }
    return expr_width;
}

static NetExpr* elab_and_eval_case(Design* des,
                                   NetScope* scope,
                                   PExpr* pe,
                                   bool context_is_real,
...
NetProc* PCase::elaborate(Design* des, NetScope* scope) const {
    ...
    for (unsigned idx = 0; idx < items_->size(); idx += 1) {
        ...
        for (list<PExpr*>::iterator idx_expr = cur->expr.begin(); idx_expr != cur->expr.end();
             ++idx_expr) {
    ...
    if (context_is_real) {
        ...
        for (unsigned idx = 0; idx < items_->size(); idx += 1) {
            ...
            for (list<PExpr*>::iterator idx_expr = cur->expr.begin(); idx_expr != cur->expr.end();
                ...
                unsigned cur_width = test_case_width(des, scope, cur_expr, context_mode);
                if (cur_width > context_width)
                    context_width = cur_width;
            }
        }
    ...
    if (debug_elaborate) {
        ...
        if (context_is_real) {
            ...
            cerr << (context_unsigned ? "unsigned" : "signed") << " vector, width=" << context_width
                 << endl;
        }
    }
    NetExpr* expr =
        elab_and_eval_case(des, scope, expr_, context_is_real, context_unsigned, context_width);
    if (expr == 0) {
        cerr << get_fileline()
    ...
    for (unsigned idx = 0; idx < items_->size(); idx += 1) {
        ...
        if (cur->expr.empty()) {
            ...
            for (list<PExpr*>::iterator idx_expr = cur->expr.begin(); idx_expr != cur->expr.end();
                ...
                PExpr* cur_expr = *idx_expr;
                ivl_assert(*this, cur_expr);
                NetExpr* gu = elab_and_eval_case(
                    des, scope, cur_expr, context_is_real, context_unsigned, context_width);
        ...
        for (size_t idx = 1; idx < parms.size(); idx += 1) {
            if (args[idx - 1]) {
                parms[idx] = elaborate_rval_expr(
                    des, scope, def->port(idx)->net_type(), args[idx - 1], false);
                continue;
            }
...
NetProc* PCallTask::elaborate_sys(Design* des, NetScope* scope) const {
    ...
    for (unsigned idx = 0; idx < parm_count; idx += 1) {
        ...
        if (!parm.name.nil()) {
            cerr << parm.get_fileline() << ": error: "
                 << "The system task `" << name << "` has no argument called `" << parm.name << "`."
                 << endl;
            des->errors++;
        }
    ...
    if (gn_specify_blocks_flag == false && gn_interconnect_flag == false &&
        name == "$sdf_annotate") {
...
NetProc* PCallTask::elaborate_usr(Design* des, NetScope* scope) const {
    ...
    if (def == 0) {
        cerr << get_fileline() << ": internal error: task " << path_
             << " doesn't have a definition in " << scope_path(scope) << "." << endl;
        des->errors += 1;
        return 0;
    }
    ivl_assert(*this, def);
 ...
 */
NetProc* PCallTask::elaborate_sys_task_method_(Design* des,
                                               NetScope* scope,
                                               NetNet* net,
    ...
    if (method_name == "delete") {
        ...
        cerr << get_fileline() << ": error: " << method_name << "() method takes "
             << parm_names.size() << " arguments, got " << parms_.size() << "." << endl;
        des->errors++;
    }

    auto args = map_named_args(des, parm_names, parms_);
    for (unsigned idx = 0; idx < nparms; idx += 1) {
        argv[idx + 1] = elab_sys_task_arg(des, scope, method_name, idx, args[idx]);
    }
 ...
 * sys_task_name is the internal system-task name to use.
 */
NetProc* PCallTask::elaborate_queue_method_(Design* des,
                                            NetScope* scope,
                                            NetNet* net,
                                            perm_string method_name,
                                            const char* sys_task_name,
    ...
    if ((method_name == "insert") && (nparms != 2)) {
        cerr << get_fileline() << ": error: " << method_name << "() method requires two arguments."
             << endl;
        des->errors += 1;
    }
    ...
    if (method_name != "insert") {
        if (nparms == 0 || !args[0]) {
            ...
            argv[1] = elab_and_eval(des, scope, args[0], context_width, false, false, base_type);
        }
    } else {
        if (nparms == 0 || !args[0]) {
            argv[1] = nullptr;
            ...
            argv[1] = elab_and_eval(des, scope, args[0], context_width, false, false, IVL_VT_LOGIC);
        }
        ...
        if (nparms < 2 || !args[1]) {
            ...
            argv[2] = elab_and_eval(des, scope, args[1], context_width, false, false, base_type);
        }
    }
...
NetProc* PCallTask::elaborate_method_func_(NetScope* scope,
    ...
    if (!void_cast_) {
        cerr << get_fileline() << ": warning: method function '" << method_name
...
NetProc* PCallTask::elaborate_method_(Design* des, NetScope* scope, bool add_this_flag) const {
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PCallTask::elaborate_method_: "
             ...
             << ", add_this_flag=" << add_this_flag << endl;
    }
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PCallTask::elaborate_method_: "
             << "Try to match method " << method_name << " of object " << net->name() << "."
             << endl;
        if (net->net_type())
            cerr << get_fileline() << ": PCallTask::elaborate_method_: " << net->name()
                 << ".net_type() --> " << *net->net_type() << endl;
        cerr << get_fileline() << ": PCallTask::elaborate_method_: " << net->name()
             << ".data_type() --> " << net->data_type() << endl;
    }
    ...
    if (dynamic_cast<const netstring_t*>(net->net_type())) {
        if (method_name == "itoa") {
            ...
            return elaborate_sys_task_method_(
                des, scope, net, method_name, "$ivl_string_method$itoa", parm_names);
        } else if (method_name == "hextoa") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("i")};

            return elaborate_sys_task_method_(
                des, scope, net, method_name, "$ivl_string_method$hextoa", parm_names);
        } else if (method_name == "octtoa") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("i")};

            return elaborate_sys_task_method_(
                des, scope, net, method_name, "$ivl_string_method$octtoa", parm_names);
        } else if (method_name == "bintoa") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("i")};

            return elaborate_sys_task_method_(
                des, scope, net, method_name, "$ivl_string_method$bintoa", parm_names);
        } else if (method_name == "realtoa") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("r")};

            return elaborate_sys_task_method_(
                des, scope, net, method_name, "$ivl_string_method$realtoa", parm_names);
        }
    }
    ...
    if (net->darray_type()) {
        if (method_name == "delete") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("index")};

            return elaborate_sys_task_method_(
                des, scope, net, method_name, "$ivl_darray_method$delete", parm_names);
        } else if (method_name == "size") {
            // This returns an int. It could be removed, but keep for now.
            return elaborate_method_func_(scope, net, &netvector_t::atom2s32, method_name, "$size");
    ...
    if (net->queue_type()) {
        ...
        if (method_name == "push_back") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("item")};

            return elaborate_queue_method_(
                des, scope, net, method_name, "$ivl_queue_method$push_back", parm_names);
        } else if (method_name == "push_front") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("item")};

            return elaborate_queue_method_(
                des, scope, net, method_name, "$ivl_queue_method$push_front", parm_names);
        } else if (method_name == "insert") {
            static const std::vector<perm_string> parm_names = {perm_string::literal("index"),
                                                                perm_string::literal("item")};

            return elaborate_queue_method_(
                des, scope, net, method_name, "$ivl_queue_method$insert", parm_names);
        } else if (method_name == "pop_front") {
            return elaborate_method_func_(
                scope, net, use_darray->element_type(), method_name, "$ivl_queue_method$pop_front");
        } else if (method_name == "pop_back") {
            return elaborate_method_func_(
                scope, net, use_darray->element_type(), method_name, "$ivl_queue_method$pop_back");
        }
    }
    ...
    if (const netclass_t* class_type = dynamic_cast<const netclass_t*>(sr.type)) {
        ...
        if (task == 0) {
            ...
            if (!add_this_flag) {
                cerr << get_fileline() << ": error: "
                     << "Can't find task " << method_name << " in class " << class_type->get_name()
                     << endl;
                des->errors += 1;
            }
            return 0;
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": PCallTask::elaborate_method_: "
                 << "Elaborate " << class_type->get_name() << " method " << task->basename()
                 << endl;
        }
...
NetProc* PCallTask::elaborate_non_void_function_(Design* des, NetScope* scope) const {
    ...
    if (!void_cast_) {
        cerr << get_fileline() << ": warning: User function '" << peek_tail_name(path_)
             << "' is being called as a task." << endl;
    }
...
NetProc* PCallTask::elaborate_void_function_(Design* des, NetScope* scope, NetFuncDef* def) const {
...
NetProc* PCallTask::elaborate_build_call_(Design* des,
                                          NetScope* scope,
                                          NetScope* task,
    ...
    if (task->type() == NetScope::TASK) {
        ...
        if (void_cast_) {
            cerr << get_fileline() << ": error: void casting user task '" << peek_tail_name(path_)
                 << "' is not allowed." << endl;
            des->errors++;
        }
    ...
    if (parms_.size() > parm_count) {
        cerr << get_fileline() << ": error: "
             << "Too many arguments (" << parms_.size() << ", expecting " << parm_count << ")"
    ...
    for (unsigned int idx = off; idx < parm_count; idx++) {
        ...
        if (args[parms_idx]) {
            rv = elaborate_rval_expr(des, scope, port->net_type(), args[parms_idx]);
            ...
            switch (lv->expr_type()) {
                ...
                default:
                    ...
                    cerr << get_fileline() << ": error: "
                         ...
                         << " is not compatible with the argument type." << endl;
                    des->errors += 1;
                    delete rv;
                    delete lv;
                    continue;
...
static bool check_parm_is_const(NetExpr* param) {
    ...
    if (dynamic_cast<NetEConstParam*>(param))
        return true;
    if (dynamic_cast<NetECReal*>(param))
        return true;
    if (dynamic_cast<NetECRealParam*>(param))
        return true;
    if (dynamic_cast<NetEConst*>(param))
        return true;
...
bool PCallTask::elaborate_elab(Design* des, NetScope* scope) const {
    ...
    if (!gn_system_verilog()) {
        cerr << get_fileline() << ": error: Elaboration task '" << name
             << "' requires SystemVerilog." << endl;
        des->errors += 1;
        return false;
    }

    if (name != "$fatal" && name != "$error" && name != "$warning" && name != "$info") {
        cerr << get_fileline() << ": error: '" << name << "' is not a valid elaboration task."
             << endl;
        des->errors += 1;
        return true;
    }
    ...
    for (unsigned idx = 0; idx < parm_count; idx += 1) {
        ...
        if (!parm.name.nil()) {
            cerr << parm.get_fileline() << ": error: "
                 << "The elaboration system task `" << name << "` has no argument called `"
                 << parm.name << "`." << endl;
            des->errors++;
        }
        ...
        if (!check_parm_is_const(eparms[idx])) {
            cerr << get_fileline() << ": error: Elaboration task " << name << "() parameter ["
                 << idx + 1 << "] '" << *eparms[idx] << "' is not constant." << endl;
            des->errors += 1;
            const_parms = false;
        }
    }
    ...
    cerr << string(sstr.size(), ' ') << "  During elaboration  Scope: " << scope->fullname()
         << endl;
...
NetCAssign* PCAssign::elaborate(Design* des, NetScope* scope) const {
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": debug: Elaborate cassign,"
             << " lval width=" << lwid << " rval width=" << rexp->expr_width() << " rval=" << *rexp
             << endl;
    }
...
NetProc* PDelayStatement::elaborate(Design* des, NetScope* scope) const {
    ...
    if (const NetEConst* tmp = dynamic_cast<NetEConst*>(dex)) {
        if (statement_)
            obj = new NetPDelay(tmp->value().as_ulong64(), statement_->elaborate(des, scope));
        else
...
NetProc* PDisable::elaborate(Design* des, NetScope* scope) const {
    ...
    if (scope_.empty()) {
        if (gn_system_verilog()) {
            ...
            cerr << get_fileline() << ": error: 'disable fork' requires SystemVerilog." << endl;
            des->errors += 1;
            return 0;
        }
    }
...
NetProc* PEventStatement::elaborate_st(Design* des, NetScope* scope, NetProc* enet) const {
    ivl_assert(*this, scope);
    ...
    if (expr_.size() == 0) {
        ...
        NetEvProbe* pr =
            new NetEvProbe(scope, scope->local_symbol(), ev, NetEvProbe::ANYEDGE, nset->size());
...
// FIXME: create a part select to get the correct bits to connect.
        ...
        for (unsigned idx = 0; idx < expr_.size(); idx += 1) {
            ...
            if (const PEIdent* id = dynamic_cast<PEIdent*>(expr_[idx]->expr())) {
                ...
                if (sr.scope && sr.eve) {
                    ...
                    if (expr_[idx]->type() != PEEvent::ANYEDGE) {
                        ...
                        cerr << " can not be used with a named event (" << sr.eve->name() << ")."
                             << endl;
                        des->errors += 1;
                    }
                    continue;
            ...
            if ((tmp->expr_type() == IVL_VT_REAL) && (expr_[idx]->type() != PEEvent::ANYEDGE)) {
                cerr << get_fileline() << ": error: '";
                switch (expr_[idx]->type()) {
                    case PEEvent::POSEDGE:
                        cerr << "posedge";
                ...
                cerr << "' cannot be used with real expressions '" << *expr_[idx]->expr() << "'."
                     << endl;
                des->errors += 1;
                continue;
            }
            ...
            unsigned pins = (expr_[idx]->type() == PEEvent::ANYEDGE) ? expr->pin_count() : 1;

            NetEvProbe* pr;
            switch (expr_[idx]->type()) {
                case PEEvent::POSEDGE:
                    pr =
                        new NetEvProbe(scope, scope->local_symbol(), ev, NetEvProbe::POSEDGE, pins);
                    break;

                case PEEvent::NEGEDGE:
                    pr =
                        new NetEvProbe(scope, scope->local_symbol(), ev, NetEvProbe::NEGEDGE, pins);
                    break;

                case PEEvent::EDGE:
                    pr = new NetEvProbe(scope, scope->local_symbol(), ev, NetEvProbe::EDGE, pins);
                    break;

                case PEEvent::ANYEDGE:
                    pr =
                        new NetEvProbe(scope, scope->local_symbol(), ev, NetEvProbe::ANYEDGE, pins);
                    break;
 ...
 *     end
 */
NetProc* PEventStatement::elaborate_wait(Design* des, NetScope* scope, NetProc* enet) const {
    ivl_assert(*this, scope);
    ...
    NetExpr* expr = pe->elaborate_expr(des, scope, pe->expr_width(), PExpr::NO_FLAGS);
    if (expr == 0) {
        cerr << get_fileline()
             << ": error: Unable to elaborate"
    ...
    NetEvProbe* wait_pr = new NetEvProbe(
        scope, scope->local_symbol(), wait_event, NetEvProbe::ANYEDGE, wait_set->size());
...
NetProc* PEventStatement::elaborate_wait_fork(Design* des, const NetScope* scope) const {
    ...
    if (gn_system_verilog()) {
        ...
        cerr << get_fileline() << ": error: 'wait fork' requires SystemVerilog." << endl;
        des->errors += 1;
        return 0;
    }
}
...
NetForce* PForce::elaborate(Design* des, NetScope* scope) const {
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": debug: Elaborate force,"
             << " lval width=" << lval->lwidth() << " rval width=" << rexp->expr_width()
             << " rval=" << *rexp << endl;
    }
...
static void find_property_in_class(const LineInfo& loc,
...
NetProc* PForeach::elaborate(Design* des, NetScope* scope) const {
    ...
    if (debug_elaborate && class_scope) {
        cerr << get_fileline() << ": PForeach::elaborate: "
             ...
             << class_scope->get_name() << " as " << *class_scope->get_prop_type(class_property)
             << "." << endl;
    }
    ...
    if (class_scope != 0 && class_property >= 0) {
        ...
        if (atype == 0) {
            cerr << get_fileline() << ": error: "
                 << "I can't handle the type of " << array_var_ << " as a foreach loop." << endl;
            des->errors += 1;
            return 0;
        }
    ...
    if (array_sig == 0) {
        cerr << get_fileline() << ": error:"
             << " Unable to find foreach array " << array_name << " in scope " << scope_path(scope)
             << "." << endl;
        des->errors += 1;
        return 0;
    }
    ...
    if (debug_elaborate) {
        cerr << get_fileline() << ": PForeach::elaborate: "
             << "Scan array " << array_sig->name() << " of " << array_sig->data_type() << " with "
             << array_sig->unpacked_dimensions() << " unpacked"
             << " and " << array_sig->packed_dimensions() << " packed dimensions." << endl;
    }
 ...
 */
NetProc*
PForeach::elaborate_static_array_(Design* des, NetScope* scope, const netranges_t& dims) const {
    if (debug_elaborate) {
...
NetProc* PForStatement::elaborate(Design* des, NetScope* scope) const {
    ...
    if (!name1_) {
        ...
        if (sig == 0) {
            cerr << id1->get_fileline() << ": register ``" << id1->path() << "'' unknown in "
                 << scope_path(scope) << "." << endl;
            des->errors += 1;
            return 0;
        }
        ...
        initial_expr = elaborate_rval_expr(des, scope, sig->net_type(), expr1_);
        if (!initial_expr)
            error_flag = true;
...
NetProc* PReturn::elaborate(Design* des, NetScope* scope) const {
    ...
    for (;;) {
        if (target == 0) {
            cerr << get_fileline() << ": error: "
                 << "Return statement is not in a function or task." << endl;
            des->errors += 1;
            return 0;
        }
    ...
    if (expr_ == 0) {
        cerr << get_fileline() << ": error: "
             << "Return from " << scope_path(target) << " requires a return value expression."
             << endl;
        des->errors += 1;
        return 0;
    }
...
NetProc* PNBTrigger::elaborate(Design* des, NetScope* scope) const {
    ...
    NetExpr* dly = 0;
    if (dly_)
        dly = elab_and_eval(des, scope, dly_, -1);
...
bool PProcess::elaborate(Design* des, NetScope* scope) const {
    ...
    do {
        if ((top->type() != IVL_PR_ALWAYS) && (top->type() != IVL_PR_ALWAYS_COMB) &&
            (top->type() != IVL_PR_ALWAYS_FF) && (top->type() != IVL_PR_ALWAYS_LATCH))
            break;
...
void PSpecPath::elaborate(Design* des, NetScope* scope) const {
    ...
    if (!full_flag_ && ((src.size() != 1) || (dst.size() != 1))) {
        ...
        for (str_vec_iter cur = src.begin(); (cur != src.end() && all_single); ++cur) {
        ...
        for (str_vec_iter cur = dst.begin(); (cur != dst.end() && all_single); ++cur) {
            const NetNet* psig = scope->find_signal(*cur);
    ...
    for (str_vector_iter cur = dst.begin(); cur != dst.end(); ++cur) {
        ...
        if (dst_sig == 0) {
            cerr << get_fileline() << ": error: No wire '" << *cur << "' in this module." << endl;
            des->errors += 1;
            continue;
        }
        ...
        if (dst_sig->port_type() != NetNet::POUTPUT && dst_sig->port_type() != NetNet::PINOUT) {
            cerr << get_fileline() << ": error: Path destination " << *cur
                 << " must be an output or inout port." << endl;
            des->errors += 1;
        }

        NetDelaySrc* path = new NetDelaySrc(
            scope, scope->local_symbol(), src.size(), condit_sig, conditional, !full_flag_);
        path->set_line(*this);
        ...
        unsigned idx = 0;
        for (str_vector_iter cur_src = src.begin(); cur_src != src.end(); ++cur_src) {
            NetNet* src_sig = scope->find_signal(*cur_src);
            if (src_sig == 0) {
                cerr << get_fileline() << ": error: No wire '" << *cur_src << "' in this module."
                     << endl;
                des->errors += 1;
                continue;
            }
...
void PRecRem::elaborate(Design* des, NetScope* scope) const {
    ...
    if (delayed_reference_ != nullptr) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PRecRem::elaborate: Assigning " << reference_event_->name
        ...
        if (sig == nullptr) {
            cerr << get_fileline() << ": error: Cannot find: " << reference_event_->name << endl;
            des->errors += 1;
            return;
        }
        ...
        if (sig_delayed == nullptr) {
            cerr << get_fileline() << ": error: Cannot find: " << *delayed_reference_ << endl;
            des->errors += 1;
            return;
        }
    ...
    if (delayed_data_ != nullptr) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PRecRem::elaborate: Assigning " << data_event_->name
        ...
        if (sig == nullptr) {
            cerr << get_fileline() << ": error: Cannot find: " << data_event_->name << endl;
            des->errors += 1;
            return;
        }
        ...
        if (sig_delayed == nullptr) {
            cerr << get_fileline() << ": error: Cannot find: " << *delayed_data_ << endl;
            des->errors += 1;
            return;
        }
...
void PSetupHold::elaborate(Design* des, NetScope* scope) const {
    ...
    if (delayed_reference_ != nullptr) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PSetupHold::elaborate: Assigning" << reference_event_->name
                 << " to " << *delayed_reference_ << endl;
        }
    ...
    if (delayed_data_ != nullptr) {
        if (debug_elaborate) {
            cerr << get_fileline() << ": PSetupHold::elaborate: Assigning" << data_event_->name
                 << " to " << *delayed_data_ << endl;
        }
...
static void
elaborate_functions(Design* des, NetScope* scope, const map<perm_string, PFunction*>& funcs) {
...
static void elaborate_tasks(Design* des, NetScope* scope, const map<perm_string, PTask*>& tasks) {
...
static void
elaborate_classes(Design* des, NetScope* scope, const map<perm_string, PClass*>& classes) {
    for (map<perm_string, PClass*>::const_iterator cur = classes.begin(); cur != classes.end();
         ++cur) {
...
void netclass_t::elaborate(Design* des, PClass* pclass) {
    ...
    for (map<perm_string, PFunction*>::iterator cur = pclass->funcs.begin();
         cur != pclass->funcs.end();
         ++cur) {
        if (debug_elaborate) {
            cerr << cur->second->get_fileline() << ": netclass_t::elaborate: "
                 << "Elaborate class " << scope_path(class_scope_) << " function method "
                 << cur->first << endl;
        }
    ...
    for (map<perm_string, PTask*>::iterator cur = pclass->tasks.begin(); cur != pclass->tasks.end();
         ++cur) {
        if (debug_elaborate) {
            cerr << cur->second->get_fileline() << ": netclass_t::elaborate: "
                 << "Elaborate class " << scope_path(class_scope_) << " task method " << cur->first
                 << endl;
        }
...
bool PGenerate::elaborate(Design* des, NetScope* container) const {
    ...
    if (debug_elaborate) {
        cerr << get_fileline()
             << ": PGenerate::elaborate: "
                "generate "
             << scheme_type << " elaborating in scope " << scope_path(container) << "." << endl;
        cerr << get_fileline()
             << ": PGenerate::elaborate: "
                "generate scope_name="
    ...
    if (scheme_type == PGenerate::GS_CASE) {
        ...
        for (generate_it_t cur = generate_schemes.begin(); cur != generate_schemes.end(); ++cur) {
            PGenerate* item = *cur;
    ...
    for (scope_list_it_t cur = scope_list_.begin(); cur != scope_list_.end(); ++cur) {
        NetScope* scope = *cur;
    ...
    for (generate_it_t cur = generate_schemes.begin(); cur != generate_schemes.end(); ++cur) {
        ...
        if (debug_elaborate) {
            cerr << get_fileline() << ": PGenerate::elaborate_direct_: "
                 ...
                 << ", item->scope_list_.size()=" << item->scope_list_.size() << "." << endl;
        }
        ...
        if (item->scheme_type == PGenerate::GS_CASE) {
            for (generate_it_t icur = item->generate_schemes.begin();
                 icur != item->generate_schemes.end();
                 ++icur) {
...
bool PGenerate::elaborate_(Design* des, NetScope* scope) const {
    ...
    if (result_flag) {
        ...
        for (const auto bh : behaviors)
            bh->elaborate(des, scope);

        for (const auto gs : generate_schemes)
            gs->elaborate(des, scope);
    }
...
bool PScope::elaborate_behaviors_(Design* des, NetScope* scope) const {
    ...
    // involves scanning the PProcess* list, creating a NetProcTop
    // for each process.
    for (list<PProcess*>::const_iterator st = behaviors.begin(); st != behaviors.end(); ++st) {
        result_flag &= (*st)->elaborate(des, scope);
    }

    for (list<AProcess*>::const_iterator st = analog_behaviors.begin();
         st != analog_behaviors.end();
         ++st) {
        result_flag &= (*st)->elaborate(des, scope);
    }
...
bool Design::check_proc_delay() const {
    ...
    for (const NetProcTop* pr = procs_; pr; pr = pr->next_) {
        ...
        if ((pr->type() == IVL_PR_ALWAYS_COMB) || (pr->type() == IVL_PR_ALWAYS_FF) ||
 ...
 */
static void check_timescales(bool& some_explicit, bool& some_implicit, const PScope* scope) {
...
static void check_timescales() {
    ...
    if (gn_system_verilog() && !(some_explicit && some_implicit)) {
        for (pkg = pform_units.begin(); pkg != pform_units.end(); ++pkg) {
            ...
            if (pp->parameters.empty() && pp->wires.empty() && pp->tasks.empty() &&
                pp->funcs.empty() && pp->classes.empty())
                continue;
    ...
    for (mod = pform_modules.begin(); mod != pform_modules.end(); ++mod) {
        ...
        if (mp->has_explicit_timescale())
            continue;
        cerr << "       :   -- module " << (*mod).first << " declared here: " << mp->get_fileline()
             << endl;
    }
    ...
    for (pkg = pform_units.begin(); pkg != pform_units.end(); ++pkg) {
        ...
        if (pp->parameters.empty() && pp->wires.empty() && pp->tasks.empty() && pp->funcs.empty() &&
            pp->classes.empty())
            continue;
...
Design* elaborate(list<perm_string> roots) {
    ...
    if (gn_system_verilog()) {
        for (vector<PPackage*>::iterator pkg = pform_units.begin(); pkg != pform_units.end();
             ++pkg) {
            PPackage* unit = *pkg;
    ...
    // assumes they can.
    for (vector<PPackage*>::iterator pkg = pform_packages.begin(); pkg != pform_packages.end();
         ++pkg) {
        PPackage* pack = *pkg;
    ...
    i = 0;
    for (list<perm_string>::const_iterator root = roots.begin(); root != roots.end(); ++root) {
        ...
        NetScope* scope =
            des->make_root_scope(*root, unit_scope, rmod->program_block, rmod->is_interface);
    ...
    for (i = 0; i < root_elems.size(); i++) {
        ...
        if (debug_elaborate) {
            cerr << "<toplevel>" << ": debug: " << rmod->mod_name() << ": port elaboration root "
        ...
        for (unsigned idx = 0; idx < rmod->port_count(); idx += 1) {
            ...
            if (debug_elaborate) {
                cerr << "<toplevel>" << ": debug: " << rmod->mod_name() << ": adding module port "
    ...
    if (debug_elaborate) {
        cerr << "<toplevel>" << ": debug: "
             << " finishing with " << des->find_root_scopes().size() << " root scopes " << endl;
    }
