...
void PGate::eval_delays(Design* des,
                        NetScope* scope,
                        NetExpr*& rise_expr,
                        NetExpr*& fall_expr,
                        NetExpr*& decay_expr,
...
PGAssign::PGAssign(list<PExpr*>* pins, list<PExpr*>* dels) : PGate(perm_string(), pins, dels) {
    ivl_assert(*this, pin_count() == 2);
}

PGAssign::~PGAssign() {}

PGBuiltin::PGBuiltin(Type t, perm_string name, list<PExpr*>* pins, const list<PExpr*>* del)
...
PGModule::PGModule(perm_string type, perm_string name, named_pexpr_t* pins, unsigned npins)
    : PGate(name, 0),
      bound_type_(0),
      type_(type),
      overrides_(0),
