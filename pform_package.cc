...
void pform_end_package_declaration(const struct vlltype& loc) {
    ...
    if (test != packages_by_name.end()) {
        ostringstream msg;
        msg << "error: Package " << use_name
            << " was already declared here: " << test->second->get_fileline() << ends;
    ...
    for (auto search_pkg : scope->potential_imports) {
        ...
        if (found_pkg && found_pkg != decl_pkg && make_explicit) {
            cerr << loc.get_fileline()
                 ...
                 << found_pkg->pscope_name() << "' and by '" << search_pkg->pscope_name() << "'."
                 << endl;
            error_count++;
            continue;
        }
...
PPackage* pform_package_importable(PPackage* pkg, perm_string name) {
    ...
    for (const auto& exp : pkg->exports) {
        ...
        // from a package.
        if ((!exp.pkg || exp.pkg == import_pkg->second) && (exp.name.nil() || exp.name == name))
    ...
    if (ident) {
        ...
        if (!pkg_decl) {
            cerr << loc.get_fileline()
                 << ": error: "
                    "'"
                 << use_ident << "' is not exported by '" << pkg->pscope_name() << "'." << endl;
            error_count += 1;
            return;
        }
        ...
        list<PPackage*>::const_iterator cur_pkg =
            find(scope->potential_imports.begin(), scope->potential_imports.end(), pkg);
...
static bool
pform_package_exportable(const struct vlltype& loc, PPackage* pkg, const perm_string& ident) {
    ...
    if (pform_cur_package->local_symbols.find(ident) == pform_cur_package->local_symbols.end()) {
        if (pform_find_potential_import(loc, pform_cur_package, ident, false, true))
            return true;
    }
    ...
    cerr << loc.get_fileline()
         << ": error: "
            "`"
         << ident << "` has not been imported from " << pkg->pscope_name() << "." << endl;
    error_count++;
...
PExpr*
pform_package_ident(const struct vlltype& loc, PPackage* pkg, const pform_name_t* ident_name) {
