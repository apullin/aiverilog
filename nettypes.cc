...
unsigned long netrange_width(const netranges_t& packed, unsigned int base_width) {
    unsigned wid = base_width;
    for (netranges_t::const_iterator cur = packed.begin(); cur != packed.end(); ++cur) {
 ...
 */
bool prefix_to_slice(const netranges_t& dims,
