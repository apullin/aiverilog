...
NetScope* Design::make_root_scope(perm_string root,
    ...
    root_scope_ = new NetScope(
        0, hname_t(root), NetScope::MODULE, unit_scope, false, program_block, is_interface);
...
NetScope* Design::make_package_scope(perm_string name, NetScope* unit_scope, bool is_unit) {
    NetScope* scope;

    scope =
        new NetScope(0, hname_t(name), NetScope::PACKAGE, unit_scope, false, false, false, is_unit);
 ...
 */
NetScope*
Design::find_scope_(NetScope* scope, const std::list<hname_t>& path, NetScope::TYPE type) const {
    std::list<hname_t> tmp = path;
    ...
    do {
        ...
        if (scope->type() == NetScope::MODULE && (type == NetScope::MODULE || tmp.size() > 1) &&
 ...
 */
NetScope*
Design::find_scope(NetScope* scope, const std::list<hname_t>& path, NetScope::TYPE type) const {
    assert(scope);
    if (path.empty())
        return scope;
 ...
 * fails.
 */
NetScope* Design::find_scope_(NetScope* scope, const hname_t& path, NetScope::TYPE type) const {
    /* If we are looking for a module or we are not
     * looking at the last path component check for
     * a name match (second line). */
 ...
 * parent scopes until I find it, or I run out of parent scopes.
 */
NetScope* Design::find_scope(NetScope* scope, const hname_t& path, NetScope::TYPE type) const {
    assert(scope);
...
void NetScope::evaluate_parameter_logic_(Design* des, param_ref_t cur) {
    ...
    if (param_type) {
        ...
        if (param_vect && param_vect->get_implicit() && param_vect->get_scalar())
            lv_width = -2;
    ...
    if (param_type && dynamic_cast<PEAssignPattern*>(val_expr)) {
        ...
        expr = elab_and_eval(
            des, val_scope, val_expr, lv_width, true, cur->second.is_annotatable, use_type);
    }
    if (!expr)
        return;
    ...
    switch (expr->expr_type()) {
        case IVL_VT_REAL:
            if (!dynamic_cast<const NetECReal*>(expr)) {
                cerr << expr->get_fileline() << ": error: Unable to evaluate real parameter "
        ...
        case IVL_VT_BOOL:
            ...
            if (param_type == 0) {
                param_type =
                    new netvector_t(expr->expr_type(), expr->expr_width() - 1, 0, expr->has_sign());
                cur->second.ivl_type = param_type;
            }
        ...
        default:
            cerr << expr->get_fileline() << ": internal error: "
                 << "Unhandled expression type " << expr->expr_type() << "?" << endl;
    ...
    for (range_t* value_range = (*cur).second.range; value_range; value_range = value_range->next) {
    ...
    if (!from_flag) {
        cerr << val->get_fileline() << ": error: "
             << "Parameter value " << value << " is out of range for parameter " << (*cur).first
             << "." << endl;
        des->errors += 1;
    }
}
...
void NetScope::evaluate_parameter_real_(Design* des, param_ref_t cur) {
    ...
    NetExpr* expr = elab_and_eval(
        des, val_scope, val_expr, -1, true, cur->second.is_annotatable, param_type->base_type());
    if (!expr)
        return;
        ...
        case IVL_VT_REAL:
            if (NetECReal* tmp = dynamic_cast<NetECReal*>(expr)) {
                ...
                cerr << expr->get_fileline() << ": error: "
                     << "Unable to evaluate real parameter " << (*cur).first << " value: " << *expr
                     << endl;
                des->errors += 1;
                return;
            }
    ...
    if (!from_flag) {
        cerr << res->get_fileline() << ": error: "
             << "Parameter value " << value << " is out of range for real parameter "
             << (*cur).first << "." << endl;
        des->errors += 1;
    }
}
...
void NetScope::evaluate_type_parameter_(Design* des, param_ref_t cur) {
    ...
    if (!type_expr) {
        cerr << this->get_fileline() << ": error: "
             << "Type parameter `" << cur->first << "` value `" << *cur->second.val_expr
             << "` is not a type." << endl;
        des->errors++;
...
void NetScope::evaluate_parameter_(Design* des, param_ref_t cur) {
    ...
    if (cur->second.val_expr == 0) {
        cerr << this->get_fileline() << ": error: "
             << "Missing value for parameter `" << cur->first << "`." << endl;
        des->errors += 1;
    ...
    if (cur->second.solving) {
        cerr << cur->second.get_fileline() << ": error: "
             << "Recursive parameter reference found involving " << cur->first << "." << endl;
        des->errors += 1;
        ...
        switch (use_type) {
            ...
            default:
                cerr << cur->second.get_fileline() << ": internal error: "
                     << "Unexpected parameter type " << use_type << "." << endl;
