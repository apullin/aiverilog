...
ostream& netqueue_t::debug_dump(ostream& fd) const {
    fd << "queue of ";
    if (max_idx_ >= 0)
        fd << "(maximum of " << max_idx_ + 1 << " elements) ";
    fd << *element_type();
    return fd;
}
...
void Link::dump_link(ostream& fd, unsigned ind) const {
    ...
    for (cur = nex->first_nlink(); cur; cur = cur->next_nlink()) {
        ...
        fd << setw(ind) << "" << "Pin " << pin << " of " << show_type(obj) << ", dir=" << cur->dir_
           << endl;
    }
}
...
void NetDelaySrc::dump(ostream& o, unsigned ind) const {
    ...
    if (parallel_)
        ...
        o << " full";
    o << " src "
      << "(" << transition_delays_[IVL_PE_01] << "," << transition_delays_[IVL_PE_10] << ","
      << transition_delays_[IVL_PE_0z] << "/" << transition_delays_[IVL_PE_z1] << ","
      << transition_delays_[IVL_PE_1z] << "," << transition_delays_[IVL_PE_z0] << "/"
      << transition_delays_[IVL_PE_0x] << "," << transition_delays_[IVL_PE_x1] << ","
      << transition_delays_[IVL_PE_1x] << "/" << transition_delays_[IVL_PE_x0] << ","
...
ostream& operator<<(ostream& out, const netranges_t& rlist) {
    for (netranges_t::const_iterator cur = rlist.begin(); cur != rlist.end(); ++cur) {
        out << *cur;
    }
    return out;
}
...
void NetNet::dump_net(ostream& o, unsigned ind) const {
    ...
    for (unsigned idx = 0; idx < pin_count(); idx += 1) {
        ...
        o << setw(ind + 4) << "" << "[" << idx << "]: " << nex << " " << nex->name() << endl;
    }
...
void NetNode::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "node: ";
    o << typeid(*this).name() << " #(" << rise_time() << "," << fall_time() << "," << decay_time()
      << ") " << name() << endl;
...
void NetObj::dump_obj_attr(ostream& o, unsigned ind) const {
    ...
    for (idx = 0; idx < attr_cnt(); idx += 1) {
        o << setw(ind) << "" << attr_key(idx) << " = \"" << attr_value(idx) << "\"" << endl;
    }
}

void NetAbs::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "Absolute value (NetAbs): " << name() << " width=" << width()
...
void NetAddSub::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "Adder (NetAddSub): " << name() << " width=" << width()
      << " pin_count=" << pin_count() << endl;
...
void NetArrayDq::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "NetArrayDq: " << name() << " array=" << mem_->name() << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}

void NetCastInt2::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "Cast to int2. (NetCastInt2): " << name() << " width=" << width()
      << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}
...
void NetCastInt4::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "Cast to int4. (NetCastInt4): " << name() << " width=" << width()
      << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}
...
void NetCastReal::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "Cast to real (NetCastReal): " << name() << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}

void NetCLShift::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "Combinatorial shift (NetCLShift): " << name() << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}

void NetCompare::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "LPM_COMPARE (NetCompare " << (get_signed() ? "signed" : "unsigned")
      << "): " << name() << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}
...
void NetPow::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "LPM_POW (NetPow): " << name() << " scope=" << scope_path(scope())
      << " delay=(";
    if (rise_time())
        o << *rise_time() << "," << *fall_time() << "," << *decay_time();
    else
...
void NetMux::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "Multiplexer (NetMux): " << name() << " width=" << width_
      << " swidth=" << swidth_ << " size=" << size_ << " scope=" << scope_path(scope()) << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}

void NetBUFZ::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "NetBUFZ: " << name() << " scope=" << scope_path(scope()) << " delay=("
      << rise_time() << "," << fall_time() << "," << decay_time() << ") width=" << width()
...
void NetFF::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "LPM_FF: " << name() << " scope=" << scope_path(scope());
    if (negedge_)
        o << " negedge";
    else
        o << " posedge";
...
void NetLatch::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "LPM_LATCH: " << name() << " scope=" << scope_path(scope()) << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}

void NetLiteral::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "constant real " << real_ << ": " << name();
    if (rise_time())
        o << " #(" << *rise_time() << "," << *fall_time() << "," << *decay_time() << ")";
    else
        o << " #(.,.,.)";
...
void NetLogic::dump_node(ostream& o, unsigned ind) const {
    ...
    o << " #(" << rise_time() << "," << fall_time() << "," << decay_time() << ") " << name()
      << " scope=" << scope_path(scope()) << endl;
...
void NetPartSelect::dump_node(ostream& o, unsigned ind) const {
    ...
    o << setw(ind) << "" << "NetPartSelect(" << pt << "): " << name();
    if (rise_time())
        o << " #(" << *rise_time() << "," << *fall_time() << "," << *decay_time() << ")";
    else
        o << " #(.,.,.)";
...
void NetSubstitute::dump_node(ostream& fd, unsigned ind) const {
    fd << setw(ind) << "" << "NetSubstitute: " << name();
    if (rise_time())
...
void NetReplicate::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "NetReplicate: " << name() << " wid=" << width_
...
void NetSysFunc::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << def_->name << "(...) -->" << data_type() << " width=" << vector_width()
      << endl;
    dump_node_pins(o, ind + 4);
    dump_obj_attr(o, ind + 4);
}
...
void NetUserFunc::dump_node(ostream& o, unsigned ind) const {
    o << setw(ind) << "" << "USER FUNC: " << scope_path(def_);
    if (rise_time())
        o << " #(" << *rise_time() << "," << *fall_time() << "," << *decay_time() << ")";
    o << endl;
    dump_node_pins(o, ind + 4);
...
void NetTran::dump_node(ostream& o, unsigned ind) const {
    ...
    if (type_ == IVL_SW_TRAN_VP) {
        o << " width=" << vector_width() << " part=" << part_width() << " offset=" << part_offset();
    }
    o << " delay=(";
    if (rise_time())
        o << *rise_time() << "," << *fall_time() << "," << *decay_time();
...
void NetProcTop::dump(ostream& o, unsigned ind) const {
    switch (type_) {
        case IVL_PR_INITIAL:
            o << "initial  /* " << get_fileline() << " in " << scope_path(scope_) << " */" << endl;
            break;
        case IVL_PR_ALWAYS:
            o << "always  /* " << get_fileline() << " in " << scope_path(scope_) << " */" << endl;
            break;
        ...
        case IVL_PR_ALWAYS_LATCH:
            ...
            break;
        case IVL_PR_FINAL:
            o << "final  /* " << get_fileline() << " in " << scope_path(scope_) << " */" << endl;
            break;
    }

    for (unsigned idx = 0; idx < attr_cnt(); idx += 1) {
        o << setw(ind + 2) << "" << "(* " << attr_key(idx) << " = " << attr_value(idx) << " *)"
          << endl;
    }
...
void NetAnalogTop::dump(ostream& o, unsigned ind) const {
    switch (type_) {
        ...
        case IVL_PR_ALWAYS:
            o << "analog /* " << get_fileline() << " in " << scope_path(scope_) << " */" << endl;
            break;
...
void NetScope::dump(ostream& o) const {
    ...
    if (is_interface())
        o << " (interface)";
    o << " " << children_.size() << " children, " << classes_.size() << " classes" << endl;
    if (unit() && !is_unit())
        o << "    in compilation unit " << unit()->basename() << endl;

    for (unsigned idx = 0; idx < attr_cnt(); idx += 1)
        o << "    (* " << attr_key(idx) << " = " << attr_value(idx) << " *)" << endl;

    o << "    timescale = 10e" << time_unit() << " / 10e" << time_precision() << endl;
    ...
    /* Dump the saved defparam assignments here. */
    {
        ...
        for (pp = defparams.begin(); pp != defparams.end(); ++pp) {
            o << "    defparam " << (*pp).first << " = " << *(*pp).second << ";" << endl;
        }
    }

    {
        ...
        for (pp = defparams_later.begin(); pp != defparams_later.end(); ++pp) {
            o << "    defparam(later) " << pp->first << " = " << *(pp->second) << ";" << endl;
        }
    }
    ...
    for (NetEvent* cur = events_; cur; cur = cur->snext_) {
        o << "    event " << cur->name() << "; nprobe=" << cur->nprobe()
          << " scope=" << scope_path(cur->scope()) << " // " << cur->get_fileline() << endl;
    }
    ...
    /* Dump any sub-scopes. */
    for (map<hname_t, NetScope*>::const_iterator cur = children_.begin(); cur != children_.end();
         ++cur)
        cur->second->dump(o);
