...
class netvector_t : public ivl_type_s {
  public:
    ...
    explicit netvector_t(ivl_variable_type_t type, long msb, long lsb, bool signed_flag = false);
    ...
    // handled as vectors.
    inline void set_signed(bool flag) {
        signed_ = flag;
    }
    inline bool get_signed(void) const override {
        return signed_;
    }

    inline void set_isint(bool flag) {
        isint_ = flag;
    }
    inline bool get_isint(void) const {
        return isint_;
    }

    inline bool get_scalar(void) const override {
        return packed_dims_.empty();
    }

    void set_implicit(bool implicit) {
        implicit_ = implicit;
    }
    bool get_implicit() const {
        return implicit_;
    }
