...
void pform_mc_translate_on(bool flag) {
    pform_mc_translate_flag = flag;
}
...
void parm_to_defparam_list(const string& param) {
    ...
    if (*value == '"') {
        ...
        if (*buf_ptr == '\0')  // String end without '"'
            cerr << "<command line>: error: missing close quote of string for defparam: " << name
                 << endl;
        else if (*(buf_ptr + 1) != 0) {  // '"' appears within string with no escape
            cerr << buf_ptr << endl;
            cerr << "<command line>: error: \'\"\' appears within string value for defparam: "
...
static void add_local_symbol(LexicalScope* scope, perm_string name, PNamedItem* item) {
    ...
    map<perm_string, PNamedItem*>::const_iterator cur_sym = scope->local_symbols.find(name);
    ...
    map<perm_string, PPackage*>::const_iterator cur_pkg = scope->explicit_imports.find(name);
...
void pform_set_scope_timescale(const struct vlltype& loc) {
    ...
    if (warn_timescale && used_global_timescale && (strcmp(pform_timescale_file, loc.text) != 0)) {
        cerr << loc.get_fileline() << ": warning: "
             << "timescale for " << scope->pscope_name() << " inherited from another file." << endl;
...
PPackage* pform_push_package_scope(const struct vlltype& loc,
                                   perm_string name,
...
PTask* pform_push_task_scope(const struct vlltype& loc,
                             const char* name,
                             LexicalScope::lifetime_t lifetime) {
...
PBlock* pform_push_block_scope(const struct vlltype& loc, const char* name, PBlock::BL_TYPE bt) {
    ...
    if (name)
        add_local_symbol(lexical_scope, block_name, block);
    lexical_scope = block;
...
PTrigger* pform_new_trigger(const struct vlltype& loc,
                            PPackage* pkg,
...
void pform_put_enum_type_in_scope(enum_type_t* enum_set) {
    if (std::find(lexical_scope->enum_sets.begin(), lexical_scope->enum_sets.end(), enum_set) !=
...
void pform_forward_typedef(const struct vlltype& loc,
                           perm_string name,
                           enum typedef_t::basic_type basic_type) {
    typedef_t* td = pform_get_typedef(loc, name);

    if (!td->set_basic_type(basic_type)) {
        cout << loc << " error: Incompatible basic type `" << basic_type << "` for `" << name
             << "`. Previously declared in this scope as `" << td->get_basic_type() << "` at "
             << td->get_fileline() << "." << endl;
        error_count++;
    }
}

void pform_set_typedef(const struct vlltype& loc,
                       perm_string name,
    ...
    if (!td->set_data_type(data_type)) {
        cerr << loc << " error: Type identifier `" << name
             ...
             << td->get_data_type()->get_fileline() << "." << endl;
        error_count++;
        delete data_type;
    }
}
...
void pform_make_var(const struct vlltype& loc,
    ...
    pform_makewire(loc, 0, str, assign_list, NetNet::REG, data_type, attr, is_const);
}

void pform_make_foreach_declarations(const struct vlltype& loc, std::list<perm_string>* loop_vars) {
    list<decl_assignment_t*> assign_list;
    for (list<perm_string>::const_iterator cur = loop_vars->begin(); cur != loop_vars->end();
         ++cur) {
        if (cur->nil())
            continue;
...
void pform_set_default_nettype(NetNet::Type type, const char* file, unsigned lineno) {
    ...
    if (!pform_cur_module.empty()) {
        ...
        cerr << file << ":" << lineno << ":      : "
             << "module " << pform_cur_module.back()->mod_name() << " starts on line "
 ...
 */
void pform_set_timescale(int unit, int prec, const char* file, unsigned lineno) {
    assert(unit >= prec);
...
bool get_time_unit(const char* cp, int& unit) {
    ...
    if (strchr(cp, '_')) {
        VLerror(yylloc,
                "error: Invalid timeunit constant ('_' is not "
                "supported).");
        return false;
    }
...
static bool get_time_unit_prec(const char* cp, int& res, bool is_unit) {
    ...
    if (strchr(cp, '_')) {
        if (is_unit) {
            VLerror(yylloc,
                    "error: Invalid timeunit constant ('_' "
                    "is not supported).");
        } else {
            VLerror(yylloc,
                    "error: Invalid timeprecision constant ('_' "
                    "is not supported).");
        }
        return true;
    }
    ...
    if (*cp != '1') {
        if (is_unit) {
            VLerror(yylloc,
                    "error: Invalid timeunit constant "
                    "(1st digit).");
        } else {
            VLerror(yylloc,
                    "error: Invalid timeprecision constant "
                    "(1st digit).");
        }
        return true;
    }
...
void pform_set_timeunit(const char* txt, bool initial_decl) {
    ...
    if (get_time_unit_prec(txt, val, true))
        return;
...
void pform_set_timeprec(const char* txt, bool initial_decl) {
    ...
    if (get_time_unit_prec(txt, val, false))
        return;
...
verinum* pform_verinum_with_size(verinum* siz, verinum* val, const char* file, unsigned lineno) {
...
void pform_startmodule(const struct vlltype& loc,
                       const char* name,
                       bool program_block,
                       bool is_interface,
                       LexicalScope::lifetime_t lifetime,
    ...
    if (!pform_cur_module.empty() && !gn_system_verilog()) {
        cerr << loc << ": error: Module definition " << name << " cannot nest into module "
    ...
    if (gn_system_verilog() && !pform_cur_module.empty()) {
        ...
        if (pform_cur_module.front()->is_interface && !(program_block || is_interface)) {
            cerr << loc
 ...
 */
Module::port_t* pform_module_port_reference(const struct vlltype& loc, perm_string name) {
...
void pform_endmodule(const char* name, bool inside_celldefine, Module::UCDriveType uc_drive_def) {
    ...
    if (strcmp(name, mod_name) != 0) {
        while (!pform_cur_module.empty()) {
            ...
            pform_cur_module.pop_front();
            ostringstream msg;
            msg << "error: Module " << mod_name << " was nested within " << tmp_name
                << " but broken.";
            VLerror(msg.str().c_str());
    ...
    map<perm_string, Module*>& use_module_map =
        (pform_cur_module.empty()) ? pform_modules : pform_cur_module.front()->nested_modules;

    map<perm_string, Module*>::const_iterator test = use_module_map.find(mod_name);

    if (test != use_module_map.end()) {
        ostringstream msg;
        msg << "error: Module " << name
            << " was already declared here: " << test->second->get_fileline() << endl;
        VLerror(msg.str().c_str());
    } else {
...
void pform_start_generate_for(const struct vlltype& li,
    ...
    if (strcmp(ident1, ident2)) {
        cerr << li << ": error: "
             << "A generate \"loop\" requires the initialization genvar (" << ident1
             << ") to match the iteration genvar (" << ident2 << ")." << endl;
        error_count += 1;
    }
...
void pform_start_generate_nblock(const struct vlltype& li, char* name) {
    ...
    add_local_symbol(
        pform_cur_generate->parent_scope(), pform_cur_generate->scope_name, pform_cur_generate);
}
...
static void process_udp_table(PUdp* udp, list<string>* table, const struct vlltype& loc) {
    ...
    {
        unsigned idx = 0;
        for (list<string>::iterator cur = table->begin(); cur != table->end(); ++cur, idx += 1) {
            string tmp = *cur;
            ...
            if (tmp.find(':') != (udp->ports.size() - 1)) {
                cerr << loc << ": error: "
                     ...
                     << ") does not match the number of input table entries (" << tmp.find(':')
                     << ") in primitive \"" << udp->name_ << "\"." << endl;
                error_count += 1;
                break;
            }
...
void pform_make_udp(const struct vlltype& loc,
                    perm_string name,
                    list<pform_ident_t>* parms,
                    vector<PWire*>* decl,
                    list<string>* table,
                    Statement* init_expr) {
    ...
    */
    ...
    do {
        if (pins[0] == 0) {
            cerr << loc << ": error: "
                 << "Output port of primitive " << name << " missing output declaration." << endl;
            cerr << loc << ":      : "
    ...
    for (unsigned idx = 1; idx < pins.size(); idx += 1) {
        ...
        if (pins[idx]->get_wire_type() == NetNet::REG) {
            cerr << loc << ": error: "
                 << "Port " << (idx + 1) << " of primitive " << name << " is an input port"
...
void pform_make_udp(const struct vlltype& loc,
    ...
    pins[0] = new PWire(out_name.first,
                        out_name.second,
    ...
    /* Make the PWire objects for the input ports. */
    {
        ...
        for (cur = parms->begin(), idx = 1; cur != parms->end(); idx += 1, ++cur) {
            ...
            pins[idx] = new PWire(cur->first, cur->second, NetNet::WIRE, NetNet::PINPUT);
    ...
    if (pform_primitives[name]) {
        ostringstream msg;
        msg << "error: Primitive " << name
            << " was already declared here: " << pform_primitives[name]->get_fileline() << endl;
        ...
        if (table) {
            ...
            ostringstream msg;
            msg << "error: Invalid table for UDP primitive " << name << "." << endl;
            // Some compilers warn if there is just a single C string.
            VLerror(loc, msg.str().c_str(), "");
        }
    }
...
static void pform_makegate(PGBuiltin::Type type,
    ...
    if (info.parms_by_name) {
        cerr << info.get_fileline() << ": error: Gates do not have port names." << endl;
        error_count += 1;
        return;
    }

    if (info.parms) {
        for (list<PExpr*>::iterator cur = info.parms->begin(); cur != info.parms->end(); ++cur) {
    ...
    if (pform_cur_generate) {
        if (dev_name != "")
            add_local_symbol(pform_cur_generate, dev_name, cur);
...
static void pform_make_modgate(perm_string type,
                               ...
                               const LineInfo& li,
                               std::list<named_pexpr_t>* attr) {
    for (list<PExpr*>::iterator idx = wires->begin(); idx != wires->end(); ++idx) {
    ...
    if (overrides && overrides->by_name) {
        ...
        std::copy(overrides->by_name->begin(), overrides->by_name->end(), byname);
...
void pform_make_modgates(const struct vlltype& loc,
    ...
    if (pform_cur_module.empty()) {
        cerr << loc << ": internal error: "
             << "Module instantiations outside module scope are not possible." << endl;
        error_count += 1;
        delete gates;
        return;
    }
    ...
    for (unsigned idx = 0; idx < gates->size(); idx += 1) {
        ...
        if (cur.parms_by_name) {
            pform_make_modgate(type, cur_name, overrides, cur.parms_by_name, cur.ranges, cur, attr);
            ...
            pform_make_modgate(type, cur_name, overrides, cur.parms, cur.ranges, cur, attr);

        } else {
            list<PExpr*>* wires = new list<PExpr*>;
            pform_make_modgate(type, cur_name, overrides, wires, cur.ranges, cur, attr);
        }
    }
...
static PGAssign*
pform_make_pgassign(PExpr* lval, PExpr* rval, list<PExpr*>* del, struct str_pair_t str) {
 ...
 */
void pform_make_var_init(const struct vlltype& li, const pform_ident_t& name, PExpr* expr) {
...
void pform_module_define_port(const struct vlltype& li,
                              ...
                              data_type_t* vtype,
                              list<named_pexpr_t>* attr) {
    for (list<pform_port_t>::iterator cur = ports->begin(); cur != ports->end(); ++cur) {
        data_type_t* use_type = vtype;

        pform_module_define_port(li, cur->name, port_kind, type, use_type, cur->udims, attr, true);
 ...
 */
PWire* pform_makewire(const vlltype& li,
                      const pform_ident_t& name,
                      NetNet::Type type,
...
void pform_makewire(const struct vlltype& li,
    ...
    for (list<decl_assignment_t*>::iterator cur = assign_list->begin(); cur != assign_list->end();
         ++cur) {
    ...
    while (!assign_list->empty()) {
        ...
        if (PExpr* expr = first->expr.release()) {
            if (type == NetNet::REG || type == NetNet::IMPLICIT_REG) {
                ...
                PEIdent* lval = new PEIdent(first->name.first, first->name.second);
                FILE_NAME(lval, li);
...
vector<pform_tf_port_t>* pform_make_task_ports(const struct vlltype& loc,
    ...
    for (list<pform_port_t>::iterator cur = ports->begin(); cur != ports->end(); ++cur) {
        PWire* curw = pform_get_or_make_wire(loc, cur->name, NetNet::IMPLICIT_REG, pt, rt);
        if (rt == SR_BOTH)
...
PExpr* pform_genvar_compressed(const struct vlltype& loc, const char* name, char op, PExpr* rval) {
...
LexicalScope::range_t* pform_parameter_value_range(
    bool exclude_flag, bool low_open, PExpr* low_expr, bool hig_open, PExpr* hig_expr) {
...
static void pform_set_type_parameter(const struct vlltype& loc,
                                     perm_string name,
...
void pform_set_parameter(const struct vlltype& loc,
                         ...
                         bool is_type,
                         data_type_t* data_type,
                         const list<pform_range_t>* udims,
                         PExpr* expr,
                         LexicalScope::range_t* value_range) {
    LexicalScope* scope = lexical_scope;
    if (is_compilation_unit(scope) && !gn_system_verilog()) {
        VLerror(loc,
                "error: %s declarations must be contained within a module.",
    ...
    if (expr == 0) {
        if (is_local) {
            ...
            VLerror(loc,
                    "error: parameter declared outside parameter "
    ...
    bool in_module = dynamic_cast<Module*>(scope) && scope == pform_cur_module.front();

    if (!pform_in_parameter_port_list && in_module && scope->has_parameter_port_list)
        overridable = false;
...
void pform_set_specparam(const struct vlltype& loc,
                         perm_string name,
...
void pform_make_let(const struct vlltype& loc,
    ...
    cerr << loc.get_fileline() << ": sorry: let declarations (" << name
...
extern PRecRem* pform_make_recrem(const struct vlltype& li,
    ...
    PRecRem* recrem = new PRecRem(reference_event,
                                  data_event,
                                  setup_limit,
                                  hold_limit,
                                  args->notifier,
...
extern PSetupHold* pform_make_setuphold(const struct vlltype& li,
    ...
    PSetupHold* setuphold = new PSetupHold(reference_event,
                                           data_event,
                                           setup_limit,
                                           hold_limit,
                                           args->notifier,
...
void pform_set_port_type(const struct vlltype& li,
    ...
    for (list<pform_port_t>::iterator cur = ports->begin(); cur != ports->end(); ++cur) {
        PWire* wire = pform_get_or_make_wire(li, cur->name, NetNet::IMPLICIT, pt, SR_PORT);
 ...
 */
void pform_set_data_type(const struct vlltype& li,
                         data_type_t* data_type,
    ...
    for (std::vector<PWire*>::iterator it = wires->begin(); it != wires->end(); ++it) {
        PWire* wire = *it;
...
vector<PWire*>* pform_make_udp_input_ports(list<pform_ident_t>* names) {
    ...
    unsigned idx = 0;
    for (list<pform_ident_t>::iterator cur = names->begin(); cur != names->end(); ++cur) {
        PWire* pp = new PWire(cur->first, cur->second, NetNet::IMPLICIT, NetNet::PINPUT);
        (*out)[idx] = pp;
        idx += 1;
    }
...
PProcess* pform_make_behavior(ivl_process_type_t type, Statement* st, list<named_pexpr_t>* attr) {
    ...
    if (pform_cur_module.front()->program_block &&
        ((type == IVL_PR_ALWAYS) || (type == IVL_PR_ALWAYS_COMB) || (type == IVL_PR_ALWAYS_FF) ||
         (type == IVL_PR_ALWAYS_LATCH))) {
...
void pform_add_modport_port(const struct vlltype& loc,
    ...
    if (pform_cur_modport->simple_ports.find(name) != pform_cur_modport->simple_ports.end()) {
        cerr << loc << ": error: duplicate declaration of port '" << name << "' in modport list '"
...
void pform_check_net_data_type(const struct vlltype& loc,
                               NetNet::Type net_type,
...
int pform_parse(const char* path) {
    ...
    if (strcmp(path, "-") == 0) {
        ...
        char* cmdline = static_cast<char*>(malloc(strlen(ivlpp_string) + strlen(path) + 4));
