...
ivl_scope_s::ivl_scope_s() : func_type(IVL_VT_NO_TYPE) {
    func_signed = false;
    func_width = 0;
}
 ...
 * nexus.
 */
static ivl_nexus_ptr_t nexus_log_add(ivl_nexus_t nex, ivl_net_logic_t net, unsigned pin) {
    unsigned top = nex->ptrs_.size();
    nex->ptrs_.resize(top + 1);
...
static void nexus_con_add(
    ivl_nexus_t nex, ivl_net_const_t net, unsigned pin, ivl_drive_t drive0, ivl_drive_t drive1) {
    unsigned top = nex->ptrs_.size();
    nex->ptrs_.resize(top + 1);
...
static void nexus_lpm_add(
    ivl_nexus_t nex, ivl_lpm_t net, unsigned pin, ivl_drive_t drive0, ivl_drive_t drive1) {
    unsigned top = nex->ptrs_.size();
    nex->ptrs_.resize(top + 1);
...
void scope_add_event(ivl_scope_t scope, ivl_event_t net) {
    if (scope->nevent_ == 0) {
        ...
        scope->nevent_ += 1;
        scope->event_ =
            static_cast<ivl_event_t*>(realloc(scope->event_, scope->nevent_ * sizeof(ivl_event_t)));
...
static void scope_add_lpm(ivl_scope_t scope, ivl_lpm_t net) {
    if (scope->nlpm_ == 0) {
        ...
        scope->nlpm_ += 1;
        scope->lpm_ =
            static_cast<ivl_lpm_t*>(realloc(scope->lpm_, scope->nlpm_ * sizeof(ivl_lpm_t)));
...
ivl_parameter_t dll_target::scope_find_param(ivl_scope_t scope, const char* name) {
    unsigned idx = 0;
...
void dll_target::make_scope_parameters(ivl_scope_t scop, const NetScope* net) {
    ...
    for (pit_t cur_pit = net->parameters.begin(); cur_pit != net->parameters.end(); ++cur_pit) {
        assert(idx < scop->param.size());
        ivl_parameter_t cur_par = &scop->param[idx];
        cur_par->basename = cur_pit->first;
        cur_par->local = cur_pit->second.local_flag || !cur_pit->second.overridable;
        cur_par->is_type = cur_pit->second.type_flag;

        if (cur_pit->second.ivl_type == 0) {
            cerr << "?:?: internal error: "
                 << "No type for parameter " << cur_pit->first << " in scope " << net->fullname()
                 << "?" << endl;
        }
...
bool dll_target::start_design(const Design* des) {
    ...
    for (list<NetScope*>::const_iterator cur = scope_list.begin(); cur != scope_list.end(); ++cur) {
        add_root(*cur);
    }
...
void dll_target::switch_attributes(struct ivl_switch_s* obj, const NetNode* net) {
    obj->nattr = net->attr_cnt();
    obj->attr = fill_in_attributes(net);
}

void dll_target::logic_attributes(struct ivl_net_logic_s* obj, const NetNode* net) {
    obj->nattr = net->attr_cnt();
    obj->attr = fill_in_attributes(net);
}
...
void dll_target::make_logic_delays_(struct ivl_net_logic_s* obj, const NetObj* net) {
    make_delays_(obj->delay, net);
}

void dll_target::make_switch_delays_(struct ivl_switch_s* obj, const NetObj* net) {
    make_delays_(obj->delay, net);
}
...
void dll_target::make_const_delays_(struct ivl_net_const_s* obj, const NetObj* net) {
    make_delays_(obj->delay, net);
}
...
ivl_event_t dll_target::make_lpm_trigger(const NetEvWait* net) {
    ...
    if (net) {
        ...
        for (unsigned bit = 0; bit < pr->pin_count(); bit += 1) {
            ivl_nexus_t nex = static_cast<ivl_nexus_t>(pr->pin(bit).nexus()->t_cookie());
            assert(nex);
...
bool dll_target::lpm_arith1_(ivl_lpm_type_t lpm_type,
                             unsigned width,
                             bool signed_flag,
                             const NetNode* net) {
    ivl_lpm_t obj = new struct ivl_lpm_s;
...
void dll_target::lpm_mux(const NetMux* net) {
    ...
    nexus_lpm_add(obj->u_.mux.q, obj, 0, net->pin_Result().drive0(), net->pin_Result().drive1());
    ...
    for (unsigned sdx = 0; sdx < selects; sdx += 1) {
        ...
        if (tmp == 0) {
            cerr << net->get_fileline() << ": internal error: "
                 << "dll_target::lpm_mux: "
                 << "Missing data port " << sdx << " of mux " << obj->name << "." << endl;
        }
        ivl_assert(*net, tmp);
...
void dll_target::signal(const NetNet* net) {
...

    {
        ...
        for (cur = net->packed_dims().begin(), idx = 0; cur != net->packed_dims().end();
             ++cur, idx += 1) {
    ...
    obj->forced_net_ = (net->type() != NetNet::REG) && (net->peek_lref() > 0) ? 1 : 0;
        ...
        for (unsigned pin = 0; pin < src->src_count(); pin += 1) {
            ...
            if (!nex->t_cookie()) {
                cerr << src->get_fileline() << ": internal error: "
                     << "No signal connected to pin " << pin << " of delay path to " << net->name()
                     << "." << endl;
            }
            assert(nex->t_cookie());
...
void dll_target::test_version(const char* target_name) {
    ...
    if (dll_ == 0) {
        cout << "\n\nUnable to load " << target_name << " for version details." << endl;
        return;
    }

    target_query_f targ_query =
        reinterpret_cast<target_query_f>(ivl_dlsym(dll_, LU "target_query" TU));
    if (targ_query == 0) {
