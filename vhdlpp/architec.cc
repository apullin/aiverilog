...
GenerateStatement::GenerateStatement(perm_string gname, std::list<Architecture::Statement*>& s)
    : name_(gname) {
    statements_.splice(statements_.end(), s);
}
...
ForGenerate::ForGenerate(perm_string gname,
    ...
    : GenerateStatement(gname, s), genvar_(genvar), lsb_(rang->lsb()), msb_(rang->msb()) {}

ForGenerate::~ForGenerate() {}

IfGenerate::IfGenerate(perm_string gname, Expression* cond, std::list<Architecture::Statement*>& s)
    : GenerateStatement(gname, s), cond_(cond) {}

IfGenerate::~IfGenerate() {}

SignalAssignment::SignalAssignment(ExpName* name, list<Expression*>& rv) : lval_(name) {
    rval_.splice(rval_.end(), rv);
}

SignalAssignment::SignalAssignment(ExpName* name, Expression* rv) : lval_(name) {
    rval_.push_back(rv);
}
...
ComponentInstantiation::ComponentInstantiation(perm_string i,
    ...
    while (parms && !parms->empty()) {
        ...
        if (!rc.second) {
            cerr << "?:?: error: Duplicate map of generic " << cur->name() << " ignored." << endl;
            parse_errors += 1;
        }
    }
    ...
    while (ports && !ports->empty()) {
        ...
        if (!rc.second) {
            cerr << "?:?: error: Duplicate map of port " << cur->name() << " ignored." << endl;
            parse_errors += 1;
        }
    }
}
...
ComponentInstantiation::~ComponentInstantiation() {
    ...
    for (map<perm_string, Expression*>::iterator it = port_map_.begin(); it != port_map_.end();
         ++it) {
        delete it->second;
    }
}
