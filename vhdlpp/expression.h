...
class ExpAggregate : public Expression {
  ...
  private:
    ...
    int
    emit_array_(std::ostream& out, Entity* ent, ScopeBase* scope, const VTypeArray* ltype) const;
    int
    emit_record_(std::ostream& out, Entity* ent, ScopeBase* scope, const VTypeRecord* ltype) const;
...
class ExpCharacter : public Expression {
  ...
  private:
    int emit_primitive_bit_(std::ostream& out,
                            Entity* ent,
                            ScopeBase* scope,
...
class ExpConditional : public Expression {
  ...
  public:
    ExpConditional(Expression* cond, std::list<Expression*>* tru, std::list<case_t*>* options);
...
class ExpName : public Expression {
  ...
  private:
    ...
    const VType*
    elaborate_adjust_type_with_range_(Entity* ent, ScopeBase* scope, const VType* type);
    ...
    bool try_workarounds_(std::ostream& out,
                          Entity* ent,
                          ScopeBase* scope,
                          std::list<index_t*>& indices,
                          int& data_size) const;

    bool check_const_array_workaround_(const VTypeArray* arr,
                                       ScopeBase* scope,
                                       std::list<index_t*>& indices,
                                       int& data_size) const;

    bool check_const_record_workaround_(const VTypeRecord* rec,
                                        ScopeBase* scope,
                                        std::list<index_t*>& indices,
                                        int& data_size) const;

    int emit_workaround_(std::ostream& out,
                         Entity* ent,
                         ScopeBase* scope,
...
class ExpString : public Expression {
  ...
  private:
    int
    emit_as_array_(std::ostream& out, Entity* ent, ScopeBase* scope, const VTypeArray* arr) const;
...
class ExpUMinus : public ExpUnary {
  public:
    ...
    Expression* clone() const override {
        return new ExpUMinus(peek_operand()->clone());
    }
