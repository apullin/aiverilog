...
int Scope::emit_signals(ostream& out, Entity* entity, ScopeBase* scope) {
    ...
    for (map<perm_string, Signal*>::iterator cur = new_signals_.begin(); cur != new_signals_.end();
         ++cur) {
...
int Architecture::emit(ostream& out, Entity* entity) {
    ...
    for (map<perm_string, const VType*>::iterator cur = use_types_.begin(); cur != use_types_.end();
         ++cur) {
    ...
    for (map<perm_string, const VType*>::iterator cur = cur_types_.begin(); cur != cur_types_.end();
         ++cur) {
        if (const VTypeDef* def = dynamic_cast<const VTypeDef*>(cur->second))
            errors += def->emit_typedef(out, typedef_ctx);
    }
...
int CondSignalAssignment::emit(ostream& out, Entity* ent, Architecture* arc) {
    ...
    for (list<ExpConditional::case_t*>::iterator it = options_.begin(); it != options_.end();
         ++it) {
    ...
    for (list<const ExpName*>::const_iterator it = sens_list_.begin(); it != sens_list_.end();
         ++it) {
        if (first)
            first = false;
        else
...
int ComponentInstantiation::emit(ostream& out, Entity* ent, Architecture* arc) {
    ...
    if (ComponentBase* comp = arc->find_component(cname_)) {
        ...
        if (generics.size() != generic_map_.size())
            ...
            for (vector<InterfacePort*>::const_iterator it = generics.begin(); it != generics.end();
                 ++it) {
                if (!(*it)->expr && generic_map_.count((*it)->name) == 0) {
                    cerr << get_fileline() << ": generic " << (*it)->name << "value is not defined"
                         << endl;
                    ++errors;
                }
            }
    ...
    out << " \\" << iname_ << " (";
    comma = "";
    for (map<perm_string, Expression*>::iterator cur = port_map_.begin(); cur != port_map_.end();
         ++cur) {
...
int ForGenerate::emit(ostream& out, Entity* ent, Architecture* arc) {
    ...
    out << "; \\" << get_name() << ":" << genvar_ << " = \\" << get_name() << ":" << genvar_
        << " + 1)"
        << " begin : \\" << get_name() << endl;
...
int StatementList::emit(ostream& out, Entity* ent, ScopeBase* scope) {
    ...
    for (std::list<SequentialStmt*>::iterator it = statements_.begin(); it != statements_.end();
         ++it) {
