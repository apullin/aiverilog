...
int Architecture::elaborate(Entity* entity) {
    ...
    for (map<perm_string, Signal*>::iterator cur = old_signals_.begin(); cur != old_signals_.end();
         ++cur) {
    ...
    if (errors > 0) {
        cerr << errors << " errors in " << name_ << " architecture of " << entity->get_name() << "."
             << endl;
    }
...
int ComponentInstantiation::elaborate(Entity* ent, Architecture* arc) {
    ...
    for (map<perm_string, Expression*>::const_iterator cur = generic_map_.begin();
        ...
        if (iparm == 0) {
            cerr << get_fileline() << ": warning: No generic " << cur->first << " in component "
                 << cname_ << "." << endl;
            continue;
        }
    ...
    for (map<perm_string, Expression*>::const_iterator cur = port_map_.begin();
        ...
        if (iport == 0) {
            cerr << get_fileline() << ": error: No port " << cur->first << " in component "
                 << cname_ << "." << endl;
            errors += 1;
            continue;
        }
...
int SignalAssignment::elaborate(Entity* ent, Architecture* arc) {
    ...
    if (lval_type == 0) {
        if (errors == 0) {
            errors += 1;
            cerr << get_fileline() << ": error: Unable to calculate type for l-value expression."
                 << endl;
        }
        return errors;
    }

    for (list<Expression*>::iterator cur = rval_.begin(); cur != rval_.end(); ++cur) {
...
int CondSignalAssignment::elaborate(Entity* ent, Architecture* arc) {
    ...
    struct name_extractor_t : public ExprVisitor {
        explicit name_extractor_t(list<const ExpName*>& name_list) : name_list_(name_list) {}
