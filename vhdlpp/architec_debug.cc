...
void Architecture::dump(ostream& out, perm_string of_entity, int indent) const {
    out << setw(indent) << "" << "architecture " << name_ << " of entity " << of_entity
...
void ComponentInstantiation::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "Component Instantiation "
        << "instance=" << iname_ << " of component=" << cname_ << ", file=" << get_fileline()
        << endl;
...
void ForGenerate::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "for " << genvar_ << " in" << endl;
...
void SignalAssignment::dump(ostream& out, int indent) const {
    ...
    for (list<Expression*>::const_iterator cur = rval_.begin(); cur != rval_.end(); ++cur) {
...
void CondSignalAssignment::dump(ostream& out, int indent) const {
    ...
    for (list<ExpConditional::case_t*>::const_iterator it = options_.begin(); it != options_.end();
         ++it) {
...
void StatementList::dump(ostream& out, int indent) const {
    ...
    for (list<SequentialStmt*>::const_iterator cur = statements_.begin(); cur != statements_.end();
         ++cur) {
...
void ProcessStatement::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "ProcessStatement name_=" << iname_ << " file=" << get_fileline()
        << endl;
