...
int SubprogramHeader::emit_package(ostream& fd) const {
    ...
    for (list<InterfacePort*>::const_iterator cur = ports_->begin(); cur != ports_->end(); ++cur) {
        if (cur != ports_->begin())
            fd << ", ";
...
void emit_subprogram_sig(ostream& out, perm_string name, const list<const VType*>& arg_types) {
    out << name << "(";
    bool first = true;
    for (list<const VType*>::const_iterator it = arg_types.begin(); it != arg_types.end(); ++it) {
        if (first)
            first = false;
        else
            out << ", ";
