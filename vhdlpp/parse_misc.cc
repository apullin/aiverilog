...
void bind_architecture_to_entity(const char* ename, Architecture* arch) {
    ...
    std::map<perm_string, Entity*>::const_iterator idx = design_entities.find(ekey);
    if (idx == design_entities.end()) {
        cerr << arch->get_fileline() << ": error: No entity " << ekey << " for architecture "
             << arch->get_name() << "." << endl;
        parse_errors += 1;
        return;
    }
...
static const VType* calculate_subtype_array(const YYLTYPE& loc,
    ...
    if (base_array) {
        ...
        VTypeArray* subtype = new VTypeArray(element, range, base_array->signed_vector());
...
const VType* calculate_subtype_array(const YYLTYPE& loc,
    ...
    if (ranges->size() == 1) {
        ...
        return calculate_subtype_array(loc,
                                       ...
                                       scope,
                                       lef,
                                       (tmpr->direction() == ExpRange::DOWNTO ? true : false),
                                       rig);
    }
...
const VType* calculate_subtype_range(const YYLTYPE& loc,
                                     const char* base_name,
                                     ScopeBase* scope,
