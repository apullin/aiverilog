...
const char COPYRIGHT[] = "Copyright (c) 2011-2025 Stephen Williams (steve@icarus.com)\n"
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, "D:L:vVw:")) != EOF)
        switch (opt) {
            ...
            case 'v':
                fprintf(stderr,
                        "Icarus Verilog VHDL Parse version " VERSION " (" VERSION_TAG ")\n\n");
                fprintf(stderr, "%s\n\n", COPYRIGHT);
                fputs(NOTICE, stderr);
                verbose_flag = true;
                break;
    ...
    if ((mkdir(work_path, 0777)) < 0) {
        if (errno != EEXIST) {
            fprintf(stderr,
                    "Icarus Verilog VHDL unable to create work directory %s, errno=%d\n",
                    work_path,
                    errno);
            return -1;
        }
    ...
    for (int idx = optind; idx < argc; idx += 1) {
        ...
        if (parse_sorrys > 0) {
            fprintf(stderr,
                    "Encountered %d unsupported constructs parsing %s\n",
                    parse_sorrys,
                    argv[idx]);
        }
