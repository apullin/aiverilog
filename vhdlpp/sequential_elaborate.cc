...
int SignalSeqAssignment::elaborate(Entity* ent, ScopeBase* scope) {
    ...
    for (list<Expression*>::iterator cur = waveform_.begin(); cur != waveform_.end(); ++cur) {
...
int ProcedureCall::elaborate(Entity* ent, ScopeBase* scope) {
    ...
    if (param_list_) {
        for (list<named_expr_t*>::iterator it = param_list_->begin(); it != param_list_->end();
             ++it) {
    ...
    if (param_list_) {
        size_t idx = 0;
        for (list<named_expr_t*>::iterator cur = param_list_->begin(); cur != param_list_->end();
             ++cur) {
...
int WaitStmt::elaborate(Entity* ent, ScopeBase* scope) {
    if (type_ == UNTIL) {
        struct fill_sens_list_t : public ExprVisitor {
            explicit fill_sens_list_t(set<ExpName*>& sig_list) : sig_list_(sig_list) {};
