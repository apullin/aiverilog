...
void ComponentBase::dump_generics(ostream& out, int indent) const {
    if (parms_.empty()) {
        ...
        for (vector<InterfacePort*>::const_iterator cur = parms_.begin(); cur != parms_.end();
            ...
            out << setw(indent + 2) << "" << item->name << " : " << item->mode << ", type=";
            if (item->type)
                item->type->show(out);
            else
                out << "<nil>";
...
void Entity::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "entity " << get_name() << " file=" << get_fileline() << endl;
    dump_ports(out, indent + 2);

    for (map<perm_string, Architecture*>::const_iterator cur = arch_.begin(); cur != arch_.end();
         ++cur) {
...
void ExpTypeAttribute::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "Attribute (type-related) " << name_ << " at " << get_fileline()
        << endl;
    base_->show(out);
}

void ExpObjAttribute::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "Attribute (object-related) " << name_ << " at " << get_fileline()
        << endl;
...
void ExpBitstring::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "Bit string " << value_.size() << "b\"";
...
void ExpConditional::dump(ostream& out, int indent) const {
    ...
    for (list<case_t*>::const_iterator cur = options_.begin(); cur != options_.end(); ++cur) {
        (*cur)->dump(out, indent);
    }
}
...
void ExpConditional::case_t::dump(ostream& out, int indent) const {
    ...
    for (list<Expression*>::const_iterator cur = true_clause_.begin(); cur != true_clause_.end();
         ++cur) {
...
void ExpFunc::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "function " << name_ << " has " << argv_.size()
        << " arguments:" << endl;
...
void ExpInteger::dump(ostream& out, int indent) const {
    out << setw(indent) << "" << "Integer " << value_ << " at " << get_fileline() << endl;
}
...
void ExpLogical::dump(ostream& out, int indent) const {
    ...
    out << setw(indent) << "" << "Logical " << fun_name << " at " << get_fileline() << endl;
