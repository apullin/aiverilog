...
class SubprogramToInteger : public SubprogramStdHeader {
  public:
    ...
    int emit_name(const std::vector<Expression*>& argv,
                  std::ostream& out,
                  Entity* ent,
                  ScopeBase* scope) const override {
...
class SubprogramSizeCast : public SubprogramStdHeader {
  public:
    ...
    int
    emit_name(const std::vector<Expression*>&, std::ostream&, Entity*, ScopeBase*) const override {
        return 0;
    }
...
void preload_std_funcs(void) {
    ...
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("integer"), perm_string::literal("int'"), args, &primitive_INTEGER));
    ...
    */
    ...
    register_std_subprogram(new SubprogramSizeCast(
        perm_string::literal("resize"), &primitive_UNSIGNED, &primitive_UNSIGNED));
    ...
    args->push_back(new InterfacePort(&primitive_STDLOGIC_VECTOR));
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("to_bitvector"), empty_perm_string, args, &primitive_BIT_VECTOR));
    ...
    args->push_back(new InterfacePort(&primitive_STDLOGIC_VECTOR));
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("or_reduce"), perm_string::literal("|"), args, &primitive_STDLOGIC));
    ...
    args->push_back(new InterfacePort(&primitive_STDLOGIC_VECTOR));
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("and_reduce"), perm_string::literal("&"), args, &primitive_STDLOGIC));
    ...
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("file_open"), perm_string::literal("$ivlh_file_open"), args, NULL));

    /* procedure file_open (status: out file_open_status, file f: text; filename: in string,
    ...
    args->push_back(new InterfacePort(&primitive_INTEGER, PORT_IN));
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("file_close"), perm_string::literal("$fclose"), args, NULL));
    ...
    register_std_subprogram(
        new SubprogramReadWrite(perm_string::literal("read"), perm_string::literal("$ivlh_read")));
     ...
     */
    register_std_subprogram(new SubprogramReadWrite(
        perm_string::literal("hwrite"), perm_string::literal("$ivlh_write"), true));
    ...
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("readline"), perm_string::literal("$ivlh_readline"), args, NULL));
    ...
    args->push_back(new InterfacePort(&primitive_STRING, PORT_IN));
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("writeline"), perm_string::literal("$ivlh_writeline"), args, NULL));
    ...
    args->push_back(new InterfacePort(&primitive_INTEGER, PORT_IN));
    register_std_subprogram(new SubprogramBuiltin(
        perm_string::literal("endfile"), perm_string::literal("$feof"), args, &type_BOOLEAN));
}

void delete_std_funcs() {
    for (std::map<perm_string, SubHeaderList>::iterator cur = std_subprograms.begin();
         cur != std_subprograms.end();
         ++cur) {
        for (SubHeaderList::const_iterator it = cur->second.begin(); it != cur->second.end();
             ++it) {
            delete *it;
        }
    }
