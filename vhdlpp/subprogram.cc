...
SubprogramHeader::SubprogramHeader(perm_string nam,
...
SubprogramHeader::~SubprogramHeader() {
    ...
    if (ports_) {
        for (list<InterfacePort*>::iterator it = ports_->begin(); it != ports_->end(); ++it) {
            delete *it;
        }
        delete ports_;
    }
...
const InterfacePort* SubprogramHeader::find_param(perm_string nam) const {
    ...
    for (std::list<InterfacePort*>::const_iterator it = ports_->begin(); it != ports_->end();
         ++it) {
        if ((*it)->name == nam)
            return *it;
    }
...
const VType* SubprogramHeader::exact_return_type(const std::vector<Expression*>& argv,
                                                 Entity* ent,
                                                 ScopeBase* scope) {
    ...
    VTypeArray* ret =
        new VTypeArray(orig_ret->element_type(), arg->dimensions(), orig_ret->signed_vector());
...
int SubprogramHeader::elaborate_argument(Expression* expr, int idx, Entity* ent, ScopeBase* scope) {
    const VType* type = expr->probe_type(ent, scope);
    const InterfacePort* param = peek_param(idx);

    if (!param) {
        cerr << expr->get_fileline() << ": error: Too many arguments when calling " << name_ << "."
             << endl;
        return 1;
    }
...
SubprogramHeader* SubprogramHeader::make_instance(const std::vector<Expression*>& arguments,
    ...
    // the function call
    for (std::list<InterfacePort*>::iterator it = ports_->begin(); it != ports_->end(); ++it) {
