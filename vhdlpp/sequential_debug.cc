...
void IfSequential::dump(ostream& out, int indent) const {
    ...
    for (list<SequentialStmt*>::const_iterator cur = if_.begin(); cur != if_.end(); ++cur)
        (*cur)->dump(out, indent + 4);

    for (list<IfSequential::Elsif*>::const_iterator cur = elsif_.begin(); cur != elsif_.end();
         ++cur)
        (*cur)->dump(out, indent);

    out << setw(indent + 3) << "" << "FALSE clause (" << else_.size() << "):" << endl;
    for (list<SequentialStmt*>::const_iterator cur = else_.begin(); cur != else_.end(); ++cur)
        (*cur)->dump(out, indent + 4);
}
...
void SignalSeqAssignment::dump(ostream& out, int indent) const {
    ...
    for (list<Expression*>::const_iterator cur = waveform_.begin(); cur != waveform_.end(); ++cur)
        (*cur)->dump(out, indent + 4);
}
...
void CaseSeqStmt::CaseStmtAlternative::dump(ostream& out, int indent) const {
    ...
    for (list<SequentialStmt*>::const_iterator cur = stmts_.begin(); cur != stmts_.end(); ++cur)
...
void ProcedureCall::dump(ostream& out, int indent) const {
    ...
    for (list<named_expr_t*>::const_iterator it = param_list_->begin(); it != param_list_->end();
         ++it)
        (*it)->dump(out, indent);
    out << ")" << endl;
}
...
void LoopStatement::dump(ostream& out, int indent) const {
    for (list<SequentialStmt*>::const_iterator it = stmts_.begin(); it != stmts_.end(); ++it)
        (*it)->dump(out, indent);
}
