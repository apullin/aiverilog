...
ComponentBase::~ComponentBase() {
    for (std::vector<InterfacePort*>::iterator it = ports_.begin(); it != ports_.end(); ++it)
        delete *it;
}
...
Entity::Entity(perm_string name) : ComponentBase(name) {
    bind_arch_ = 0;
}

Entity::~Entity() {
    for (map<perm_string, Architecture*>::reverse_iterator it = arch_.rbegin(); it != arch_.rend();
         ++it)
        delete it->second;
}
