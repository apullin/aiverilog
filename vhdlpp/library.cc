...
static void
import_ieee_use(const YYLTYPE& loc, ActiveScope* res, perm_string package, perm_string name);
static void
import_std_use(const YYLTYPE& loc, ActiveScope* res, perm_string package, perm_string name);

static void
dump_library_package(ostream& file, perm_string lname, perm_string pname, const Package* pack) {
...
static void dump_library_packages(ostream& file,
                                  perm_string lname,
                                  const map<perm_string, Package*>& packages) {
    for (map<perm_string, Package*>::const_iterator cur = packages.cbegin(); cur != packages.cend();
         ++cur) {
...
void library_save_package(perm_string parse_library_name, Package* pack) {
    perm_string use_libname =
        parse_library_name.str() ? parse_library_name : perm_string::literal("work");
...
void library_import(const YYLTYPE& loc, const std::list<perm_string>* names) {
    for (std::list<perm_string>::const_iterator cur = names->begin(); cur != names->end(); ++cur) {
        if (*cur == "ieee") {
...
void library_use(const YYLTYPE& loc,
                 ActiveScope* res,
                 const char* libname,
                 const char* package,
                 const char* name) {
...
static void import_std_use(const YYLTYPE& /*loc*/,
                           ActiveScope* res,
                           perm_string package,
                           perm_string /*name*/) {
    if (package == "standard") {
...
static int emit_packages(perm_string, const map<perm_string, Package*>& packages) {
    int errors = 0;
    for (map<perm_string, Package*>::const_iterator cur = packages.begin(); cur != packages.end();
         ++cur) {
        errors += cur->second->emit_package(cout);
    }

    for (list<const Package*>::const_iterator cur = work_packages.begin();
         cur != work_packages.end();
         ++cur) {
...
static int elaborate_library_packages(map<perm_string, Package*> packages) {
    ...
    for (map<perm_string, Package*>::iterator cur = packages.begin(); cur != packages.end();
         ++cur) {
