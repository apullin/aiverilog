...
ExpAttribute::ExpAttribute(perm_string nam, list<Expression*>* args) : name_(nam), args_(args) {}

ExpAttribute::~ExpAttribute() {
    if (args_) {
        for (list<Expression*>::iterator it = args_->begin(); it != args_->end(); ++it) {
            delete *it;
        }
    }
...
Expression* ExpObjAttribute::clone() const {
    return new ExpObjAttribute(static_cast<ExpName*>(base_->clone()), name_, clone_args());
}
...
ExpBinary::ExpBinary(Expression* op1, Expression* op2) : operand1_(op1), operand2_(op2) {}
...
ExpAggregate::ExpAggregate(std::list<element_t*>* el) : elements_(el ? el->size() : 0) {
    assert(el);
    size_t idx = 0;
    while (!el->empty()) {
...
ExpAggregate::~ExpAggregate() {
    for (std::vector<element_t*>::iterator it = elements_.begin(); it != elements_.end(); ++it) {
        delete *it;
    }

    for (std::vector<choice_element>::iterator it = aggregate_.begin(); it != aggregate_.end();
         ++it) {
        delete it->choice;
        if (!it->alias_flag)
            delete it->expr;
...
Expression* ExpAggregate::clone() const {
    ...
    if (!elements_.empty()) {
        ...
        for (std::vector<element_t*>::const_iterator it = elements_.begin(); it != elements_.end();
             ++it) {
...
ExpConcat::ExpConcat(Expression* op1, Expression* op2) : operand1_(op1), operand2_(op2) {}
...
Expression* ExpConditional::clone() const {
    ...
    if (!options_.empty()) {
        ...
        for (std::list<case_t*>::const_iterator it = options_.begin(); it != options_.end(); ++it) {
...
void ExpConditional::visit(ExprVisitor& func) {
    ...
    for (std::list<case_t*>::iterator it = options_.begin(); it != options_.end(); ++it)
        (*it)->visit(func);

    func.up();
}

ExpConditional::case_t::case_t(Expression* cond, std::list<Expression*>* tru) : cond_(cond) {
    if (tru)
...
void ExpConditional::case_t::visit(ExprVisitor& func) {
    ...
    for (std::list<Expression*>::iterator it = true_clause_.begin(); it != true_clause_.end(); ++it)
        (*it)->visit(func);
    func.up();
}

ExpEdge::ExpEdge(ExpEdge::fun_t typ, Expression* op) : ExpUnary(op), fun_(typ) {}
...
Expression* ExpFunc::clone() const {
    ...
    if (!argv_.empty()) {
        ...
        for (std::vector<Expression*>::const_iterator it = argv_.begin(); it != argv_.end(); ++it)
            new_args->push_back((*it)->clone());
    }
...
void ExpFunc::visit(ExprVisitor& func) {
    ...
    if (!argv_.empty()) {
        for (std::vector<Expression*>::iterator it = argv_.begin(); it != argv_.end(); ++it)
            (*it)->visit(func);
    }
...
SubprogramHeader* ExpFunc::match_signature(Entity* ent, ScopeBase* scope) const {
    ...
    for (vector<Expression*>::const_iterator it = argv_.begin(); it != argv_.end(); ++it) {
...
ExpName::ExpName(perm_string nn, list<Expression*>* indices) : name_(nn), indices_(indices) {}
...
ExpName::~ExpName() {
    if (indices_) {
        for (list<Expression*>::iterator it = indices_->begin(); it != indices_->end(); ++it) {
            delete *it;
        }
...
Expression* ExpName::clone() const {
    ...
    if (indices_) {
        ...
        for (list<Expression*>::const_iterator it = indices_->begin(); it != indices_->end();
             ++it) {
            new_indices->push_back((*it)->clone());
        }
    }

    return new ExpName(static_cast<ExpName*>(safe_clone(prefix_.get())), name_, new_indices);
}
...
ExpCast::ExpCast(Expression* base, const VType* type) : base_(base), type_(type) {}
...
ExpTime::ExpTime(uint64_t amount, timeunit_t unit) : amount_(amount), unit_(unit) {}
...
Expression* ExpRange::left() {
    if (range_expr_ && !left_)
        ...
        left_ = new ExpObjAttribute(
            static_cast<ExpName*>(range_base_->clone()), ExpAttribute::LEFT, NULL);
...
Expression* ExpRange::right() {
    if (range_expr_ && !right_)
        // TODO check if it is an object or type
        right_ = new ExpObjAttribute(
            static_cast<ExpName*>(range_base_->clone()), ExpAttribute::RIGHT, NULL);
    return right_;
}

ExpDelay::ExpDelay(Expression* expr, Expression* delay) : expr_(expr), delay_(delay) {}
