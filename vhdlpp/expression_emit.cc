...
int ExpAggregate::emit_array_(ostream& out,
                              Entity* ent,
                              ScopeBase* scope,
    ...
    if (aggregate_.size() == 1 && aggregate_[0].choice->others()) {
        ...
        if (rc_msb && rc_lsb) {
            int asize = (use_msb >= use_lsb) ? (use_msb - use_lsb) + 1 : (use_lsb - use_msb) + 1;
            out << "{" << asize << "{";
...
int ExpAggregate::emit_record_(ostream& out,
                               Entity* ent,
                               ScopeBase* scope,
...
int ExpCharacter::emit_primitive_bit_(ostream& out,
                                      Entity*,
                                      ScopeBase*,
    ...
    if (res)
        cerr << get_fileline() << ": internal error: "
             << "Don't know how to handle bit " << value_ << " with etype==" << etype->type()
             << endl;
...
int ExpConditional::emit(ostream& out, Entity* ent, ScopeBase* scope) const {
    ...
    if (options_.size() > 1) {
        ...
        for (list<case_t*>::const_iterator cur = options_.begin(); cur != last; ++cur) {
...
int ExpConditional::case_t::emit_option(ostream& out, Entity* ent, ScopeBase* scope) const {
    ...
    if (true_clause_.size() > 1) {
        cerr << get_fileline() << ": sorry: Multiple expression waveforms not supported here."
             << endl;
        errors += 1;
    }
...
int ExpName::emit(ostream& out, Entity* ent, ScopeBase* scope) const {
    ...
    if (try_workarounds_(out, ent, scope, indices, field_size)) {
        ...
        for (list<index_t*>::iterator it = indices.begin(); it != indices.end(); ++it) {
            delete *it;
        }
        return 0;
    }
...
bool ExpName::try_workarounds_(
    ostream& out, Entity* ent, ScopeBase* scope, list<index_t*>& indices, int& data_size) const {
    Expression* exp = NULL;
...
int ExpName::emit_workaround_(ostream& out,
                              Entity* ent,
                              ScopeBase* scope,
    ...
    for (list<index_t*>::const_iterator it = indices.begin(); it != indices.end(); ++it) {
        errors += (*it)->emit(out, ent, scope);
        out << "+";
    }
