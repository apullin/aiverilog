...
void ExpAggregate::element_t::write_to_stream(ostream& fd) const {
    for (vector<choice_t*>::const_iterator cur = fields_.begin(); cur != fields_.end(); ++cur) {
        (*cur)->write_to_stream(fd);
    }
...
void ExpBitstring::write_to_stream(ostream& fd) const {
    fd << "B\"";
    for (vector<char>::const_reverse_iterator it = value_.rbegin(); it != value_.rend(); ++it) {
        fd << *it;
    }
    fd << "\"";
}
...
void ExpFunc::write_to_stream(ostream& fd) const {
    const char* comma = "";
    fd << name_ << "(";
    for (vector<Expression*>::const_iterator cur = argv_.begin(); cur != argv_.end(); ++cur) {
        fd << comma;
        (*cur)->write_to_stream(fd);
        comma = ", ";
    }
