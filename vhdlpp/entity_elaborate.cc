...
int Entity::elaborate() {
    ...
    if (arch_.empty()) {
        cerr << get_fileline() << ": error: "
             << "No architectures to choose from for entity " << get_name() << "." << endl;
        return 1;
    }
    ...
    if (verbose_flag)
        cerr << "For entity " << get_name() << ", choosing architecture " << bind_arch_->get_name()
             << "." << endl;
...
int Entity::elaborate_generic_exprs_() {
    int errors = 0;
    for (vector<InterfacePort*>::const_iterator cur = parms_.begin(); cur != parms_.end(); ++cur) {
...
int Entity::elaborate_ports_(void) {
    int errors = 0;
    for (std::vector<InterfacePort*>::const_iterator cur = ports_.begin(); cur != ports_.end();
         ++cur) {
        ...
        if (type == 0) {
            cerr << get_fileline() << ": error: "
                 << "Giving up on unknown type for port " << cur_port->name << "." << endl;
            errors += 1;
            continue;
        }
