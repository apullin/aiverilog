...
int IfSequential::emit(ostream& out, Entity* ent, ScopeBase* scope) {
    ...
    for (list<SequentialStmt*>::iterator cur = if_.begin(); cur != if_.end(); ++cur)
        errors += (*cur)->emit(out, ent, scope);

    for (list<IfSequential::Elsif*>::iterator cur = elsif_.begin(); cur != elsif_.end(); ++cur) {
        out << "end else if (";
    ...
    if (!else_.empty()) {
        ...
        for (list<SequentialStmt*>::iterator cur = else_.begin(); cur != else_.end(); ++cur)
            errors += (*cur)->emit(out, ent, scope);
    }
...
int ProcedureCall::emit(ostream& out, Entity* ent, ScopeBase* scope) {
    ...
    if (param_list_) {
        ...
        for (std::list<named_expr_t*>::iterator it = param_list_->begin(); it != param_list_->end();
             ++it)
...
int LoopStatement::emit_substatements(ostream& out, Entity* ent, ScopeBase* scope) {
    int errors = 0;
    for (list<SequentialStmt*>::iterator cur = stmts_.begin(); cur != stmts_.end(); ++cur) {
...
int CaseSeqStmt::emit(ostream& out, Entity* ent, ScopeBase* scope) {
    ...
    for (list<CaseStmtAlternative*>::iterator cur = alt_.begin(); cur != alt_.end(); ++cur) {
...
int ForLoopStatement::emit(ostream& out, Entity* ent, ScopeBase* scope) {
    ...
    if (!start_rc || !finish_rc) {
        ...
        if ((dir == ExpRange::DOWNTO && start_val < finish_val) ||
            ...
            out << "begin /* Degenerate loop at " << get_fileline() << ": " << start_val;
...
int ReportStmt::emit(ostream& out, Entity* ent, ScopeBase* scope) {
    ...
    struct emitter : public ExprVisitor {
        emitter(ostream& outp, Entity* enti, ScopeBase* scop)
            : out_(outp), ent_(enti), scope_(scop), level_lock_(numeric_limits<int>::max()) {}
        ...
        void operator()(Expression* s) {
            if (!dynamic_cast<ExpConcat*>(s)) {
                ...
                if (dynamic_cast<ExpName*>(s) && type && type->type_match(&primitive_STRING)) {
                    out_ << "$sformatf(\"%s\", (";
    ...
    switch (type_) {
        ...
        case UNTIL:
            if (!sens_list_.empty()) {
                ...
                for (std::set<ExpName*>::iterator it = sens_list_.begin(); it != sens_list_.end();
                     ++it) {
