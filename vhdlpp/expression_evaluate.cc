...
bool ExpAttribute::test_array_type(const VType* type) const {
    ...
    if (arr == 0) {
        cerr << endl
             << get_fileline() << ": error: "
             << "Cannot apply the '" << name_ << " attribute to non-array objects" << endl;
    ...
    if (arr->dimensions().size() > 1) {
        cerr << endl
             << get_fileline() << ": error: "
             << "Cannot apply the '" << name_ << " attribute to multidimensional arrays" << endl;
        return false;
    }
...
bool ExpAttribute::evaluate_type_attr(const VType* type,
                                      Entity* ent,
                                      ScopeBase* scope,
                                      int64_t& val) const {
