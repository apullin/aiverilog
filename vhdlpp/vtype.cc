...
int VTypePrimitive::get_width(ScopeBase*) const {
    switch (type_) {
        ...
        default:
            std::cerr << "sorry: primitive type " << type_ << " has no get_width() implementation."
                      << std::endl;
            break;
    }
...
VTypeArray::VTypeArray(const VType* element, std::list<ExpRange*>* r, bool sv)
    ...
    for (size_t idx = 0; idx < ranges_.size(); idx += 1) {
        ...
        r->pop_front();
        ranges_[idx] = range_t(
            curp->msb(), curp->lsb(), (curp->direction() == ExpRange::DOWNTO ? true : false));
    }
}
...
VType* VTypeArray::clone() const {
    ...
    for (std::vector<range_t>::const_iterator it = ranges_.begin(); it != ranges_.end(); ++it) {
...
void VTypeArray::show(ostream& out) const {
    out << "array ";
    for (vector<range_t>::const_iterator cur = ranges_.begin(); cur != ranges_.end(); ++cur) {
        out << "(";
        if (cur->msb())
...
int VTypeArray::get_width(ScopeBase* scope) const {
    ...
    for (vector<range_t>::const_iterator it = ranges_.begin(); it != ranges_.end(); ++it) {
...
VTypeRangeExpr::VTypeRangeExpr(const VType* base,
...
VType* VTypeRangeExpr::clone() const {
    return new VTypeRangeExpr(base_type()->clone(), start_->clone(), end_->clone(), downto_);
}

VTypeEnum::VTypeEnum(const std::list<perm_string>* names) : names_(names->size()) {
    size_t idx = 0;

    for (list<perm_string>::const_iterator cur = names->begin(); cur != names->end();
         ++cur, ++idx) {
        names_[idx] = *cur;
    }
}
...
VTypeRecord::VTypeRecord(std::list<element_t*>* elements) : elements_(elements->size()) {
...
int VTypeRecord::get_width(ScopeBase* scope) const {
    ...
    for (vector<element_t*>::const_iterator it = elements_.begin(); it != elements_.end(); ++it) {
...
const VTypeRecord::element_t* VTypeRecord::element_by_name(perm_string name, int* index) const {
    for (vector<element_t*>::const_iterator cur = elements_.begin(); cur != elements_.end();
         ++cur) {
        element_t* curp = *cur;
...
VTypeRecord::element_t::element_t(perm_string name, const VType* typ) : name_(name), type_(typ) {}

VTypeDef::VTypeDef(perm_string nam) : name_(nam), type_(0) {}

VTypeDef::VTypeDef(perm_string nam, const VType* typ) : name_(nam), type_(typ) {}
