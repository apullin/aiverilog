...
class InterfacePort : public LineInfo {
  public:
    ...
    explicit InterfacePort(const VType* typ) : mode(PORT_NONE), type(typ), expr(NULL) {}

    InterfacePort(const VType* typ, port_mode_t mod) : mode(mod), type(typ), expr(NULL) {}
...
class ComponentBase : public LineInfo {
  public:
    ...
    void set_interface(std::list<InterfacePort*>* parms, std::list<InterfacePort*>* ports);
