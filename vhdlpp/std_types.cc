...
void delete_global_types() {
    for (map<perm_string, VTypeDef*>::iterator cur = std_types.begin(); cur != std_types.end();
         ++cur) {
...
const VTypeEnum* find_std_enum_name(perm_string name) {
    for (list<const VTypeEnum*>::const_iterator it = std_enums.begin(); it != std_enums.end();
         ++it) {
        if ((*it)->has_name(name))
            return *it;
    }
