...
class LexicalScope {
  public:
    ...
    explicit LexicalScope(LexicalScope* parent)
        : default_lifetime(INHERITED),
          has_parameter_port_list(false),
          generate_counter(0),
          parent_(parent) {}
    ...
    struct param_expr_t : public PNamedItem {
        inline param_expr_t()
            : data_type(0), expr(0), range(0), local_flag(false), overridable(true) {}
        // Type information.
    ...
    LexicalScope* parent_scope() const {
        return parent_;
    }
