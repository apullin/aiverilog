 ...
 */
static void link_scope_to_nexus_signal(nexus_private_t* priv,
                                       vhdl_scope* scope,
                                       ivl_signal_t sig,
                                       unsigned pin) {
    scope_nexus_t* sn;
 ...
 */
static void link_scope_to_nexus_tmp(nexus_private_t* priv, vhdl_scope* scope, const string& name) {
 ...
 */
static string
visible_nexus_signal_name(nexus_private_t* priv, const vhdl_scope* scope, unsigned* pin) {
...
void draw_nexus(ivl_nexus_t nexus) {
    ...
    for (int i = 0; i < nptrs; i++) {
        ...
        if ((log = ivl_nexus_ptr_log(nexus_ptr))) {
            ...
            if (visible_nexus(priv, vhdl_scope)) {
                ...
                const vhdl_type* type = vhdl_type::type_for(ivl_logic_width(log), false);
            ...
            if (ivl_const_type(con) == IVL_VT_REAL) {
                error("No VHDL translation for real constant (%g)", ivl_const_real(con));
                continue;
            }
            if (ivl_const_width(con) == 1)
                priv->const_driver = new vhdl_const_bit(ivl_const_bits(con)[0]);
            else
                priv->const_driver = new vhdl_const_bits(
                    ivl_const_bits(con), ivl_const_width(con), ivl_const_signed(con) != 0);
    ...
    if (ndrivers == 0) {
        ...
        if (def) {
            if (width > 1)
                priv->const_driver =
                    new vhdl_bit_spec_expr(vhdl_type::std_logic(), new vhdl_const_bit(def));
            else
...
vhdl_var_ref* nexus_to_var_ref(vhdl_scope* scope, ivl_nexus_t nexus) {
    ...
    nexus_private_t* priv = static_cast<nexus_private_t*>(ivl_nexus_get_private(nexus));
    unsigned pin;
...
static void declare_one_signal(vhdl_entity* ent, ivl_signal_t sig, ivl_scope_t scope) {
    ...
    if (dimensions > 0) {
        ...
        const vhdl_type* array_type = vhdl_type::array_of(base_type, type_name, msb, lsb);
    ...
    switch (mode) {
        case IVL_SIP_NONE: {
            ...
            if (ivl_signal_local(sig)) {
                ...
                ss << "Declared at " << ivl_signal_file(sig) << ":" << ivl_signal_lineno(sig);
            }
        ...
        case IVL_SIP_INPUT:
            ent->get_scope()->add_decl(new vhdl_port_decl(name.c_str(), sig_type, VHDL_PORT_IN));
            break;
        case IVL_SIP_OUTPUT: {
            ...
            if (ivl_signal_type(sig) == IVL_SIT_REG) {
                ...
                ent->get_arch()->get_scope()->add_decl(new vhdl_signal_decl(newname, reg_type));

                // Create a concurrent assignment statement to
                // connect the register to the output
                ent->get_arch()->add_stmt(new vhdl_cassign_stmt(new vhdl_var_ref(name, NULL),
                                                                new vhdl_var_ref(newname, NULL)));
            }
            break;
        case IVL_SIP_INOUT:
            ent->get_scope()->add_decl(new vhdl_port_decl(name.c_str(), sig_type, VHDL_PORT_INOUT));
            break;
        default:
            assert(false);
    }
 ...
 */
static void map_signal(ivl_signal_t to, const vhdl_entity* parent, vhdl_comp_inst* inst) {
    ...
    if (visible_nexus(priv, arch_scope)) {
        ...
        const vhdl_decl* from_decl = parent->get_arch()->get_scope()->get_decl(ref->get_name());
        if (!from_decl->is_readable() && !arch_scope->have_declared(name + "_Readable")) {
            vhdl_decl* tmp_decl = new vhdl_signal_decl(name + "_Readable", ref->get_type());
 ...
 */
static void port_map(ivl_scope_t scope, const vhdl_entity* parent, vhdl_comp_inst* inst) {
...
static int draw_function(ivl_scope_t scope, ivl_scope_t parent) {
    ...
    debug_msg("Generating function %s (%s)", ivl_scope_tname(scope), ivl_scope_name(scope));
    ...
    ostringstream ss;
    ss << "Generated from function " << funcname << " at " << ivl_scope_def_file(scope) << ":"
...
static void create_skeleton_entity_for(ivl_scope_t scope, int depth) {
    ...
    ent->set_time_units(ivl_scope_time_units(scope), ivl_scope_time_precision(scope));
...
extern "C" int draw_skeleton_scope(ivl_scope_t scope, void*) {
    ...
    debug_msg("Initial visit to scope type %s at depth %d", ivl_scope_tname(scope), depth);
...
extern "C" int draw_constant_drivers(ivl_scope_t scope, void*) {
    ...
    if (ivl_scope_type(scope) == IVL_SCT_MODULE) {
        ...
        for (int i = 0; i < nsigs; i++) {
            ...
            for (unsigned j = ivl_signal_array_base(sig); j < ivl_signal_array_count(sig); j++) {
                ...
                nexus_private_t* priv = static_cast<nexus_private_t*>(ivl_nexus_get_private(nex));
                assert(priv);
                ...
                if (priv->const_driver &&
                    ...
                    ent->get_arch()->add_stmt(new vhdl_cassign_stmt(ref, priv->const_driver));
                    priv->const_driver = NULL;
                }
                ...
                if (ivl_signal_port(sn->sig) != IVL_SIP_INPUT) {
                    for (list<ivl_signal_t>::const_iterator it = sn->connect.begin();
                         it != sn->connect.end();
                         ++it) {
                        const vhdl_type* rtype = vhdl_type::type_for(ivl_signal_width(sn->sig),
                                                                     ivl_signal_signed(sn->sig));
                        const vhdl_type* ltype =
                            vhdl_type::type_for(ivl_signal_width(*it), ivl_signal_signed(*it));
...
extern "C" int draw_hierarchy(ivl_scope_t scope, void* _parent) {
    if (ivl_scope_type(scope) == IVL_SCT_MODULE && _parent) {
        ...
        if (inst_name == ent->get_name() || parent_scope->name_collides(inst_name) ||
            find_entity(inst_name) != NULL || is_vhdl_reserved_word(inst_name)) {
        ...
        vhdl_comp_inst* inst = new vhdl_comp_inst(inst_name.c_str(), ent->get_name().c_str());
