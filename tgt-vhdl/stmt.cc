 ...
 * -puse-vhpi-finish=1.
 */
static int draw_stask_finish(vhdl_procedural*, stmt_container* container, ivl_statement_t) {
    ...
    if (strcmp(use_vhpi, "1") == 0) {
        ...
        container->add_stmt(
            new vhdl_report_stmt(new vhdl_const_string("SIMULATION FINISHED"), SEVERITY_FAILURE));
    }
...
static int
draw_stask_display(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
    vhdl_binop_expr* text = new vhdl_binop_expr(VHDL_BINOP_CONCAT, vhdl_type::string());
 ...
 * these are supported.
 */
static int draw_stask(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
    ...
    if (strcmp(name, "$display") == 0)
        ...
        ostringstream ss;
        ss << "Unsupported system task " << name << " omitted here (" << ivl_stmt_file(stmt) << ":"
 ...
 */
static int
draw_block(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt, bool is_last) {
 ...
 */
static int draw_noop(vhdl_procedural*, stmt_container* container, ivl_statement_t) {
...
static vhdl_var_ref* make_assign_lhs(ivl_lval_t lval, vhdl_scope* scope) {
    ...
    if (decl->assignment_type() == vhdl_decl::ASSIGN_CONST) {
        ...
        vhdl_var_decl* shadow_decl = new vhdl_var_decl(shadow_name, decl->get_type());
...
static bool
assignment_lvals(ivl_statement_t stmt, vhdl_procedural* proc, list<vhdl_var_ref*>& lvals) {
...
bool check_valid_assignment(vhdl_decl::assign_type_t atype,
                            vhdl_procedural* proc,
                            ivl_statement_t stmt) {
    if (atype == vhdl_decl::ASSIGN_NONBLOCK && !proc->get_scope()->allow_signal_assignment()) {
...
static void emit_wait_for_0(vhdl_procedural* proc,
    ...
    for (vhdl_var_set_t::const_iterator it = read.begin(); it != read.end(); ++it) {
        if (proc->is_blocking_target(*it))
            need_wait_for_0 = true;
    }

    const stmt_container::stmt_list_t& stmts = container->get_stmts();
    bool last_was_wait = !stmts.empty() && dynamic_cast<vhdl_wait_stmt*>(stmts.back());

    if (need_wait_for_0 && !last_was_wait) {
        debug_msg("Generated wait-for-0 for %s:%d", ivl_stmt_file(stmt), ivl_stmt_lineno(stmt));

        vhdl_seq_stmt* wait = new vhdl_wait_stmt(VHDL_WAIT_FOR0);

        ostringstream ss;
        ss << "Read target of blocking assignment (" << ivl_stmt_file(stmt) << ":"
           << ivl_stmt_lineno(stmt) << ")";
...
void make_assignment(vhdl_procedural* proc,
                     stmt_container* container,
                     ivl_statement_t stmt,
                     bool emul_blocking,
    ...
    if (lvals.size() == 1) {
        ...
        if (ivl_expr_type(rval) == IVL_EX_TERNARY) {
            ...
            vhdl_var_ref* lhs2 = make_assign_lhs(ivl_stmt_lval(stmt, 0), proc->get_scope());
            ...
            // True part
            {
                vhdl_abstract_assign_stmt* a = assign_for(decl->assignment_type(), lhs, rhs);
                if (after)
                    a->set_after(after);
            ...
            // False part
            {
                vhdl_abstract_assign_stmt* a = assign_for(decl->assignment_type(), lhs2, rhs2);
                if (after)
        ...
        if (proc->get_scope()->initializing() &&
            ivl_signal_port(ivl_lval_sig(lval)) == IVL_SIP_NONE && !decl->has_initial() &&
            rhs->constant() && decl->get_type()->get_name() != VHDL_TYPE_ARRAY) {
        ...
        for (it = lvals.begin(); it != lvals.end(); ++it) {
            ...
            vhdl_abstract_assign_stmt* a = assign_for(decl->assignment_type(), *it, tmp_rhs);
            if (after)
                a->set_after(after);
 ...
 * assignment.
 */
static int draw_nbassign(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
...
static int draw_assign(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
 ...
 * VHDL wait statement.
 */
static int draw_delay(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
 ...
 */
static bool
draw_synthesisable_wait(vhdl_process* proc, stmt_container* container, ivl_statement_t stmt) {
    ...
    set_difference(edge_triggered.begin(),
                   edge_triggered.end(),
                   test_nexuses.begin(),
                   test_nexuses.end(),
    ...
    for (set<ivl_nexus_t>::const_iterator it = edge_triggered.begin(); it != edge_triggered.end();
         ++it) {
 ...
 */
static int draw_wait(vhdl_procedural* _proc, stmt_container* container, ivl_statement_t stmt) {
    ...
    if (combinatorial) {
        ...
        vhdl_binop_expr* test = new vhdl_binop_expr(VHDL_BINOP_OR, vhdl_type::boolean());
        ...
        for (int i = 0; i < nevents; i++) {
            ...
            for (int j = 0; j < nneg; j++) {
                ...
                vhdl_var_ref* ref = nexus_to_var_ref(proc->get_scope(), nexus);
                vhdl_fcall* detect = new vhdl_fcall("falling_edge", vhdl_type::boolean());
                detect->add_expr(ref);
            ...
            for (int j = 0; j < npos; j++) {
                ...
                vhdl_var_ref* ref = nexus_to_var_ref(proc->get_scope(), nexus);
                vhdl_fcall* detect = new vhdl_fcall("rising_edge", vhdl_type::boolean());
                detect->add_expr(ref);
...
static int
draw_if(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt, bool is_last) {
...
static vhdl_var_ref*
draw_case_test(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
    vhdl_expr* test = translate_expr(ivl_stmt_cond_expr(stmt));
    if (NULL == test)
        return NULL;
    ...
    if (typeid(*test) != typeid(vhdl_var_ref)) {
        ...
        if (!proc->get_scope()->have_declared(tmp_name)) {
            proc->get_scope()->add_decl(new vhdl_var_decl(tmp_name, new vhdl_type(*test_type)));
        }
...
static int
draw_case(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt, bool is_last) {
    ...
    if (!have_others) {
        vhdl_case_branch* others = new vhdl_case_branch(new vhdl_var_ref("others", NULL));
    ...
    switch (ivl_expr_type(expr)) {
        ...
        case IVL_EX_NUMBER: {
            ...
            for (unsigned idx = 0; idx < nbits; idx++) {
                ...
                if (ivl_expr_signed(expr) && bits[nbits - 1] == '1' && nbits < 8 * sizeof(long))
                    imm |= -1UL << nbits;
            }
            break;
        }
 ...
 * or undefined select.
 */
static void check_against_x(vhdl_binop_expr* all,
                            const vhdl_var_ref* test,
                            ivl_expr_t expr,
                            unsigned width,
                            unsigned base,
                            bool is_casez) {
    if (is_casez) {
        ...
        for (unsigned i = 0; i < ivl_expr_width(expr); i++) {
            vhdl_binop_expr* sub_expr = new vhdl_binop_expr(VHDL_BINOP_OR, vhdl_type::boolean());
            const vhdl_type* type;
            vhdl_var_ref* ref;
            ...
            vhdl_binop_expr* cmp = new vhdl_binop_expr(VHDL_BINOP_EQ, vhdl_type::boolean());
            cmp->add_expr(ref);
 ...
 */
static void process_number(vhdl_binop_expr* all,
                           const vhdl_var_ref* test,
                           ivl_expr_t expr,
                           unsigned width,
                           unsigned base,
 ...
 */
static bool process_signal(vhdl_binop_expr* all,
                           const vhdl_var_ref* test,
                           ivl_expr_t expr,
                           unsigned width,
                           unsigned base,
    ...
    if (ivl_expr_oper1(expr) != 0 || ivl_signal_dimensions(ivl_expr_signal(expr)) != 0) {
 ...
 */
static bool process_expr_bits(vhdl_binop_expr* all,
                              vhdl_var_ref* test,
                              ivl_expr_t expr,
                              unsigned width,
                              unsigned base,
    ...
    switch (ivl_expr_type(expr)) {
        ...
        case IVL_EX_SIGNAL:
            if (process_signal(all, test, expr, width, base, is_casez, ivl_expr_width(expr), 0))
                return true;
            break;

        case IVL_EX_SELECT: {
            ivl_expr_t bexpr = ivl_expr_oper2(expr);
            if (ivl_expr_type(bexpr) != IVL_EX_NUMBER && ivl_expr_type(bexpr) != IVL_EX_ULONG) {
        ...
        default:
            error("Sorry, expression type %d is not currently supported.", ivl_expr_type(expr));
            return true;
            break;
    }
 ...
 */
int draw_casezx(vhdl_procedural* proc,
                stmt_container* container,
                ivl_statement_t stmt,
                bool is_last) {
    vhdl_var_ref* test = draw_case_test(proc, container, stmt);
    ...
    for (int i = 0; i < nbranches; i++) {
        ...
        ivl_expr_t net = ivl_stmt_case_expr(stmt, i);
        if (net) {
            vhdl_binop_expr* all = new vhdl_binop_expr(VHDL_BINOP_AND, vhdl_type::boolean());
    ...
    ostringstream ss;
    ss << "Generated from case" << (is_casez ? 'z' : 'x') << " statement at " << ivl_stmt_file(stmt)
       << ":" << ivl_stmt_lineno(stmt);
    result->set_comment(ss.str());
...
int draw_while(vhdl_procedural* proc,
               stmt_container* container,
               ivl_statement_t stmt,
               ivl_statement_t step = 0) {
...
int draw_for_loop(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
...
int draw_forever(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
...
int draw_repeat(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
 ...
 */
int draw_utask(vhdl_procedural* proc, stmt_container* container, ivl_statement_t stmt) {
 ...
 */
int draw_stmt(vhdl_procedural* proc,
              stmt_container* container,
              ivl_statement_t stmt,
              bool is_last) {
    assert(stmt);
