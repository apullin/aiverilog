...
static vhdl_expr* binop_lpm_to_expr(vhdl_scope* scope, ivl_lpm_t lpm, vhdl_binop_t op) {
    ...
    const vhdl_type* result_type = vhdl_type::type_for(out_width, ivl_lpm_signed(lpm) != 0);
    ...
    for (unsigned i = 0; i < ivl_lpm_size(lpm); i++) {
        ...
        e = e->cast(vhdl_type::type_for(e->get_type()->get_width(), ivl_lpm_signed(lpm) != 0));
    ...
    if (op == VHDL_BINOP_MULT) {
        ...
        vhdl_fcall* resize = new vhdl_fcall("Resize", vhdl_type::nsigned(out_width));
...
static vhdl_expr*
reduction_lpm_to_expr(vhdl_scope* scope, ivl_lpm_t lpm, support_function_t f, bool invert) {
...
static vhdl_expr* shift_lpm_to_expr(vhdl_scope* scope, ivl_lpm_t lpm, vhdl_binop_t shift_op) {
    vhdl_expr* lhs = readable_ref(scope, ivl_lpm_data(lpm, 0));
    vhdl_expr* rhs = readable_ref(scope, ivl_lpm_data(lpm, 1));
    if (!lhs || !rhs)
        return NULL;
...
static int draw_mux_lpm(vhdl_arch* arch, ivl_lpm_t lpm) {
    ...
    vhdl_expr* t1 = new vhdl_binop_expr(sel, VHDL_BINOP_EQ, b1, vhdl_type::boolean());
...
int draw_lpm(vhdl_arch* arch, ivl_lpm_t lpm) {
    ...
    bool bool_to_logic = out->get_type()->get_name() == VHDL_TYPE_STD_LOGIC &&
                         f->get_type()->get_name() == VHDL_TYPE_BOOLEAN;

    if (bool_to_logic) {
        vhdl_cassign_stmt* s = new vhdl_cassign_stmt(out, new vhdl_const_bit('0'));
