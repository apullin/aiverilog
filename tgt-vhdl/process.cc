    ...
    if (ivl_process_type(proc) == IVL_PR_ALWAYS) {
        ...
        if (ivl_statement_type(stmt) == IVL_ST_WAIT) {
            ...
            if (ivl_statement_type(stmt) == IVL_ST_BLOCK) {
                ...
                if (proc_scope)
                    name = ivl_scope_basename(proc_scope);
            }
        }
    }
...
static int generate_vhdl_process(vhdl_entity* ent, ivl_process_t proc) {
    ...
    vhdl_proc->get_scope()->set_initializing(ivl_process_type(proc) == IVL_PR_INITIAL);
    ...
    const char* type = ivl_process_type(proc) == IVL_PR_INITIAL ? "initial" : "always";
...
extern "C" int draw_process(ivl_process_t proc, void*) {
    ...
    while (ivl_scope_type(scope) == IVL_SCT_GENERATE || ivl_scope_type(scope) == IVL_SCT_BEGIN)
