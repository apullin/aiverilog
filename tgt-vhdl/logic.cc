 ...
 */
static vhdl_expr* inputs_to_expr(vhdl_scope* scope, vhdl_binop_t op, ivl_net_logic_t log) {
 ...
 */
static vhdl_expr* input_to_expr(vhdl_scope* scope, vhdl_unaryop_t op, ivl_net_logic_t log) {
...
static void comb_udp_logic(vhdl_arch* arch, ivl_net_logic_t log) {
    ...
    ss.str("");
    ss << "Input to " << ivl_logic_basename(log) << " " << ivl_udp_name(udp) << " UDP";
    ...
    vhdl_with_select_stmt* ws = new vhdl_with_select_stmt(new vhdl_var_ref(*tmp_ref), out);
...
static void seq_udp_logic(vhdl_arch* arch, ivl_net_logic_t log) {
    ...
    if (nin == 1) {
        vhdl_var_ref* ref = nexus_to_var_ref(arch->get_scope(), ivl_logic_pin(log, 1));
        ...
        for (int i = 1; i < nin; i++) {
            vhdl_var_ref* ref = nexus_to_var_ref(arch->get_scope(), ivl_logic_pin(log, i));
            concat->add_expr(ref);
            proc->add_sensitivity(ref->get_name());
        }
...
static vhdl_expr* translate_logic_inputs(vhdl_scope* scope, ivl_net_logic_t log) {
    switch (ivl_logic_type(log)) {
        ...
        default:
            error("Don't know how to translate logic type = %d to expression", ivl_logic_type(log));
            return NULL;
    }
}
