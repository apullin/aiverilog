...
vhdl_scope::vhdl_scope() : parent_(NULL), init_(false), sig_assign_(true), hoisted_init_(false) {}
...
void stmt_container::find_vars(vhdl_var_set_t& read, vhdl_var_set_t& write) {
    // Iterate over each sub-statement and find all its
    // read/written variables

    for (stmt_list_t::const_iterator it = stmts_.begin(); it != stmts_.end(); ++it)
...
void vhdl_wait_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t&) {
    if (expr_)
...
void vhdl_expr_list::find_vars(vhdl_var_set_t& read) {
    for (list<vhdl_expr*>::const_iterator it = exprs_.begin(); it != exprs_.end(); ++it)
        (*it)->find_vars(read);
}
...
void vhdl_pcall_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t&) {
...
void vhdl_abstract_assign_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t& write) {
...
vhdl_const_bits::vhdl_const_bits(const char* value, int width, bool issigned, bool qualify)
    : vhdl_expr(issigned ? vhdl_type::nsigned(width) : vhdl_type::nunsigned(width), true),
      qualified_(qualify),
      signed_(issigned) {
...
void vhdl_const_bits::emit(std::ostream& of, int) const {
    ...
    if ((!signed_ || ival >= 0) && !has_meta_bits() && bits <= 64 && bits % 4 == 0) {
...
void vhdl_cassign_stmt::emit(std::ostream& of, int level) const {
    ...
    of << " <= ";
    if (!whens_.empty()) {
        for (std::list<when_part_t>::const_iterator it = whens_.begin(); it != whens_.end(); ++it) {
...
vhdl_report_stmt::vhdl_report_stmt(vhdl_expr* text, vhdl_severity_t severity)
    : severity_(severity), text_(text) {}
...
void vhdl_if_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t& write) {
    test_->find_vars(read);

    then_part_.find_vars(read, write);
    else_part_.find_vars(read, write);

    for (list<elsif>::const_iterator it = elsif_parts_.begin(); it != elsif_parts_.end(); ++it) {
...
vhdl_binop_expr::vhdl_binop_expr(vhdl_expr* left,
                                 vhdl_binop_t op,
                                 vhdl_expr* right,
...
void vhdl_binop_expr::find_vars(vhdl_var_set_t& read) {
    for (list<vhdl_expr*>::const_iterator it = operands_.begin(); it != operands_.end(); ++it)
        (*it)->find_vars(read);
}
...
void vhdl_case_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t& write) {
    test_->find_vars(read);

    for (case_branch_list_t::const_iterator it = branches_.begin(); it != branches_.end(); ++it) {
...
void vhdl_while_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t& write) {
    test_->find_vars(read);
...
void vhdl_loop_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t& write) {
...
void vhdl_for_stmt::find_vars(vhdl_var_set_t& read, vhdl_var_set_t& write) {
...
vhdl_function::vhdl_function(const char* name, vhdl_type* ret_type) : vhdl_decl(name, ret_type) {
