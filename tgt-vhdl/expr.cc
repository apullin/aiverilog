...
static vhdl_expr* correct_signedness(vhdl_expr* vhd_e, ivl_expr_t vl_e) {
    ...
    } else if (vhd_e->get_type()->get_name() == VHDL_TYPE_SIGNED && !should_be_signed) {
        // operand->print();
...
static vhdl_var_ref* translate_signal(ivl_expr_t e) {
    ...
    vhdl_var_ref* ref = new vhdl_var_ref(renamed, new vhdl_type(*decl->get_type()));
...
static vhdl_expr* translate_number(ivl_expr_t e) {
    if (ivl_expr_width(e) == 1)
        ...
        return new vhdl_const_bits(ivl_expr_bits(e), ivl_expr_width(e), ivl_expr_signed(e) != 0);
}
...
static vhdl_expr* translate_reduction(support_function_t f, bool neg, vhdl_expr* operand) {
    vhdl_expr* result;
    if (operand->get_type()->get_name() == VHDL_TYPE_STD_LOGIC)
        ...
        vhdl_fcall* fcall =
            new vhdl_fcall(support_function::function_name(f), vhdl_type::std_logic());
    ...
    if (neg)
        return new vhdl_unaryop_expr(VHDL_UNARYOP_NOT, result, vhdl_type::std_logic());
    else
        return result;
}
 ...
 */
static vhdl_expr* translate_numeric(vhdl_expr* lhs, vhdl_expr* rhs, vhdl_binop_t op) {
    ...
    if (op == VHDL_BINOP_MULT)
        rtype =
            new vhdl_type(lhs->get_type()->get_name(), (lhs->get_type()->get_width() * 2) - 1, 0);
    else
        rtype = new vhdl_type(*lhs->get_type());
    return new vhdl_binop_expr(lhs, op, rhs, rtype);
}

static vhdl_expr* translate_relation(vhdl_expr* lhs, vhdl_expr* rhs, vhdl_binop_t op) {
 ...
 */
static vhdl_expr* translate_logical(vhdl_expr* lhs, vhdl_expr* rhs, vhdl_binop_t op) {
    vhdl_type boolean(VHDL_TYPE_BOOLEAN);

    return translate_relation(lhs->cast(&boolean), rhs->cast(&boolean), op);
}

static vhdl_expr* translate_shift(vhdl_expr* lhs, vhdl_expr* rhs, vhdl_binop_t op) {
...
static vhdl_expr* translate_power(ivl_expr_t e, vhdl_expr* lhs, vhdl_expr* rhs) {
    ...
    vhdl_expr* result =
        new vhdl_binop_expr(lhs_int, VHDL_BINOP_POWER, rhs_int, vhdl_type::integer());

    int width = ivl_expr_width(e);
    const char* func = ivl_expr_signed(e) ? "To_Signed" : "To_Unsigned";
    const vhdl_type* type =
        ivl_expr_signed(e) ? vhdl_type::nsigned(width) : vhdl_type::nunsigned(width);
...
static vhdl_expr* translate_binary(ivl_expr_t e) {
    ...
    switch (ivl_expr_opcode(e)) {
        ...
        case 'E':
            if (vectorop)
                result = translate_relation(
                    lhs->cast(&std_logic_vector), rhs->cast(&std_logic_vector), VHDL_BINOP_EQ);
            else
                result = translate_relation(lhs, rhs, VHDL_BINOP_EQ);
            break;
        case 'n':
        ...
        case 'N':
            if (vectorop)
                result = translate_relation(
                    lhs->cast(&std_logic_vector), rhs->cast(&std_logic_vector), VHDL_BINOP_NEQ);
            else
                result = translate_relation(lhs, rhs, VHDL_BINOP_NEQ);
            break;
        case '&':  // Bitwise AND
    ...
    if (vectorop) {
        ...
        if (actual_width != result_width) {
            // result->print();
            // std::cout << "^ should be " << result_width << " but is " << actual_width <<
            // std::endl;
        }
    }
        ...
        if (NULL == from_var_ref) {
            ...
            if (ivl_expr_signed(ivl_expr_oper1(e))) {
                ...
                return new vhdl_binop_expr(
                    from, VHDL_BINOP_SR, base->to_integer(), from->get_type());
...
static vhdl_expr* translate_ufunc(ivl_expr_t e) {
    ...
    const vhdl_type* rettype = vhdl_type::type_for(ivl_expr_width(e), ivl_expr_signed(e) != 0);
    ...
    for (int i = 0; i < nparams; i++) {
        ...
        vhdl_type* param_type =
            vhdl_type::type_for(ivl_signal_width(param_sig), ivl_signal_signed(param_sig) != 0);
...
static vhdl_expr* translate_ternary(ivl_expr_t e) {
    ...
    vhdl_fcall* fcall =
        new vhdl_fcall(support_function::function_name(sf), vhdl_type::type_for(width, issigned));
    fcall->add_expr(test);
...
static vhdl_expr* translate_concat(ivl_expr_t e) {
    const vhdl_type* rtype = vhdl_type::type_for(ivl_expr_width(e), ivl_expr_signed(e) != 0);
