...
vhdl_expr* vhdl_expr::to_vector(vhdl_type_name_t name, int w) {
    if (type_->get_name() == VHDL_TYPE_STD_LOGIC) {
        ...
        vhdl_bit_spec_expr* bs = new vhdl_bit_spec_expr(new vhdl_type(name, w - 1, 0), others);
        bs->add_bit(0, this);
...
vhdl_expr* vhdl_expr::to_string() {
    bool numeric = type_->get_name() == VHDL_TYPE_UNSIGNED || type_->get_name() == VHDL_TYPE_SIGNED;
    ...
    if (numeric) {
        ...
        vhdl_fcall* image = new vhdl_fcall(type_->get_string() + "'image", vhdl_type::string());
        image->add_expr(this);
        return image;
    }
}
...
vhdl_expr* vhdl_expr::to_boolean() {
    if (type_->get_name() == VHDL_TYPE_STD_LOGIC) {
        ...
        vhdl_fcall* conv = new vhdl_fcall(support_function::function_name(SF_UNSIGNED_TO_BOOLEAN),
                                          vhdl_type::boolean());
        conv->add_expr(this);
        return conv;
        ...
        vhdl_fcall* conv = new vhdl_fcall(support_function::function_name(SF_SIGNED_TO_BOOLEAN),
                                          vhdl_type::boolean());
        conv->add_expr(this);
        return conv;
    }
...
vhdl_expr* vhdl_expr::to_std_logic() {
    if (type_->get_name() == VHDL_TYPE_BOOLEAN) {
        ...
        vhdl_fcall* ah = new vhdl_fcall(support_function::function_name(SF_BOOLEAN_TO_LOGIC),
                                        vhdl_type::std_logic());
        ah->add_expr(this);
        ...
        vhdl_fcall* ah = new vhdl_fcall(support_function::function_name(SF_SIGNED_TO_LOGIC),
                                        vhdl_type::std_logic());
        ah->add_expr(this);
        ...
        vhdl_fcall* ah = new vhdl_fcall(support_function::function_name(SF_UNSIGNED_TO_LOGIC),
                                        vhdl_type::std_logic());
        ah->add_expr(this);
...
vhdl_expr* vhdl_expr::resize(int newwidth) {
    ...
    if (type_->get_name() == VHDL_TYPE_SIGNED)
        ...
        // Pad it with zeros
        vhdl_expr* zeros =
            new vhdl_const_bits(string(newwidth - 1, '0').c_str(), newwidth - 1, false, true);

        vhdl_binop_expr* concat =
            new vhdl_binop_expr(zeros, VHDL_BINOP_CONCAT, this, vhdl_type::nunsigned(newwidth));
        return concat;
    } else
