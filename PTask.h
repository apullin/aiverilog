...
class PTaskFunc : public PScope, public PNamedItem {
  public:
    ...
    // to the class type.
    inline class_type_t* method_of() const {
        return this_type_;
    }
  ...
  protected:
    ...
    void elaborate_sig_ports_(Design* des,
                              NetScope* scope,
...
class PFunction : public PTaskFunc {
  public:
    ...
    inline Statement* get_statement() {
        return statement_;
    }
...
class PLet : public PTaskFunc {
  public:
    ...
    explicit PLet(perm_string name,
                  LexicalScope* parent,
                  std::list<let_port_t*>* ports,
                  PExpr* expr);
    ~PLet() override;
